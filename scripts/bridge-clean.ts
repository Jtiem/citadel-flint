#!/usr/bin/env node
/**
 * bridge-clean — scripts/bridge-clean.ts
 *
 * CLI wrapper for `auditSnippet`. Applies Commandment 3 + the Mithril
 * perceptual colour gate to any raw JSX snippet.
 *
 * Usage:
 *   npm run bridge-clean "<jsx snippet>"
 *   cat snippet.tsx | npm run bridge-clean
 *   npm run bridge-clean --tokens path/to/tokens.json "<jsx snippet>"
 *
 * --tokens <path>
 *   Path to a JSON file containing a DesignToken[] array (same shape as the
 *   Bridge tokenStore export). When omitted, the colour gate is skipped —
 *   only Commandment 3 (index + data-bridge-id injection) is enforced.
 *
 * Exit codes:
