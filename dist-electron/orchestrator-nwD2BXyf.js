...
Anthropic.Beta = Beta;
const CONFIG_PATH = path$1.join(homedir(), ".bridge", "config.json");
...
async function writeConfig(config) {
  const dir = path$1.dirname(CONFIG_PATH);
...
const BRIDGE_TOOLS = [
  {
    ...
    description: "Read the current source code of the active file. Use this first to understand the existing component structure before proposing any changes.",
    input_schema: {
      type: "object",
      properties: {},
      required: []
  ...
  },
  {
    ...
    description: "Read all design tokens from the Bridge token store. You MUST use only token values from this list when proposing styling changes. Never invent arbitrary hex colours or pixel values.",
    input_schema: {
      type: "object",
      properties: {},
      required: []
  ...
  },
  {
    ...
    description: "Read all current Mithril Safety violations (color drift, typography drift, spacing drift, shadow, opacity). Use this to understand existing design system debt before proposing changes.",
    input_schema: {
      type: "object",
      properties: {},
      required: []
  ...
  },
  {
    ...
    description: "Read all current WCAG 2.1 AA accessibility violations. Use this to understand existing a11y debt and to verify your proposed changes do not introduce new violations.",
    input_schema: {
      type: "object",
      properties: {},
      required: []
  ...
  {
    name: "bridge_apply_batch",
    description: `
Propose one or more AST mutations to the active file. This is the ONLY way to modify component code.
This call will be shown to the user as a diff preview. The user must approve before mutations are applied.

Rules (mandatory — these enforce the 13 Commandments):
1. Never invent colour values. Only use tokens from bridge_read_tokens output.
2. Never remove a data-bridge-id attribute.
3. Every className value must use Tailwind utility classes that map to known tokens.
4. Prefer the smallest batch possible — one mutation per logical change.

Supported mutation ops:
- setAttr: set a JSX attribute (e.g., className, aria-label, href)
- updateTextContent: change the visible text inside a JSX element
- deleteNode: remove a node by its data-bridge-id (also clears overrides)
- injectComponent: insert a new component instance as a child
...
`,
    input_schema: {
      ...
      properties: {
        mutations: {
          type: "array",
          description: "Array of AST mutation objects.",
          items: {
            type: "object",
            properties: {
              op: {
                type: "string",
                enum: ["setAttr", "updateTextContent", "deleteNode", "injectComponent", "wrapNode"]
              },
              nodeId: { type: "string", description: "data-bridge-id of the target node." },
              attr: { type: "string", description: "For setAttr: JSX attribute name." },
              value: { type: "string", description: "For setAttr / updateTextContent: new value." },
              componentName: { type: "string", description: "For injectComponent: name of the React component to insert." },
              wrapperElement: { type: "string", description: "For wrapNode: JSX element tag to wrap with." }
            },
            required: ["op", "nodeId"]
          }
        },
        reasoning: {
          type: "string",
          description: "Brief human-readable explanation of what this batch does and why. Shown in the UI diff card."
        },
        tier: {
          type: "string",
          enum: ["flash", "thinking"],
          description: "Complexity tier. flash = 1-3 mutations. thinking = multi-step or cross-file."
        }
      },
      required: ["mutations", "reasoning", "tier"]
    }
  }
];
const SYSTEM_PROMPT = `You are the Bridge Auditor, an AI assistant integrated directly into Bridge IDE — the world's first Agentic UI Operating System.

Your role is to help engineers make precise, design-system-compliant, accessible component edits using the Bridge tool catalog.

## The 3 Rules You Must Never Break

1. **No Hallucinated Styling**: Before proposing any className or style change, call bridge_read_tokens. Only use token values that appear in the result. Never invent hex colours or pixel values.

2. **Preserve data-bridge-id**: Never remove or change a data-bridge-id attribute in any mutation.

3. **Accessibility First**: Before completing a task, if it touches interactive elements (button, a, input, img), call bridge_audit_a11y to verify no WCA violations exist. If violations exist, fix them as part of your response.

## Workflow

For simple tasks (colour/text change):
1. bridge_read_code → understand structure
2. bridge_read_tokens → find correct token
3. bridge_apply_batch (tier: flash) → propose mutation

For complex tasks (new component, cross-file, restructuring):
1. bridge_read_code → understand structure
2. bridge_read_tokens + bridge_audit_mithril + bridge_audit_a11y → full audit
3. bridge_apply_batch (tier: thinking) → propose mutations with clear reasoning

Always be concise in your explanations. The user is an engineer; skip preamble.`;
async function sendChatMessage(messages, onChunk) {
  const config = await readConfig();
  if (!config.apiKey) {
    onChunk({ type: "error", error: "No API key configured. Open the Agent panel and add your Anthropic API key." });
    return;
  }
  const client = new Anthropic({ apiKey: config.apiKey });
  const model = config.model && config.model.length > 0 ? config.model : DEFAULT_MODEL;
  try {
    ...
    onChunk({ type: "error", error: `Anthropic API error: ${msg}` });
  }
}
export {
  BRIDGE_TOOLS,
