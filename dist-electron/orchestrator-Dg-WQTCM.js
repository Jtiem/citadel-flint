...
Anthropic.Beta = Beta;
if (!isMainThread) {
  let createHost = function(snippet) {
  ...
  parentPort.on("message", (msg) => {
    ...
    try {
      ...
      if (errors.length === 0) {
        const res = { type: "result", error: null };
        parentPort.postMessage(res);
      } else {
        const message = errors.slice(0, 3).map((d) => ts.flattenDiagnosticMessageText(d.messageText, " ")).join("; ");
        const res = { type: "result", error: `TypeScript: ${message}` };
        parentPort.postMessage(res);
      }
    } catch (err) {
      const res = {
...
class TypeScriptLspClient {
  ...
  async validateSnippet(snippet) {
    ...
    return new Promise((resolve) => {
      ...
      const req = { type: "validate", snippet };
...
const tsLspClient = new TypeScriptLspClient();
if (!isMainThread) {
...
${msg.snippet}
      ...
      const res = {
        type: "result",
        error: errors.length > 0 ? `Vue: ${errors.slice(0, 3).join("; ")}` : null
      };
      parentPort.postMessage(res);
    } catch (err) {
      const res = {
...
const vueLspClient = new VueLspClient();
const CONFIG_PATH = path__default.join(homedir(), ".bridge", "config.json");
...
async function writeConfig(config) {
  const dir = path__default.dirname(CONFIG_PATH);
...
const SYSTEM_PROMPT = `You are the Bridge Auditor, an AI assistant integrated into Bridge IDE — the world's first Agentic UI Operating System. Your role is to make precise, design-system-compliant, accessible component edits using the Bridge tool catalog.
...
const MUTATION_TOOL_NAMES = /* @__PURE__ */ new Set([
  "bridge_update_props",
  "bridge_update_text",
  "bridge_insert_node",
  "bridge_wrap_node",
...
async function validateToolInput(toolName, input, lsp = tsLspClient) {
  if (!MUTATION_TOOL_NAMES.has(toolName)) return null;
...
async function sendChatMessage(messages, onChunk, activeFilePath) {
  ...
  try {
    if (config.provider === "openai") {
      ...
      const { GoogleGenAI } = await import("./index-DqFSC0nt.js").then((n) => n.i);
