...
function validateToolInput(toolName, input) {
  ...
  try {
    if (toolName === "bridge_insert_node") {
      ...
      const snippet = `const __v = <${nodeType}>${children}</${nodeType}>;`;
      transformSync(snippet, {
      ...
      const snippet = `const __v = <${wrapperType}><div /></${wrapperType}>;`;
      transformSync(snippet, {
        filename: "validate.tsx",
        plugins: [
          ["@babel/plugin-transform-typescript", { isTSX: true, allExtensions: true }],
    ...
    } else if (toolName === "bridge_update_props") {
    ...
    } else if (toolName === "bridge_add_class" || toolName === "bridge_remove_class") {
    ...
    return `Babel validation failed: ${msg}`;
  }
}
        ...
        if (event.type === "content_block_delta") {
          ...
          for (const block of finalMsg.content) {
            if (block.type === "tool_use") {
              ...
              const validationError = validateToolInput(block.name, toolInput);
              if (validationError) {
