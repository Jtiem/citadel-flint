...
export default function DemoCard({
  ...
  return <div data-flint-id="card-root" className="bg-[#FEFEFE] rounded-[10px] p-[20px] max-w-[var(--spacing.12, 48px)] border border-[var(--color.border, #E5E7EB)]" style={{
    boxShadow: '0 4px 12px rgba(0,0,0,0.08)'
  }}>
      {/* Violation: img missing alt attribute (A11Y-001) */}
      <img data-flint-id="card-image" src="/placeholder.png" className="w-full h-[var(--spacing.12, 48px)] object-cover rounded-[8px] mb-[16px]" alt="" />

      <h3 data-flint-id="card-title" className="text-[#111827] text-[20px] font-bold leading-[1.3] m-0">
        {title}
      </h3>

      {/* Violation: arbitrary font-size 15px, not in token set */}
      <p data-flint-id="card-description" className="text-[#6B7280] text-[var(--spacing.4, 16px)] leading-[1.5] mt-[8px] mb-[16px]">
        {description}
      </p>
      ...
      <div data-flint-id="card-footer" className="flex items-center justify-between">
        ...
        <span data-flint-id="card-price" className="text-[var(--color.primary-hover, #0052CC)] text-[24px] font-bold">
          {price}
        </span>

        {/* Violation: button missing accessible name for icon-only button */}
        <button data-flint-id="card-cta" onClick={onAddToCart} className="px-[20px] py-[var(--spacing.2, 8px)] bg-[var(--color.primary-hover, #0052CC)] text-[#FFFFFF] rounded-[8px] text-[14px] font-semibold cursor-pointer border-none">
          Add to Cart
        </button>
      </div>
    </div>;
