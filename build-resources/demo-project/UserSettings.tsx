...
export default function UserSettings() {
  ...
  return <div data-flint-id="settings-root" className="min-h-screen bg-zinc-950 text-zinc-100">
      {/* Header */}
      <header data-flint-id="settings-header" className="border-b border-zinc-800 bg-zinc-900">
        <div className="mx-auto flex max-w-4xl items-center justify-between px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <img data-flint-id="header-logo" src="/logo.svg" className="h-8 w-8" alt="" />
            <span data-flint-id="header-title" className="text-lg font-semibold">Settings</span>
          </div>
          <button data-flint-id="header-back" className="rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-1.5 text-sm text-zinc-300 transition hover:bg-zinc-700">
            Back to Dashboard
          </button>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-4xl px-6 py-8">
        ...
        <nav data-flint-id="settings-tabs" className="flex gap-1 rounded-lg bg-zinc-900 p-1">
          {tabs.map(tab => <button key={tab.id} data-flint-id={`tab-${tab.id}`} onClick={() => setActiveTab(tab.id)} className={`flex-1 rounded-md px-4 py-2 text-sm font-medium transition ${activeTab === tab.id ? 'bg-zinc-800 text-white shadow-sm' : 'text-zinc-400 hover:text-zinc-200'}`}>
              {tab.label}
            </button>)}
        </nav>

        {/* Profile Tab */}
        {activeTab === 'profile' && <div data-flint-id="tab-content-profile" className="mt-8 space-y-8">
            {/* Avatar + Name section */}
            <div data-flint-id="profile-card" className="flex items-start gap-6 rounded-xl border border-zinc-800 bg-zinc-900 p-6">
              <div data-flint-id="avatar-wrapper" className="relative">
                {/* Violation: img missing alt (A11Y-001) */}
                <img data-flint-id="profile-avatar" src={MOCK_USER.avatar} className="h-20 w-20 rounded-full border-2 border-zinc-700 object-cover" alt="" />
                {/* Violation: button with no accessible label (A11Y-002) */}
                <button data-flint-id="avatar-edit-btn" className="absolute -bottom-1 -right-1 rounded-full bg-indigo-600 p-1.5 text-white shadow-lg transition hover:bg-indigo-500">
              ...
              <div className="flex-1">
                ...
                <span data-flint-id="profile-role-badge" className="mt-2 inline-block rounded-full bg-indigo-600/20 px-2.5 py-0.5 text-xs font-medium text-indigo-400">
                  {MOCK_USER.role}
                </span>
              </div>
              <button data-flint-id="profile-edit-btn" className="rounded-lg border border-zinc-700 bg-zinc-800 px-4 py-2 text-sm font-medium text-zinc-300 transition hover:bg-zinc-700 hover:text-white">
                Edit Profile
              </button>
            </div>
            ...
            <div data-flint-id="profile-form" className="space-y-6 rounded-xl border border-zinc-800 bg-zinc-900 p-6">
              ...
              <div className="grid gap-6 md:grid-cols-2">
                <div data-flint-id="field-name">
                  ...
                  <input data-flint-id="input-name" type="text" defaultValue={MOCK_USER.name} className="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-2 text-sm text-zinc-100 placeholder-zinc-500 transition focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500" aria-label="[NEEDS LABEL]" />
                </div>
                <div data-flint-id="field-email">
                  <label className="mb-1.5 block text-sm font-medium text-zinc-300">Email Address</label>
                  <input data-flint-id="input-email" type="email" defaultValue={MOCK_USER.email} className="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-2 text-sm text-zinc-100 placeholder-zinc-500 transition focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500" aria-label="[NEEDS LABEL]" />
                </div>
                <div data-flint-id="field-role">
                  ...
                  <select data-flint-id="select-role" defaultValue={MOCK_USER.role} className="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-2 text-sm text-zinc-100 transition focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500" aria-label="[NEEDS LABEL]">
                ...
                <div data-flint-id="field-bio">
                  ...
                  <textarea data-flint-id="textarea-bio" rows={3} placeholder="Tell us about yourself..." className="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-2 text-sm text-zinc-100 placeholder-zinc-500 transition focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500" aria-label="[NEEDS LABEL]" />
                </div>
              </div>
              <div className="flex justify-end gap-3 border-t border-zinc-800 pt-4">
                <button data-flint-id="form-cancel" className="rounded-lg border border-zinc-700 bg-zinc-800 px-4 py-2 text-sm text-zinc-400 transition hover:text-zinc-200">
                  Cancel
                </button>
                {/* Violation: hardcoded hex color #4F46E5 instead of design token */}
                <button data-flint-id="form-save" className="rounded-lg bg-[var(--color.primary-hover, #0052CC)] px-4 py-2 text-sm font-semibold text-white transition hover:bg-[var(--color.primary-hover, #0052CC)]">
                  Save Changes
                </button>
              </div>
            </div>
        ...
        {/* Notifications Tab */}
        {activeTab === 'notifications' && <div data-flint-id="tab-content-notifications" className="mt-8 space-y-4">
            <div data-flint-id="notif-card" className="rounded-xl border border-zinc-800 bg-zinc-900 p-6">
              ...
              <div className="mt-6 space-y-5">
                ...
                <div data-flint-id="notif-email" className="flex items-center justify-between">
                  ...
                  <button data-flint-id="toggle-email" onClick={() => setEmailNotifs(!emailNotifs)} className={`relative inline-flex h-6 w-11 items-center rounded-full transition ${emailNotifs ? 'bg-indigo-600' : 'bg-zinc-700'}`}>
                    <span className={`inline-block h-4 w-4 rounded-full bg-white shadow transition ${emailNotifs ? 'translate-x-6' : 'translate-x-1'}`} />
                  </button>
                </div>
                ...
                <div data-flint-id="notif-slack" className="flex items-center justify-between border-t border-zinc-800 pt-5">
                  ...
                  <button data-flint-id="toggle-slack" onClick={() => setSlackNotifs(!slackNotifs)} className={`relative inline-flex h-6 w-11 items-center rounded-full transition ${slackNotifs ? 'bg-indigo-600' : 'bg-zinc-700'}`}>
                    <span className={`inline-block h-4 w-4 rounded-full bg-white shadow transition ${slackNotifs ? 'translate-x-6' : 'translate-x-1'}`} />
                  </button>
                </div>
                ...
                <div data-flint-id="notif-digest" className="flex items-center justify-between border-t border-zinc-800 pt-5">
                  ...
                  <button data-flint-id="toggle-digest" onClick={() => setWeeklyDigest(!weeklyDigest)} className={`relative inline-flex h-6 w-11 items-center rounded-full transition ${weeklyDigest ? 'bg-indigo-600' : 'bg-zinc-700'}`}>
                    <span className={`inline-block h-4 w-4 rounded-full bg-white shadow transition ${weeklyDigest ? 'translate-x-6' : 'translate-x-1'}`} />
                  </button>
                </div>
              </div>
            </div>
        ...
        {/* Security Tab */}
        {activeTab === 'security' && <div data-flint-id="tab-content-security" className="mt-8 space-y-6">
            {/* 2FA */}
            <div data-flint-id="security-2fa" className="rounded-xl border border-zinc-800 bg-zinc-900 p-6">
              <div className="flex items-center justify-between">
                ...
                <button data-flint-id="2fa-toggle" onClick={() => setTwoFactor(!twoFactor)} className={`rounded-lg px-4 py-2 text-sm font-medium transition ${twoFactor ? 'bg-emerald-600/20 text-emerald-400 border border-emerald-500/30' : 'bg-zinc-800 text-zinc-300 border border-zinc-700 hover:bg-zinc-700'}`}>
            ...
            <div data-flint-id="security-sessions" className="rounded-xl border border-zinc-800 bg-zinc-900 p-6">
              ...
              <div className="mt-4 space-y-3">
                ...
                <div data-flint-id="session-other" className="flex items-center justify-between rounded-lg bg-zinc-800/50 px-4 py-3">
                  ...
                  <button data-flint-id="session-revoke" className="text-xs text-red-400 transition hover:text-red-300">
                    Revoke
                  </button>
                </div>
              </div>
            ...
            <div data-flint-id="danger-zone" className="rounded-xl border border-red-900/30 bg-red-950/10 p-6">
              ...
              {!showDeleteConfirm ? <button data-flint-id="delete-account-btn" onClick={() => setShowDeleteConfirm(true)} className="mt-4 rounded-lg border border-red-700/40 bg-red-900/20 px-4 py-2 text-sm font-medium text-red-400 transition hover:bg-red-900/40">
                  Delete Account
                </button> : <div data-flint-id="delete-confirm" className="mt-4 flex items-center gap-3">
                  <p className="text-sm text-red-300">Are you sure? Type your email to confirm.</p>
                  {/* Violation: input missing label (A11Y-004) */}
                  <input data-flint-id="delete-confirm-input" type="email" placeholder="your@email.com" className="rounded-lg border border-red-700/40 bg-red-950/20 px-3 py-1.5 text-sm text-red-300 placeholder-red-800 transition focus:border-red-500 focus:outline-none" aria-label="[NEEDS LABEL]" aria-invalid="true" />
                  <button data-flint-id="delete-confirm-btn" className="rounded-lg bg-red-600 px-4 py-1.5 text-sm font-semibold text-white transition hover:bg-red-500">
                    Confirm Delete
                  </button>
                  <button data-flint-id="delete-cancel-btn" onClick={() => setShowDeleteConfirm(false)} className="text-sm text-zinc-400 transition hover:text-zinc-200">
                    Cancel
                  </button>
                </div>}
            </div>
        ...
        {/* Billing Tab */}
        {activeTab === 'billing' && <div data-flint-id="tab-content-billing" className="mt-8 space-y-6">
            {/* Current plan */}
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="mt-2 flex items-baseline gap-2">
                    ...
                    <span data-flint-id="plan-status" className="rounded-full bg-[#22C55E]/20 px-2 py-0.5 text-xs font-medium text-[var(--color.success, #16A34A)]">
                      Active
                    </span>
                  </div>
                  <p className="mt-1 text-sm text-zinc-400">$29/month · Renews April 15, 2026</p>
                </div>
                <button data-flint-id="billing-manage" className="rounded-lg border border-zinc-700 bg-zinc-800 px-4 py-2 text-sm font-medium text-zinc-300 transition hover:bg-zinc-700">
                  Manage Plan
                </button>
              </div>
            </div>
            ...
            <div data-flint-id="billing-payment" className="rounded-xl border border-zinc-800 bg-zinc-900 p-6">
              ...
              <div data-flint-id="payment-card" className="mt-4 flex items-center gap-4 rounded-lg bg-zinc-800/50 px-4 py-3">
                ...
                <button data-flint-id="payment-update" className="ml-auto text-xs text-indigo-400 transition hover:text-indigo-300">
                  Update
                </button>
              </div>
            </div>
