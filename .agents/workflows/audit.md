---
description: Run a full Mithril compliance audit and generate a session-end FLINT-PULSE
---
...
1. Read `.flint-context/FLINT-PULSE.md` for the current session state.
...
3. Scan all files modified during this session. For each file under `src/`:
   - Verify all new JSX elements have a `data-flint-id` attribute (or are handled by `injectFlintIds`).
...
5. Update `.flint-context/FLINT-PULSE.md` with:
...
6. Generate the final `[FLINT-PULSE-v5.7]` block and report it to the user.
