/**
 * SessionValidator — flint-mcp/src/core/governance/sessionValidator.ts
 *
 ...
 * independent checks:
 *
 *   CHECK-1  DUPLICATE_FLINT_ID      — all data-flint-id values are unique
 ...
 *   CHECK-4  MISSING_FLINT_ID        — every JSX element carries a data-flint-id
 *
 * Design goals
...
// CJS/ESM interop (mirrors the pattern used throughout flint-mcp)
const traverse =
...
export interface SessionMutation {
    /** data-flint-id of the node that was targeted by this mutation. */
    nodeId?: string
...
/**
 * Extract the string value of a JSX attribute named `data-flint-id` from
 * a JSXOpeningElement, or null when absent / non-string.
 */
function getFlintId(openingElement: JSXOpeningElement): string | null {
    for (const attr of openingElement.attributes) {
        ...
        const name = t.isJSXNamespacedName(nameNode)
            ...
            : nameNode.name
        if (name !== 'data-flint-id') continue
...
export function validateSessionState(
    ...
    // CHECK-1: track flint IDs seen so far — map to first-seen nodeId for msg
    const seenFlintIds = new Map<string, string>()   // flintId → first occurrence
    const duplicateFlintIds = new Set<string>()      // flintIds already flagged

    // CHECK-4: track JSX elements without a flint ID
    const missingFlintIdElements: string[] = []       // tag names for messages
    ...
    traverse(ast, {
        ...
        JSXOpeningElement(nodePath) {
            const flintId = getFlintId(nodePath.node)

            // CHECK-4 — every JSX element must carry a flint ID
            if (flintId === null) {
                const tagName = t.isJSXIdentifier(nodePath.node.name)
                    ? nodePath.node.name.name
                    : 'unknown'
                missingFlintIdElements.push(tagName)
            } else {
                // Track for CHECK-2
                existingNodeIds.add(flintId)

                // CHECK-1 — duplicate detection
                if (seenFlintIds.has(flintId)) {
                    if (!duplicateFlintIds.has(flintId)) {
                        duplicateFlintIds.add(flintId)
                        errors.push({
                            code: 'DUPLICATE_FLINT_ID',
                            message: `Duplicate data-flint-id "${flintId}" found in ${filePath}. ` +
                    ...
                    seenFlintIds.set(flintId, flintId)
                }
            }
        },
    })
    ...
    // ── CHECK-4 — emit accumulated missing-flint-id errors ─────────────────
    for (const tagName of missingFlintIdElements) {
        errors.push({
            code: 'MISSING_FLINT_ID',
            message: `JSX element <${tagName}> in ${filePath} is missing a ` +
                `data-flint-id attribute. Run injectFlintIds after every ` +
