...
// ── Known rule set — all rules enforced by flint-mcp ──────────────────────────
...
describe('resolveProvenance — known ruleId lookup', () => {
    ...
    it('resolves MITHRIL-COL as Flint Design System authority', () => {
        const p = resolveProvenance('MITHRIL-COL')
        expect(p.ruleId).toBe('MITHRIL-COL')
        expect(p.sourceAuthority).toBe('Flint Design System')
        expect(p.regulatoryReference).toContain('Flint Design System')
    })
    ...
    it('resolves EXP-001 as Flint Design System authority', () => {
...
describe('resolveProvenance — unknown ruleId fallback', () => {
    it('returns a fallback with Flint Design System authority for an unknown ruleId', () => {
...
describe('buildComplianceSummary — authority breakdown accuracy', () => {
    it('correctly separates WCAG 2.1 AA and Flint Design System violations', () => {
        const summary = buildComplianceSummary([
            { ruleId: 'A11Y-001', severity: 'critical' },  // WCAG 2.1 AA
            { ruleId: 'A11Y-002', severity: 'critical' },  // WCAG 2.1 AA
            { ruleId: 'MITHRIL-COL', severity: 'amber' },  // Flint Design System
        ])

        expect(summary.byAuthority['WCAG 2.1 AA']).toBe(2)
        expect(summary.byAuthority['Flint Design System']).toBe(1)
        expect(summary.byAuthority['WCAG 2.2 AA']).toBe(0)
        expect(summary.byAuthority['SOC2']).toBe(0)
    })

    it('attributes unknown ruleIds to Flint Design System (fallback)', () => {
        const summary = buildComplianceSummary([
    ...
    it('all authority keys are present in the output even at 0', () => {
        ...
        expect('Flint Design System' in summary.byAuthority).toBe(true)
...
describe('getByAuthority — filter by source authority', () => {
    ...
    it('returns only Flint Design System rules when filtering by that authority', () => {
        const flint = getByAuthority('Flint Design System')
        expect(flint.length).toBeGreaterThan(0)
        for (const entry of flint) {
            expect(entry.sourceAuthority).toBe('Flint Design System')
        }
    })
