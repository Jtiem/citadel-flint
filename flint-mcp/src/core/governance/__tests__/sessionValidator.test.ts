 ...
 *
 * flint-mcp/src/core/governance/__tests__/sessionValidator.test.ts
 *
 * Coverage:
 *   - Clean AST with all flint IDs → valid, no errors
 *   - Duplicate flint IDs → DUPLICATE_FLINT_ID error for each pair
 ...
 *   - Missing flint ID on a JSX element → MISSING_FLINT_ID error
...
// ---------------------------------------------------------------------------
// CHECK-1: Unique flint IDs
// ---------------------------------------------------------------------------

describe('CHECK-1 — Unique flint IDs', () => {
    it('clean AST with unique flint IDs → valid, no DUPLICATE_FLINT_ID errors', () => {
        const ast = parseJSX(`
            const C = () => (
                <div data-flint-id="root">
                    <span data-flint-id="child-1">Hello</span>
                    <span data-flint-id="child-2">World</span>
                </div>
            )
        `)
        const result = validateSessionState(ast, 'Button.tsx', [])
        const dupeErrors = result.errors.filter((e) => e.code === 'DUPLICATE_FLINT_ID')
        expect(dupeErrors).toHaveLength(0)
    })

    it('duplicate flint IDs → exactly one DUPLICATE_FLINT_ID error per duplicated id', () => {
        const ast = parseJSX(`
            const C = () => (
                <div data-flint-id="root">
                    <span data-flint-id="dup-id">A</span>
                    <span data-flint-id="dup-id">B</span>
                </div>
            )
        `)
        const result = validateSessionState(ast, 'Button.tsx', [])
    ...
    it('two distinct duplicate pairs → two DUPLICATE_FLINT_ID errors', () => {
        const ast = parseJSX(`
            const C = () => (
                <div data-flint-id="root">
                    <span data-flint-id="a">1</span>
                    <span data-flint-id="a">2</span>
                    <p data-flint-id="b">3</p>
                    <p data-flint-id="b">4</p>
                </div>
            )
        `)
        const result = validateSessionState(ast, 'File.tsx', [])
        const codes = result.errors
            .filter((e) => e.code === 'DUPLICATE_FLINT_ID')
            .map((e) => e.nodeId)
    ...
    it('triplicate flint ID → only one error emitted (de-duplication guard)', () => {
        const ast = parseJSX(`
            const C = () => (
                <div>
                    <span data-flint-id="triple">A</span>
                    <span data-flint-id="triple">B</span>
                    <span data-flint-id="triple">C</span>
                </div>
            )
        `)
        const result = validateSessionState(ast, 'File.tsx', [])
        const dupes = result.errors.filter((e) => e.code === 'DUPLICATE_FLINT_ID')
        expect(dupes).toHaveLength(1)
    })

    it('valid=false when DUPLICATE_FLINT_ID error present', () => {
        const ast = parseJSX(`
            const C = () => (
                <div>
                    <span data-flint-id="x">A</span>
                    <span data-flint-id="x">B</span>
                </div>
            )
        `)
        const result = validateSessionState(ast, 'File.tsx', [])
...
describe('CHECK-2 — Orphaned mutation references', () => {
    it('node referenced in session mutations exists in AST → no ORPHANED_MUTATION warning', () => {
        const ast = parseJSX(`
            const C = () => <div data-flint-id="alive">Hello</div>
        `)
    ...
    it('node referenced in session mutations is absent from AST → ORPHANED_MUTATION warning', () => {
        const ast = parseJSX(`
            const C = () => <div data-flint-id="survivor">Hello</div>
        `)
    ...
    it('mutation with undefined nodeId is skipped (no spurious error)', () => {
        const ast = parseJSX(`
            const C = () => <div data-flint-id="root">Hello</div>
        `)
...
describe('CHECK-3 — Stale imports', () => {
    it('all imports are used → no STALE_IMPORT errors', () => {
        ...
        const ast = parseJSX(`
            ...
            const C = () => (
                <React.Fragment>
                    <Button data-flint-id="btn">Click</Button>
                </React.Fragment>
            )
        `)
        const result = validateSessionState(ast, 'File.tsx', [])
    ...
    it('namespace import that is used → no STALE_IMPORT error', () => {
        const ast = parseJSX(`
            ...
            const C = () => <div data-flint-id="root">{Icons.Star}</div>
        `)
        const result = validateSessionState(ast, 'File.tsx', [])
        const stale = result.errors.filter((e) => e.code === 'STALE_IMPORT')
        expect(stale).toHaveLength(0)
...
// ---------------------------------------------------------------------------
// CHECK-4: Flint ID coverage
// ---------------------------------------------------------------------------

describe('CHECK-4 — Flint ID coverage', () => {
    it('all JSX elements have flint IDs → no MISSING_FLINT_ID errors', () => {
        const ast = parseJSX(`
            const C = () => (
                <div data-flint-id="root">
                    <span data-flint-id="s1">Text</span>
                </div>
            )
        `)
        const result = validateSessionState(ast, 'File.tsx', [])
        const missing = result.errors.filter((e) => e.code === 'MISSING_FLINT_ID')
        expect(missing).toHaveLength(0)
    })

    it('JSX element without flint ID → MISSING_FLINT_ID error', () => {
        const ast = parseJSX(`
    ...
    it('multiple elements missing flint IDs → one error per element', () => {
        const ast = parseJSX(`
            const C = () => (
                <div>
                    <span>A</span>
    ...
    it('MISSING_FLINT_ID makes valid=false', () => {
        const ast = parseJSX(`
            const C = () => <div>Hello</div>
        `)
        const result = validateSessionState(ast, 'File.tsx', [])
...
describe('Multiple errors in one AST', () => {
    it('reports DUPLICATE_FLINT_ID + STALE_IMPORT in same result', () => {
        const ast = parseJSX(`
            import { Ghost } from './ghost'
            const C = () => (
                <div data-flint-id="root">
                    <span data-flint-id="dup">A</span>
                    <span data-flint-id="dup">B</span>
                </div>
            )
        `)
        const result = validateSessionState(ast, 'File.tsx', [])
        const codes = result.errors.map((e) => e.code)
        expect(codes).toContain('DUPLICATE_FLINT_ID')
    ...
    it('reports all four check types simultaneously', () => {
        ...
        expect(codes).toContain('STALE_IMPORT')
        expect(codes).toContain('MISSING_FLINT_ID')
        expect(result.valid).toBe(false)
    })
})
...
describe('Edge cases', () => {
    ...
    it('clean, fully-covered JSX component → valid=true', () => {
        // No unused imports; all JSX elements have flint IDs; no orphaned mutations.
        const ast = parseJSX(`
            const Button = () => (
                <button data-flint-id="btn" className="px-4 py-2">
                    <span data-flint-id="btn-label">Click me</span>
                </button>
            )
