 ...
 *
 * Intercepts eligible tool calls and prepends a structured "Flint Context
 ...
 *   - flint_ast_mutate → MutateEnrichment (target node props, violations,
 *                          parent tag, sibling IDs)
 *   - flint_fix        → FixEnrichment (violation details, suggested fix ops)
 *   - flint_audit      → Token context appended to result (top violated
 *                          categories, top-3 suggested fixes)
 *
 * Read-only tools (flint_status, flint_query_registry, etc.) pass through
 * unchanged.
 *
...
/**
 * Parse a file and extract node context for a specific flint ID.
 *
    ...
    try {
        ...
        traverse(ast, {
            JSXElement(nodePath) {
                ...
                // Check if this element has the matching data-flint-id
                ...
                for (const attr of openingEl.attributes) {
                    ...
                    // Extract data-flint-id
                    if (attrName === 'data-flint-id') {
                ...
                if (parentPath && parentPath.isJSXElement()) {
                    ...
                    for (const pAttr of parentOpeningEl.attributes) {
                        ...
                        if (
                            ...
                            pAttr.name.name === 'data-flint-id'
                        ) {
                            if (t.isStringLiteral(pAttr.value)) {
                                parentFlintId = pAttr.value.value
                            }
                        }
                    }

                    if (parentFlintId) {
                        parentContext = { tagName: parentTagName, flintId: parentFlintId }
                    }
                }
 ...
 * Used to append suggestions to flint_audit results.
 */
            ...
            const suggestion =
                ruleId.startsWith('MITHRIL') && tokenNames.length > 0
                    ...
                    : ruleId.startsWith('A11Y')
                        ...
                        : 'Apply nearest design token from .flint/design-tokens.json'
            return {
                ruleId,
...
function formatMutatePreamble(
    ...
    const lines: string[] = ['--- Flint Context Preamble ---']

    lines.push(`Target node: data-flint-id="${nodeId}"`)
    ...
    if (astInfo) {
        ...
        if (astInfo.parentContext) {
            lines.push(
                `Parent: <${astInfo.parentContext.tagName} data-flint-id="${astInfo.parentContext.flintId}">`,
            )
        }
...
function formatAuditAppendix(ctx: AuditTokenContext): string {
    const lines: string[] = ['', '--- Flint Token Context ---']
 ...
 *
 * Used for mutation tools (flint_ast_mutate, flint_fix) where the
...
export function enrichToolCall(
    ...
    try {
        ...
        if (toolName === 'flint_ast_mutate') {
            return enrichAstMutate(args, ctx)
        }

        if (toolName === 'flint_fix') {
 ...
 *
 *   - Mutation tools (flint_ast_mutate, flint_fix): context preamble
 *     prepended before the original result
 *   - Audit tools (flint_audit): token context block appended after the
 *     original result
...
function buildMinimalContext(
    ...
    try {
        const contextPath = path.join(projectRoot, '.flint', 'context.json')
        let violations = {
            mithrilCount: 0,
            a11yCount: 0,
            amberCount: 0,
