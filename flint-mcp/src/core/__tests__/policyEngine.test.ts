 ...
 * flint-mcp/src/core/__tests__/policyEngine.test.ts
 *
...
/**
 * Creates a temporary directory with an optional .flint/policy.json,
 * returns the temp dir path, and registers cleanup.
 */
function makeTempProject(policy?: unknown): string {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'flint-pol-test-'))
    if (policy !== undefined) {
