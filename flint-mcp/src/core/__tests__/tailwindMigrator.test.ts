/**
 * tailwindMigrator tests — flint-mcp/src/core/__tests__/tailwindMigrator.test.ts
 *
...
function tmpFile(content: string, ext = '.tsx'): string {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'flint-tw-test-'))
...
describe('migrateFile — write mode', () => {
    it('dryRun=false writes migrated content to disk via migrateFileAtPath', () => {
        const source = wrap('<div className="flex-grow" />')
