/**
 * colorMath.ts — Shared CIEDE2000 color distance engine
 *
 * Extracted from MithrilLinter.ts and fix.ts to eliminate duplication.
 * Used by both the Mithril linter (detection) and the fix tool (remediation).
 */
...
/**
 * Parse a CSS dimension value (e.g. "16px", "1.5rem") to a numeric value in px.
 * Returns null if the value cannot be parsed.
 */
export function parseDimensionToPx(value: string): number | null {
    const match = /^([\d.]+)(px|rem|em)?$/.exec(value.trim())
    if (!match) return null
    const num = parseFloat(match[1]!)
    if (isNaN(num)) return null
    const unit = match[2] ?? 'px'
    // Approximate rem/em as 16px for sorting purposes
