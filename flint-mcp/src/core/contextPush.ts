 ...
 *
 * Watches .flint/context.json (and .flint/design-tokens.json) for changes
 ...
 * change is significant — writes the delta to .flint/mcp-events.jsonl using
...
import { appendMCPEvent, flintEvents, EVENTS } from './events.js'
import type {
    ContextDelta,
    ContextDeltaTrigger,
    ContextDeltaPayload,
...
// ── Internal snapshot shape — mirrors the FlintContext fields we care about ─
...
export class ContextPushManager {
    ...
    /**
     * Start watching the project's .flint/ directory for state changes.
    ...
    start(projectRoot: string): void {
        ...
        flintEvents.on(EVENTS.TOKENS_UPDATED, this.onTokensUpdated)
    }

    /** Stop all watchers and release resources. */
    stop(): void {
        flintEvents.off(EVENTS.TOKENS_UPDATED, this.onTokensUpdated)
    ...
    /**
     * Read .flint/context.json and extract the fields needed for diffing.
    ...
    private readContextSnapshot(): ContextSnapshot {
        ...
        try {
            const contextPath = path.join(this.projectRoot, '.flint', 'context.json')
    ...
    /**
     * Read .flint/design-tokens.json and return the total token count.
    ...
    private readTokenCount(): number {
        ...
        try {
            const tokensPath = path.join(this.projectRoot, '.flint', 'design-tokens.json')
     ...
     * flintEvents bus (server.ts listens to this and calls server.notification).
     */
    private emitDelta(delta: ContextDelta): void {
        if (this.projectRoot === null) return

        // Write to .flint/mcp-events.jsonl
        ...
        flintEvents.emit(EVENTS.CONTEXT_DELTA)
    }
}
