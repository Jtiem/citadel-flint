/**
 * planService.ts — flint-mcp/src/core/planService.ts
 *
 * Intent classification + execution plan generation for the flint_plan MCP tool.
 *
 * Design principles:
 ...
 *   - Scope estimation reads .flint/ files only; never throws
...
import { queryRegistry } from './registryService.js'
import type { ComponentEntry } from './registryService.js'

// ── Intent Classification Types ───────────────────────────────────────────────

/**
 * The 6 supported intent categories. Each maps to a fixed plan template.
...
export interface ExecutionPlan {
    ...
    dryRun: boolean
    /**
     * For component-composition intent: registry component names that match
     * the composition request. Empty array when no manifest is available or
     * no components matched the intent. Undefined for all other intent types.
     */
...
const COMPONENT_COMPOSITION_TEMPLATE: PlanStep[] = [
    {
        step: 1,
        kind: 'tool',
        tool: 'flint_query_registry',
        params: { query: '{{intent}}', limit: 10 },
        purpose: 'Search component registry for components matching the intent.',
        requiresReview: true,
    },
    {
        step: 2,
        kind: 'decision',
        description: 'Review matched components and select which to use for this composition.',
        rationale: 'Component selection requires understanding the design intent and which registry components are semantically appropriate for the requested UI.',
        suggestedOptions: [
            'Use all matched components as suggested by the registry',
            'Select a subset of matched components manually',
            'Search for additional components with a refined query',
        ],
    },
    {
        step: 3,
        ...
        tool: 'flint_audit',
        params: { filePaths: '{{glob}}' },
        purpose: 'Pre-audit target file for baseline violations (if file exists).',
        requiresReview: false,
    },
    {
        step: 4,
        ...
        params: { filePath: '{{filePath}}', ops: [] },
        purpose: 'Compose UI from selected registry components using AST operations.',
        requiresReview: false,
    },
    {
        step: 5,
        ...
        purpose: 'Post-composition audit to verify design system compliance.',
        requiresReview: true,
    },
    {
        step: 6,
        kind: 'decision',
        description: 'Review audit results and decide on fixes.',
        rationale: 'Post-composition audit may surface violations from the newly composed UI that require design judgment to resolve (e.g., token selection, accessibility requirements).',
        suggestedOptions: [
            'Apply all auto-fixable violations immediately',
            'Review violations one by one before applying fixes',
            'Defer non-critical violations to a follow-up session',
        ],
    },
]
...
const UNKNOWN_INTENT_TEMPLATE: PlanStep[] = [
    {
        ...
        rationale: 'flint_plan supports 6 task types: token-migration, accessibility-sweep, full-governance-audit, figma-sync, debt-remediation, and component-composition. The provided intent did not match any of these categories with sufficient confidence.',
        suggestedOptions: [
            ...
            'Build a UI component from the registry',
        ],
    },
]
...
const PLAN_META: Record<PlanIntentType, PlanMeta> = {
    ...
    'component-composition': {
        riskLevel: 'medium',
        successCriteria: [
            'All components used are from the project registry',
            'Zero Mithril violations in composed output',
    ...
    'unknown': {
        riskLevel: 'low',
        successCriteria: [
            'User clarifies intent and re-invokes flint_plan with a more specific request',
        ],
    },
}
...
function parameterizePlan(steps: PlanStep[], glob: string, projectRoot: string, intent: string): PlanStep[] {
    return steps.map(step => {
        if (step.kind === 'tool') {
            return {
                ...step,
                params: substituteParams(step.params, glob, projectRoot, intent),
            }
        }
        return { ...step }
    })
...
function getTemplate(intentType: PlanIntentType): PlanStep[] {
    switch (intentType) {
        ...
        case 'component-composition': return COMPONENT_COMPOSITION_TEMPLATE
...
function buildScopeString(scope: ScopeEstimate | null, glob: string, projectRoot?: string): string {
    if (!projectRoot || scope === null) {
        return `Files matching ${glob} (scope estimate unavailable -- no .flint/ data found)`
    }
    ...
    // If we only have file count or the flint dir exists but has no data
...
// ── Flint Manifest Shape ─────────────────────────────────────────────────────

interface FlintManifestShape {
 ...
 *   5. For component-composition: look up matching registry components
 *   6. Build the ExecutionPlan
 *
...
export function generatePlan(intent: string, options: GeneratePlanOptions = {}): ExecutionPlan {
    ...
    // 4. Parameterize
    const steps = parameterizePlan(rawTemplate, glob, projectRoot, intent)
    ...
    // 6. For component-composition: read manifest and find matching registry components
    if (planIntent.type === 'component-composition') {
        let suggestedComponents: string[] = []

        if (projectRoot) {
            const manifestPath = path.join(projectRoot, 'flint-manifest.json')
            const manifest = safeReadJson<FlintManifestShape>(manifestPath)

            if (manifest && manifest.components && Object.keys(manifest.components).length > 0) {
                const matched = queryRegistry(manifest.components, intent, 5)
                suggestedComponents = matched.map(c => c.name)
            }
        }

        plan.suggestedComponents = suggestedComponents
    }
