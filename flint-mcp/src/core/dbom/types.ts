/**
 * DBOM Types — flint-mcp/src/core/dbom/types.ts
 *
...
export interface DBOMViolation {
    /** Flint rule identifier (e.g. 'MITHRIL-COL', 'MITHRIL-TYP-001'). */
    ruleId: string
    /** Severity: 'amber' (ΔE 2-10), 'critical' (ΔE > 10), or 'advisory'. */
    severity: 'amber' | 'critical' | 'advisory'
    /** Human-readable violation message. */
    message: string
    /** The data-flint-id of the JSX element with the violation. */
    nodeId: string
}

/** An accessibility (WCAG) violation on a specific node. */
export interface DBOMA11yViolation {
    /** Flint rule identifier (e.g. 'A11Y-001', 'A11Y-007'). */
    ruleId: string
    /** Human-readable violation message. */
    message: string
}
...
export interface DBOMOverride {
    /** The data-flint-id of the overridden element. */
    nodeId: string
 ...
 * established via `flint baseline:set` or the Glass baseline API.
 */
