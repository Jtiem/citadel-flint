/**
 * DBOM Generator Tests — flint-mcp/src/core/dbom/__tests__/dbom.test.ts
 *
...
function makeTmpProject(): string {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'flint-dbom-test-'))
    // Minimal .flint/ structure
...
export function Dashboard() {
    return (
        <section data-flint-id="dash-001" className="p-[32px] bg-[#aabbcc] text-lg">
            <img src="/chart.png" data-flint-id="chart-img" />
            <p data-flint-id="dash-text" className="font-semibold">Hello</p>
        </section>
    )
}
`
...
describe('DBOM generator — token inventory', () => {
    ...
    it('sets usedIn to an array', async () => {
        ...
        writeSrc(tmpDir, 'Red.tsx', `
            export function Red() {
                return <div data-flint-id="r1" className="bg-[#ff0000]">red</div>
            }
        `)
        const dbom = await generateDBOM(tmpDir)
...
describe('DBOM generator — overrides', () => {
    ...
    it('reads overrides from .flint/overrides.json', async () => {
        const overrides = [
            { flint_id: 'btn-001', property_key: 'style', property_value: 'color: red;', updated_at: Date.now() },
        ]
        fs.writeFileSync(
            path.join(tmpDir, '.flint', 'overrides.json'),
...
describe('DBOM generator — baseline', () => {
    ...
    it('includes baseline when violation_baselines.json exists', async () => {
        ...
        fs.writeFileSync(
            path.join(tmpDir, '.flint', 'violation_baselines.json'),
