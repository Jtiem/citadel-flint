/**
 * healOnAudit.test.ts — flint-mcp/src/__tests__/healOnAudit.test.ts
 *
 * Phase ING.3 integration tests for the optional `healOnAudit` parameter
 * on `flint_audit`.
 *
 * Test IDs covered:
 *   ING-18 — flint_audit({ filePath, healOnAudit: true }) → healed section present
 *           — flint_audit({ filePath })                   → no healed section
 *           — flint_audit({ filePath, healOnAudit: true }) with zero tokens → no-op
 *
 * Architecture note:
 ...
 *   5. `FLINT_AUDIT_TOOL` schema registers the property as optional boolean.
 */
...
import { handleFlintAudit, FLINT_AUDIT_TOOL } from '../tools/audit.js'
 ...
 *   - No .flint/design-tokens.json is found → empty token list (zero-tokens case).
 *   - Policy uses defaults (Mithril + A11y enabled).
 */
const CONFIG_NO_TOKENS: FlintConfig = {
    ...DEFAULT_CONFIG,
    projectRoot: '/tmp/__flint_ing18_no_tokens__',
}
 ...
 */
const tmpProjectRoot = path.join(os.tmpdir(), '__flint_ing18_with_tokens__')
const CONFIG_WITH_TOKENS: FlintConfig = {
    ...DEFAULT_CONFIG,
...
const Card = () => (
  <div data-flint-id="card-root" className="bg-[#3B82F6] p-4">
    <span>Content</span>
  </div>
)
export default Card
...
async function audit(
    ...
    return handleFlintAudit({ source, filePath: 'Component.tsx', ...extra }, config)
}

// ── Test suite ─────────────────────────────────────────────────────────────────

describe('ING-18: healOnAudit parameter on flint_audit', () => {
    ...
    describe('Scenario 3 — zero tokens → heal is no-op, standard audit runs', () => {
        ...
        it('with tokens present: audit detects drift, healOnAudit still skipped in MCP mode', async () => {
            ...
            const flintDir = path.join(tmpProjectRoot, '.flint')
    ...
    describe('FLINT_AUDIT_TOOL schema', () => {
        it('schema properties include healOnAudit', () => {
            const props = FLINT_AUDIT_TOOL.inputSchema.properties as Record<string, unknown>
            expect(props).toHaveProperty('healOnAudit')
        })

        it('healOnAudit schema type is boolean', () => {
            const props = FLINT_AUDIT_TOOL.inputSchema.properties as Record<string, { type: string }>
            expect(props.healOnAudit.type).toBe('boolean')
        })

        it('healOnAudit is not in required (optional parameter)', () => {
            const required = FLINT_AUDIT_TOOL.inputSchema.required as string[]
