/**
 * Phase ING.3 — flint_audit healOnAudit parameter tests
 * flint-mcp/src/__tests__/audit-heal.test.ts
 *
 * Test ID: ING-18
 *
 * Validates the schema extension and graceful-degradation behaviour of the
 * optional `healOnAudit` parameter on `handleFlintAudit`.
 *
...
/**
 * Minimal FlintConfig that enables both Mithril and A11y auditing with default
...
const config: FlintConfig = {
    ...DEFAULT_CONFIG,
    projectRoot: '/tmp/__flint_test_nonexistent__',
}
...
const Drifted = () => (
  <div data-flint-id="drifted-root" className="bg-[#ff0000]">
...
async function audit(source: string, extra: Partial<AuditArgs> = {}): Promise<AuditResult> {
    return handleFlintAudit({ source, filePath: 'Test.tsx', ...extra }, config)
}

// ── Test suite ─────────────────────────────────────────────────────────────────

describe('ING-18: flint_audit healOnAudit parameter — graceful degradation in headless MCP mode', () => {
    ...
    // ── 4. Schema sanity — FLINT_AUDIT_TOOL includes the new property ─────────

    describe('4. FLINT_AUDIT_TOOL schema includes healOnAudit property', () => {
        it('FLINT_AUDIT_TOOL.inputSchema.properties contains healOnAudit', () => {
