...
/** Create a hermetic temp directory with a .flint sub-directory. */
function makeProjectDir(): string {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'flint-ctx-push-test-'))
...
/** Write .flint/context.json in the given project root. */
function writeContext(
    ...
    fs.writeFileSync(
        path.join(projectRoot, '.flint', 'context.json'),
        JSON.stringify(ctx, null, 2),
        'utf-8',
    )
}

/** Write .flint/design-tokens.json as an array of N dummy tokens. */
...
/** Read .flint/mcp-events.jsonl and return parsed events. */
...
describe('ACX-13: ContextPushManager — graceful handling of missing context.json', () => {
    it('start() does not throw when .flint/context.json does not exist', () => {
        const projectRoot = makeProjectDir()
    ...
    it('checkContextNow() does not throw when .flint directory does not exist', () => {
        const projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'flint-no-dot-flint-'))
        // Intentionally do NOT create .flint/
