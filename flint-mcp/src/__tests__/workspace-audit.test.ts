 ...
 * flint-mcp/src/__tests__/workspace-audit.test.ts
 *
...
function auditSource(filePath: string, source: string, tokens: DesignToken[]): FileAuditResult {
    ...
    // All A11y violations are blocking/critical by convention in Flint
 ...
 * Includes data-flint-id so the linter can key the warning.
 */
const COLOR_DRIFT_SOURCE = `
const Drifted = () => (
    <div data-flint-id="drifted-bg" className="bg-[#ff0000]">
 ...
 * Uses data-flint-id to ensure the warning is keyed to this node.
 */
const CRITICAL_DRIFT_SOURCE = `
const CriticalDrift = () => (
    <div data-flint-id="crit-bg" className="bg-[#ff0000]">
...
const Mixed = () => (
    <div data-flint-id="mixed-bg" className="bg-[#ff0000]">
        <img src="photo.png" />
    </div>
)
...
const AmberDrift = () => (
    <div data-flint-id="amber-bg" className="bg-[#4090f7]">
