 ...
 *   .flint-context/architect-reviews/TestStrategy-Plan.md
 *
 * | # | What it proves                                    |
 * |---|---------------------------------------------------|
 * | 11| Batch mutations preserve data-flint-id           |
 ...
 * | 18| ID injection produces unique data-flint-id attrs |
 */
...
// ── Test 11: Batch mutations preserve data-flint-id ─────────────────────────
//
// Applies updateClassName mutations to three elements that each carry a
// data-flint-id. After the batch, re-traverses the resulting AST and asserts
// that every original flint ID is still present and that no ID is duplicated.

describe('Test 11 — batch mutations preserve data-flint-id', () => {
    const SOURCE = `
export default function Card() {
  return (
    <div data-flint-id="card-root" className="bg-white p-4">
      <h2 data-flint-id="card-title" className="text-zinc-900 font-bold">Hello</h2>
      <p data-flint-id="card-body" className="text-zinc-600 text-sm">World</p>
    </div>
  )
}
`

    it('preserves every original data-flint-id after a batch of updateClassName mutations', () => {
        ...
        // Every original data-flint-id must still be present
        expect(result).toContain('data-flint-id="card-root"')
        expect(result).toContain('data-flint-id="card-title"')
        expect(result).toContain('data-flint-id="card-body"')

        // No duplicate flint IDs — each appears exactly once
    ...
    it('data-flint-id is preserved even when className attribute is absent before the mutation', () => {
        const source = `
export default function Badge() {
  return <span data-flint-id="badge-1">label</span>
}
`
        const ast = parseCode(source)
        ...
        expect(result).toContain('data-flint-id="badge-1"')
    })
})
...
export default function Button() {
  return (
    <button data-flint-id="btn-primary" className="bg-blue-500 text-white px-4 py-2">
      Click me
    </button>
  )
}
`
    ...
    it('applying the inverse of an updateClassName restores the original class string', () => {
        ...
        // data-flint-id must have survived both passes
        expect(restoredCode).toContain(`data-flint-id="${nodeId}"`)
    })
...
export default function List() {
  return (
    <ul data-flint-id="list-root">
      <li data-flint-id="item-a">Alpha</li>
      <li data-flint-id="item-b">Beta</li>
    </ul>
  )
}
`
        ...
        const mutatedCode = generateCode(ast)
        expect(mutatedCode).not.toContain('data-flint-id="item-a"')
        ...
        expect(restoredCode).toContain('data-flint-id="item-a"')
        expect(restoredCode).toContain('data-flint-id="item-b"')
    })
})
...
describe('Test 13 — FileTransactionManager large-content round-trip', () => {
    ...
    beforeEach(async () => {
        dir = join(tmpdir(), `flint-di-13-${randomUUID()}`)
    ...
    it('writes a large TSX component (>10 KB) and reads back identical content', async () => {
        ...
        const repeat = (n: number) =>
            Array.from(
                { length: n },
                (_, i) =>
                    `  <div data-flint-id="item-${i}" className="p-4 bg-white border rounded">` +
...
export default function LargeComponent() {
  return (
    <div data-flint-id="root" className="flex flex-col gap-2">
${repeat(200)}
    </div>
  )
}
...
export default function Grid() {
  return (
    <div data-flint-id="grid-root" className="grid grid-cols-3 gap-4">
      ${Array.from({ length: 30 }, (_, i) => `<div data-flint-id="cell-${i}" className="p-2">{${i}}</div>`).join('\n      ')}
    </div>
  )
}
`
        ...
        for (let i = 0; i < 30; i++) {
            expect(code).toContain(`data-flint-id="cell-${i}"`)
        }
    })
})
...
describe('Test 14 — GitManager shadow commit → gitShow content round-trip', () => {
    ...
    beforeEach(async () => {
        dir = join(tmpdir(), `flint-di-14-${randomUUID()}`)
        await mkdir(dir, { recursive: true })
    })
    ...
    it('committed TSX source including Unicode characters is retrieved byte-for-byte by git show', async () => {
        ...
        await execFileAsync('git', ['config', 'user.email', 'test@flint.test'], { cwd: dir })
        await execFileAsync('git', ['config', 'user.name', 'Flint Test'], { cwd: dir })
...
`
        ...
        await execFileAsync(
            'git',
            ['commit', '-m', 'flint:sync:test', '--allow-empty'],
            { cwd: dir },
        )
        ...
        // data-flint-id must be present
        expect(stdout).toContain('data-flint-id="intl-btn"')
    })
})
...
describe('Test 15 — GovernanceEventService all-fields round-trip fidelity', () => {
    it('stores and retrieves every field including all optional fields populated', () => {
        ...
        expect(event.nodeId).toBe('flint-card-root')
...
export default function Hero() {
  return (
    <section data-flint-id="hero-root" className="bg-[#1a1a2e] text-white min-h-screen flex items-center">
      <div data-flint-id="hero-inner" className="container mx-auto px-8">
        <h1 data-flint-id="hero-title" className="text-[72px] font-bold leading-[1.1]">
          The future of UI governance
        </h1>
        <p data-flint-id="hero-sub" className="text-[20px] text-[#a0a0b0] mt-6 max-w-[600px]">
          Flint enforces your design system deterministically at the AST level.
        </p>
      </div>
    </section>
  )
...
`.trim()
        ...
        expect(entry.approvedBy).toBe('flint-autofix')
...
export default function Brand() {
  return (
    <div data-flint-id="brand-box" className="bg-[#3b82f6]">
      Brand colour
    </div>
  )
}
...
export default function Safe() {
  return <div data-flint-id="safe-node" className="bg-blue-500 text-white">OK</div>
}
`
        const ast = parseCode(source)
...
// ── Test 18: injectFlintIds produces unique IDs ──────────────────────────────
//
// injectFlintIds lives in the Glass renderer process (src/core/ast-parser.ts)
...
// data-flint-id formatted as "tagName:line:col".

describe('Test 18 — ID injection produces unique data-flint-id attributes', () => {
    /**
     * Mirrors the injectFlintIds algorithm from src/core/ast-parser.ts.
     * Mutates ast in-place. Idempotent: elements already carrying a
     * data-flint-id are skipped.
     */
    function injectFlintIds(ast: File): void {
        traverse(ast, {
            JSXElement(path) {
                ...
                opening.attributes.push(
                    t.jsxAttribute(t.jsxIdentifier('data-flint-id'), t.stringLiteral(flintId)),
                )
            },
        })
    }

    it('injects data-flint-id on every JSXElement that lacks one', () => {
        const source = `
...
`
        ...
        // Every element must now carry a data-flint-id
        expect(code).toContain('data-flint-id="main:')
        expect(code).toContain('data-flint-id="header:')
        expect(code).toContain('data-flint-id="h1:')
        expect(code).toContain('data-flint-id="section:')
        expect(code).toContain('data-flint-id="p:')
    })
...
`
    ...
    it('is idempotent — running injectFlintIds twice does not create duplicate attributes', () => {
        const source = `
export default function Card() {
  return (
    <div>
...
`
        const ast = parseCode(source)
        injectFlintIds(ast)
        injectFlintIds(ast)  // second call must be a no-op

        const ids: string[] = []
        traverse(ast, {
            JSXElement(path) {
                const flintAttrs = path.node.openingElement.attributes.filter(
                    (attr) =>
                        t.isJSXAttribute(attr) &&
                        t.isJSXIdentifier((attr as t.JSXAttribute).name, { name: 'data-flint-id' }),
                )
                for (const attr of flintAttrs) {
        ...
        // 2 elements: div + span — each with exactly one data-flint-id
...
`
    ...
    it('pre-existing data-flint-id is preserved and not overwritten', () => {
        const source = `
export default function Widget() {
  return (
    <div data-flint-id="my-stable-id" className="p-4">
      <span>text</span>
    </div>
  )
}
`
        ...
        expect(code).toContain('data-flint-id="my-stable-id"')

        // Must not gain a second data-flint-id attribute
        const divMatches = code.match(/data-flint-id="my-stable-id"/g) ?? []
