 ...
 *   CX1-23 through CX1-25 — flint_ast_mutate summary (inline in server.ts)
 */
...
import {
    ...
    handleFlintAudit,
    handleFlintAuditBatch,
    FLINT_AUDIT_TOOL,
} from '../tools/audit.js'
import type { AuditResult, BatchAuditResult } from '../tools/audit.js'
import { generateFixSummary, handleFlintFix } from '../tools/fix.js'
...
function makeTempDir(): string {
    return fs.mkdtempSync(path.join(os.tmpdir(), 'flint-cx1-test-'))
}
...
describe('CX1-11: loadProjectContext — no history file', () => {
    ...
    beforeEach(() => {
        tmpDir = makeTempDir()
        makeFlintDir(tmpDir) // .flint dir exists but no debt-history.json
    })
    ...
    it('does not throw when .flint directory does not exist', () => {
...
describe('CX1-13: loadProjectContext — empty array in history', () => {
    ...
    beforeEach(() => {
        tmpDir = makeTempDir()
        const flintDir = makeFlintDir(tmpDir)
        writeDebtHistory(flintDir, []) // empty array
    })
...
// ── CX1-14: handleFlintAudit includes summary field ──────────────────────────

describe('CX1-14: handleFlintAudit includes summary field', () => {
    let tmpDir: string
...
// ── CX1-15: handleFlintAudit includes project_context when history exists ────

describe('CX1-15: handleFlintAudit includes project_context when history exists', () => {
    let tmpDir: string

    beforeEach(() => {
        tmpDir = makeTempDir()
        const flintDir = makeFlintDir(tmpDir)
        writeDebtHistory(flintDir, [{ score: 75, grade: 'C', totalViolations: 20 }])
    })
...
// ── CX1-16: handleFlintAudit omits project_context when no history ────────────

describe('CX1-16: handleFlintAudit omits project_context when no history', () => {
    let tmpDir: string

    beforeEach(() => {
        tmpDir = makeTempDir()
        makeFlintDir(tmpDir) // no debt-history.json
    })
...
// ── CX1-17: handleFlintFix includes summary and dryRun fields ────────────────

describe('CX1-17: handleFlintFix includes summary and dryRun fields', () => {
    let tmpDir: string
...
// ── CX1-18: handleFlintFix with dryRun: true — summary says DRY RUN ──────────

describe('CX1-18: handleFlintFix dryRun=true — summary says DRY RUN', () => {
    let tmpDir: string
...
// ── CX1-19: handleFlintFix with dryRun: true — still returns fixedSource ─────

describe('CX1-19: handleFlintFix dryRun=true — still returns fixedSource', () => {
    let tmpDir: string
...
describe('CX1-20: BatchAuditResult.summary.text is populated', () => {
    ...
    it('summary.text is a non-empty string', async () => {
        const config = makeConfig(tmpDir)
        const result: BatchAuditResult = await handleFlintAuditBatch(
...
describe('CX1-22: Server instructions field', () => {
    it('server.ts imports Server from MCP SDK and sets instructions', async () => {
        ...
        // in flint-mcp/src/server.ts contains the onboarding hint text.
        ...
        if (fs.existsSync(serverPath)) {
            ...
            expect(content).toContain('workflow-guide')
...
describe('CX1-23 through CX1-25: flint_ast_mutate summary generation (inline logic)', () => {
...
describe('loadProjectContext performance budget < 5ms', () => {
    ...
    beforeEach(() => {
        tmpDir = makeTempDir()
        const flintDir = makeFlintDir(tmpDir)
        writeDebtHistory(flintDir, [{ score: 80, grade: 'B', totalViolations: 10 }])
    })
