 ...
 *
 * These tests prove that data transits Flint's process boundaries faithfully:
...
import type { FlintAnnotation } from '../core/annotations/types.js'
...
describe('Test 20: buildComplianceSummary — correct ComplianceSummary shape', () => {
    ...
    it('byAuthority has Flint Design System count >= 1 for a Mithril violation', () => {
        const summary = buildComplianceSummary([
            { ruleId: 'A11Y-001', severity: 'critical' },
            { ruleId: 'MITHRIL-COL', severity: 'amber' },
        ])

        expect(summary.byAuthority['Flint Design System']).toBeGreaterThanOrEqual(1)
    })
...
// ── Test 21: Context Flint file is valid JSON ────────────────────────────────

describe('Test 21: .flint/context.json — valid JSON, correct top-level keys', () => {
    let tmpDir: string
    ...
    it('round-trips from write to JSON.parse without errors', () => {
        // Reproduce the exact shape that useContextSync assembles (FlintContext)
        const ctx = {
            timestamp: Date.now(),
        ...
        const contextPath = path.join(tmpDir, '.flint', 'context.json')
        ...
        // Verify shape expected by MCP server reading flint://context
...
describe('Test 22: .flint/annotations.json — round-trip fidelity', () => {
    let tmpDir: string
    ...
    it('annotations round-trip through JSON with all required fields intact', () => {
        const annotations: FlintAnnotation[] = [
            {
                id: 'anno-001',
                nodeId: 'card-root',
        ...
        const annotationsPath = path.join(tmpDir, '.flint', 'annotations.json')
        fs.writeFileSync(annotationsPath, JSON.stringify(annotations, null, 2), 'utf-8')

        const raw = fs.readFileSync(annotationsPath, 'utf-8')
        let parsed: FlintAnnotation[]
        expect(() => { parsed = JSON.parse(raw) as FlintAnnotation[] }).not.toThrow()
    ...
    it('each annotation has id, targetId (nodeId), content (body), author, status', () => {
        ...
        const parsed = JSON.parse(fs.readFileSync(annotationsPath, 'utf-8')) as FlintAnnotation[]
        const a = parsed[0]!

        expect(typeof a.id).toBe('string')
        // The field is nodeId in the FlintAnnotation schema
...
// ── Test 23: flint://dashboard resource returns DashboardData ────────────────
...
// ── Test 24: flint://violations for a specific file ─────────────────────────
...
export const Drifted = () => (
  <div data-flint-id="drift-root" className="bg-[#ff00ff]">
...
// ── Test 25: flint://capabilities lists all tools ────────────────────────────
