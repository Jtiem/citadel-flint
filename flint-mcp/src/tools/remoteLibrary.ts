/**
 * remoteLibrary — flint-mcp/src/tools/remoteLibrary.ts
 *
 * Implements flint_add_remote_library: fetch a component library manifest
 * from a GitHub repository URL, merge it into the in-memory RAG cache, and
 * persist metadata to .flint/remote-libraries.json.
 */
...
export const FLINT_ADD_REMOTE_LIBRARY_TOOL = {
    name: 'flint_add_remote_library',
    description:
        'Fetch and cache a component library from a GitHub repository URL. ' +
        'Makes all components instantly available via flint_query_registry.',
    inputSchema: {
        ...
        properties: {
            ...
            manifestPath: {
                type: 'string',
                description:
                    'Path within the repository to flint-manifest.json ' +
                    '(default: "flint-manifest.json").',
            },
            alias: {
                type: 'string',
 ...
 *
 * Accepted shapes:
 *   1. { "components": { "Button": { ... } } }       — flint-manifest.json
 *   2. { "flint": { "components": { ... } } }       — package.json flint section
...
function extractComponents(json: unknown): Record<string, ComponentEntry> {
    ...
    // Shape 1 — flint-manifest.json
    if (obj.components && typeof obj.components === 'object') {
        return obj.components as Record<string, ComponentEntry>;
    }

    // Shape 2 — package.json with flint section
    if (
        obj.flint &&
        typeof obj.flint === 'object' &&
        (obj.flint as Record<string, unknown>).components
    ) {
        return (obj.flint as Record<string, unknown>).components as Record<
            string,
            ComponentEntry
        >;
    }
...
const REMOTE_LIBRARIES_FILE = '.flint/remote-libraries.json';
 ...
 *  2. Fetch flint-manifest.json from raw.githubusercontent.com.
 ...
 *  7. Persist metadata to .flint/remote-libraries.json.
 *  8. Return summary result.
 */
export async function handleFlintAddRemoteLibrary(
    args: AddRemoteLibraryArgs,
): Promise<AddRemoteLibraryResult> {
    const {
        githubUrl,
        branch = 'main',
        manifestPath = 'flint-manifest.json',
        projectRoot,
    } = args;
    ...
    try {
        ...
        } else if (res.status === 404 && manifestPath === 'flint-manifest.json') {
    ...
    if (rawJson === null) {
        throw new Error(
            ...
            `Ensure the repository contains a flint-manifest.json or package.json ` +
