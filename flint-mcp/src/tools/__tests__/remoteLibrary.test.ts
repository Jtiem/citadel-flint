 ...
 *   - handleFlintAddRemoteLibrary — happy path + 404 fallback + error path
 *   - Persistence to .flint/remote-libraries.json
...
/** Build a minimal flint-manifest.json payload */
...
/** Read and parse .flint/remote-libraries.json from a project root */
function readLibrariesFile(projectRoot: string) {
    const fp = path.join(projectRoot, '.flint', 'remote-libraries.json');
...
describe('buildRawUrl', () => {
    it('builds the correct raw URL', () => {
        const url = buildRawUrl('shadcn-ui', 'ui', 'main', 'flint-manifest.json');
        expect(url).toBe(
            'https://raw.githubusercontent.com/shadcn-ui/ui/main/flint-manifest.json',
        );
    });
...
describe('handleFlintAddRemoteLibrary', () => {
    ...
    it('fetches manifest, populates cache, writes file, returns result', async () => {
        ...
        const result = await handleFlintAddRemoteLibrary({
    ...
    it('uses custom branch', async () => {
        ...
        await handleFlintAddRemoteLibrary({
            githubUrl: 'https://github.com/shadcn-ui/ui',
            branch: 'canary',
            projectRoot,
        });
    ...
    it('uses a custom manifestPath', async () => {
        ...
        await handleFlintAddRemoteLibrary({
            ...
            manifestPath: 'packages/ui/flint-manifest.json',
            projectRoot,
        });

        expect(mockFetch).toHaveBeenCalledWith(
            expect.stringContaining('packages/ui/flint-manifest.json'),
        );
    });

    // ── 404 fallback to package.json ──────────────────────────────────────────

    it('falls back to package.json when flint-manifest.json returns 404', async () => {
        const mockFetch = vi.fn()
            // First call (flint-manifest.json) → 404
    ...
    it('creates the .flint directory if it does not exist', async () => {
        vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
            ok: true,
            json: async () => ({ components: FIXTURE_COMPONENTS }),
        }));
        ...
        expect(fs.existsSync(flintDir)).toBe(false);
