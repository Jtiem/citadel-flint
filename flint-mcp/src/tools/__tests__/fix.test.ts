/**
 * Tests for handleFlintFix — flint-mcp/src/tools/__tests__/fix.test.ts
 *
 * Covers:
...
function makeTmpDir(): string {
    return fs.mkdtempSync(path.join(os.tmpdir(), 'flint-fix-test-'))
}
...
/**
 * Build a minimal FlintConfig with the given tokens written into
 * .flint/design-tokens.json inside a temp directory.
 */
function makeConfig(tokens: DesignToken[]): { config: FlintConfig; tmpDir: string } {
...
describe('handleFlintFix — no violations', () => {
    it('returns source unchanged with fixesApplied: 0 when there are no violations', async () => {
        const source = `
import React from 'react'
export const Clean = () => (
  <div data-flint-id="root" className="bg-zinc-900 text-white p-4">
    Hello
  </div>
)
`
        ...
        try {
            const result = await handleFlintFix({ source, filePath: '/src/Clean.tsx' }, config)
...
`
        ...
        try {
            const result = await handleFlintFix({ source, filePath: '/src/Comp.tsx' }, config)
...
describe('handleFlintFix — color drift in className', () => {
...
export const Drifted = () => (
  <div data-flint-id="root" className="bg-[#ff0000] p-4">
    Red
  </div>
)
`
...
export const Matched = () => (
  <div data-flint-id="root" className="bg-[#ff00ff]">
    Magenta
  </div>
)
`
        ...
        try {
            const result = await handleFlintFix({ source, filePath: '/src/Matched.tsx' }, config)
            expect(result.fixesApplied).toBe(0)
...
export const RgbDrifted = () => (
  <div data-flint-id="root" className="text-[rgb(255,0,0)]">
    Red text
  </div>
)
`
...
describe('handleFlintFix — color drift in inline style object', () => {
    it('replaces a hardcoded color in style={{ color }} with a CSS var', async () => {
        const source = `
import React from 'react'
export const Styled = () => (
  <div data-flint-id="root" style={{ color: '#ff0000', padding: '8px' }}>
    Inline
  </div>
)
`
...
export const BgStyled = () => (
  <section data-flint-id="section" style={{ backgroundColor: '#ff0000' }}>
    Section
  </section>
)
`
...
describe('handleFlintFix — multiple violations in one pass', () => {
    it('fixes multiple hardcoded class colors across different elements', async () => {
        const source = `
import React from 'react'
export const Multi = () => (
  <div data-flint-id="outer" className="bg-[#ff0000]">
    <span data-flint-id="inner" className="text-[#ff0000] font-bold">
      Multi
    </span>
  </div>
)
`
        const { config, tmpDir } = makeConfig([DARK_NEUTRAL_TOKEN])
        try {
            const result = await handleFlintFix({ source, filePath: '/src/Multi.tsx' }, config)
...
`
        const { config, tmpDir } = makeConfig([DARK_NEUTRAL_TOKEN])
        try {
            const result = await handleFlintFix({ source, filePath: '/src/Both.tsx' }, config)
            expect(result.fixesApplied).toBeGreaterThanOrEqual(2)
            expect(result.fixedSource).not.toContain('#ff0000')
        } finally {
            rmTmpDir(tmpDir)
...
export const TokenSpaced = () => (
  <div data-flint-id="root" className="p-[16px]">
    Exact
  </div>
)
`
        const { config, tmpDir } = makeConfig([SPACING_TOKEN])
        try {
            const result = await handleFlintFix({ source, filePath: '/src/TokenSpaced.tsx' }, config)
