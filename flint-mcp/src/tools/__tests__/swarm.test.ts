/**
 * Tests for handleFlintSwarmAuditFix — flint-mcp/src/tools/__tests__/swarm.test.ts
 *
 * Covers:
...
import { handleFlintSwarmAuditFix } from '../swarm.js'
...
function makeTmpDir(): string {
    return fs.mkdtempSync(path.join(os.tmpdir(), 'flint-swarm-test-'))
}
...
function buildConfig(projectRoot: string, tokens: DesignToken[] = []): FlintConfig {
...
export const Clean = () => (
  <button data-flint-id="clean-btn" aria-label="Submit" className="bg-zinc-900 p-4 text-white">
    Submit
  </button>
)
`
...
describe('handleFlintSwarmAuditFix — empty glob match', () => {
    let tmpDir: string
    ...
    it('returns zero counts when no files match the glob', async () => {
        ...
        const report = await handleFlintSwarmAuditFix(
...
describe('handleFlintSwarmAuditFix — all files violation-free', () => {
    let tmpDir: string
...
describe('handleFlintSwarmAuditFix — audit-only mode', () => {
    let tmpDir: string
...
describe('handleFlintSwarmAuditFix — autoFix mode', () => {
    let tmpDir: string
    ...
    it('writes fixed source to disk', async () => {
        const config = buildConfig(tmpDir, [DARK_NEUTRAL_TOKEN])
        await handleFlintSwarmAuditFix(
            { glob: 'src/**/*.tsx', projectRoot: tmpDir, autoFix: true },
            config,
        )
...
describe('handleFlintSwarmAuditFix — dryRun mode', () => {
    let tmpDir: string
    let driftedPath: string
...
describe('handleFlintSwarmAuditFix — SwarmReport shape', () => {
    let tmpDir: string
