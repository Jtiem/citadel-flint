 ...
 * bridge-mcp/src/__tests__/safety-promises.test.ts
 *
 * 10 tests that prove Bridge's governance promises hold end-to-end.
 ...
 *   5  — bridge_audit returns structured violations (ruleIds + severity)
 *   6  — bridge_fix auto-fixes and re-audit passes (token replacement)
 *   7  — bridge_debt_report returns accurate score (healthScore, grade, byFile, topRules)
 *   8  — bridge_accessibility_report returns WCAG breakdown (criterionResults, compliancePercent)
 *   9  — bridge_audit_report enriches with provenance (sourceAuthority, regulatoryReference)
...
describe('Test 1: Color drift gets caught', () => {
    it('flags bg-[#ff0000] as a MITHRIL-COL violation with amber or critical severity', () => {
        ...
        const source = `
            const C = () => (
                <div data-bridge-id="node-col" className="bg-[#ff0000]">Hello</div>
            )
        `
    ...
    it('does not flag a class that exactly matches a token value', () => {
        ...
        const source = `
            const C = () => (
                <div data-bridge-id="node-clean" className="bg-[#3b82f6]">Hello</div>
            )
        `
        const ast = parseTSX(source)
...
// ── Test 5: bridge_audit_report returns structured violations ──────────────────

describe('Test 5: bridge_audit returns structured violations', () => {
    it('returns violations array with ruleIds and severity for multi-violation TSX', () => {
        ...
        const source = `
            const C = () => (
                <div data-bridge-id="node-multi" className="bg-[#ff0000]">
                    <img src="photo.png" />
                </div>
            )
        `
...
// ── Test 6: bridge_fix auto-fixes and re-audit passes ─────────────────────────

describe('Test 6: bridge_fix auto-fixes and re-audit passes', () => {
    it('replacing arbitrary hex with token class reduces violation count to zero', () => {
        ...
        // This tests the "fix" by simulating what bridge_fix does:
        ...
        const violatingSource = `
            const C = () => (
                <div data-bridge-id="node-fix" className="bg-[#ff0000]">Hello</div>
            )
        `
        ...
        const fixedSource = `
            const C = () => (
                <div data-bridge-id="node-fix" className="bg-zinc-900">Hello</div>
            )
        `
    ...
    it('token that exactly matches passes zero-violation re-audit', () => {
        ...
        const fixedSource = `
            const C = () => (
                <div data-bridge-id="node-exact" className="bg-[#3b82f6]">Hello</div>
            )
        `
...
// ── Test 7: bridge_debt_report returns accurate score ─────────────────────────

describe('Test 7: bridge_debt_report returns accurate score', () => {
    let tmpDir: string

    beforeAll(() => {
        // Create isolated temp project with .bridge directory and 2 TSX files
        tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'bridge-debt-test-'))
        const bridgeDir = path.join(tmpDir, '.bridge')
        fs.mkdirSync(bridgeDir, { recursive: true })

        // Write design-tokens.json (empty — we don't need tokens for these files)
        fs.writeFileSync(path.join(bridgeDir, 'design-tokens.json'), '[]', 'utf-8')
...
`
        ...
        const violatingFile = `
const Violated = () => (
    <div data-bridge-id="v1" className="bg-[#ff0000] p-[7px]">
        <img src="photo.jpg" />
    </div>
)
...
// ── Test 8: bridge_accessibility_report returns WCAG breakdown ─────────────────

describe('Test 8: bridge_accessibility_report returns WCAG breakdown', () => {
...
// ── Test 9: bridge_audit_report enriches with provenance ──────────────────────

describe('Test 9: bridge_audit_report enriches violations with provenance', () => {
    it('every violation in the response has sourceAuthority and regulatoryReference', () => {
        const source = `
            const C = () => (
                <div data-bridge-id="node-prov" className="bg-[#ff0000]">
                    <img src="photo.png" />
                </div>
            )
        `
    ...
    it('Mithril violations map to Bridge Design System authority', () => {
        const source = `
            const C = () => (
                <div data-bridge-id="node-mith" className="bg-[#ff0000]">Hello</div>
            )
        `
        ...
        for (const v of mithrilViolations) {
            expect(v.provenance.sourceAuthority).toBe('Bridge Design System')
        }
    })
})
...
describe('Test 10: CIEDE2000 ΔE reference values', () => {
    ...
    function getDeltaE(hexColor: string, tokenHex: string): number {
        ...
        const source = `
            const C = () => (
                <div data-bridge-id="delta-node" className="bg-[${hexColor}]">x</div>
            )
        `
        const ast = parseTSX(source)
    ...
    it('same color produces ΔE = 0 (no violation)', () => {
        ...
        const source = `
            const C = () => (
                <div data-bridge-id="same-color" className="bg-[#3b82f6]">x</div>
            )
        `
        const ast = parseTSX(source)
        const token: DesignToken = {
    ...
    it('near-imperceptible difference (ΔE < 1.0) produces no violation', () => {
        ...
        const source = `
            const C = () => (
                <div data-bridge-id="near-match" className="bg-[#3b83f6]">x</div>
            )
        `
        const ast = parseTSX(source)
        const token: DesignToken = {
    ...
    it('brand-violation-level difference (ΔE > 2.0) triggers amber or critical violation', () => {
        ...
        const source = `
            const C = () => (
                <div data-bridge-id="brand-violation" className="bg-[#ff0000]">x</div>
            )
        `
        const ast = parseTSX(source)
        const token: DesignToken = {
