 ...
 *
 * Unit tests for bridge-mcp/src/core/projectContext.ts
 *
 * Tests:
 ...
 *   - Missing .bridge directory: returns null
...
function makeTempDir(): string {
    return fs.mkdtempSync(path.join(os.tmpdir(), 'bridge-ctx-test-'))
}

function makeBridgeDir(projectRoot: string): string {
    const bridgeDir = path.join(projectRoot, '.bridge')
    fs.mkdirSync(bridgeDir, { recursive: true })
    return bridgeDir
}

function writeHistory(bridgeDir: string, content: unknown): void {
    fs.writeFileSync(
...
describe('loadProjectContext — snapshots wrapper format', () => {
    ...
    it('returns null when snapshots array is empty', () => {
        const bridgeDir = makeBridgeDir(tmpDir)
        writeHistory(bridgeDir, { snapshots: [] })
...
describe('loadProjectContext — missing files', () => {
    ...
    it('returns null when debt-history.json does not exist', () => {
        makeBridgeDir(tmpDir) // .bridge exists but no history
        expect(loadProjectContext(tmpDir)).toBeNull()
    })

    it('returns null when .bridge directory does not exist', () => {
        // No .bridge dir created
        expect(loadProjectContext(tmpDir)).toBeNull()
    })
    ...
    it('does not throw for any missing path scenario', () => {
        ...
        expect(() => loadProjectContext(tmpDir)).not.toThrow() // no .bridge dir
    })
})
...
describe('loadProjectContext — empty or corrupt files', () => {
    ...
    it('returns null for empty file without throwing', () => {
        const bridgeDir = makeBridgeDir(tmpDir)
        fs.writeFileSync(path.join(bridgeDir, 'debt-history.json'), '', 'utf-8')
    ...
    it('returns null for whitespace-only file without throwing', () => {
        const bridgeDir = makeBridgeDir(tmpDir)
        fs.writeFileSync(path.join(bridgeDir, 'debt-history.json'), '   \n\t  ', 'utf-8')
        expect(() => loadProjectContext(tmpDir)).not.toThrow()
        expect(loadProjectContext(tmpDir)).toBeNull()
    })
    ...
    it('returns null when file contains null', () => {
        const bridgeDir = makeBridgeDir(tmpDir)
        fs.writeFileSync(path.join(bridgeDir, 'debt-history.json'), 'null', 'utf-8')
        expect(loadProjectContext(tmpDir)).toBeNull()
    })

    it('returns null when file contains a non-array non-object primitive (string)', () => {
        const bridgeDir = makeBridgeDir(tmpDir)
        fs.writeFileSync(path.join(bridgeDir, 'debt-history.json'), '"just a string"', 'utf-8')
        expect(loadProjectContext(tmpDir)).toBeNull()
    })
})
...
describe('loadProjectContext — incomplete entry fields', () => {
    ...
    it('returns null when entry is missing score and healthScore', () => {
        const bridgeDir = makeBridgeDir(tmpDir)
        writeHistory(bridgeDir, [{ grade: 'B', totalViolations: 10 }])
        expect(loadProjectContext(tmpDir)).toBeNull()
    })

    it('returns null when entry is missing grade', () => {
        const bridgeDir = makeBridgeDir(tmpDir)
        writeHistory(bridgeDir, [{ score: 75, totalViolations: 10 }])
        expect(loadProjectContext(tmpDir)).toBeNull()
    })

    it('returns null when entry is missing totalViolations and violationCount', () => {
        const bridgeDir = makeBridgeDir(tmpDir)
        writeHistory(bridgeDir, [{ score: 75, grade: 'C' }])
        expect(loadProjectContext(tmpDir)).toBeNull()
    })

    it('returns null when entry is null', () => {
        const bridgeDir = makeBridgeDir(tmpDir)
        writeHistory(bridgeDir, [null])
        expect(loadProjectContext(tmpDir)).toBeNull()
    })

    it('returns null for empty-string grade', () => {
        const bridgeDir = makeBridgeDir(tmpDir)
        writeHistory(bridgeDir, [{ score: 75, grade: '', totalViolations: 10 }])
        expect(loadProjectContext(tmpDir)).toBeNull()
    })
})
...
describe('loadProjectContext — return type shape', () => {
    ...
    it('returned object has exactly { health_score, grade, total_violations, blocked_files }', () => {
        const bridgeDir = makeBridgeDir(tmpDir)
        writeHistory(bridgeDir, [{ score: 80, grade: 'B', totalViolations: 5 }])
    ...
    it('blocked_files is always 0 (history lacks per-file data)', () => {
        const bridgeDir = makeBridgeDir(tmpDir)
        writeHistory(bridgeDir, [{ score: 70, grade: 'C', totalViolations: 20 }])
        const ctx = loadProjectContext(tmpDir)!
        expect(ctx.blocked_files).toBe(0)
    })
    ...
    it('health_score is a number, grade is a string, total_violations is a number', () => {
        const bridgeDir = makeBridgeDir(tmpDir)
        writeHistory(bridgeDir, [{ score: 95, grade: 'A', totalViolations: 0 }])
        const ctx = loadProjectContext(tmpDir)!
...
describe('loadProjectContext — performance budget < 5ms', () => {
    ...
    beforeEach(() => {
        ...
        writeHistory(bridgeDir, Array.from({ length: 100 }, (_, i) => ({
            score: i,
