...
import { handleBridgeAudit, handleBridgeAuditBatch } from '../tools/audit.js'
import { handleBridgeFix } from '../tools/fix.js'
import { handleBridgeSwarmAuditFix } from '../tools/swarm.js'
...
function makeTempDir(): string {
    return fs.mkdtempSync(path.join(os.tmpdir(), 'bridge-cx1-int-'))
}
...
// ── bridge_audit single-file response shape (CX.1) ────────────────────────────

describe('bridge_audit single-file: CX.1 response shape', () => {
    let tmpDir: string

    beforeEach(() => {
        tmpDir = makeTempDir()
        const bridgeDir = makeBridgeDir(tmpDir)
        writeDebtHistory(bridgeDir, [{ score: 80, grade: 'B', totalViolations: 8 }])
    })
...
// ── bridge_audit batch response shape (CX.1) ──────────────────────────────────

describe('bridge_audit batch: CX.1 response shape', () => {
    let tmpDir: string

    beforeEach(() => {
        tmpDir = makeTempDir()
        const bridgeDir = makeBridgeDir(tmpDir)
        writeDebtHistory(bridgeDir, [{ score: 75, grade: 'C', totalViolations: 15 }])
    ...
    it('batch result has summary.text and project_context', async () => {
        const config = makeConfig(tmpDir)
        const result = await handleBridgeAuditBatch(
...
// ── bridge_fix response shape (CX.1) ──────────────────────────────────────────

describe('bridge_fix: CX.1 response shape', () => {
    let tmpDir: string

    beforeEach(() => {
        tmpDir = makeTempDir()
        const bridgeDir = makeBridgeDir(tmpDir)
        writeDebtHistory(bridgeDir, [{ score: 70, grade: 'C', totalViolations: 25 }])
    })
...
// ── bridge_swarm_audit_fix response shape (CX.1) ─────────────────────────────

describe('bridge_swarm_audit_fix: CX.1 summary field', () => {
    let tmpDir: string
    ...
    it('swarm result has a summary field', async () => {
        const config = makeConfig(tmpDir)
        const result = await handleBridgeSwarmAuditFix(
...
describe('CX.1: project_context omitted gracefully when no history', () => {
    ...
    beforeEach(() => {
        tmpDir = makeTempDir()
        makeBridgeDir(tmpDir) // .bridge exists but no debt-history.json
    })
    ...
    it('bridge_audit: project_context absent when no debt history', async () => {
        const config = makeConfig(tmpDir)
        const result = await handleBridgeAudit(
            { source: CLEAN_SOURCE, filePath: 'Clean.tsx' },
            config,
    ...
    it('bridge_fix: project_context absent when no debt history', async () => {
        const config = makeConfig(tmpDir)
        const result = await handleBridgeFix(
            { source: CLEAN_SOURCE, filePath: 'Clean.tsx' },
            config,
