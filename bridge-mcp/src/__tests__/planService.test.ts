...
function makeTempDir(): string {
    return fs.mkdtempSync(path.join(os.tmpdir(), 'bridge-plan-test-'))
}

function setupBridgeDir(dir: string, opts: {
    debtHistory?: object
    tokens?: unknown[]
} = {}): string {
...
describe('generatePlan — accessibility-sweep (CX2-22)', () => {
    it('CX2-22: accessibility-sweep intent has bridge_accessibility_report as step 1', () => {
        ...
        expect(step1.tool).toBe('bridge_accessibility_report')
    })
})
...
describe('generatePlan — summary and successCriteria (CX2-29 to CX2-30)', () => {
    it('CX2-29: summary is a non-empty string containing the intent type name', () => {
        const intentTypes: Array<{ intent: string; expectedType: PlanIntentType }> = [
            { intent: 'Migrate all hardcoded colors to design tokens', expectedType: 'token-migration' },
            { intent: 'Run an accessibility sweep', expectedType: 'accessibility-sweep' },
            { intent: 'Audit the entire codebase for governance compliance', expectedType: 'full-governance-audit' },
            { intent: 'Sync tokens from Figma', expectedType: 'figma-sync' },
            { intent: 'Reduce design debt and improve health score', expectedType: 'debt-remediation' },
        ]

        for (const { intent, expectedType } of intentTypes) {
            const plan = generatePlan(intent)
            expect(typeof plan.summary).toBe('string')
            expect(plan.summary.length).toBeGreaterThan(0)
            expect(plan.summary).toContain(expectedType)
        }
    })
...
describe('estimateScope — with .bridge/ data (CX2-31)', () => {
    let tmpDir: string
    ...
    it('CX2-32: valid projectRoot with no .bridge/ directory -- all scope fields are null, no throw', () => {
        // tmpDir exists but has no .bridge/ subdirectory
        const noBridgeDir = path.join(tmpDir, 'no-bridge-subdir')
        fs.mkdirSync(noBridgeDir)

        expect(() => estimateScope(noBridgeDir)).not.toThrow()
        const scope = estimateScope(noBridgeDir)
