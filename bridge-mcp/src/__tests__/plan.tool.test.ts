 ...
 *
 * Integration tests for the bridge_plan MCP tool handler.
 *
 * Tests CX2-36 through CX2-45 from the contract test matrix.
 */

import { describe, it, expect } from 'vitest'
import { handleBridgePlan, BRIDGE_PLAN_TOOL } from '../../src/tools/plan.js'
import type { BridgePlanArgs, ExecutionPlan, ToolStep } from '../../src/tools/plan.js'
import type { BridgeConfig } from '../../src/core/config.js'
import { DEFAULT_CONFIG } from '../../src/core/config.js'

// ── Known Bridge tool names (from contract §13) ───────────────────────────────
...
const TEST_CONFIG: BridgeConfig = {
    ...DEFAULT_CONFIG,
    projectRoot: '/tmp/bridge-plan-test',
}

function callPlan(args: BridgePlanArgs, config: BridgeConfig = TEST_CONFIG) {
    return handleBridgePlan(args, config)
}

// ── CX2-36: Happy path ────────────────────────────────────────────────────────

describe('handleBridgePlan — happy path (CX2-36)', () => {
...
describe('handleBridgePlan — JSON validity (CX2-37)', () => {
...
describe('handleBridgePlan — ExecutionPlan shape (CX2-38)', () => {
...
describe('handleBridgePlan — missing intent (CX2-39)', () => {
...
describe('handleBridgePlan — glob parameter (CX2-40)', () => {
...
describe('handleBridgePlan — projectRoot parameter (CX2-41)', () => {
...
describe('handleBridgePlan — dry_run flag (CX2-42)', () => {
...
describe('handleBridgePlan — config.projectRoot fallback (CX2-43)', () => {
...
describe('handleBridgePlan — unknown intent fallback (CX2-44)', () => {
...
// ── CX2-45: Every ToolStep.tool is a known Bridge tool ───────────────────────

describe('handleBridgePlan — tool names are known Bridge tools (CX2-45)', () => {
    ...
    for (const intent of intentExamples) {
        it(`all ToolStep.tool names are known for: "${intent.slice(0, 50)}..."`, () => {
            ...
            for (const step of toolSteps) {
                expect(KNOWN_BRIDGE_TOOLS.has(step.tool)).toBe(true)
            }
        })
    }
})

// ── BRIDGE_PLAN_TOOL schema validation ────────────────────────────────────────

describe('BRIDGE_PLAN_TOOL definition', () => {
    it('has the correct tool name', () => {
        expect(BRIDGE_PLAN_TOOL.name).toBe('bridge_plan')
    })

    it('has a description', () => {
        expect(typeof BRIDGE_PLAN_TOOL.description).toBe('string')
        expect(BRIDGE_PLAN_TOOL.description.length).toBeGreaterThan(0)
    })

    it('requires "intent" as the only required field', () => {
        expect(BRIDGE_PLAN_TOOL.inputSchema.required).toContain('intent')
        expect(BRIDGE_PLAN_TOOL.inputSchema.required).toHaveLength(1)
    })

    it('schema has all 4 input properties', () => {
        const props = Object.keys(BRIDGE_PLAN_TOOL.inputSchema.properties)
