/**
 * bridge-sentinel — domain-configurable governance engine prompt.
 *
 * Exports:
 *   BRIDGE_SENTINEL_PROMPT_DEF  — the MCP prompt registration object used by server.ts
 *   getBridgeSentinelContent()  — assembles the full prompt string for a resolved domain
 *
 * Domain resolution order:
 *   1. Explicit `domain` argument when provided
 *   2. `.bridge/policy.json` → domain field when `projectRoot` is provided
 *   3. Default: "general"
 */
...
const BASE_BLOCK = `You are the Bridge Governance Sentinel — the compliance enforcement layer for AI-generated UI code.
...
To change the domain for this project, update .bridge/policy.json → { "domain": "${resolvedDomain}" }`
}
...
/**
 * Attempt to read the domain from `.bridge/policy.json` in the given project root.
...
function resolveDomain(domain: string | undefined, projectRoot: string | undefined): GovernanceDomain {
    ...
    if (domain && typeof domain === 'string' && domain.length > 0) {
        ...
        console.warn(
            `[bridge-sentinel] Unknown domain "${domain}" — falling back to "general". ` +
    ...
    if (projectRoot) {
        ...
        if (policyDomain) {
            ...
            console.warn(
                `[bridge-sentinel] policy.json declares unknown domain "${policyDomain}" — ` +
...
/**
 * Assemble the full Bridge Sentinel prompt string for the resolved governance domain.
 *
 * @param domain      - Explicit domain override. If undefined, reads from policy.json.
 * @param projectRoot - Project root path used to read .bridge/policy.json when domain is undefined.
