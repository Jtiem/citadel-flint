/**
 * Keyboard rules — bridge-mcp/src/core/a11y/rules/keyboard.ts
 *
...
const rule007: A11yRule = {
    ...
    visitElement(path, _context) {
        ...
        const elementId = getBridgeId(opening, `${tag}-tabindex-positive`)
...
const rule020: A11yRule = {
    ...
    visitElement(path, _context) {
        ...
        const elementId = getBridgeId(opening, `${tag}-click-no-keyboard`)
...
const rule021: A11yRule = {
    ...
    visitElement(path, _context) {
        ...
        const tag = getTagName(path) ?? 'element'
        const elementId = getBridgeId(opening, `${tag}-mouse-only`)
        ...
        if (classNameAttr) {
            ...
            if (hasFocusOutlineNone) {
                ...
                if (!hasFocusReplacement) {
                    const tag = getTagName(path) ?? 'element'
                    const elementId = getBridgeId(opening, `${tag}-no-focus-indicator`)
                    return {
                        ruleId: 'A11Y-022',
                        elementId,
                        message:
        ...
        if (styleAttr?.type === 'JSXAttribute' && styleAttr.value) {
            ...
            if (styleAttr.value.type === 'StringLiteral') {
                ...
                if (style.includes('outline') && (style.includes('none') || style.includes(': 0'))) {
                    const tag = getTagName(path) ?? 'element'
                    const elementId = getBridgeId(opening, `${tag}-style-no-focus`)
                    return {
                        ruleId: 'A11Y-022',
                        elementId,
                        message:
