/**
 * Structure rules — bridge-mcp/src/core/a11y/rules/structure.ts
 *
...
const rule008: A11yRule = {
    ...
    visitElement(path, _context) {
        ...
        const elementId = getBridgeId(opening, 'table-no-summary')
        return {
            ruleId: 'A11Y-008',
            elementId,
            message:
...
const rule009: A11yRule = {
    ...
    visitElement(path, _context) {
        ...
        const elementId = getBridgeId(opening, 'html-no-lang')
        return {
            ruleId: 'A11Y-009',
            elementId,
            message:
...
const rule010: A11yRule = {
    ...
    visitElement(path, context) {
        ...
        const opening = path.node.openingElement
        const elementId = getBridgeId(opening, `${tag}-level-skip`)
...
const rule015: A11yRule = {
    ...
    visitElement(path, _context) {
        ...
        const opening = path.node.openingElement
        const elementId = getBridgeId(opening, `${tag}-invalid-children`)
...
const rule016: A11yRule = {
    ...
    visitElement(path, _context) {
        ...
        const opening = path.node.openingElement
        const elementId = getBridgeId(opening, 'dl-invalid-children')
