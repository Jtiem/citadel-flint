/**
 * A11y Rule Runner — bridge-mcp/src/core/a11y/runner.ts
 *
...
export function auditSync(ast: BabelFile, options: RunnerOptions): A11yAuditResult {
    ...
    traverse(ast, {
        JSXElement(path) {
            ...
            if (effectiveRole) {
                context.elementRoles.set(bridgeId, effectiveRole)
            ...
            if (idVal) {
                context.idToElementMap.set(idVal, bridgeId)
            }
            ...
            if (classNameAttr) {
                ...
                context.colorContext.set(bridgeId, colors)
            }
