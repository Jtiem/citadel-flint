/**
 * Error Taxonomy — bridge-mcp/src/core/errorTaxonomy.ts
 *
 * CX.3: Canonical registry of all Bridge error codes with plain-language
...
export interface ErrorEntry {
    /** Stable Bridge error code, e.g. "BRIDGE-MITH-001". */
    code: string
    ...
    recovery: string
    /** The specification, standard, or Bridge commandment that mandates this rule. */
...
/**
 * Complete static registry of all Bridge error entries.
...
const REGISTRY: Record<string, ErrorEntry> = {
    ...
    'BRIDGE-MITH-001': {
        ...
        recovery:
            ...
            '(e.g. `bg-error-500`). Run `bridge_sync_tokens` to ensure your token set is current. ' +
            'If no token fits, add the new color to your design token file and re-run `bridge_fix`.',
        sourceAuthority: 'Bridge Commandment 2 (No Hallucinated Styling); Bridge Commandment 9 (CIEDE2000 Delta-E Logic)',
        regulatoryRef: 'CIEDE2000 ΔE > 2.0 threshold',
    },

    'BRIDGE-MITH-002': {
        code: 'BRIDGE-MITH-002',
        ...
        sourceAuthority: 'Bridge Commandment 2 (No Hallucinated Styling)',
    },

    'BRIDGE-MITH-003': {
        code: 'BRIDGE-MITH-003',
    ...
    'BRIDGE-A11Y-034': {
        ...
        recovery:
            ...
            'Bridge auto-fix can remove invalid attributes; re-add the corrected version.',
        sourceAuthority: 'WCAG 2.1 AA; WAI-ARIA 1.2',
        regulatoryRef: '§ 4.1.2 Name, Role, Value (Level A)',
    },

    'BRIDGE-A11Y-035': {
        code: 'BRIDGE-A11Y-035',
        ruleId: 'A11Y-035',
        category: 'a11y',
        severity: 'critical',
    ...
    'BRIDGE-A11Y-060': {
        ...
        recovery:
            ...
            'Use the WebAIM Contrast Checker or Bridge\'s token picker to find compliant color pairs. ' +
    ...
    'BRIDGE-SES-001': {
        ...
        category: 'session',
        severity: 'error',
        title: 'Duplicate Bridge Node IDs',
        explanation:
            'Bridge uses `data-bridge-id` attributes to track individual AST nodes across edits. ' +
        ...
        recovery:
            'Re-run `injectBridgeIds` on the affected file to regenerate unique IDs. ' +
            'Duplicate IDs typically arise from copy-paste operations that bypass the AST surgery layer. ' +
            'Use the Bridge undo system to restore the pre-duplicate state if needed.',
        sourceAuthority: 'Bridge Commandment 7 (ID Preservation)',
    },
    ...
    'BRIDGE-SES-002': {
        ...
        explanation:
            'Orphaned nodes are AST elements with `data-bridge-id` attributes that are not registered ' +
            'in the Bridge node index. ' +
        ...
        recovery:
            'Reload the file in Bridge to rebuild the node index from the current AST. ' +
    ...
    'BRIDGE-SES-003': {
        ...
        recovery:
            'Run `bridge_fix` with the import cleanup option to remove unused imports automatically. ' +
            'The Import Synthesizer (`synthesizeImports`) will remove stale entries when the next mutation batch is applied.',
        sourceAuthority: 'Bridge Commandment 1 (Code is Truth)',
    },
    ...
    'BRIDGE-SES-004': {
        ...
        category: 'session',
        severity: 'error',
        title: 'Missing Bridge IDs',
        explanation:
            'JSX elements without `data-bridge-id` attributes cannot be targeted by Bridge\'s mutation ' +
            'engine. ' +
            'They become invisible to the visual layer and cannot be selected, moved, or mutated via ' +
            'Bridge\'s AST surgery operations.',
        recovery:
            'Run `injectBridgeIds` on the affected file to assign IDs to all eligible JSX elements. ' +
            'This happens automatically on file load and after every structural mutation — if IDs are ' +
            'missing, the file may have been edited outside Bridge.',
        sourceAuthority: 'Bridge Commandment 7 (ID Preservation)',
    },
}
...
/**
 * Returns the ErrorEntry for the given Bridge error code, or null if not found.
