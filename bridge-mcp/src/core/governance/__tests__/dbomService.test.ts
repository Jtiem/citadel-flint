/**
 * DBOM Service Tests — bridge-mcp/src/core/governance/__tests__/dbomService.test.ts
 *
...
function makeTmpProject(): string {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'bridge-dbom-gov-'))
...
describe('DBOM governance — empty project', () => {
    ...
    it('returns a valid DBOM with version 1.0 for an empty project', async () => {
        ...
        expect(dbom.bridgeVersion).toBe('1.0.0')
...
describe('DBOM governance — CycloneDX format', () => {
    ...
    it('includes metadata with tools and timestamp', async () => {
        ...
        expect(parsed.metadata.tools[0].name).toBe('bridge-mcp')
    ...
    it('embeds the full DBOM in extensions', async () => {
        ...
        expect(parsed.extensions['bridge:dbom']).toBeDefined()
        expect(parsed.extensions['bridge:dbom'].version).toBe('1.0')
        expect(parsed.extensions['bridge:dbom'].posture).toBeDefined()
    })

    it('includes bridge properties at top level', async () => {
        writeSrc(tmpDir, 'Button.tsx', CLEAN_COMPONENT)
        const dbom = await generateDBOM(tmpDir, { format: 'cyclonedx', dryRun: true })
        const parsed = JSON.parse(formatDBOMOutput(dbom, 'cyclonedx'))

        const propNames = parsed.properties.map((p: { name: string }) => p.name)
        expect(propNames).toContain('bridge:healthScore')
        expect(propNames).toContain('bridge:grade')
        expect(propNames).toContain('bridge:totalViolations')
    })
})
...
describe('DBOM governance — disk write', () => {
    ...
    it('writes .bridge/dbom.json when dryRun is false', async () => {
        writeSrc(tmpDir, 'Button.tsx', CLEAN_COMPONENT)
        await generateDBOM(tmpDir, { dryRun: false })

        const outputPath = path.join(tmpDir, '.bridge', 'dbom.json')
    ...
    it('does not write .bridge/dbom.json when dryRun is true', async () => {
        writeSrc(tmpDir, 'Button.tsx', CLEAN_COMPONENT)
    ...
    it('creates .bridge/ directory if missing when writing', async () => {
        // Remove .bridge to test creation
        fs.rmSync(path.join(tmpDir, '.bridge'), { recursive: true, force: true })
        writeSrc(tmpDir, 'Button.tsx', CLEAN_COMPONENT)
        await generateDBOM(tmpDir, { dryRun: false })
