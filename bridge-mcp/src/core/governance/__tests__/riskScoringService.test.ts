 ...
 * RiskScoringService class tests in bridge-mcp/src/__tests__/riskScoring.test.ts.
 *
 * Coverage (30+ tests):
...
describe('scoreMutation — familiarity factor', () => {
    ...
    it('projectRoot with no provenance.db → familiarity is neutral, no throw', () => {
        ...
        try {
            // .bridge dir exists but provenance.db does not
            fs.mkdirSync(path.join(tmpDir, '.bridge'))
    ...
    it('projectRoot pointing to a non-existent directory → no throw', () => {
        expect(() => scoreMutation({
            ...
            affectedNodeCount: 1,
            filePath: '/src/components/Button.tsx',
            projectRoot: '/tmp/bridge-mrs-nonexistent-root-xyz',
        })).not.toThrow()
    })
})
