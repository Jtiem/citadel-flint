/**
 * RiskScoringService — bridge-mcp/src/core/governance/riskScoringService.ts
 *
...
/**
 * Query .bridge/provenance.db to count how many prior mutations exist for
...
function queryProvenance(projectRoot: string, filePath: string): number {
    try {
        const dbPath = path.join(projectRoot, '.bridge', 'provenance.db')
