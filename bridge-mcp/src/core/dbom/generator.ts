/**
 * DBOM Generator — bridge-mcp/src/core/dbom/generator.ts
 *
 * Generates a Design Bill of Materials (DBOM) for a Bridge project by:
 *   1. Reading .bridge/policy.json for active governance settings.
 *   2. Reading .bridge/design-tokens.json for the token inventory.
 ...
 *   8. Reading .bridge/violation_baselines if a baseline exists.
...
const EXCLUDED_DIRS = new Set(['node_modules', 'dist', 'dist-electron', '.git', '.bridge', 'coverage', 'out', 'build'])
...
/**
 * Reads violation baselines from .bridge/violation_baselines.json.
...
function readViolationBaselines(projectRoot: string): BaselineEntry[] | null {
    ...
    const candidates = [
        path.join(projectRoot, '.bridge', 'violation_baselines.json'),
        path.join(projectRoot, '.bridge', 'violation-baselines.json'),
    ]
...
/**
 * Reads the baseline timestamp from .bridge/violation-baseline-meta.json.
 * Returns null when no metadata file exists.
 */
function readBaselineMeta(projectRoot: string): { setAt: string } | null {
    const metaPath = path.join(projectRoot, '.bridge', 'violation-baseline-meta.json')
 ...
 */
export async function generateDBOM(projectRoot: string): Promise<DesignBillOfMaterials> {
    ...
    const sourceFiles = findFiles(scanRoot, ['.tsx', '.ts'])
...
/**
 * Reads active component_overrides from .bridge/overrides.json.
...
function readOverrides(projectRoot: string): DBOMOverride[] {
    const overridesPath = path.join(projectRoot, '.bridge', 'overrides.json')
    ...
    try {
        ...
        return raw
            .filter(
                (r): r is { bridge_id: string; property_key: string; property_value: string } =>
                    typeof r?.bridge_id === 'string' &&
