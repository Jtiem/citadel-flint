/**
 * DebtReportService — bridge-mcp/src/core/dashboard/debtReportService.ts
 *
 ...
 * optional trend tracking via .bridge/debt-history.json.
 *
 ...
 * bridge-mcp's direct dependencies).
 *
 * Excluded directories: node_modules, dist, dist-electron, .git, .bridge
 */
function findFiles(rootDir: string, globPattern: string): string[] {
    const results: string[] = []
    const EXCLUDED_DIRS = new Set(['node_modules', 'dist', 'dist-electron', '.git', '.bridge'])
...
/**
 * Loads design tokens from .bridge/design-tokens.json.
...
function scanFile(source: string, tokens: DesignToken[]): FileViolation[] {
    ...
    // Mithril violations (Map<bridgeId, LinterWarning>)
...
export interface GenerateReportOptions {
    /** Project root directory (must contain .bridge/). */
    projectRoot: string
    /** Glob pattern for files to scan (default: '**\/*.tsx'). */
    glob?: string
    /** If true, append a snapshot to .bridge/debt-history.json. */
    track?: boolean
}
...
/**
 * Appends a snapshot to .bridge/debt-history.json.
...
function appendHistory(projectRoot: string, report: DebtReport): void {
    const historyPath = path.join(projectRoot, '.bridge', 'debt-history.json')
...
/**
 * Reads the last N debt history entries from .bridge/debt-history.json.
...
/**
 * Generates the DashboardData payload for the bridge://dashboard resource.
