/**
 * registryService — bridge-mcp/src/core/registryService.ts
 *
 * Text-search component registry + Shadow Storybook artifact formatter.
 * Used directly by bridge_query_registry as a fallback when the RAG cache
 ...
 *
 * @param components  Record keyed by component name from bridge-manifest.json
...
export function queryRegistry(
    ...
    for (const [name, raw] of Object.entries(components)) {
        const entry: ComponentEntry = { name, ...raw };
