/**
 * ragRegistryService — bridge-mcp/src/core/ragRegistryService.ts
 *
 * In-memory component registry cache that backs the primary query path for
 * bridge_query_registry.  The cache is populated via setRegistryCache, which
 * is called both by bridge_add_remote_library (remote ingestion) and by the
 * bridge_query_registry handler itself (local manifest hydration).
 *
