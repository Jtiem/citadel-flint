/**
 * AnnotationStore — bridge-mcp/src/core/annotations/annotationStore.ts
 *
 * MCP-side file I/O layer for Bridge annotations (COLLAB.1).
 *
 * Responsibilities:
 *   - Read / write .bridge/annotations.json atomically
...
import type { BridgeAnnotation } from './types.js'

// ── Helpers ────────────────────────────────────────────────────────────────────

/**
 * Returns the absolute path to .bridge/annotations.json for `projectRoot`.
 * Uses the HOME-relative `.bridge` directory when no project root is provided
 ...
 */
export async function readAnnotations(filePath: string): Promise<BridgeAnnotation[]> {
    try {
 ...
 */
export async function writeAnnotations(filePath: string, annotations: BridgeAnnotation[]): Promise<void> {
 ...
 */
export async function appendAnnotation(filePath: string, annotation: BridgeAnnotation): Promise<void> {
