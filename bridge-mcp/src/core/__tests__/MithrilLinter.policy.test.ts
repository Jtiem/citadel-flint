 ...
 * bridge-mcp/src/core/__tests__/MithrilLinter.policy.test.ts
 *
 * Test map:
 ...
 *
 * The element has a data-bridge-id so the linter can identify it.
 * Uses text-[#ff0000] — a vivid red that is far from any token color.
 */
function makeColorJSX(hex = '#ff0000'): File {
    return parseJSX(`
        const C = () => (
            <div data-bridge-id="node-1" className="text-[${hex}]" />
        )
    `)
}
...
function makeTypographyJSX(): File {
    return parseJSX(`
        const C = () => (
            <div data-bridge-id="node-1" className="font-[Arial]" />
        )
    `)
}
...
function makeSpacingJSX(): File {
    return parseJSX(`
        const C = () => (
            <div data-bridge-id="node-1" className="p-[99px]" />
        )
    `)
}
...
function makeShadowJSX(): File {
    return parseJSX(`
        const C = () => (
            <div data-bridge-id="node-1" className="shadow-[0_2px_4px_rgba(0,0,0,0.5)]" />
        )
    `)
}
...
function makeOpacityJSX(): File {
    return parseJSX(`
        const C = () => (
            <div data-bridge-id="node-1" className="opacity-[0.73]" />
        )
    `)
}
...
describe('Group F — auditAll policy propagation', () => {
    ...
    it('F3: auditAll mixed modes — off rule absent, advisory rule tagged, blocking rule normal', () => {
        ...
        const ast = parseJSX(`
            const C = () => (
                <div>
                    <span data-bridge-id="node-1" className="text-[#800000]" />
                    <div data-bridge-id="node-2" className="p-[99px]" />
                </div>
            )
        `) as unknown as File
