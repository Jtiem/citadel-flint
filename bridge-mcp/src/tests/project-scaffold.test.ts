/**
 * project-scaffold.test.ts — bridge-mcp/src/tests/project-scaffold.test.ts
 *
 * Journey 10: Create New Project — MCP-side tests.
 *
 * Test map:
 *   #35 — project:initialize creates the .bridge directory inside the project
 ...
 *
 * Environment: node (bridge-mcp/vitest.config.ts)
 *
 ...
 *   bridge-mcp MithrilLinter (`auditAll`), exactly as workspace-audit.test.ts
...
// ── Template paths (resolve from bridge-mcp/src/tests/ up to repo root) ───────

// Derive __dirname from import.meta.url (Node ESM — matches the pattern used
// in bridge-mcp/src/domains/index.ts for cross-version compatibility).
...
/**
 * Scaffold a minimal Bridge workspace into `workspaceRoot`:
 *   <root>/
 *     .bridge/
 *       design-tokens.json
 *     src/
 ...
 * the `.bridge/` sentinel directory that the IPC handler creates separately.
 */
beforeAll(() => {
    workspaceRoot = mkdtempSync(path.join(tmpdir(), 'bridge-scaffold-test-'))

    // Create .bridge/ directory and design-tokens.json
    const bridgeDir = path.join(workspaceRoot, '.bridge')
...
// ── #35: project:initialize creates .bridge directory ─────────────────────────

describe('#35 — project:initialize creates .bridge directory', () => {
    it('.bridge/ exists in the scaffolded workspace', () => {
        const bridgeDir = path.join(workspaceRoot, '.bridge')
        expect(existsSync(bridgeDir)).toBe(true)
    })

    it('.bridge/ is a directory, not a file', () => {
        const bridgeDir = path.join(workspaceRoot, '.bridge')
        expect(statSync(bridgeDir).isDirectory()).toBe(true)
    })

    it('.bridge/ contains at least design-tokens.json', () => {
        const tokensPath = path.join(workspaceRoot, '.bridge', 'design-tokens.json')
