#!/usr/bin/env node
/**
 * bridge-dbom CLI — bridge-mcp/src/cli-dbom.ts
 *
 * Command-line interface for generating a Design Bill of Materials (DBOM).
 *
 * Usage:
 *   npx bridge-dbom                           # Generate DBOM as JSON to stdout
 *   npx bridge-dbom --output dbom.json        # Write JSON to file
 *   npx bridge-dbom --format markdown         # Human-readable Markdown to stdout
 *   npx bridge-dbom --format markdown --output dbom.md
 *   npx bridge-dbom --project-root /path/to/project
 *
 * Options:
...
bridge-dbom — Design Bill of Materials generator

USAGE
  npx bridge-dbom [options]
...
EXAMPLES
  npx bridge-dbom
  npx bridge-dbom --output dbom.json
  npx bridge-dbom --format markdown
  npx bridge-dbom --format markdown --output dbom.md
  npx bridge-dbom --project-root /path/to/my-project --format json --output dbom.json
  npx bridge-dbom --strict  # exits 2 if project is non-compliant
...
async function main(): Promise<void> {
    ...
    stderr(`[bridge-dbom] Scanning ${projectRoot}…`)
    ...
    if (output !== null) {
        ...
        stderr(`[bridge-dbom] DBOM written to ${output}`)
        stderr(`[bridge-dbom] Health: ${dbom.summary.healthScore}/100 (Grade ${dbom.summary.grade}) — ${dbom.summary.complianceStatus}`)
    } else {
    ...
    if (strict && dbom.summary.complianceStatus === 'non-compliant') {
        stderr(`[bridge-dbom] --strict: project is non-compliant. Exiting with code 2.`)
        process.exit(2)
    }
}

main().catch((err) => {
    process.stderr.write(`[bridge-dbom] Fatal: ${err instanceof Error ? err.message : String(err)}\n`)
    process.exit(1)
})
