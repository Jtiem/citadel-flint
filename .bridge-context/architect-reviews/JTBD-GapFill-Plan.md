# JTBD Gap-Fill Plan — Bridge 7.5 → 9.0
...
- Context Bridge syncs Glass state to MCP via `.bridge/context.json`
...
| 7 | W.3 | Bidirectional Action Bridge | — | XL | 3 | REMAINING |
...
Wave 1 — Foundation (parallel, no inter-gap dependencies):
  ...
  └── MCP   Tool Discoverability (bridge://capabilities)
...
### Problem
Activity Feed (`src/components/ui/ActivityFeed.tsx`) shows MCP tool calls but is read-only — no filtering, search, or actions. Worse, the 'activity' tab in `App.tsx:374` renders the legacy `AgentChatPanel` instead of `ActivityFeed`. Per CLAUDE.md: "Chat lives in the host IDE via MCP — there is no chat panel inside Bridge Glass."
...
**No new stores, IPC channels, or modules needed.** All changes are local component state (filter/search are transient UI state, reset on mount). Polling loop (3s interval, reads `.bridge/activity-log.jsonl` via `window.bridgeAPI.readFile`) unchanged.

**Scope boundary:** Filtering + search + view button ship now. "Retry" and "Approve/Reject" buttons depend on Phase W.3 (Bidirectional Action Bridge) and will be added after Wave 3.
...
| `electron/preload.ts` | Add `bridgeAPI.figma.status()` IPC channel |
...
- C7 (ID Preservation): ShieldOverlay keys everything by `data-bridge-id` ✓
...
### Problem
New users (and AI agents) don't know what Bridge MCP can do. No structured catalog of tools, parameters, or example invocations.
...
| File | Type |
|------|------|
| `bridge-mcp/src/core/capabilities/index.ts` | `bridge://capabilities` resource handler |
| `bridge-mcp/src/prompts/workflow-guide.ts` | Multi-tool sequence composition prompt |

**Changed files:**
| File | Change |
|------|--------|
| `bridge-mcp/src/server.ts` | Register new resource + prompt |
...
**Key decision: IPC push via `fs.watch`, not polling.** The Activity Feed's 3-second poll model already shows scaling issues. Annotations originate in the MCP/main process (written to `.bridge/annotations.json`). Glass should be notified immediately when they change.

**Data flow:**
```
MCP tool (bridge_annotate) → writes .bridge/annotations.json
  → main process fs.watch detects change
  → fires bridge:annotations-changed IPC event to renderer
...
|------|------|-----------|
| `src/store/annotationStore.ts` | New Zustand store | bridge-state-architect |
| `src/components/ui/AnnotationList.tsx` | Properties sub-section | bridge-design-engineer |
| `src/components/ui/AnnotationBadge.tsx` | ShieldOverlay integration | bridge-design-engineer |

**Changed files:**
| File | Change | Specialist |
|------|--------|-----------|
| `electron/preload.ts` | Add `annotations` namespace (readAll, onChanged) | bridge-electron-ipc |
| `electron/main.ts` | IPC handlers + `fs.watch` on `.bridge/annotations.json` | bridge-electron-ipc |
| `src/components/editor/ShieldOverlay.tsx` | Render AnnotationBadge at node positions | bridge-design-engineer |
| `src/components/ui/PropertiesPanel.tsx` | Embed AnnotationList when node has annotations | bridge-design-engineer |
| `src/components/ui/LayerTree.tsx` | Indigo dot next to nodes with unresolved annotations | bridge-design-engineer |
| `src/App.tsx` | Register `bridge:annotations-changed` listener on mount | bridge-electron-ipc |
| `src/types/bridge-api.ts` | Mirror `BridgeAnnotation` type from bridge-mcp | bridge-state-architect |

**Store shape (`annotationStore`):**
```typescript
interface AnnotationState {
  annotations: BridgeAnnotation[]
  annotationsForNode: (nodeId: string) => BridgeAnnotation[]  // derived selector
...
**Commandment compliance:**
- C4 (Local-First): Reads from local `.bridge/` directory ✓
...
### Problem
`bridge://dashboard` returns health score (0–100), grade (A–F), top violated files/rules, override telemetry — but users must ask the AI to see project health. No persistent visual surface in Glass.
...
**Key decision: Synthesize from local `.bridge/` files, not MCP server.** The dashboard data can be assembled from sources already available to the Electron main process:
- Linter warnings from renderer (already computed in `editorStore`)
- `.bridge/activity-log.jsonl` for recent mutations
- `.bridge/rule-overrides.json` for override count
- Run audit on open files (cached per file, invalidate on `bridge:file-changed`)
...
| File | Type | Specialist |
|------|------|-----------|
| `src/components/ui/GovernanceDashboard.tsx` | New panel | bridge-design-engineer |
| `electron/dashboardService.ts` | Main process data aggregation | bridge-electron-ipc |
...
**Risk:** Auditing all workspace files on every refresh is expensive. Mitigation: cache audit results per file, invalidate only on `bridge:file-changed` events.
...
| File | Type |
|------|------|
| `bridge-mcp/src/core/events.ts` | `appendMCPEvent()` + file rotation (200 lines) |
| `src/hooks/useMCPEventListener.ts` | Renderer-side event dispatcher |

**Changed files:**
| File | Change |
|------|--------|
| `electron/main.ts` | fs.watch + tail-follow + `bridge:mcp-event` push |
| `electron/preload.ts` | Expose `bridgeAPI.mcp.onEvent(callback)` |
...
## Phase W.3 — Bidirectional Action Bridge
...
### Success Criteria
- Glass button "Run Audit" invokes `bridge_audit` MCP tool and displays result
- Glass button "Auto-Fix" invokes `bridge_fix` MCP tool on selected violations
...
```
Renderer
  → IPC invoke 'bridge:mcp-call-tool'
  → Main process
...
| File | Change |
|------|--------|
| `electron/preload.ts` | Add `bridgeAPI.mcp.callTool(name, args)`, `.readResource(uri)`, `.status()` |
| `electron/main.ts` | Start/stop MCP client on project open/close, register IPC handlers |
| `src/types/bridge-api.ts` | Add `mcp` namespace types |
...
Wave 3 (sequential) ················ ⬜ REMAINING
  W.1   Push Channel ············ L ──→ W.3   Action Bridge ·· XL ──→ 9.0
```

**Critical path gate:** W.3 (Bidirectional Action Bridge) must ship before Glass can trigger any MCP tool. All "action" affordances (fix buttons, approve buttons, canvas context menus) are blocked on W.3.
