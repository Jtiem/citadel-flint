...
|------|------------|-------------|-------|
| `electron/mcp-policy.ts` | CREATE | `bridge-electron-ipc` | Static allowlist constant |
| `electron/main.ts` | MODIFY | `bridge-electron-ipc` | Import allowlist, add guard to `mcp:call-tool` handler |
| `electron/__tests__/mcp-policy.test.ts` | CREATE | `bridge-test-writer` | Tests for allowlist enforcement |
...
|--------|------|-----------|---------|
| `ExportModal.handleDownloadDBOM` | `src/components/ui/ExportModal.tsx:178` | `bridge_generate_dbom` | Download Design Bill of Materials as JSON |
...
| Tool | Justification |
|------|--------------|
| `bridge_status` | Health check -- used by status indicators |
| `bridge_audit` | Run governance audit -- used by GovernanceOverlay and ExportModal pre-flight |
| `bridge_debt_report` | Health score data -- used by GovernanceDashboard |
| `bridge_query_registry` | Component search -- used by Asset Management Hub |
| `bridge_generate_dbom` | DBOM export -- used by ExportModal (the one confirmed callTool site) |
| `bridge_accessibility_report` | VPAT report -- used by ExportModal compliance section |
| `bridge_audit_report` | Provenance report -- used by ExportModal compliance summary |

Tools explicitly **excluded** from the allowlist (write-oriented or agent-only):

| Tool | Reason for Exclusion |
|------|---------------------|
| `bridge_ast_mutate` | AST mutations from the renderer go through `editorStore.applyBatch` -> `ast:save-file` IPC, not through MCP. MCP mutations are agent-initiated and should only come from the host IDE via MCP protocol, not from Glass. |
| `bridge_fix` | Auto-fix is triggered by GovernanceOverlay buttons which call `editorStore.applyBatch` directly, not MCP. |
| `bridge_ingest_figma` | Ingestion is triggered by the HTTP server route, not by Glass. |
| `bridge_sync_tokens` | Token sync is triggered by the ingestion server, not by Glass. |
| `bridge_swarm_audit_fix` | Agent-only swarm orchestration tool. Never called from Glass. |
| `bridge_add_remote_library` | Agent-only library management. Never called from Glass. |
| `bridge_annotate` | Annotations are created by MCP agents via the protocol, not by Glass buttons. Glass reads annotations via `annotations:read-all` IPC, not MCP. |
...
None. No React components are modified. The `ExportModal.tsx` call to `bridge_generate_dbom` is already in the allowlist and continues to work.
...
```
- Assert: RENDERER_ALLOWED_MCP_TOOLS includes 'bridge_status'
- Assert: RENDERER_ALLOWED_MCP_TOOLS includes 'bridge_audit'
- Assert: RENDERER_ALLOWED_MCP_TOOLS includes 'bridge_debt_report'
- Assert: RENDERER_ALLOWED_MCP_TOOLS includes 'bridge_query_registry'
- Assert: RENDERER_ALLOWED_MCP_TOOLS includes 'bridge_generate_dbom'
- Assert: RENDERER_ALLOWED_MCP_TOOLS includes 'bridge_accessibility_report'
- Assert: RENDERER_ALLOWED_MCP_TOOLS includes 'bridge_audit_report'
```

Test SEC3-03: The allowlist does NOT contain write-oriented tools.

```
- Assert: RENDERER_ALLOWED_MCP_TOOLS does NOT include 'bridge_ast_mutate'
- Assert: RENDERER_ALLOWED_MCP_TOOLS does NOT include 'bridge_fix'
- Assert: RENDERER_ALLOWED_MCP_TOOLS does NOT include 'bridge_ingest_figma'
- Assert: RENDERER_ALLOWED_MCP_TOOLS does NOT include 'bridge_sync_tokens'
- Assert: RENDERER_ALLOWED_MCP_TOOLS does NOT include 'bridge_swarm_audit_fix'
- Assert: RENDERER_ALLOWED_MCP_TOOLS does NOT include 'bridge_add_remote_library'
```
...
- Invoke the mcp:call-tool handler with name='bridge_status', args={}
- Assert: mcpClient.callTool was called with ('bridge_status', {})
...
```
- Invoke the mcp:call-tool handler with name='bridge_ast_mutate', args={}
- Assert: the call throws/rejects
- Assert: error message contains 'not in the renderer allowlist'
- Assert: error message contains 'bridge_ast_mutate'
...
|------|-------|---------------|-------------|
| 1 | `bridge-electron-ipc` | A | Create `electron/mcp-policy.ts` with the allowlist |
| 2 | `bridge-electron-ipc` | A | Modify `electron/main.ts`: import allowlist, add guard to `mcp:call-tool` |
| 3 | `bridge-test-writer` | B (after A) | Write tests SEC3-01 through SEC3-06 |
| 4 | `bridge-test-writer` | B | Run full test suite, TSC, report counts |
...
| `ExportModal` DBOM download breaks | Very Low | `bridge_generate_dbom` is explicitly in the allowlist. Verified via grep. |
| Future Glass features need write tools | Medium | If a future Glass feature needs to invoke `bridge_fix` or `bridge_ast_mutate` from a button click, the allowlist must be expanded. This is intentional friction: each addition requires a conscious security decision. For write-oriented tools, consider adding a user-confirmation dialog in the main process (future SEC enhancement). |
...
- [ ] The array contains exactly: `bridge_status`, `bridge_audit`, `bridge_debt_report`, `bridge_query_registry`, `bridge_generate_dbom`, `bridge_accessibility_report`, `bridge_audit_report`
