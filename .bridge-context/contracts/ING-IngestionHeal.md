...
|------|------------|-------------|-------|
| `electron/ingestion/IngestionAuditor.ts` | NEW | `bridge-electron-ipc` | Core heal logic: classify + fix. Main process only. |
| `src/store/importSummaryStore.ts` | NEW | `bridge-state-architect` | Zustand store for IngestionSummary |
| `src/components/ui/ImportSummary.tsx` | NEW | `bridge-design-engineer` | Toast + panel variants |
| `electron/ingestion-server.ts` | MODIFY | `bridge-electron-ipc` | Insert heal pass in `/ingest-ast` handler |
| `electron/preload.ts` | MODIFY | `bridge-electron-ipc` | Add `importSummary` IPC namespace |
| `src/types/bridge-api.d.ts` | MODIFY | `bridge-electron-ipc` | Add ING type declarations + ImportSummaryAPI |
| `src/components/editor/LivePreview.tsx` | MODIFY | `bridge-design-engineer` | Listen for `bridge:import-summary`, populate store |
| `bridge-mcp/src/tools/audit.ts` | MODIFY | `bridge-ast-surgeon` | Add optional `healOnAudit` parameter (ING.3) |
| `bridge-mcp/src/core/MithrilLinter.ts` | UNCHANGED | -- | Reused by IngestionAuditor (MCP-side copy used in main process) |
...
| `electron/__tests__/IngestionAuditor.test.ts` | NEW | `bridge-test-writer` | Unit tests ING-01 through ING-12 |
| `src/store/__tests__/importSummaryStore.test.ts` | NEW | `bridge-test-writer` | Store state transition tests |
| `src/components/ui/__tests__/ImportSummary.test.tsx` | NEW | `bridge-test-writer` | Component render + interaction tests ING-15, ING-16, ING-17 |
| `electron/__tests__/ingestion-integration.test.ts` | NEW | `bridge-test-writer` | Integration tests ING-13, ING-14 |

---

## 2. Type Contracts

### 2.1 Core ING Types (new, in `src/types/bridge-api.d.ts`)
 ...
 * Pushed from main -> renderer via `bridge:import-summary` IPC.
...
export interface IngestionFix {
    /** data-bridge-id of the element that was fixed. */
    nodeId: string
...
**LinterWarning (renderer-side, `src/types/bridge-api.d.ts` line 109):**
```typescript
interface LinterWarning {
    id: string                    // data-bridge-id
 ...
 * Runs in the main process (electron/) after hydration and injectBridgeIds.
 *
...
|---------|-----------|-------------|-------------|---------|
| `bridge:import-summary` | main -> renderer | `IngestionSummary` | -- (push event) | Pushes heal results to renderer for toast |
...
importSummary: {
    /**
     * Subscribes to 'bridge:import-summary' push events from the main process.
...
### 3.2 BridgeAPI Type Addition (`src/types/bridge-api.d.ts`)
...
Add to the `BridgeAPI` interface:
```typescript
...
**Anti-pattern guard:** This store does NOT call `window.bridgeAPI`. IPC calls happen in the LivePreview.tsx or App.tsx listener hooks that populate this store.

### 4.2 editorStore (UNCHANGED)

No changes to editorStore. The heal pass runs in the main process. The healed code arrives at the renderer via the existing `bridge:hydro-paste-auto` flow, which ultimately calls `editorStore.setCode()`. The importSummaryStore is a separate concern.
...
| | `window.bridgeAPI.importSummary.undoAllHeals()` on "Undo all heals" click |
...
Add a `useEffect` that subscribes to `bridge:import-summary` via `window.bridgeAPI.importSummary.onSummary()` and calls `importSummaryStore.getState().setSummary(summary)`. This listener sits alongside the existing `onHydroPasteAuto` subscription. Cleanup on unmount.
...
- `bridge-mcp/src/core/MithrilLinter.ts` patterns -- regex patterns for arbitrary value detection, CIEDE2000 math

Since importing from `bridge-mcp/src/` directly creates a dependency coupling, the IngestionAuditor should **inline the necessary functions** (CIEDE2000 distance calculation, `cssColorToHex`, the regex patterns) rather than importing from MithrilLinter. This keeps the module self-contained within `electron/`.
...
1. Find the JSXElement by `data-bridge-id`
...
| C7 | ID Preservation | YES | `injectBridgeIds` runs BEFORE the heal pass. Heal mutates className values only, never adds/removes/reorders elements. All `data-bridge-id` attributes remain intact. |
...
**Agents:** `bridge-electron-ipc` + `bridge-test-writer`
...
2. Modify `electron/ingestion-server.ts` `/ingest-ast` handler:
   - After payload extraction, before `windows[0].webContents.send('bridge:hydro-paste-auto', ...)`
   ...
   - Send `bridge:import-summary` IPC with the summary
...
3. Modify `src/components/editor/LivePreview.tsx`:
   - Add `useEffect` subscribing to `bridge:import-summary` IPC
...
**Agents:** `bridge-integration-validator` + `bridge-ast-surgeon`
...
3. Modify `bridge-mcp/src/tools/audit.ts` to optionally run heal pass
...
| ING-10 | Healed AST preserves all data-bridge-id values | `electron/__tests__/IngestionAuditor.test.ts` | HIGH |
...
| ING-18 | MCP bridge_audit({ healOnAudit: true }) -> tier-1 fixes | `bridge-mcp/src/__tests__/audit-heal.test.ts` | MEDIUM |
...
The current `/ingest-ast` handler simply forwards the raw payload to the renderer via `bridge:hydro-paste-auto`. The heal pass must run BETWEEN receiving the payload and forwarding it. This means the main process must:

1. Call `window.bridgeAPI.ai.hydroPaste` equivalent logic (the `hydrateFigmaAST` function in `electron/orchestrator.ts`) to get the hydrated JSX code
2. Run `injectBridgeIds` on the hydrated AST
...
| `bridge-mcp/src/core/MithrilLinter.ts` | CIEDE2000 math and regex patterns to inline |
...
| `src/types/bridge-api.d.ts` | LinterWarning shape (no ruleId on renderer side) |
| `bridge-mcp/src/types.ts` | LinterWarning shape (has ruleId on MCP side) |
