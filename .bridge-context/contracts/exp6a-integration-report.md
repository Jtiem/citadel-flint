...
All interfaces from the contract types section are implemented in `bridge-mcp/src/core/a11y/types.ts`:
...
- **AccessibilityReportArgs** -- Matches contract in `bridge-mcp/src/tools/accessibility.ts`. PASS.
- **AccessibilityReportOutput** -- Has `status`, `auditResult`, `fixedSource`, `appliedFixes`. Matches contract. PASS.
- **LinterWarning** -- `bridge-mcp/src/types.ts` has `wcag?: string` and `fixable?: boolean` as optional fields. PASS.
...
All new rule modules live in `bridge-mcp/src/core/a11y/`. The `accessibility.ts` tool handler uses `import fs from 'node:fs'`, which is correct because it runs in the MCP server process (Node.js), not the renderer. No `fs`, `path`, or `child_process` imports exist in `src/` (the renderer side). PASS.
...
Auto-fix functions return `A11yFixMutation[]` with `type: 'updateProp' | 'inject'`. These map directly to the bridge_ast_mutate operation catalog. No raw code string generation. The `applyFixMutationToAst` function in `fixer.ts` does perform direct AST node manipulation, but only on freshly-parsed ephemeral ASTs within the tool handler (not the live editor AST). PASS.

### C16 -- In-Memory Validation

The `handleAccessibilityReport` tool handler parses the source, runs the audit, optionally applies fixes, and generates code from the modified AST. The result is returned as data -- it does not write to disk. The existing orchestrator validation loop applies when these mutations flow through `bridge_ast_mutate`. PASS.
...
The `A11yLinter.audit()` API in `bridge-mcp/src/core/A11yLinter.ts` is preserved as a backward-compatible shim:
...
All files reviewed:
- `bridge-mcp/src/core/a11y/types.ts`
- `bridge-mcp/src/core/a11y/runner.ts`
- `bridge-mcp/src/core/a11y/fixer.ts`
- `bridge-mcp/src/core/a11y/helpers.ts`
- `bridge-mcp/src/core/a11y/contrast-utils.ts`
- `bridge-mcp/src/core/a11y/index.ts`
- `bridge-mcp/src/core/a11y/rules/names-labels.ts`
- `bridge-mcp/src/core/a11y/rules/keyboard.ts`
- `bridge-mcp/src/core/a11y/rules/structure.ts`
- `bridge-mcp/src/core/a11y/rules/aria.ts`
- `bridge-mcp/src/core/a11y/rules/landmarks.ts`
- `bridge-mcp/src/core/a11y/rules/contrast.ts`
- `bridge-mcp/src/core/a11y/rules/forms.ts`
- `bridge-mcp/src/core/a11y/rules/live-regions.ts`
- `bridge-mcp/src/core/a11y/rules/motion.ts`
- `bridge-mcp/src/core/A11yLinter.ts`
- `bridge-mcp/src/tools/accessibility.ts`
- `bridge-mcp/src/server.ts`
- `bridge-mcp/src/types.ts`
- `bridge-mcp/src/core/dashboard/debtReportService.ts`
- All 13 test files in `bridge-mcp/src/core/a11y/__tests__/`
