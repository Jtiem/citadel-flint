...
| IPC Symmetry | PASS | `figma:status` return type aligned across main/preload/bridge-api.d.ts; `mcp:call-tool` allowlist enforced in main.ts |
| Store Isolation | PASS | No store files modified; pre-existing `window.bridgeAPI` usage in stores is out of scope |
...
- **Type declaration** (`src/types/bridge-api.d.ts:410`): `FigmaStatus` interface has 4 fields: `running`, `lastWebhookAt`, `tokenCount`, `port`. No `secret`.
...
- **Secret lifecycle**: `bridgeSecret` set on start, cleared to `null` on `stopIngestionServer()`.
...
- Pre-existing `window.bridgeAPI` calls in `orchestratorStore.ts`, `tokenStore.ts`, `canvasStore.ts`, `annotationStore.ts`, `governanceStore.ts`, `assetStore.ts`, `editorStore.ts`, `astBufferStore.ts` are an architectural debt item documented in the CLAUDE.md anti-patterns section. They are NOT regressions from this sprint.
