...
**UX Plan:** `.bridge-context/architect-reviews/FigmaConnect-UX-Plan.md`
...
|------|------------|-------------|
| `src/types/bridge-api.d.ts` | MODIFY -- extend `FigmaStatus`, extend `FigmaAPI` with new methods | bridge-electron-ipc |
| `electron/ingestion-server.ts` | MODIFY -- expose `port`+`secret` in `getFigmaStatus()`, fire `bridge:figma-connected` and `bridge:figma-error` IPC push events | bridge-electron-ipc |
| `electron/main.ts` | MODIFY -- register `figma:disconnect` IPC handler, pass `stopIngestionServer`/`startIngestionServer` | bridge-electron-ipc |
| `electron/preload.ts` | MODIFY -- extend `figma` namespace with `onConnected()`, `onError()`, `disconnect()`, `removeListeners()` | bridge-electron-ipc |
| `src/components/ui/FigmaSetupWizard.tsx` | NEW FILE -- 3-step guided wizard component | bridge-design-engineer |
| `src/components/ui/LaunchScreen.tsx` | MODIFY -- replace inline text block with `<FigmaSetupWizard>` | bridge-design-engineer |
| `src/components/editor/StatusBar.tsx` | MODIFY -- add copy buttons, disconnect button, "Refresh Status" to Figma popover | bridge-design-engineer |
| `src/components/ui/__tests__/LaunchScreen.test.tsx` | MODIFY -- update Figma wizard test | bridge-test-writer |
| `src/components/ui/__tests__/FigmaSetupWizard.test.tsx` | NEW FILE -- wizard-specific tests | bridge-test-writer |
| `src/components/editor/__tests__/StatusBar.test.tsx` | MODIFY -- add tests for popover actions | bridge-test-writer |
...
| `bridge:figma-connected` | main -> renderer (push) | `FigmaConnectedEvent` | N/A (broadcast) |
| `bridge:figma-error` | main -> renderer (push) | `FigmaErrorEvent` | N/A (broadcast) |
...
- Push events (`bridge:figma-connected`, `bridge:figma-error`) are consumed
...
|-------|----------------------|
| `notificationStore` | `push()` called to show success/error toasts when `bridge:figma-connected` or `bridge:figma-error` fires |
...
| Consumes Store | None |
| Calls IPC | `window.bridgeAPI.figma.status()` on mount to check server |
| Subscribes IPC | `window.bridgeAPI.figma.onConnected()` to detect first sync |
...
**"Setup guide" link:** Dispatches `window.dispatchEvent(new CustomEvent('bridge:open-figma-wizard'))`.
...
**"Disconnect" behavior:** Calls `window.bridgeAPI.figma.disconnect()`, then
re-fetches `figma.status()` to update the dot. Pushes a toast:
"Figma server stopped. Restart Bridge to reconnect." No undo path in v1 -- the
...
**`bridge:figma-connected` push -- fire after successful POST /ingest:**
...
**`bridge:figma-error` push -- fire on 401 (bad secret) and 400 (bad payload):**
...
if (windows.length > 0) {
    windows[0].webContents.send('bridge:figma-error', {
...
- [x] **C14 Bypass Prohibition** -- `stopIngestionServer()` is called through IPC, not by importing `ingestion-server.ts` in the renderer. The renderer calls `window.bridgeAPI.figma.disconnect()` which routes through preload -> main -> `stopIngestionServer()`.

**Not applicable:**
- C1 (Code is Truth) -- no AST mutations
- C2 (No Hallucinated Styling) -- no visual edits to user code
- C3 (Composite IDs) -- no dynamic lists with bridge IDs
...
No separate step needed -- the type changes in `bridge-api.d.ts` are part of
...
| Agent | Files | What |
|-------|-------|------|
| `bridge-electron-ipc` | `src/types/bridge-api.d.ts`, `electron/ingestion-server.ts`, `electron/main.ts`, `electron/preload.ts` | Extend `FigmaStatus`/`FigmaAPI` types, add `port`+`secret` to `getFigmaStatus()`, add `bridge:figma-connected` and `bridge:figma-error` push events, add `figma:disconnect` handler, extend preload `figma` namespace |

### Group B (parallel, after A): UI + Tests

| Agent | Files | What |
|-------|-------|------|
| `bridge-design-engineer` | `src/components/ui/FigmaSetupWizard.tsx` (NEW), `src/components/ui/LaunchScreen.tsx`, `src/components/editor/StatusBar.tsx` | Build wizard component, replace LaunchScreen text block, add StatusBar popover actions |
| `bridge-test-writer` | `src/components/ui/__tests__/FigmaSetupWizard.test.tsx` (NEW), `src/components/ui/__tests__/LaunchScreen.test.tsx`, `src/components/editor/__tests__/StatusBar.test.tsx` | Wizard tests, update existing tests |

### Group C (after B): Review

| Agent | Files | What |
|-------|-------|------|
| `bridge-code-reviewer` | All modified files | IPC symmetry check, process boundary audit, commandment compliance |
...
Test 9 currently checks for "Bridge Figma plugin setup" text. After the change,
...
- It is already visible in the terminal output when Bridge starts.
- The renderer displays it in a copy field (the whole point of this feature).
- An attacker with renderer access already has full `bridgeAPI` access.
...
Mitigation: the disconnect toast says "Restart Bridge to reconnect." A future
`figma:reconnect` handler can call `startIngestionServer()` if needed.

### 4. Race between `bridge:figma-connected` and wizard mount (LOW)
...
Mitigation: `bridge-test-writer` receives explicit instructions to update these
assertions.
...
Mitigation: `bridge-test-writer` updates the mock factory to include `port: 4545`
...
### Modified
- `/Users/tiemann/Lunar-Elevator-Bridge/src/types/bridge-api.d.ts`
- `/Users/tiemann/Lunar-Elevator-Bridge/electron/ingestion-server.ts`
- `/Users/tiemann/Lunar-Elevator-Bridge/electron/main.ts`
- `/Users/tiemann/Lunar-Elevator-Bridge/electron/preload.ts`
- `/Users/tiemann/Lunar-Elevator-Bridge/src/components/ui/LaunchScreen.tsx`
- `/Users/tiemann/Lunar-Elevator-Bridge/src/components/editor/StatusBar.tsx`
- `/Users/tiemann/Lunar-Elevator-Bridge/src/components/ui/__tests__/LaunchScreen.test.tsx`
- `/Users/tiemann/Lunar-Elevator-Bridge/src/components/editor/__tests__/StatusBar.test.tsx`

### New
- `/Users/tiemann/Lunar-Elevator-Bridge/src/components/ui/FigmaSetupWizard.tsx`
- `/Users/tiemann/Lunar-Elevator-Bridge/src/components/ui/__tests__/FigmaSetupWizard.test.tsx`
