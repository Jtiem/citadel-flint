...
EXP.6 expands the A11yLinter from 10 rules to 50+ WCAG 2.1 AA rules, adds deterministic auto-fix for a subset of those rules, and introduces a new `bridge_accessibility_report` MCP tool. This is the largest single expansion of the governance engine since Phase B.

### Key Architectural Decisions

**1. Single Source of Truth -- MCP-side owns the linter.**
Today the codebase has two near-identical A11yLinter files: `bridge-mcp/src/core/A11yLinter.ts` (MCP server) and `src/core/A11yLinter.ts` (renderer). With 50+ rules and an auto-fix engine, maintaining two copies is untenable. The solution:
- The canonical implementation lives in `bridge-mcp/src/core/a11y/` as a modular rule directory.
- The renderer-side `src/core/A11yLinter.ts` becomes a thin re-export facade that imports the shared rule definitions from a process-neutral shared types package. However, because the process boundary law prohibits the renderer from importing Node.js code, and the MCP server runs in a separate process entirely, the actual sharing mechanism is: **the MCP server is the single source of truth**, and the renderer-side linter is deprecated in favor of calling `bridge_audit` via IPC or consuming `bridge://violations/{filePath}` resources. For the transition period, we build a code-generation script (`scripts/sync-a11y-rules.ts`) that copies the rule catalog to `src/core/A11yLinter.ts` at build time.

**2. Modular rule files organized by WCAG success criterion category.**
A single 1500-line file is unmaintainable. Rules are organized into category modules under `bridge-mcp/src/core/a11y/rules/`, each exporting a standard `A11yRule` interface. The linter becomes a rule runner that loads and executes all registered rules.

**3. Auto-fix is a separate concern from detection.**
Each rule optionally exports a `fix` function. The fix function receives the AST and the violation location, and returns the mutated AST. Fix functions are only called via the `bridge_fix` tool or the new `bridge_accessibility_report` tool with `autoFix: true`. Detection and fix are never coupled.

**4. `bridge_accessibility_report` is a new dedicated tool, not an extension of `bridge_audit`.**
Rationale: `bridge_audit` returns a combined Mithril + A11y result optimized for the IDE workflow. The new tool returns a WCAG-structured compliance report with pass/fail per success criterion, compliance percentage, and is designed for compliance officers and CI/CD gates. Different audiences, different output shapes.
...
| 9 | Process Boundary | New rules live in `bridge-mcp/` (Node.js). Renderer gets results via IPC/MCP resource. |
| 13 | Deterministic Surgery | All rules use Babel AST traversal. Auto-fix uses Babel AST mutation. No regex. |
| 15 | Granular AST Tools Only | Auto-fix emits `bridge_ast_mutate` ops, not raw code strings. |
...
|------|------------|-------------|
| `bridge-mcp/src/core/a11y/types.ts` | NEW FILE -- A11yRule interface, A11yViolation, A11yFixResult types | bridge-ast-surgeon |
| `bridge-mcp/src/core/a11y/runner.ts` | NEW FILE -- Rule runner (loads rules, executes audit, aggregates results) | bridge-ast-surgeon |
| `bridge-mcp/src/core/a11y/helpers.ts` | NEW FILE -- Shared AST helpers (extracted from current A11yLinter) | bridge-ast-surgeon |
| `bridge-mcp/src/core/a11y/rules/names-labels.ts` | NEW FILE -- A11Y-001 through A11Y-006 (migrated + enhanced) | bridge-ast-surgeon |
| `bridge-mcp/src/core/a11y/rules/keyboard.ts` | NEW FILE -- A11Y-007, A11Y-020..025 (tab order, interactive keyboard) | bridge-ast-surgeon |
| `bridge-mcp/src/core/a11y/rules/structure.ts` | NEW FILE -- A11Y-008..A11Y-010 (tables, lang, headings, migrated + enhanced) | bridge-ast-surgeon |
| `bridge-mcp/src/core/a11y/rules/aria.ts` | NEW FILE -- A11Y-030..A11Y-042 (valid roles, required children/parent, required attrs) | bridge-ast-surgeon |
| `bridge-mcp/src/core/a11y/rules/landmarks.ts` | NEW FILE -- A11Y-050..A11Y-054 (main, nav, banner, contentinfo, complementary) | bridge-ast-surgeon |
| `bridge-mcp/src/core/a11y/rules/contrast.ts` | NEW FILE -- A11Y-060..A11Y-063 (AA ratio, APCA, large text, UI components) | bridge-ast-surgeon |
| `bridge-mcp/src/core/a11y/rules/forms.ts` | NEW FILE -- A11Y-070..A11Y-077 (fieldset/legend, error, required, autocomplete) | bridge-ast-surgeon |
| `bridge-mcp/src/core/a11y/rules/live-regions.ts` | NEW FILE -- A11Y-080..A11Y-083 (aria-live, aria-atomic, role=alert, aria-relevant) | bridge-ast-surgeon |
| `bridge-mcp/src/core/a11y/rules/motion.ts` | NEW FILE -- A11Y-090..A11Y-091 (prefers-reduced-motion, autoplay) | bridge-ast-surgeon |
| `bridge-mcp/src/core/a11y/fixer.ts` | NEW FILE -- Auto-fix engine: applies deterministic fixes via AST mutation | bridge-ast-surgeon |
| `bridge-mcp/src/core/a11y/index.ts` | NEW FILE -- Public API barrel (re-exports runner, fixer, types) | bridge-ast-surgeon |
| `bridge-mcp/src/core/a11y/contrast-utils.ts` | NEW FILE -- APCA Lc calculation + WCAG 2.x AA contrast ratio helpers | bridge-ast-surgeon |
| `bridge-mcp/src/core/A11yLinter.ts` | MODIFY -- Delegate to new `a11y/runner.ts`, keep backward-compat API | bridge-ast-surgeon |
| `bridge-mcp/src/types.ts` | MODIFY -- Add `'a11y-fix'` to LinterWarning type union, add A11yReportResult type | bridge-ast-surgeon |
| `bridge-mcp/src/tools/accessibility.ts` | NEW FILE -- `bridge_accessibility_report` tool handler | bridge-ast-surgeon |
| `bridge-mcp/src/server.ts` | MODIFY -- Register `bridge_accessibility_report` tool + import | bridge-electron-ipc |
| `bridge-mcp/src/core/dashboard/debtReportService.ts` | MODIFY -- Update A11y scanning to use new runner | bridge-ast-surgeon |
| `src/core/A11yLinter.ts` | MODIFY -- Delegate to shared rule definitions (build-time sync) | bridge-ast-surgeon |
| `bridge-mcp/src/core/a11y/__tests__/runner.test.ts` | NEW FILE -- Runner unit tests | bridge-test-writer |
| `bridge-mcp/src/core/a11y/__tests__/fixer.test.ts` | NEW FILE -- Auto-fix unit tests | bridge-test-writer |
| `bridge-mcp/src/core/a11y/__tests__/rules-names-labels.test.ts` | NEW FILE -- Names/labels rule tests | bridge-test-writer |
| `bridge-mcp/src/core/a11y/__tests__/rules-aria.test.ts` | NEW FILE -- ARIA rule tests | bridge-test-writer |
| `bridge-mcp/src/core/a11y/__tests__/rules-landmarks.test.ts` | NEW FILE -- Landmark rule tests | bridge-test-writer |
| `bridge-mcp/src/core/a11y/__tests__/rules-contrast.test.ts` | NEW FILE -- Contrast rule tests | bridge-test-writer |
| `bridge-mcp/src/core/a11y/__tests__/rules-forms.test.ts` | NEW FILE -- Forms rule tests | bridge-test-writer |
| `bridge-mcp/src/core/a11y/__tests__/rules-keyboard.test.ts` | NEW FILE -- Keyboard rule tests | bridge-test-writer |
| `bridge-mcp/src/core/a11y/__tests__/rules-live-regions.test.ts` | NEW FILE -- Live regions rule tests | bridge-test-writer |
| `bridge-mcp/src/core/a11y/__tests__/rules-motion.test.ts` | NEW FILE -- Motion rule tests | bridge-test-writer |
| `bridge-mcp/src/core/a11y/__tests__/contrast-utils.test.ts` | NEW FILE -- APCA/WCAG contrast math tests | bridge-test-writer |
| `bridge-mcp/src/core/a11y/__tests__/accessibility-tool.test.ts` | NEW FILE -- MCP tool integration tests | bridge-test-writer |

---

## Type Contracts (Source of Truth for Phase 2)

### Core Types -- `bridge-mcp/src/core/a11y/types.ts`
...
export interface A11yRuleContext {
    ...
    /** Map of element bridge-id to its ARIA role (explicit or implicit). */
    elementRoles: Map<string, string>
    /** Map of id attribute values to their bridge-ids (for label associations). */
...
export interface A11yViolationDetail {
    ...
    ruleId: string
    /** The bridge-id of the offending element (or positional fallback). */
    elementId: string
...
export interface A11yFixResult {
    ...
    description: string
    /**
     * The mutations to apply via bridge_ast_mutate.
 ...
 * These map 1:1 to bridge_ast_mutate operation types.
 */
...
### Updated LinterWarning type -- `bridge-mcp/src/types.ts`
...
### MCP Tool Input/Output -- `bridge_accessibility_report`
...
## Integration with Existing `bridge_fix` Tool

The existing `bridge_fix` MCP tool handles Mithril token fixes. EXP.6 extends it:

1. `bridge_fix` currently receives `violationIds` and applies fixes.
2. We add A11y violations to the fixable set. When `bridge_fix` receives an A11Y violation ID, it delegates to `a11y/fixer.ts`.
3. The fixer receives the AST and the violation detail, runs the rule's `fix()` function, and returns the mutations.
4. `bridge_fix` applies the mutations via the same AST mutation pipeline used for Mithril fixes.
...
1. **Types** (`bridge-mcp/src/core/a11y/types.ts`) -- Define all interfaces. No dependencies. Owner: `bridge-ast-surgeon`.
2. **Helpers** (`bridge-mcp/src/core/a11y/helpers.ts`) -- Extract shared AST helpers from current A11yLinter. Owner: `bridge-ast-surgeon`.
3. **Contrast Utils** (`bridge-mcp/src/core/a11y/contrast-utils.ts`) -- WCAG contrast ratio + APCA math. Owner: `bridge-ast-surgeon`.

### Group 1 -- Core Engine (parallel after Group 0)

4. **Runner** (`bridge-mcp/src/core/a11y/runner.ts`) -- Rule loading + execution engine. Depends on types + helpers. Owner: `bridge-ast-surgeon`.
5. **Fixer** (`bridge-mcp/src/core/a11y/fixer.ts`) -- Auto-fix engine. Depends on types. Owner: `bridge-ast-surgeon`.
...
6a. `rules/names-labels.ts` -- Owner: `bridge-ast-surgeon`
6b. `rules/keyboard.ts` -- Owner: `bridge-ast-surgeon`
6c. `rules/structure.ts` -- Owner: `bridge-ast-surgeon`
6d. `rules/aria.ts` -- Owner: `bridge-ast-surgeon`
6e. `rules/landmarks.ts` -- Owner: `bridge-ast-surgeon`
6f. `rules/contrast.ts` -- Owner: `bridge-ast-surgeon`
6g. `rules/forms.ts` -- Owner: `bridge-ast-surgeon`
6h. `rules/live-regions.ts` -- Owner: `bridge-ast-surgeon` (EXP.6b)
6i. `rules/motion.ts` -- Owner: `bridge-ast-surgeon` (EXP.6b)

### Group 3 -- Integration (after Group 2)

7. **Barrel export** (`bridge-mcp/src/core/a11y/index.ts`) -- Owner: `bridge-ast-surgeon`.
8. **Backward-compat shim** (modify `bridge-mcp/src/core/A11yLinter.ts`) -- Delegate to new runner, preserve `audit()` API. Owner: `bridge-ast-surgeon`.
9. **MCP tool handler** (`bridge-mcp/src/tools/accessibility.ts`) -- Owner: `bridge-ast-surgeon`.
10. **Server registration** (modify `bridge-mcp/src/server.ts`) -- Owner: `bridge-electron-ipc`.
11. **Types update** (modify `bridge-mcp/src/types.ts`) -- Add `wcag` and `fixable` fields. Owner: `bridge-ast-surgeon`.
12. **DebtReport integration** (modify `bridge-mcp/src/core/dashboard/debtReportService.ts`) -- Use new runner. Owner: `bridge-ast-surgeon`.
13. **Renderer-side sync** (modify `src/core/A11yLinter.ts`) -- Delegate to shared rules. Owner: `bridge-ast-surgeon`.

### Group 4 -- Tests (parallel with Groups 2-3)

14. All test files listed in Impact Map. Owner: `bridge-test-writer`.

### Group 5 -- Review (after all above)

15. Full integration review. Owner: `bridge-code-reviewer`.
...
**Mitigation:** Short-term: build-time sync script. Medium-term (EXP.6c): deprecate renderer-side A11yLinter entirely and have Glass consume `bridge://violations/{filePath}` for all A11y data. The MCP server is always the source of truth.
...
```
[bridge-mcp]: 120/120 passing (110 new)
TSC: 0 errors
```
