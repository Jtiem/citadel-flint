...
**Depends on:** Phase W.1 (MCP Push Channel) ONLINE, Phase W.3 (Bidirectional Action Bridge) ONLINE, Phase 1A (Context Bridge) ONLINE, EXP.2 (Design Debt Report) ONLINE

---

## 0. Problem Statement

An AI agent connecting to Bridge MCP today must make 3-4 sequential tool calls (`bridge_read_code`, `bridge_read_tokens`, `bridge_audit_mithril`, `bridge_audit_a11y`) before performing any useful work. This costs approximately 2,000-4,000 tokens per session bootstrap, adds 3-8 seconds of latency, and means every agent session starts cold with no awareness of the user's current activity.
...
|------|------------|-------------|-------|
| `bridge-mcp/src/core/sessionContext.ts` | NEW | `bridge-ast-surgeon` | Core assembly logic: reads context.json, tokens, violations, debt, mutations |
| `bridge-mcp/src/core/contextPush.ts` | NEW | `bridge-ast-surgeon` | Event-driven delta computation + JSONL emission |
| `bridge-mcp/src/core/complexityRouter.ts` | NEW | `bridge-ast-surgeon` | Task complexity analysis + model tier recommendation |
| `bridge-mcp/src/core/toolEnricher.ts` | NEW | `bridge-ast-surgeon` | Pre-flight context injection for mutation tools |
| `bridge-mcp/src/prompts/sentinel.ts` | NEW | `bridge-ast-surgeon` | Domain-configurable governance sentinel prompt |
| `bridge-mcp/src/domains/fintech.ts` | NEW | `bridge-ast-surgeon` | Fintech domain rules (PCI-DSS) |
| `bridge-mcp/src/domains/healthcare.ts` | NEW | `bridge-ast-surgeon` | Healthcare domain rules (HIPAA) |
| `bridge-mcp/src/domains/ecommerce.ts` | NEW | `bridge-ast-surgeon` | E-commerce domain rules |
| `bridge-mcp/src/domains/index.ts` | MODIFY | `bridge-ast-surgeon` | Register new domains |
| `bridge-mcp/src/server.ts` | MODIFY | `bridge-ast-surgeon` | Register new resource, tool enrichment intercept, context push wiring |
| `bridge-mcp/src/core/events.ts` | MODIFY | `bridge-ast-surgeon` | Add new event types for context push |
| `bridge-mcp/src/core/capabilities/index.ts` | MODIFY | `bridge-ast-surgeon` | Add `bridge://session-context` to catalog, add `bridge_get_context` tool |
| `bridge-mcp/src/types.ts` | MODIFY | `bridge-ast-surgeon` | Add SessionContext, ContextDelta, ComplexityAssessment types |
| `src/hooks/useContextSync.ts` | MODIFY | `bridge-state-architect` | Extend BridgeContext with source excerpt + structured violations |
| `src/types/bridge-api.d.ts` | MODIFY | `bridge-state-architect` | Extend BridgeContext type with new fields |
| `electron/main.ts` | MODIFY | `bridge-electron-ipc` | Add `context:get-enriched` IPC for Glass-side enriched context reads |
| `electron/preload.ts` | MODIFY | `bridge-electron-ipc` | Expose `context.getEnriched()` on bridgeAPI |
| `bridge-mcp/src/__tests__/sessionContext.test.ts` | NEW | `bridge-test-writer` | ACX-01 through ACX-08 |
| `bridge-mcp/src/__tests__/contextPush.test.ts` | NEW | `bridge-test-writer` | ACX-09 through ACX-12 |
| `bridge-mcp/src/__tests__/complexityRouter.test.ts` | NEW | `bridge-test-writer` | ACX-13 through ACX-17 |
| `bridge-mcp/src/__tests__/toolEnricher.test.ts` | NEW | `bridge-test-writer` | ACX-18 through ACX-22 |
| `bridge-mcp/src/__tests__/sentinel.test.ts` | NEW | `bridge-test-writer` | ACX-23 through ACX-26 |

---

## 2. Type Contracts

### 2.1 SessionContext (new, in `bridge-mcp/src/types.ts`)

This is the primary payload for `bridge://session-context`. It is the rich, pre-assembled snapshot that eliminates the 3-4 cold-start tool calls.
 ...
 * Returned by `bridge://session-context` resource and the
 * `bridge_get_context` tool. Eliminates cold-start round-trips.
 *
...
export interface SessionContext {
    ...
    /** Absolute path to the project root (.bridge directory parent). */
    projectRoot: string
...
export interface ActiveFileContext {
    ...
    totalLines: number
    /** data-bridge-id of the currently selected node, or null. */
...
export interface NodeSummary {
    ...
    childCount: number
    /** data-bridge-id of the parent element, or null if root. */
...
export interface ViolationDetail {
    /** data-bridge-id of the affected element. */
    nodeId: string
 ...
 * to understand the token vocabulary without reading bridge://tokens.
 */
...
export interface CanvasSnapshot {
    ...
    saveState: string
    /** data-bridge-id of selected node, or null. */
    selectedNodeId: string | null
 ...
 * state changes occur in Bridge.
 *
 * Written to .bridge/mcp-events.jsonl using the existing
...
/**
 * Context for bridge_ast_mutate: target node details.
 */
export interface MutateEnrichment {
    ...
    /** Target node's parent tag + bridge ID. */
...
/**
 * Context for bridge_fix: violation details + suggested fix.
 */
...
/**
 * Context for bridge_insert_node: insertion environment.
 */
...
```typescript
/**
 * Domain-specific rule set for the bridge-sentinel prompt.
...
### 3.1 New Resource: `bridge://session-context`
...
1. `.bridge/context.json` -- written by Glass `useContextSync` hook (activeFile, selectedNode, canvasMode, saveState, violation counts)
2. `.bridge/design-tokens.json` -- token count, type breakdown, collection list
3. Active file source -- `fs.readFileSync(activeFilePath)` first 200 lines
4. `.bridge/debt-history.json` -- latest entry for health score/grade
...
### 3.2 New Resource: `bridge://complexity`

| Property | Value |
|----------|-------|
| URI | `bridge://complexity?task={description}` |
| MIME type | `application/json` |
...
### 3.3 Modified Resource: `bridge://capabilities`

Add `bridge://session-context` and `bridge://complexity` to the resources catalog in `bridge-mcp/src/core/capabilities/index.ts`.

---

## 4. MCP Tools

### 4.1 New Tool: `bridge_get_context`
...
Returns: `SessionContext` (same payload as `bridge://session-context` resource).

### 4.2 New Tool: `bridge_assess_complexity`
...
|------|----------------|-----------------|
| `bridge_ast_mutate` | `MutateEnrichment` | Target node props, className, parent context, sibling IDs, active violations |
| `bridge_fix` | `FixEnrichment` | Violation details for the target file, suggested fix ops |
| `bridge_insert_node` (via `bridge_ast_mutate`) | `InsertEnrichment` | Sibling nodes, parent summary, relevant design tokens |
...
This channel allows Glass to request an enriched context snapshot from the main process, which in turn queries the MCP server. Used for debugging and for the Context Bridge to write richer data.
...
### 5.2 Extended IPC Push: `bridge:context-delta`
...
| Property | Value |
|----------|-------|
| Channel | `bridge:mcp-event` (existing) |
| Direction | MCP server -> .bridge/mcp-events.jsonl -> main (fs.watch) -> renderer |
...
The existing `useContextSync` hook is extended to write richer data to `.bridge/context.json`. Specifically, the `BridgeContext` type gains:

```typescript
// Added to existing BridgeContext in src/types/bridge-api.d.ts
...
**Anti-pattern guard:** The `useContextSync` hook reads from Zustand stores only. It does NOT call `window.bridgeAPI` -- it only writes via the existing `syncContext` call. The source excerpt read happens in the main process when assembling SessionContext, NOT in the renderer.

**IMPORTANT REVISION:** On review, injecting `sourceExcerpt` into the renderer-side BridgeContext would require the renderer to read file contents, which violates the process boundary. Instead, the MCP server reads the source file directly when assembling SessionContext. The renderer-side BridgeContext extensions are limited to `violationDetails`, `exportBlocked`, and `exportBlockReason` -- all of which are already available in renderer stores.
...
**Location:** `bridge-mcp/src/core/sessionContext.ts`
 ...
 *   1. .bridge/context.json          -- fs.readFileSync (< 1ms)
 *   2. .bridge/design-tokens.json    -- fs.readFileSync (< 5ms)
 *   3. Active file source            -- fs.readFileSync first 200 lines (< 2ms)
 *   4. .bridge/debt-history.json     -- fs.readFileSync last entry (< 1ms)
...
**Process boundary:** This module runs in the MCP server child process. It reads `.bridge/` files directly via `fs` (allowed -- MCP server is a separate Node.js process, not the Electron main or renderer). For the mutation ledger, it reads from the `.bridge/mutations.db` SQLite file (if managed by the MCP server) or from the `.bridge/mcp-events.jsonl` file.

**Critical design decision:** The MCP server does NOT have access to the Electron main process SQLite (`bridge.db`). Mutation history must be sourced from either:
- Option A: The `mcp-events.jsonl` file (already written by `appendMCPEvent`)
- Option B: A dedicated `.bridge/mutations.json` ledger file written by the MCP server
...
**Location:** `bridge-mcp/src/core/contextPush.ts`

```typescript
/**
 * ContextPushManager
 *
 * Listens for state changes via the bridgeEvents emitter and
 * fs.watch on key .bridge/ files. When a significant change is
 * detected, computes a ContextDelta and writes it to mcp-events.jsonl
 * with type 'context-delta'.
 *
 * Watches:
 *   - .bridge/context.json       -- file switch, selection change
 *   - .bridge/design-tokens.json -- token CRUD
 *   - .bridge/debt-history.json  -- health score changes
 *
...
**Location:** `bridge-mcp/src/core/complexityRouter.ts`
...
**Location:** `bridge-mcp/src/core/toolEnricher.ts`
 ...
 *   - bridge_ast_mutate -> MutateEnrichment
 *   - bridge_fix        -> FixEnrichment
 *
 * The enrichment is added as a leading content block in the tool's
 * response, formatted as:
 *
 *   --- Bridge Context Preamble ---
 *   Target node: <div data-bridge-id="bridge-card-root" className="bg-blue-500 p-4">
 *   Parent: <section data-bridge-id="bridge-main">
 *   Violations on this node: MITHRIL-COL (deltaE 3.2, fixable)
 *   Siblings: [bridge-header, bridge-footer]
 *   ---
 *
...
**Location:** `bridge-mcp/src/prompts/sentinel.ts`

```typescript
/**
 * BRIDGE_SENTINEL_PROMPT_DEF — prompt definition for ListPrompts.
 * getBridgeSentinelContent(domain: string) — returns the full prompt body.
 *
 * Base structure (always included):
 *   1. Identity: "You are the Bridge Governance Sentinel."
 ...
 */
export const BRIDGE_SENTINEL_PROMPT_DEF = {
...
Healthcare (`bridge-mcp/src/domains/healthcare.ts`):
```
...
Fintech (`bridge-mcp/src/domains/fintech.ts`):
```
...
E-commerce (`bridge-mcp/src/domains/ecommerce.ts`):
```
...
1. A new `bridge_get_context` case that calls `assembleSessionContext()`
2. A new `bridge_assess_complexity` case that calls `assessComplexity()`
...
case "bridge_ast_mutate": {
    ...
    const enrichment = enrichToolCall("bridge_ast_mutate", args, cachedContext);
    if (enrichment) {
...
Add `bridge://session-context` handling to the existing ReadResource handler:
...
The `ContextPushManager` is instantiated once during MCP server startup (after `projectRoot` is resolved) and destroyed on server shutdown. It watches `.bridge/` files and emits deltas to `mcp-events.jsonl`.
...
| C4 | Local-First Only | YES | All data sources are local files (`.bridge/`) and local SQLite. No external URLs. |
...
| C14 | Bypass Prohibition | N/A | ACX does not modify source files. Reads from `.bridge/` are standard MCP server behavior. |
| C15 | Granular AST Tools Only | YES | `bridge_get_context` and `bridge_assess_complexity` are read-only tools. The enrichment layer does not add new mutation verbs. |
...
|-----------|--------|----------|
| `assembleSessionContext()` full assembly | < 100ms | Parallel fs.readFile for .bridge/ files; cached audit results |
...
| Tool enrichment | < 20ms | Targeted Babel traversal for single node by bridge-id |
...
**Agents:** `bridge-ast-surgeon`
**Blocking:** Nothing -- can start immediately.

1. Create `bridge-mcp/src/core/sessionContext.ts`
...
2. Create `bridge-mcp/src/prompts/sentinel.ts`
   - Implement `BRIDGE_SENTINEL_PROMPT_DEF` and `getBridgeSentinelContent()`
   - Create base governance prompt body
3. Create `bridge-mcp/src/domains/healthcare.ts`, `fintech.ts`, `ecommerce.ts`
   - Register domain rules
4. Modify `bridge-mcp/src/domains/index.ts`
   - Register new domains
5. Modify `bridge-mcp/src/types.ts`
   - Add all types from Section 2.1 through 2.5
6. Modify `bridge-mcp/src/server.ts`
   - Register `bridge://session-context` resource
   - Implement `bridge_get_context` tool handler
   - Implement `bridge_assess_complexity` tool handler
7. Modify `bridge-mcp/src/core/capabilities/index.ts`
...
1. Create `bridge-mcp/src/core/contextPush.ts`
   - Implement `ContextPushManager` with fs.watch on .bridge/ files
   - Implement delta computation logic
   - Implement debounce (300ms per file)
2. Modify `bridge-mcp/src/core/events.ts`
   - Add `'context-delta'` to `MCPEventType`
3. Modify `bridge-mcp/src/server.ts`
...
1. Create `bridge-mcp/src/core/toolEnricher.ts`
   - Implement `enrichToolCall()` for `bridge_ast_mutate` and `bridge_fix`
   - Implement AST-based node summary extraction (Babel traverse)
2. Modify `bridge-mcp/src/server.ts`
   - Add enrichment intercept to `bridge_ast_mutate` and `bridge_fix` handlers
...
1. Create `bridge-mcp/src/core/complexityRouter.ts`
   - Implement `assessComplexity()` with weighted scoring
   - Implement tier mapping (fast/balanced/powerful)
2. Wire into `bridge_assess_complexity` tool (already registered in ACX.1)

### Phase ACX.5 -- Extended Context Sync (Parallel Group A)

**Agents:** `bridge-state-architect` + `bridge-electron-ipc`
**Blocking:** Nothing -- can start immediately (only extends existing hook).

1. Modify `src/types/bridge-api.d.ts`
   - Extend `BridgeContext` with `violationDetails`, `exportBlocked`, `exportBlockReason`
...
4. Modify `electron/preload.ts`
   - Expose `context.getEnriched()` on bridgeAPI

### Phase ACX.6 -- Tests (Parallel -- each subphase gets tests)

**Agents:** `bridge-test-writer`
...
**Agents:** `bridge-integration-validator`
**Blocking:** ACX.1 through ACX.6 must be ONLINE.

1. End-to-end: MCP client connects -> reads `bridge://session-context` -> gets full context in one call
2. End-to-end: File switch in Glass -> `bridge:mcp-event` with `context-delta` -> delta arrives in host IDE
3. End-to-end: `bridge_ast_mutate` call -> enrichment prepended -> agent sees node context
4. End-to-end: `bridge_assess_complexity` -> correct tier for known test cases
5. End-to-end: `bridge-sentinel` prompt with `domain: "healthcare"` -> HIPAA rules in prompt
...
|---------|-----------------|---------------|----------|
| ACX-01 | assembleSessionContext returns valid SessionContext shape | `bridge-mcp/src/__tests__/sessionContext.test.ts` | HIGH |
| ACX-02 | assembleSessionContext completes in < 100ms | `bridge-mcp/src/__tests__/sessionContext.test.ts` | HIGH |
| ACX-03 | SessionContext includes active file source excerpt (first 200 lines) | `bridge-mcp/src/__tests__/sessionContext.test.ts` | HIGH |
| ACX-04 | SessionContext includes violation details with nodeIds | `bridge-mcp/src/__tests__/sessionContext.test.ts` | HIGH |
| ACX-05 | SessionContext token snapshot has correct byType breakdown | `bridge-mcp/src/__tests__/sessionContext.test.ts` | HIGH |
| ACX-06 | SessionContext with no active file returns activeFile: null | `bridge-mcp/src/__tests__/sessionContext.test.ts` | MEDIUM |
| ACX-07 | SessionContext caching: second call within 500ms returns cached value | `bridge-mcp/src/__tests__/sessionContext.test.ts` | HIGH |
| ACX-08 | SessionContext cache invalidation on token file change | `bridge-mcp/src/__tests__/sessionContext.test.ts` | MEDIUM |
| ACX-09 | ContextPushManager emits file-switched delta on activeFile change | `bridge-mcp/src/__tests__/contextPush.test.ts` | HIGH |
| ACX-10 | ContextPushManager emits violations-changed delta on count change | `bridge-mcp/src/__tests__/contextPush.test.ts` | HIGH |
| ACX-11 | ContextPushManager suppresses delta for cursor-only changes | `bridge-mcp/src/__tests__/contextPush.test.ts` | MEDIUM |
| ACX-12 | ContextPushManager debounces at 300ms per file | `bridge-mcp/src/__tests__/contextPush.test.ts` | HIGH |
| ACX-13 | assessComplexity returns 'fast' for single-prop edit (score < 30) | `bridge-mcp/src/__tests__/complexityRouter.test.ts` | HIGH |
| ACX-14 | assessComplexity returns 'balanced' for multi-node, single-file task | `bridge-mcp/src/__tests__/complexityRouter.test.ts` | HIGH |
| ACX-15 | assessComplexity returns 'powerful' for cross-file refactor | `bridge-mcp/src/__tests__/complexityRouter.test.ts` | HIGH |
| ACX-16 | assessComplexity factors sum to 100 | `bridge-mcp/src/__tests__/complexityRouter.test.ts` | MEDIUM |
| ACX-17 | assessComplexity with empty input returns 'fast' (safe default) | `bridge-mcp/src/__tests__/complexityRouter.test.ts` | MEDIUM |
| ACX-18 | enrichToolCall for bridge_ast_mutate includes target node props | `bridge-mcp/src/__tests__/toolEnricher.test.ts` | HIGH |
| ACX-19 | enrichToolCall for bridge_ast_mutate includes active violations on node | `bridge-mcp/src/__tests__/toolEnricher.test.ts` | HIGH |
| ACX-20 | enrichToolCall for bridge_fix includes suggested fix ops | `bridge-mcp/src/__tests__/toolEnricher.test.ts` | HIGH |
| ACX-21 | enrichToolCall returns null for non-enrichable tools | `bridge-mcp/src/__tests__/toolEnricher.test.ts` | MEDIUM |
| ACX-22 | enrichToolCall completes in < 20ms | `bridge-mcp/src/__tests__/toolEnricher.test.ts` | MEDIUM |
| ACX-23 | getBridgeSentinelContent("ui") returns base rules only | `bridge-mcp/src/__tests__/sentinel.test.ts` | HIGH |
| ACX-24 | getBridgeSentinelContent("healthcare") includes HIPAA rules | `bridge-mcp/src/__tests__/sentinel.test.ts` | HIGH |
| ACX-25 | getBridgeSentinelContent("fintech") includes PCI-DSS rules | `bridge-mcp/src/__tests__/sentinel.test.ts` | HIGH |
| ACX-26 | getBridgeSentinelContent with unknown domain falls back to "ui" | `bridge-mcp/src/__tests__/sentinel.test.ts` | MEDIUM |
| ACX-27 | bridge_get_context tool returns SessionContext | `bridge-mcp/src/__tests__/sessionContext.test.ts` | HIGH |
| ACX-28 | bridge://session-context resource returns same data as tool | `bridge-mcp/src/__tests__/sessionContext.test.ts` | MEDIUM |
| ACX-29 | BridgeContext extended fields (violationDetails, exportBlocked) populated | `src/hooks/__tests__/useContextSync.test.ts` | HIGH |
...
| **Cross-process data freshness** | Medium | LOW | None | The MCP server reads .bridge/ files which are written by the Electron main process. There is a latency window (200ms debounce + fs.watch propagation). The 500ms cache TTL absorbs this. |
...
The MCP SDK supports `notifications/resources/list_changed` (already used for token updates) but does NOT support arbitrary push messages to clients. The `mcp-events.jsonl` file channel is a Bridge-specific mechanism that works only when Glass is running. For pure CLI agents (Claude Code without Glass), context deltas must be polled via `bridge://session-context`.

Decision: **Dual path.** Glass gets deltas via `mcp-events.jsonl` -> `bridge:mcp-event` IPC. CLI agents poll `bridge://session-context` with the built-in MCP resource read. The resource emits `notifications/resources/list_changed` when context changes, which is the MCP-standard way to signal "re-read this resource."

### 15.5 Sentinel Prompt Composition vs Replacement

Should the sentinel prompt compose with `bridge-intent-composer` or replace it? Decision: **Compose.** The sentinel prompt is a standalone persona. An agent can load both prompts if desired: `bridge-intent-composer` for the design-to-code persona plus `bridge-sentinel` for domain-specific governance overlay. They are additive.
...
| File | Why |
|------|-----|
| `bridge-mcp/src/server.ts` | Resource/tool registration patterns, existing event wiring |
| `bridge-mcp/src/core/events.ts` | MCPEvent types, appendMCPEvent pattern, file rotation |
| `bridge-mcp/src/core/capabilities/index.ts` | Catalog structure for adding new resources/tools |
| `bridge-mcp/src/core/dashboard/debtReportService.ts` | How to read .bridge/debt-history.json |
| `bridge-mcp/src/core/dashboard/types.ts` | DebtReport, DashboardData shapes |
| `bridge-mcp/src/core/MithrilLinter.ts` | auditAll() signature and return type |
| `bridge-mcp/src/core/A11yLinter.ts` | A11yLinter audit signature |
| `bridge-mcp/src/types.ts` | Existing LinterWarning, DesignToken shapes |
| `bridge-mcp/src/domains/index.ts` | Domain registry pattern |
| `bridge-mcp/src/prompts/workflow-guide.ts` | Pattern for prompt module structure |
| `src/hooks/useContextSync.ts` | How context.json is assembled and written |
| `src/types/bridge-api.d.ts` | BridgeContext type, BridgeAPI surface |
...
| File | Why |
|------|-----|
| `bridge-mcp/src/core/ast-modifier.ts` | How mutations locate nodes by bridge-id |
| `bridge-mcp/src/tools/audit.ts` | handleBridgeAudit patterns |
| `bridge-mcp/src/tools/fix.ts` | handleBridgeFix patterns |
