...
Four changes, all within `bridge-mcp/src/`. Zero Glass/Electron changes. Zero new IPC channels.

| Change | Tools Affected |
|--------|---------------|
| `summary` field on tool responses | `bridge_audit`, `bridge_fix`, `bridge_ast_mutate`, `bridge_debt_report`, `audit_ui_component`, `bridge_swarm_audit_fix` |
| `project_context` footer | `bridge_audit`, `bridge_fix`, `bridge_ast_mutate` |
| `dry_run` flag formalization | `bridge_fix` (existing schema, missing implementation), `bridge_ast_mutate` (new `dryRun` param) |
...
|------|-------------|-------------|
| `bridge-mcp/src/server.ts` | Modify -- add `instructions` to Server constructor; update `bridge_ast_mutate` case to support `dryRun`; wire `summary` + `project_context` into tool response assembly for `bridge_audit`, `bridge_fix`, `bridge_ast_mutate`, `bridge_debt_report`, `audit_ui_component` | `bridge-ast-surgeon` |
| `bridge-mcp/src/tools/audit.ts` | Modify -- add `generateAuditSummary()` function; add `summary` and `project_context` fields to `AuditResult` and `BatchAuditResult` interfaces; populate them in handlers | `bridge-ast-surgeon` |
| `bridge-mcp/src/tools/fix.ts` | Modify -- add `generateFixSummary()` function; add `summary` and `project_context` fields to `FixResult`; implement `dryRun` guard in handler body; populate summary | `bridge-ast-surgeon` |
| `bridge-mcp/src/tools/debtReport.ts` | Modify -- add `summary` field to debt report handler output | `bridge-ast-surgeon` |
| `bridge-mcp/src/tools/swarm.ts` | Modify -- add `summary` field to `SwarmReport` | `bridge-ast-surgeon` |
| `bridge-mcp/src/core/projectContext.ts` | **New file** -- `loadProjectContext()` utility that reads debt history + runs a fast aggregate to produce `ProjectContext` | `bridge-ast-surgeon` |
| `bridge-mcp/src/__tests__/responseQuality.test.ts` | **New file** -- tests for summary generation, project_context population, dry_run behavior, server instructions | `bridge-test-writer` |
| `bridge-mcp/src/__tests__/safety-promises.test.ts` | Modify -- update Test 5 (`bridge_audit`) and Test 6 (`bridge_fix`) assertions to account for new `summary` field in response shape | `bridge-test-writer` |
...
```typescript
// bridge-mcp/src/core/projectContext.ts
 ...
 *   1. .bridge/debt-history.json -- read the most recent entry (O(1) file read)
...
**Data source:** `.bridge/debt-history.json` (the file already written by `generateDebtReport({ track: true })`). Read the **last entry** in the array for `health_score`, `grade`, `total_violations`. `blocked_files` defaults to 0 when sourced from history (history entries don't carry file-level data). When `.bridge/debt-history.json` does not exist, return `null`.
...
```typescript
// bridge-mcp/src/tools/swarm.ts -- add to existing SwarmReport
...
The `bridge_ast_mutate` handler in `server.ts` does not have a dedicated return type -- it builds the response inline. CX.1 adds a `summary` text content block and an optional `project_context` JSON block to the response array. No new interface needed; the contract is defined by the insertion points in `server.ts`.
...
Where `{basename}` is `path.basename(filePath)`, `{fixable}` is `mithrilCount` (Mithril violations are auto-fixable by `bridge_fix`; a11y violations require manual fix).

### 4.2 `bridge_audit` (batch)
...
|------|---------------------------|
| `bridge_audit` (single) | Yes -- in `AuditResult` |
| `bridge_audit` (batch) | Yes -- in `BatchAuditResult` |
| `bridge_fix` | Yes -- in `FixResult` |
| `bridge_ast_mutate` | Yes -- as a separate JSON content block |
| `bridge_debt_report` | **No** -- the tool IS the project context; redundant |
| `audit_ui_component` | **No** -- legacy tool, minimal changes |
| `bridge_swarm_audit_fix` | **No** -- already returns before/after health |

### 5.2 How it's loaded

In `bridge_audit` and `bridge_fix` handlers (in `audit.ts` and `fix.ts`):
```typescript
...
In `server.ts` for `bridge_ast_mutate`:
```typescript
import { loadProjectContext } from './core/projectContext.js';

// Inside the bridge_ast_mutate case, after building mutateResult:
try {
...
- `.bridge/debt-history.json` does not exist
...
**Current state:** `dryRun` exists in the input schema and `FixArgs` type, but `handleBridgeFix` ignores it. The handler always produces `fixedSource` by traversing and mutating the AST. No file writing happens in the handler itself -- writing is the caller's responsibility.
...
3. In `server.ts` `bridge_fix` case: skip provenance recording when `dryRun: true` (already done).
4. In `server.ts` `bridge_fix` case: skip file write when `dryRun: true` (note: `bridge_fix` currently never writes to disk in server.ts -- it returns fixed source for the caller to write. This means `dryRun` is already effectively true. The flag's purpose is the **summary text** and the **agent's behavioral signal**).
...
**Implementation:** In `server.ts` `bridge_ast_mutate` case, near the top:
```typescript
...
const server = new Server(
  { name: "bridge-mcp-server", version: "1.0.0" },
...
| 14 | Bypass Prohibition | Yes | `loadProjectContext` uses `fs.readFileSync` on `.bridge/debt-history.json`. This is acceptable: the function runs in the MCP server process (Node.js), not in the renderer. It reads a Bridge-managed metadata file, not user source code. It does not write. |
...
### 9.1 New Tests (`bridge-mcp/src/__tests__/responseQuality.test.ts`)
...
| CX1-14 | `handleBridgeAudit` includes `summary` field | `result.summary` is a non-empty string |
| CX1-15 | `handleBridgeAudit` includes `project_context` when history exists | `result.project_context.health_score` is a number |
| CX1-16 | `handleBridgeAudit` omits `project_context` when no history | `result.project_context` is `undefined` |
| CX1-17 | `handleBridgeFix` includes `summary` and `dryRun` fields | Both present in result |
| CX1-18 | `handleBridgeFix` with `dryRun: true` -- summary says DRY RUN | `result.summary.includes('DRY RUN')` |
| CX1-19 | `handleBridgeFix` with `dryRun: true` -- still returns fixedSource | `result.fixedSource` is non-empty |
...
| File | Test | Change |
|------|------|--------|
| `bridge-mcp/src/__tests__/safety-promises.test.ts` | Test 5 ("bridge_audit returns structured violations") | Assert `result.summary` exists and is a string |
| `bridge-mcp/src/__tests__/safety-promises.test.ts` | Test 6 ("bridge_fix auto-fixes and re-audit passes") | Assert `result.summary` exists; assert `result.dryRun` is `false` |
| `bridge-mcp/src/__tests__/healOnAudit.test.ts` | Any test checking `AuditResult` shape | Add `summary` to expected shape |
| `bridge-mcp/src/__tests__/toolEnricher.test.ts` | Tests that mock/assert tool result shapes | Update expected shapes if they assert on the full `FixResult` or `AuditResult` |
...
|------|-------|------|------------|
| A1 | `bridge-ast-surgeon` | Create `bridge-mcp/src/core/projectContext.ts` with `loadProjectContext()` | Nothing |
| A2 | `bridge-ast-surgeon` | Add summary generation functions and new type fields to `bridge-mcp/src/tools/audit.ts` | Nothing |
| A3 | `bridge-ast-surgeon` | Add summary generation function, `dryRun` field, and new type fields to `bridge-mcp/src/tools/fix.ts` | Nothing |
| A4 | `bridge-ast-surgeon` | Add `summary` field to `SwarmReport` in `bridge-mcp/src/tools/swarm.ts` | Nothing |

### Group B (depends on Group A)

| Step | Agent | What | Blocked By |
|------|-------|------|------------|
| B1 | `bridge-ast-surgeon` | Wire `server.ts`: add `instructions` to Server constructor; update `bridge_audit` case to use new summary + project_context; update `bridge_fix` case; update `bridge_ast_mutate` case (dryRun alias, summary block, project_context block); update `bridge_debt_report` case (summary); update `audit_ui_component` case (summary) | A1, A2, A3 |

### Group C (depends on Group A; can run parallel with B)

| Step | Agent | What | Blocked By |
|------|-------|------|------------|
| C1 | `bridge-test-writer` | Write `bridge-mcp/src/__tests__/responseQuality.test.ts` (CX1-01 through CX1-25) | A1, A2, A3 |
| C2 | `bridge-test-writer` | Update existing tests in `safety-promises.test.ts`, `healOnAudit.test.ts`, `toolEnricher.test.ts` | A2, A3 |

### Group D (depends on B and C)

| Step | Agent | What | Blocked By |
|------|-------|------|------------|
| D1 | `bridge-test-writer` | Run full MCP test suite (`cd bridge-mcp && npm test`), report counts | B1, C1, C2 |
| D2 | `bridge-test-writer` | Run `npx tsc --noEmit`, confirm 0 errors | B1 |
...
|------|----------------------|------------|
| `loadProjectContext` reads `.bridge/debt-history.json` which may not exist on fresh projects | C.4 (Local-First) | Returns `null` gracefully. All `project_context` usage is guarded by `!= null` check. |
...
| `bridge_ast_mutate` dryRun + writeFile interaction | C.12 (Atomic Queuing) | `dryRun: true` forces `writeFile = false`. No ambiguity. Explicit precedence rule documented above. |
...
| `bridge_fix` handler already ignores `dryRun` -- CX.1 doesn't change that | Expectation mismatch | The contract explicitly documents that `dryRun` affects **response labeling and provenance**, not handler behavior. The handler never wrote to disk anyway. |
...
- **`bridge_plan`** (Section 5) -- already implemented as Phase CX.2.
...
2. `bridge_audit`, `bridge_fix`, and `bridge_ast_mutate` include `project_context` when `.bridge/debt-history.json` exists.
3. `bridge_fix` and `bridge_ast_mutate` respect `dryRun: true` with correct summary labeling and no side effects (no provenance, no file write, no MRS).
