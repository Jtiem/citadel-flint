...
Every `governance:*` IPC handler in `electron/main.ts` has a matching `contextBridge` entry in `electron/preload.ts` AND matching `window.bridgeAPI.governance.*` usage in the renderer components.
...
**governanceStore does NOT call window.bridgeAPI.governance.** The GOV.2 telemetry IPC calls are correctly placed in the GovernancePanel component event handlers (`handleToggle`, `handleResetRule`, `handleReset`), not inside the store's `setOverride`/`resetOverride`/`resetAll` actions. This follows the contract's R3 risk mitigation and the architectural anti-pattern rule.

**Pre-existing warning (not caused by GOV.1/GOV.2):** `governanceStore.ts` calls `window.bridgeAPI.saveRuleOverrides()` and `window.bridgeAPI.getRuleOverrides()` directly inside its `saveToFile` and `loadFromFile` actions. These are pre-existing IPC calls in store actions that violate the architectural anti-pattern rule. Acknowledged in the contract as R5 (Known Gap). Not blocking for this review.

**Pre-existing warning:** Multiple stores call `window.bridgeAPI` in their actions (`tokenStore`, `orchestratorStore`, `canvasStore`, `editorStore`, `astBufferStore`, `annotationStore`, `assetStore`). These are pre-existing architectural debt, not introduced by GOV.1/GOV.2.

## Contract Fidelity: PASS (with one minor signature deviation)

### RuleProvenance -- MATCH
All six fields (`ruleId`, `ruleName`, `sourceAuthority`, `regulatoryReference`, `lastUpdated`, `rationale`) match the contract exactly in both `bridge-mcp/src/core/governance/types.ts` (lines 28-41) and `src/types/bridge-api.d.ts` (lines 862-875).
...
| **C9 (Process Boundary)** | PASS | No `fs`, `path`, `child_process`, `sqlite3`, or `@anthropic-ai/sdk` imports anywhere in `src/`. All cross-boundary calls go through `window.bridgeAPI.governance.*`. |
...
### bridge-mcp tests: 25/25 new tests passing (366/366 total)
...
**Root cause: `createMockBridgeAPI()` in `src/components/__tests__/setup.ts` is missing the `governance` namespace.** The GOV.1/GOV.2 implementation adds `window.bridgeAPI.governance.getOverrideCount()` (called by StatusBar) and `window.bridgeAPI.governance.getComplianceSummary()` (called by ExportModal). When these methods are called in tests, they throw because `window.bridgeAPI.governance` is `undefined`.
...
**Fix required:** Add the following to `createMockBridgeAPI()` in `src/components/__tests__/setup.ts`:
...
**Owner agent:** bridge-test-writer
...
- All new `window.bridgeAPI.governance` calls have corresponding type declarations in `src/types/bridge-api.d.ts` (GovernanceAPI interface, lines 898-938).
...
**Impact:** 43 React component tests fail because `window.bridgeAPI.governance` is undefined
**Fix:** Add `governance` and `figma` namespace mocks to `createMockBridgeAPI()` (see fix details in Test Coverage section above)
**Owner:** bridge-test-writer
...
**Owner:** bridge-electron-ipc

### WARNING 2: Pre-existing `window.bridgeAPI` calls inside governanceStore

**File:** `src/store/governanceStore.ts` lines 64, 73
**Evidence:** `saveToFile()` calls `window.bridgeAPI.saveRuleOverrides()` and `loadFromFile()` calls `window.bridgeAPI.getRuleOverrides()`. These methods are not defined in `preload.ts` or `bridge-api.d.ts` (confirmed by grep). The store falls back to `localStorage` when the IPC calls fail.
...
1. **bridge-test-writer** must add the `governance` and `figma` namespace mocks to `createMockBridgeAPI()` in `src/components/__tests__/setup.ts`. This unblocks all 43 failing React tests. Estimated effort: 5 minutes.

Additionally, two warnings should be addressed before the next release:

2. **bridge-electron-ipc** should replace the inefficient JS-side filtering in `governance:override-count` with `govEventService.getOverrideCount(governanceSessionId)`. Estimated effort: 2 minutes.

3. **bridge-test-writer** should add the 7 new test cases specified in the contract for ExportModal and StatusBar GOV.1/GOV.2 features. Estimated effort: 30 minutes.
