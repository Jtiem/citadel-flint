...
|----------|--------|-------------|
| `bridge-mcp/src/core/config.ts` | ONLINE | `BridgePolicy` type, `PolicyMode`, `GovernanceDomain`, `DEFAULT_POLICY`, `DEFAULT_CONFIG` |
| `bridge-mcp/src/core/config-loader.ts` | ONLINE | `loadPolicy()` reads `.bridge/policy.json` with deep-merge defaults, `loadConfig()` |
| `bridge-mcp/src/core/policyLoader.ts` | ONLINE | `readPolicy()`, `writePolicy()`, `mergePolicy()`, `getDefaultPolicy()` |
| `bridge-mcp/src/server.ts` | ONLINE | `bridge_set_policy` MCP tool (read/update/reset) with GOV.2 telemetry |
| `electron/main.ts` | ONLINE | `policy:get` IPC handler reads policy from project root |
| `electron/preload.ts` | ONLINE | `window.bridgeAPI.policy.get()` exposed |
| `src/types/bridge-api.d.ts` | ONLINE | Renderer-side `BridgePolicy`, `PolicyMode`, `PolicyAPI` types |
| `src/store/canvasStore.ts` | ONLINE | `cachedPolicy`, `loadPolicy()`, `canExport()` reads policy for export gate |
| `bridge-mcp/src/core/MithrilLinter.ts` | ONLINE | `PolicyOptions` interface, `auditAll()` accepts optional thresholds |
| `bridge-mcp/src/tools/audit.ts` | ONLINE | Reads `policy.mithril.mode`, `policy.a11y.mode`, `policy.a11y.disabled_rules` |
...
3. **Domain preset selection** -- `domain` field exists in `BridgePolicy` (optional) and sentinel reads it, but Glass has no UI to set it
...
## 1. Full `.bridge/policy.json` Schema
...
### 2.1 Core Policy Types (extend `bridge-mcp/src/core/config.ts`)
...
export interface TeamOverlay {
    mithril?: Partial<BridgePolicy['mithril']>
    a11y?: Partial<BridgePolicy['a11y']>
    export_gate?: Partial<BridgePolicy['export_gate']>
}

/**
 * The `.bridge/policy.json` schema (v2).
...
### 2.2 Policy Engine Public API (`bridge-mcp/src/core/policyEngine.ts`)

```typescript
/**
 * Loads and validates .bridge/policy.json from the project root.
 ...
 */
export function resolvePolicy(projectRoot: string, teamId?: string): BridgePolicy

/**
 * Returns the effective mode for a specific rule ID.
 * Resolution: policy.{category}.rules[ruleId] ?? policy.{category}.mode
 */
export function getRuleMode(ruleId: string, policy: BridgePolicy): PolicyMode

/**
 * Returns the effective deltaE threshold from the resolved policy.
 */
export function getDeltaEThreshold(policy: BridgePolicy): number

/**
 * Returns the effective critical deltaE threshold from the resolved policy.
 */
export function getDeltaECriticalThreshold(policy: BridgePolicy): number
...
/**
 * Validates a raw JSON object against the BridgePolicy schema.
...
export function validatePolicy(raw: unknown): {
    valid: true; policy: BridgePolicy
} | {
    valid: false; errors: string[]
}
```

### 2.3 Renderer-Side Type Updates (`src/types/bridge-api.d.ts`)

The `BridgePolicy` type must be updated to match section 2.1. Additionally, the `PolicyAPI` interface must add a `set` method:

```typescript
export interface PolicyAPI {
    get: () => Promise<BridgePolicy>
    /**
     * Writes a partial policy update to .bridge/policy.json via the main process.
     ...
     */
    set: (partial: Partial<BridgePolicy>) => Promise<BridgePolicy>
}
```
...
|------|------------|-------------|-------|
| `bridge-mcp/src/core/config.ts` | MODIFY | bridge-state-architect | Extend `BridgePolicy` with `rules` maps, `SeverityFloor`, `TeamOverlay`, `teams`, deprecation markers. Update `DEFAULT_POLICY` to v2 shape. |
| `bridge-mcp/src/core/policyEngine.ts` | NEW FILE | bridge-state-architect | `resolvePolicy()`, `getRuleMode()`, `getDeltaEThreshold()`, `shouldBlockExport()`, `validatePolicy()`. |
| `bridge-mcp/src/core/config-loader.ts` | MODIFY | bridge-state-architect | Add v1->v2 migration in `loadPolicy()`. Add field-level validation (range checks). |
| `bridge-mcp/src/core/policyLoader.ts` | MODIFY | bridge-state-architect | Update `mergePolicy()` to handle `rules` maps and `teams`. |
| `bridge-mcp/src/core/MithrilLinter.ts` | MODIFY | bridge-ast-surgeon | `auditAll()` and each visitor accept full `PolicyOptions` with per-rule mode filtering. Visitors skip nodes when their rule is `'off'` and tag warnings as `'advisory'` when mode is `'advisory'`. |
| `bridge-mcp/src/core/a11y/runner.ts` | MODIFY | bridge-ast-surgeon | `auditSync()` reads per-rule mode from policy. Rules with mode `'off'` are excluded. Rules with mode `'advisory'` produce violations tagged `severity: 'advisory'`. |
| `bridge-mcp/src/tools/audit.ts` | MODIFY | bridge-ast-surgeon | Pass per-rule modes to linters. Report `blocking` vs `advisory` in results. |
| `bridge-mcp/src/server.ts` | MODIFY (additive) | bridge-electron-ipc | Update `bridge_set_policy` tool schema to document v2 fields. No structural change -- `mergePolicy()` already handles the write. |
| `bridge-mcp/src/prompts/sentinel.ts` | MODIFY | bridge-ast-surgeon | Read `deltaE_threshold` from policy into the Mithril threshold text block (currently hardcoded as 2.0). |
| `electron/main.ts` | MODIFY | bridge-electron-ipc | Add `policy:set` IPC handler. Add `fs.watch` on `.bridge/policy.json` for hot-reload push to renderer. |
| `electron/preload.ts` | MODIFY | bridge-electron-ipc | Expose `window.bridgeAPI.policy.set()` and `window.bridgeAPI.policy.onChanged()`. |
| `src/types/bridge-api.d.ts` | MODIFY | bridge-state-architect | Update `BridgePolicy` to v2 shape. Add `PolicyAPI.set()` and `PolicyAPI.onChanged()`. |
| `src/store/canvasStore.ts` | MODIFY | bridge-state-architect | Update `canExport()` to use `shouldBlockExport()` logic (severity floor + per-rule modes). Wire `policy.onChanged` listener. |
| `src/components/ui/PolicySettings.tsx` | NEW FILE | bridge-design-engineer | Glass settings panel in right sidebar. |
| `src/components/ui/__tests__/PolicySettings.test.tsx` | NEW FILE | bridge-test-writer | Unit tests for PolicySettings. |
| `bridge-mcp/src/core/__tests__/policyEngine.test.ts` | NEW FILE | bridge-test-writer | Unit tests for policyEngine (validate, resolve, shouldBlockExport). |
...
|---------|-----------|-------------|-------------|-------|
| `policy:get` | renderer -> main | `void` | `BridgePolicy` | **Exists.** No change. |
| `policy:set` | renderer -> main | `Partial<BridgePolicy>` | `BridgePolicy` | **NEW.** Merges partial into current, writes to disk, returns merged result. |
| `bridge:policy-changed` | main -> renderer | `BridgePolicy` | `void` (broadcast) | **NEW.** Pushed by `fs.watch` on `.bridge/policy.json` change (from MCP tool, external editor, or Glass). |

### 4.1 IPC Handler Specification: `policy:set`

```typescript
ipcMain.handle('policy:set', async (_event, partial: Partial<BridgePolicy>): Promise<BridgePolicy> => {
    if (!activeProjectRoot) {
        throw new Error('No project open')
    }
    const { validatePolicy } = await import('../bridge-mcp/src/core/policyEngine.js')
    const { mergePolicy } = await import('../bridge-mcp/src/core/policyLoader.js')
    ...
    BrowserWindow.getAllWindows().forEach((w) => {
        if (!w.isDestroyed()) w.webContents.send('bridge:policy-changed', merged)
    })
...
The main process installs `fs.watch` on `.bridge/policy.json` whenever `activeProjectRoot` is set (i.e. on project open). On change:

1. Debounce 200ms (policy files are small but editors may fire multiple write events).
2. Re-read `loadPolicy(activeProjectRoot)`.
3. Broadcast `bridge:policy-changed` with the new policy to all renderer windows.

The watcher is torn down on `closeWorkspace` / project switch. This ensures external MCP tool calls (`bridge_set_policy`) and manual edits to `policy.json` are reflected in Glass without requiring a restart.
...
|-------|-----------|-------------|---------------|
| `canvasStore` | (no new state -- `cachedPolicy` already exists) | `setCachedPolicy(policy)` (exists), wire `bridge:policy-changed` listener | `canExport()` updated to use `shouldBlockExport()` from `policyEngine` |
...
**IMPORTANT architectural note:** The `shouldBlockExport` function is pure and lives in `bridge-mcp`. It cannot be imported directly into the renderer (process boundary). Two options:
...
**IPC calls:**
- `window.bridgeAPI.policy.set(partial)` -- on form save
- `window.bridgeAPI.policy.get()` -- on mount (fallback if cachedPolicy is null)
...
The `getBridgeSentinelContent()` function in `bridge-mcp/src/prompts/sentinel.ts` currently hardcodes `deltaE > 2.0` in the `BASE_BLOCK` text. POL.1 modifies it to accept an optional `policy: BridgePolicy` parameter:

```typescript
export function getBridgeSentinelContent(
    domain?: string,
    projectRoot?: string,
    policy?: BridgePolicy,  // NEW: optional policy for threshold injection
): string
```
...
The `LinterWarning.severity` type in `bridge-mcp/src/types.ts` must be extended:
...
|---|-------------|----------|----------------------|
| 1 | Code is Truth | No | POL.1 modifies `.bridge/policy.json` (metadata), not source code. |
...
| 4 | Local-First Only | YES | Policy file is local `.bridge/policy.json`. No network calls. |
...
| 12 | Atomic Queuing | PARTIALLY | `writePolicy()` writes `.bridge/policy.json` directly (not through `FileTransactionManager`) because policy.json is metadata, not source code. This is an accepted exemption documented in `policyLoader.ts`. However, the write should still be atomic: write to `.tmp`, then `fs.renameSync()`. |
...
### Group 0: Git Setup (Sequential -- bridge-git-guru)
...
| Agent | Task | Files |
|-------|------|-------|
| **bridge-state-architect** (1A) | Extend `BridgePolicy` type in `config.ts`, add `SeverityFloor`, `RuleModeMap`, `TeamOverlay`. Update `DEFAULT_POLICY` to v2 shape with backward compat. Create `policyEngine.ts` with `resolvePolicy`, `getRuleMode`, `getDeltaEThreshold`, `shouldBlockExport`, `validatePolicy`. Update `config-loader.ts` with v1->v2 migration + field validation. Update `policyLoader.ts` mergePolicy for rules maps + teams. Update `src/types/bridge-api.d.ts` with v2 types + `PolicyAPI.set`. | `bridge-mcp/src/core/config.ts`, `bridge-mcp/src/core/policyEngine.ts` (new), `bridge-mcp/src/core/config-loader.ts`, `bridge-mcp/src/core/policyLoader.ts`, `src/types/bridge-api.d.ts` |
| **bridge-electron-ipc** (1B) | Add `policy:set` IPC handler in `main.ts`. Add `fs.watch` on `.bridge/policy.json` with debounced `bridge:policy-changed` broadcast. Expose `policy.set()` and `policy.onChanged()` in `preload.ts`. | `electron/main.ts`, `electron/preload.ts` |
| **bridge-test-writer** (1C) | Write comprehensive tests for `policyEngine.ts`: validatePolicy (valid/invalid/edge cases), resolvePolicy (default/project/team merge), getRuleMode (per-rule override, category fallback), shouldBlockExport (severity floor, per-rule modes, overrides). Write v1->v2 migration tests. | `bridge-mcp/src/core/__tests__/policyEngine.test.ts` (new) |

### Group 2: Linter Integration (Parallel, depends on Group 1)

| Agent | Task | Files |
|-------|------|-------|
| **bridge-ast-surgeon** (2A) | Extend `PolicyOptions` in MithrilLinter with `ruleModes`. Update each visitor to skip rules with mode `'off'` and tag `'advisory'` warnings. Pass `PolicyOptions` to all 5 visitors in `auditAll()`. Extend `LinterWarning.severity` to include `'advisory'`. | `bridge-mcp/src/core/MithrilLinter.ts`, `bridge-mcp/src/types.ts` |
| **bridge-ast-surgeon** (2B) | Extend `RunnerOptions` with `ruleModes` and `conformanceLevel`. Update `auditSync()` rule filtering. Tag advisory violations. | `bridge-mcp/src/core/a11y/runner.ts` |
| **bridge-ast-surgeon** (2C) | Update `handleBridgeAudit()` to build `PolicyOptions.ruleModes` from `config.policy`. Pass conformance level to A11y runner. Update sentinel prompt to accept policy thresholds. | `bridge-mcp/src/tools/audit.ts`, `bridge-mcp/src/prompts/sentinel.ts` |

### Group 3: Glass UI (Depends on Group 1)

| Agent | Task | Files |
|-------|------|-------|
| **bridge-design-engineer** (3A) | Build `PolicySettings.tsx` -- form controls for all policy fields. Wire to `policy.set()` IPC and `loadPolicy()` refresh. Accessible form with labels, focus trap, keyboard nav. | `src/components/ui/PolicySettings.tsx` (new) |
| **bridge-state-architect** (3B) | Update `canvasStore.canExport()` with new `shouldBlockExport` logic (Option A inline). Wire `bridge:policy-changed` listener in `App.tsx` or a dedicated hook to auto-update `cachedPolicy`. | `src/store/canvasStore.ts` |
| **bridge-test-writer** (3C) | Tests for PolicySettings (renders, form interactions, save/cancel, a11y). Tests for updated canvasStore.canExport() with severity floor and per-rule modes. | `src/components/ui/__tests__/PolicySettings.test.tsx` (new) |

### Group 4: Integration Validation (Sequential, depends on Groups 2 + 3)

| Agent | Task |
|-------|------|
| **bridge-integration-validator** | Run full test suite. Verify: policy read->modify->read round-trip via MCP tool. Verify: Glass PolicySettings writes and hot-reload works. Verify: MithrilLinter respects per-rule modes. Verify: A11y runner respects conformance level and per-rule modes. Verify: ExportModal behavior changes with advisory rules. Verify: TSC 0 errors. |

### Group 5: Git Finalization (Sequential -- bridge-git-guru)
...
| **PolicySettings form state drift from file** | Low | -- | Hot-reload via `bridge:policy-changed` push event updates the form. If the user has unsaved changes and an external change arrives, show a "Policy changed externally -- reload?" prompt. |
...
### 13.1 policyEngine.test.ts (bridge-mcp)
...
| Save button calls policy.set | Clicking Save invokes `window.bridgeAPI.policy.set()` with correct partial |
...
- `bridge-mcp/src/server.ts` is modified **additively only** (updating the `bridge_set_policy` tool schema documentation). The tool handler itself already delegates to `mergePolicy()`, which POL.1 updates. No conflict with GOV.1/GOV.2 work.
...
|-----------|-------|-------------|---------|
| `governanceStore.overrides` | Glass session | localStorage / `.bridge/overrides.json` | Quick toggle during development. Session-scoped designer preference. |
| `policy.json rules` | Project | `.bridge/policy.json` (committed to git) | Permanent team policy. Affects MCP server, CI, and all Glass sessions. |
