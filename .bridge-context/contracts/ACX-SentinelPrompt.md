# Contract: ACX — bridge-sentinel MCP Prompt Implementation
...
The `bridge-sentinel` prompt is registered in `server.ts` and imported from `./prompts/sentinel.js`, but `sentinel.ts` does not exist. Every call to the `bridge-sentinel` prompt crashes the MCP server with a module-not-found error. Additionally, the sentinel's registered argument (`domain`) has no content behind it — the domain parameter is accepted but ignored because the content function is missing. Healthcare, fintech, government, and enterprise teams cannot instruct the sentinel to apply their industry's compliance constraints before the AI agent begins drafting UI code.
...
A designer or AI agent working in a healthcare dashboard has no way to tell Bridge's governance system "apply HIPAA rules here." The base Mithril + WCAG AA linter fires on visual violations but is silent on PHI exposure patterns, financial data formatting, or plain-language requirements. The sentinel prompt is the injection point that layers domain-specific rules on top of the base governance context before the agent starts work.

### 2.2 Success Criteria

- Calling `bridge-sentinel` with `domain: "healthcare"` gives the agent a system prompt that explicitly forbids raw SSN/MRN/DOB display patterns and requires accessible status indicators for critical values.
- Calling `bridge-sentinel` with `domain: "general"` (or no argument) reproduces the base governance persona without any domain extensions.
- A project can declare its domain once in `.bridge/policy.json` and have all subsequent sentinel invocations apply the correct preset automatically.
- The sentinel composes with the existing intent-composer: an agent can load `bridge-intent-composer` first (for Figma translation) then load `bridge-sentinel` (for governance) and the two prompts do not conflict.
- The module-not-found crash is eliminated.

### 2.3 Fits Into Bridge Because

This is a **Protect** feature. Its sole purpose is to prevent AI agents from generating UI patterns that violate industry-specific compliance requirements that the base WCAG + Mithril linter has no visibility into. It is the governance extension point the domain registry infrastructure (`bridge-mcp/src/domains/index.ts`) was built to support.
...
| Phase B-v2 — Mithril Linter | `bridge-mcp/src/core/MithrilLinter.ts` | Domain presets reference Mithril threshold overrides (e.g. fintech tightens ΔE to 1.0) |
| EXP.6a — A11y Expansion | `bridge-mcp/src/core/a11y/` | Healthcare and government domains escalate WCAG level to AAA and Section 508 respectively |
| Policy Engine | `bridge-mcp/src/core/config.ts` + `policyLoader.ts` | `BridgePolicy` already has `domain` field capacity; `policy.json` is the declaration file |
| Domain Registry | `bridge-mcp/src/domains/index.ts` | Registers governance domains; the sentinel file must align its domain IDs with this registry |
| GOV.1 — Rule Provenance | `bridge-mcp/src/core/governance/types.ts` | Domain presets introduce new `SourceAuthority` values (e.g. `'HIPAA'`, `'PCI-DSS'`, `'Section 508'`) that the provenance registry already has slots for |
...
`bridge-mcp/src/prompts/sentinel.ts`

This is the only new file required. The module-not-found crash in `server.ts` is already the only problem — the server handler at lines 464–478 is complete and correct. It passes the `domain` argument directly to `getBridgeSentinelContent(domain)`. The file just needs to exist and export the two symbols the server already imports:

```typescript
export const BRIDGE_SENTINEL_PROMPT_DEF: { name: string; description: string; arguments: [...] }
export function getBridgeSentinelContent(domain: string): string
```

### 3.2 Policy.json Domain Field

The `BridgePolicy` interface in `config.ts` does not currently include a `domain` field. Add one optional field:

```typescript
// bridge-mcp/src/core/config.ts — BridgePolicy additions
...
`getBridgeSentinelContent()` reads `domain` from the argument first, then falls back to loading `.bridge/policy.json` from the active project root when no argument is provided.

### 3.3 Domain Resolution Order

```
1. Explicit argument:      bridge-sentinel { domain: "healthcare" }
2. Policy file:            .bridge/policy.json → { domain: "healthcare" }
3. Default:                "general"
```
...
```typescript
export function getBridgeSentinelContent(domain?: string, projectRoot?: string): string
```

When `domain` is undefined and `projectRoot` is provided, the function reads `.bridge/policy.json` to resolve the domain. When both are undefined, it returns the `general` preset.
...
```
You are the Bridge Governance Sentinel — the compliance enforcement layer for AI-generated UI code.

You operate after bridge-intent-composer has translated design intent into a draft implementation plan.
...
- Before reviewing any code, call bridge_get_context to load the active file state.
- Always read bridge://tokens before any styled code is reviewed or approved.
- Always call bridge_query_registry before approving any new component to ensure existing design system
  components are used.
- Run all proposed code through bridge_audit before approving it for commit.
- bridge_ast_mutate is the ONLY approved way to modify source code. Never approve raw string replacements,
  template literals that construct JSX, or regex-based edits.
- If bridge_audit returns critical violations, HALT immediately. Display the violation list and refuse
  to proceed until bridge_fix or a targeted bridge_ast_mutate resolves them.
...
COMPLIANCE FRAME: WCAG 2.1 AA + Bridge Mithril Design System rules.
...
```
REQUIRED PATTERNS:
- All colors resolved to bridge://tokens design token names
...
- Never render patient lists with full name visible in a component that lacks an access control wrapper
  (a parent with data-bridge-access-level attribute or equivalent).
...
```
REQUIRED PATTERNS:
- All PHI display fields must have a data-phi="true" attribute so Bridge can track PHI surface area.
...
user group. Bridge enforces WCAG AAA for e-commerce interfaces because purchase barriers caused by
...
`getBridgeSentinelContent(domain, projectRoot?)` assembles the final prompt string as:
...
This sentinel prompt composes with bridge-intent-composer. If you have already loaded
bridge-intent-composer and translated a Figma design intent into an implementation plan, your next
step is to run bridge_audit against that plan and evaluate the results through the lens of the
domain rules above before approving any bridge_ast_mutate calls.

If you are starting fresh (no prior intent-composer session), begin with:
  1. bridge_get_context — load active file state
  2. bridge://tokens — read the token set
  3. bridge_query_registry — check for existing components
  4. bridge_audit — run base governance audit
  5. Apply domain rules above to all violations and proposed patterns
  6. bridge_ast_mutate — only after all domain halt conditions are clear

POLICY REFERENCE:
This project's declared domain is: [RESOLVED_DOMAIN]
Policy file: .bridge/policy.json
To change the domain for this project, update .bridge/policy.json → { "domain": "[new-domain]" }
```

---

## 6. BRIDGE_SENTINEL_PROMPT_DEF (exported constant)
    ...
    arguments: [
        {
            ...
            description:
                ...
                "government | enterprise-saas. When omitted, the domain is read from .bridge/policy.json. " +
...
## 7. BridgePolicy Domain Field (config.ts changes)

Add to `BridgePolicy` interface:
...
// Inside BridgePolicy, add after `baseline`:
...
| Agent loads `bridge-intent-composer` and `bridge-sentinel` for the same session | The prompts are additive — intent-composer is a translation persona, sentinel is an enforcement persona. They do not share state. The workflow integration block at the end of the sentinel explicitly tells the agent how to sequence them. |
| `projectRoot` not provided to `getBridgeSentinelContent` | Skip policy.json lookup, use domain argument or `"general"`. |
...
`server.ts` already calls `getBridgeSentinelContent(domain)` at line 473. No changes to `server.ts` are required. The module-not-found error is fully resolved by the existence of `sentinel.ts`.
...
- If `policy.domain` is set, call `getBridgeSentinelContent(policy.domain)` and prepend it to the orchestrator's system prompt
...
|------|------------|-------|
| `bridge-mcp/src/prompts/sentinel.ts` | NEW FILE — primary deliverable | bridge-architect |
| `bridge-mcp/src/core/config.ts` | MODIFY — add `GovernanceDomain` type + `domain?` to `BridgePolicy` | bridge-architect |
| `electron/orchestrator.ts` | MODIFY — read policy domain, prepend sentinel block to system prompt | bridge-architect |
| `bridge-mcp/src/prompts/__tests__/sentinel.test.ts` | NEW FILE — unit tests | bridge-test-writer |
| `bridge-mcp/src/core/__tests__/config.test.ts` | MODIFY — add GovernanceDomain default tests | bridge-test-writer |

---

## 12. Test Requirements

The test file `bridge-mcp/src/prompts/__tests__/sentinel.test.ts` must cover:

1. `getBridgeSentinelContent("general")` returns a string containing the base enforcement rules
2. `getBridgeSentinelContent("healthcare")` returns a string containing "PHI" and "SSN"
3. `getBridgeSentinelContent("fintech")` returns a string containing "PAN" and "currency"
4. `getBridgeSentinelContent("e-commerce")` returns a string containing "WCAG AAA"
5. `getBridgeSentinelContent("government")` returns a string containing "Section 508" and "Plain Language"
6. `getBridgeSentinelContent("enterprise-saas")` returns a string containing "SOC 2" and "confirmation dialog"
7. `getBridgeSentinelContent(undefined)` returns the `general` preset (no crash)
8. `getBridgeSentinelContent("unknown-domain")` returns the `general` preset (graceful fallback)
9. `getBridgeSentinelContent("healthcare")` returns a string containing "HALT" at least once
10. `BRIDGE_SENTINEL_PROMPT_DEF.arguments[0].required` is `false`
11. `BRIDGE_SENTINEL_PROMPT_DEF.name` is `"bridge-sentinel"`

---

## 13. Handoff Prompt for bridge-architect

```
Plan and implement Phase ACX: bridge-sentinel MCP prompt.

Context: `bridge-mcp/src/server.ts` imports `BRIDGE_SENTINEL_PROMPT_DEF` and `getBridgeSentinelContent`
...
`getBridgeSentinelContent(domain)`. The only work is creating the file and making the minor config.ts
and orchestrator.ts changes described below.

Primary deliverable:
Create `bridge-mcp/src/prompts/sentinel.ts` exporting:
  - `BRIDGE_SENTINEL_PROMPT_DEF` (see Section 6 of the ACX contract)
  - `getBridgeSentinelContent(domain?: string, projectRoot?: string): string` (see Section 5)
...
of the ACX contract at `.bridge-context/contracts/ACX-SentinelPrompt.md`. Use that text verbatim.

Domain resolution order (Section 3.3 of contract):
  1. `domain` argument if provided
  2. `.bridge/policy.json` → `domain` field if `projectRoot` is provided
  3. Default: `"general"`

Secondary deliverables:
1. `bridge-mcp/src/core/config.ts` — add `GovernanceDomain` exported type and optional `domain?:
   GovernanceDomain` field to `BridgePolicy` (Section 7 of contract). Do NOT change `DEFAULT_POLICY`.
2. `electron/orchestrator.ts` — on session start, read `policy.domain`; if set, call
   `getBridgeSentinelContent(policy.domain)` and prepend the result to the orchestrator system prompt
   string. Route the policy read through `readPolicy(projectRoot)` from `bridge-mcp/src/core/policyLoader.ts`
   (already imported in orchestrator context). Import `getBridgeSentinelContent` from the new sentinel.ts.
