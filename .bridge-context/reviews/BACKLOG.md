...
**Issue:** The `srcdoc` iframe has no `sandbox` attribute. XSS in preview code gives the attacker full access to `window.bridgeAPI` — the entire IPC surface including file write, terminal spawn, and git operations.
**Attack chain:** XSS in preview → `window.parent.bridgeAPI.saveFile()` → arbitrary file write to project
...
**Source:** Agent 1 (HIGH)
**Issue:** The Anthropic API key is read from `~/.bridge/config.json` as plaintext. Any process with file-read access to the home directory can exfiltrate it.
...
### P1-5: `editorStore.applyBatch` does not re-stamp bridge IDs after structural mutations [TRUST]
**File:** `src/store/editorStore.ts:306`
**Source:** Agent 2 (HIGH)
**Issue:** After structural mutations (move, inject, wrapNode), `injectBridgeIds` is not called. This violates Commandment 7 (ID Preservation) — newly created or moved nodes may lack `data-bridge-id`, making them unselectable in the canvas and unaddressable by future mutations.
**Fix:** Call `injectBridgeIds(ast)` at the end of `applyBatch` before calling `syncCode`.
...
| P1-5 | P1 | TRUST | applyBatch skips injectBridgeIds | No |
...
- P1-5: injectBridgeIds after applyBatch (30 min, no contract)
