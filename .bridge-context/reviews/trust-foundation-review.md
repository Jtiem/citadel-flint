...
**Files reviewed:** electron/orchestrator.ts, src/core/ASTService.ts, src/utils/astModifier.ts, src/core/MithrilLinter.ts, src/core/A11yLinter.ts, bridge-mcp/src/core/MithrilLinter.ts, bridge-mcp/src/core/A11yLinter.ts, src/core/ASTService.test.ts
...
**Issue:** When `config.provider` is `'openai'` or `'gemini'`, the orchestrator creates a plain text-only chat stream. Neither provider branch passes `BRIDGE_TOOLS` to the API call, meaning the LLM responds with free-form text instead of structured tool calls. The `validateToolInput` function is never invoked on these paths. The entire safety architecture (tool catalog enforcement, Mithril pre-commit check, data-bridge-id tampering guard, LSP validation) is bypassed.
...
### HIGH: `editorStore.applyBatch` does not call `injectBridgeIds` after structural mutations

**File:** `src/store/editorStore.ts:306-337`
**Issue:** After `adapter.applyMutationBatch()` returns new code, `applyBatch` calls `adapter.parse(newCode)` to build a fresh AST and stores it in `editorStore.ast`. However, `injectBridgeIds` is never called on this AST. For property-only mutations this is acceptable (existing IDs survive). For structural mutations (`moveNode`, `injectComponent`, `deleteNode`), newly created or repositioned nodes may lack stable `data-bridge-id` attributes in the store's AST. The `LivePreview` independently calls `injectBridgeIds` before rendering, so the iframe has correct IDs, but the `editorStore.ast` used by the Properties Panel, Layer Tree, and mutation targeting is potentially missing IDs on newly injected nodes until the next `setCode` or LivePreview refresh cycle.
**Commandment violated:** Commandment 7 (ID Preservation -- "injectBridgeIds after every structural op")
**Impact:** Between a structural mutation and the next LivePreview render, properties panel interactions targeting the new node by bridge ID will fail silently (no node match). The window is typically < 200ms in practice, but under slow rendering or rapid sequential mutations, the race is real.
**Recommended fix approach:** Call `adapter.injectBridgeIds(newAst)` inside `applyBatch` after `adapter.parse(newCode)` returns a non-null AST, before storing it in state.
**Contract required:** no
...
### MEDIUM: `deleteNode` in ASTService calls `window.bridgeAPI` inside a store-action code path

**File:** `src/core/ASTService.ts:402-404`
**Issue:** The `applyMutationBatch` function calls `window.bridgeAPI.tokens.clearOverride?.(mutation.nodeId)` when processing a `deleteNode` mutation. `applyMutationBatch` is a pure function in `src/core/` that is called from `editorStore.applyBatch`. This means a `window.bridgeAPI` IPC call fires from inside a Zustand store action's call chain. The CLAUDE.md explicitly states: "No `window.bridgeAPI` called inside a Zustand store action (belongs in hooks/components/services)."
**Commandment violated:** Anti-pattern: `window.bridgeAPI` in store action path
...
### LOW: `injectComponent` in astModifier uses `Math.random()` for bridge ID generation

**File:** `src/utils/astModifier.ts:289`
**Issue:** `const bridgeId = Math.random().toString(36).slice(2, 9)` generates a 7-character random ID. `Math.random()` is not cryptographically secure, and the 7-character base-36 space (~78 billion values) is adequate for UI element IDs but the collision probability across large component trees is non-zero.
**Commandment violated:** none
**Impact:** Extremely low probability of bridge ID collision. Not a security concern (IDs are not used for authentication).
...
- **Commandment 15 (AST Tool Catalog) for Anthropic path:** The `BRIDGE_TOOLS` array defines a closed set of 11 tools. No tool accepts a raw code string or full-file replacement payload. The LLM structurally cannot emit raw code. The `validateToolInput` function runs before any tool call reaches the renderer. Clean for the Anthropic branch.
...
| `bridge-mcp/src/core/MithrilLinter.ts` | 31 | `as unknown as { default: typeof _traverse }` | Low -- standard CJS/ESM interop pattern |
| `bridge-mcp/src/core/MithrilLinter.ts` | 88 | `rgb.map(srgbToLinear) as [number, number, number]` | Low -- array shape guaranteed by hexToRgb |

## Test Coverage Gaps

### Missing from ASTService.test.ts

1. **`injectComponent` round-trip test:** No test covers `op: 'injectComponent'` in `applyMutationBatch`. The mutation type is defined but never exercised in tests. Forward application, inverse snapshot, and bridge ID stamping are all untested.
...
4. **Malformed bridge ID input:** No test passes a `nodeId` that matches no element (e.g., `"nonexistent-id"`) to verify the mutation is a safe no-op. The code handles this (traversal finds nothing), but there is no explicit test asserting the behavior.
...
3. **[HIGH] Call `injectBridgeIds` in `editorStore.applyBatch`** after `adapter.parse(newCode)` returns, before storing the AST in state.

4. **[MEDIUM] Move `window.bridgeAPI.tokens.clearOverride` out of `applyMutationBatch`** and into the `editorStore.applyBatch` or a post-batch hook.
...
8. **[LOW] Consider deterministic bridge ID generation** in `injectComponent` for test reproducibility.
