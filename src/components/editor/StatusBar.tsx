...
function IDESyncChip({
  ...
  } = useOnboardingTooltip('ide-file-sync');

  // Progressive disclosure: hidden until first IDE sync event
  if (!ideSyncActive) return null;
  const elapsed = now - lastEventAt;
  const dotColor = elapsed < IDE_SYNC_ACTIVE_THRESHOLD_MS ? 'bg-emerald-400' : elapsed < IDE_SYNC_IDLE_THRESHOLD_MS ? 'bg-zinc-400' : 'bg-zinc-600';
  const statusLabel = elapsed < IDE_SYNC_ACTIVE_THRESHOLD_MS ? 'Following your editor' : 'Waiting for file changes';
  return <div ref={ref} className="relative">
            <button type="button" onClick={() => setOpen(v => !v)} className="flex min-h-[24px] cursor-pointer items-center gap-1.5 rounded px-1.5 py-0.5 text-xs text-zinc-400 transition-colors hover:bg-zinc-800 hover:text-zinc-200" title={statusLabel} data-testid="ide-sync-chip">
                <span className={`h-2 w-2 rounded-full ${dotColor}`} />
                IDE
            </button>

            {/* Layer 3: One-time onboarding tooltip */}
            {showTooltip && !open && <div className="absolute bottom-full left-1/2 z-50 mb-2 -translate-x-1/2 whitespace-nowrap rounded bg-zinc-800 px-2.5 py-1.5 text-[var(--spacing.3, 12px)] text-zinc-300 shadow-lg border border-zinc-700" role="status" data-testid="ide-sync-tooltip">
                    Glass is following your editor. Switch files to see it update.
                    <button type="button" onClick={dismissTooltip} className="ml-2 inline-flex rounded p-0.5 text-zinc-400 hover:text-zinc-200" aria-label="Dismiss tooltip">
                        <X className="h-3 w-3" />
                    </button>
                </div>}

            {open && <div className="absolute bottom-full left-0 mb-2 w-64 rounded-lg border border-zinc-700 bg-zinc-900 p-3 shadow-xl">
                    <div className="mb-2 flex items-center justify-between">
                        <span className="text-xs font-semibold text-zinc-100">IDE Sync</span>
                        <button type="button" onClick={() => setOpen(false)} className="rounded p-0.5 text-zinc-400 hover:text-zinc-300" aria-label="Close IDE sync popover">
                            <X className="h-3 w-3" />
                        </button>
                    </div>
                    ...
                    <dl className="space-y-1.5 text-xs">
                        ...
                        {lastFile && <div className="flex items-center justify-between">
                                <dt className="text-zinc-400">Last file</dt>
                                <dd className="truncate text-zinc-300 max-w-[var(--spacing.12, 48px)]">{lastFile}</dd>
                            </div>}
                        {lastEventAt > 0 && <div className="flex items-center justify-between">
                    ...
                    <div className="mt-2.5 border-t border-zinc-800 pt-2">
                        <p className="text-[var(--spacing.3, 12px)] leading-relaxed text-zinc-400">
                            Switch files in VS Code and Glass follows automatically.
                            Or right-click a file → &quot;Open in Flint Glass.&quot;
                        </p>
                    </div>

                    {onConnectIDE && <div className="mt-2 border-t border-zinc-800 pt-2">
                            <button type="button" onClick={() => {
          setOpen(false);
          onConnectIDE();
        }} className="text-[var(--spacing.3, 12px)] text-indigo-400 transition-colors hover:text-indigo-300">
...
function RuntimeAuditSurface() {
  ...
  const livePreviewHtml = useCanvasStore(s => s.livePreviewHtml);
  const findingCount = result?.violations?.length ?? 0;
  const onClick = useCallback(() => {
    // RUNTIME.1 Issue 5: source the active LivePreview srcdoc snapshot
    // captured by LivePreview on every successful render. When null (no
    // preview has rendered yet) the server handler short-circuits to
    // `status: 'no-preview'`.
    void run(livePreviewHtml ? { previewHtml: livePreviewHtml } : {});
  }, [run, livePreviewHtml]);
  const isNoPreview = status === 'no-preview';
  return <div className="relative flex items-center gap-1.5">
            <RuntimeAuditPill status={status} findingCount={findingCount} onClick={onClick} />
            {isNoPreview && <span data-testid="runtime-audit-no-preview-message" className="text-[var(--spacing.2, 8px)] text-zinc-500">
...
function AuditContextPill() {
  ...
  return <span role="status" aria-label={`Audit context: ${label}`} title={tooltip} tabIndex={0} data-testid="audit-context-pill" className="flex min-h-[24px] max-w-[var(--spacing.12, 48px)] items-center gap-1 truncate rounded border border-indigo-500/30 bg-zinc-800 px-1.5 py-0.5 text-xs text-indigo-400 focus:outline-none focus:ring-1 focus:ring-indigo-400">
...
export function StatusBar({
  ...
  const canExport = useMemo(() => storeCanExport(),
  ...
  useEffect(() => {
    const poll = () => {
      window.flintAPI.mcp?.status().then(s => setMcpConnected(s.connected)).catch(() => setMcpConnected(false));
    };
    poll();
  ...
  const handleMcpReconnect = useCallback(() => {
    ...
    window.flintAPI.mcp?.reconnect?.().catch(err => console.warn('[Flint] StatusBar: MCP reconnect failed', err));
  }, []);
  ...
  const fetchFigmaStatus = useCallback(() => {
    window.flintAPI.figma.status().then(setFigmaStatus).catch(err => console.warn('[Flint] StatusBar: failed to fetch Figma status', err));
  }, []);
  useEffect(() => {
    fetchFigmaStatus();
  ...
  useEffect(() => {
    ...
    const unsubError = window.flintAPI.figma.onError(event => {
      const safeReason = typeof event.reason === 'string' ? event.reason.replace(/[^\x20-\x7E]/g, '').slice(0, 200) : 'Unknown error';
      push({
        type: 'error',
  ...
  const handleSyncPull = useCallback(async () => {
    ...
    try {
      ...
      push({
        ...
        message: isError ? result?.content?.[0]?.text ?? 'Unknown error during pull.' : 'Figma tokens pulled to local project.',
  ...
  const handleSyncPush = useCallback(async () => {
    ...
    try {
      ...
      push({
        ...
        message: isError ? result?.content?.[0]?.text ?? 'Unknown error during push.' : 'Local tokens pushed to Figma.',
        severity: isError ? 'error' : 'success',
        autoDismissMs: isError ? 8000 : 4000
      });
      if (!isError) {
  ...
  useEffect(() => {
    window.flintAPI.beta?.getInfo().then(info => {
      if (info.isBeta) setBetaInfo(info);
  ...
  const isRecentSync = figmaStatus?.lastWebhookAt != null && Date.now() - figmaStatus.lastWebhookAt < 5_000;
  ...
  const figmaButtonTitle = hasNoTokens ? 'No design tokens loaded — click to connect Figma or import tokens. This unlocks design system governance.' : 'Figma connection — click for details';
  return <footer role="contentinfo" aria-label="Application status" className="relative flex shrink-0 items-center justify-between border-t border-zinc-800 bg-zinc-950 px-4 py-1">
      ...
      }} className={`flex flex-shrink-0 min-h-[24px] cursor-pointer items-center gap-1.5 text-sm font-medium transition-colors ${canExport ? 'text-emerald-500 hover:text-emerald-400' : 'text-amber-400 hover:text-amber-300'}`} title={canExport
      // EDU-01: explain what "export ready" means
      ? 'All design system checks pass — your file is ready to export. Click to open the Export panel.' : `Export blocked: ${gateLabel}. Click to see issues and fix options.`}>
                {canExport ? <>
                        <ShieldCheck className="h-3.5 w-3.5" />
                        Export Ready
                    </> : <>
                        <ShieldAlert className="h-3.5 w-3.5" />
                        <span className="max-w-[var(--spacing.12, 48px)] truncate">{gateLabel}</span>
                    </>}
            </button>
            </div>{/* end zone 1 */}
          ...
          setPopoverOpen(v => !v);
        }} className={`flex cursor-pointer min-h-[24px] items-center gap-1.5 rounded px-1.5 py-0.5 text-xs transition-colors hover:bg-zinc-800 hover:text-zinc-200 ${hasNoTokens ? 'text-amber-400' : 'text-zinc-400'}`} title={figmaButtonTitle}>
                ...
                {/* Popover */}
                {popoverOpen && <div className="absolute bottom-full left-0 mb-2 w-72 rounded-lg border border-zinc-700 bg-zinc-900 p-3 shadow-xl">
                        {/* Header */}
                        <div className="mb-2 flex items-center justify-between">
                            <span className="text-xs font-semibold text-zinc-100">Figma Connection</span>
                            <button type="button" onClick={() => {
              setPopoverOpen(false);
            }} className="rounded p-0.5 text-zinc-400 hover:text-zinc-300" aria-label="Close Figma status popover">
                                <X className="h-3 w-3" />
                            </button>
                        </div>
                        ...
                        <div className="flex gap-2">
                            <button type="button" onClick={handleSyncPull} disabled={!isFigmaConnected || syncOp !== null} className={`flex flex-1 items-center justify-center gap-1.5 rounded px-3 py-1.5 text-xs text-white transition-colors ${!isFigmaConnected || syncOp !== null ? 'bg-indigo-600 opacity-50 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-500'}`} title={!isFigmaConnected ? 'Connect to Figma first' : 'Pull token changes from Figma to local project'} data-testid="figma-sync-pull">
                                {syncOp === 'pull' ? <Loader2 className="h-3 w-3 animate-spin" /> : <Download className="h-3 w-3" />}
                                Pull from Figma
                            </button>
                            <button type="button" onClick={handleSyncPush} disabled={!isFigmaConnected || syncOp !== null} className={`flex flex-1 items-center justify-center gap-1.5 rounded px-3 py-1.5 text-xs transition-colors ${!isFigmaConnected || syncOp !== null ? 'bg-zinc-700 text-zinc-200 opacity-50 cursor-not-allowed' : 'bg-zinc-700 text-zinc-200 hover:bg-zinc-600'}`} title={!isFigmaConnected ? 'Connect to Figma first' : 'Push local token changes to Figma'} data-testid="figma-sync-push">
                                {syncOp === 'push' ? <Loader2 className="h-3 w-3 animate-spin" /> : <Upload className="h-3 w-3" />}
                                Push to Figma
                            </button>
                        </div>

                        {/* S7.4: Last synced timestamp */}
                        {lastSyncedAt != null && <p className="mt-1.5 text-[var(--spacing.2, 8px)] text-zinc-400" data-testid="figma-last-synced">
                        ...
                        {/* Action buttons */}
                        <div className="flex gap-2">
                            <button type="button" onClick={fetchFigmaStatus} className="flex flex-1 items-center justify-center gap-1.5 rounded border border-zinc-700 bg-zinc-800/60 px-2 py-1.5 text-xs text-zinc-300 transition-colors hover:border-zinc-600 hover:bg-zinc-700/60 hover:text-zinc-100" title="Re-fetch server status">
                        ...
                        {onManageFigma && <div className="mt-2.5 border-t border-zinc-800 pt-2">
                                <button type="button" onClick={() => {
              setPopoverOpen(false);
              onManageFigma();
            }} className="text-[var(--spacing.3, 12px)] text-indigo-400 transition-colors hover:text-indigo-300" data-testid="figma-manage-link">
                                    Manage connection
                                </button>
                            </div>}
              ...
              setHelpTooltipOpen(v => !v);
                            ...
                            {helpTooltipOpen && <div className="absolute bottom-full left-0 mb-1 w-56 rounded border border-zinc-700 bg-zinc-800 px-2.5 py-2 text-[var(--spacing.3, 12px)] text-zinc-300 shadow-lg">
            ...
            <div className="flex items-center gap-1.5 px-1.5 py-0.5" title={mcpConnected === null ? 'Governance engine — connecting…' : mcpConnected
      // EDU-13: plain-language tooltip explaining what MCP is
      ? 'Governance engine — connected. Flint is actively checking your code.' : 'Governance engine — not connected. Audits and fixes are unavailable.'}>
                <span className={['inline-block w-2 h-2 rounded-full', mcpConnected === null ? 'bg-zinc-500 animate-pulse' : mcpConnected ? 'bg-emerald-400' : 'bg-red-400'].join(' ')} aria-hidden="true" />
                ...
                {mcpConnected === false && <button type="button" onClick={handleMcpReconnect} className="text-xs text-zinc-400 hover:text-white underline underline-offset-2 ml-0.5" aria-label="Reconnect Flint engine">
                        Reconnect
                    </button>}
            </div>
            ...
            {updateState === 'downloading' && <span className="flex items-center gap-1.5 rounded border border-blue-700/40 bg-blue-900/10 px-2 py-0.5 text-xs text-blue-400" title={`Downloading update… ${downloadProgress ? `${Math.round(downloadProgress.percent)}%` : ''}`}>
                    <Download className="h-3 w-3 animate-pulse" />
                    {downloadProgress ? `${Math.round(downloadProgress.percent)}%` : 'Downloading…'}
                    {downloadProgress && <span className="ml-1 inline-block h-1 w-12 overflow-hidden rounded bg-blue-900/40" aria-hidden="true">
                            <span className="block h-full bg-blue-400 transition-all" style={{
            ...
            <OverflowMenu mcpConnected={mcpConnected} onConnectIDE={onConnectIDE} isDemo={isDemo} onOpenOwnProject={onOpenOwnProject} previewBreakpoint={previewBreakpoint} hasUsedBreakpoint={hasUsedBreakpoint} cyclePreviewBreakpoint={cyclePreviewBreakpoint} isScratchpad={isScratchpad} hasSeenViolation={hasSeenViolation} autopilotEnabled={autopilotEnabled} setAutopilotEnabled={setAutopilotEnabled} governedCode={governedCode} governedFixCount={governedFixCount} activeFilePath={activeFilePath} applyGovernedCode={applyGovernedCode} betaInfo={betaInfo} setFeedbackOpen={setFeedbackOpen} updateState={updateState} updateInfo={updateInfo} handleUpdateDownload={handleUpdateDownload} handleUpdateInstall={handleUpdateInstall} />
...
function OverflowMenu({
  ...
  const badgeCount = [updateState === 'available' || updateState === 'ready', isDemo, isScratchpad, mcpConnected === false && !!onConnectIDE, betaInfo != null].filter(Boolean).length;
  ...
  const hasAnyOverflowItem = mcpConnected === false && !!onConnectIDE || isDemo || showBreakpointChip || isScratchpad || hasSeenViolation || betaInfo != null || updateState === 'available' || updateState === 'ready';
  if (!hasAnyOverflowItem) return <SyncStatus />;
  return <div ref={ref} className="relative flex items-center gap-2">
            <SyncStatus />
            <button type="button" onClick={() => setOpen(v => !v)} className="relative flex items-center justify-center rounded p-1 text-zinc-400 transition-colors hover:bg-zinc-800 hover:text-zinc-100" title="More status items" aria-label="Show more status indicators" data-testid="statusbar-overflow-btn">
                <MoreHorizontal className="h-3.5 w-3.5" />
                {badgeCount > 0 && <span className="absolute -right-0.5 -top-0.5 flex h-3 w-3 items-center justify-center rounded-full bg-amber-500 text-[8px] font-bold leading-none text-zinc-950">
                        {badgeCount}
                    </span>}
            </button>

            {open && <div className="absolute bottom-full right-0 mb-2 w-56 max-h-64 overflow-y-auto rounded-lg border border-zinc-700 bg-zinc-900 py-1 shadow-xl">

                    {/* WS1: Connect IDE */}
                    {mcpConnected === false && onConnectIDE && <button type="button" onClick={() => {
        setOpen(false);
        onConnectIDE();
      }} className="flex w-full items-center gap-2 px-3 py-1.5 text-[var(--spacing.2, 8px)] text-indigo-400 transition-colors hover:bg-zinc-800 hover:text-indigo-300" title="Open IDE setup wizard to configure Flint connection" data-testid="statusbar-connect-ide">
                            Connect IDE
                        </button>}

                    {/* Demo indicator */}
                    {isDemo && <div className="px-3 py-1.5">
                            <span className="text-[var(--spacing.2, 8px)] text-amber-400">Demo Project</span>
                            {onOpenOwnProject && <button type="button" onClick={() => {
          setOpen(false);
          onOpenOwnProject();
        }} className="mt-0.5 block text-[var(--spacing.2, 8px)] text-zinc-400 underline decoration-zinc-600 underline-offset-2 transition-colors hover:text-zinc-200">
                                    Open your project
                                </button>}
                        </div>}

                    {/* Breakpoint chip */}
                    {showBreakpointChip && <button type="button" onClick={() => {
        cyclePreviewBreakpoint('up');
      }} className="flex w-full items-center gap-2 px-3 py-1.5 text-[var(--spacing.2, 8px)] text-zinc-400 transition-colors hover:bg-zinc-800 hover:text-zinc-100" title={`Preview breakpoint: ${BREAKPOINT_LABELS[previewBreakpoint as keyof typeof BREAKPOINT_LABELS]} — click to cycle`} data-testid="statusbar-breakpoint-chip">
                            {previewBreakpoint === 'mobile' ? <Smartphone className="h-3 w-3" /> : <Tablet className="h-3 w-3" />}
                            {BREAKPOINT_LABELS[previewBreakpoint as keyof typeof BREAKPOINT_LABELS]}
                        </button>}

                    {/* Scratchpad */}
                    {isScratchpad && <button type="button" title="Scratchpad project — click to save to a permanent location (Cmd+Shift+S)" onClick={() => {
        setOpen(false);
        window.dispatchEvent(new CustomEvent(`${BRAND.productLower}:save-project-as`));
      }} className="flex w-full items-center gap-2 px-3 py-1.5 text-[var(--spacing.2, 8px)] text-amber-400 transition-colors hover:bg-zinc-800 hover:text-amber-300">
                            <FolderInput className="h-3 w-3" />
                            Unsaved Project
                        </button>}

                    {/* Autopilot toggle */}
                    {hasSeenViolation && <button type="button" onClick={() => {
        setAutopilotEnabled(!autopilotEnabled);
      }} className={`flex w-full items-center gap-2 px-3 py-1.5 text-[10px] transition-colors hover:bg-zinc-800 ${autopilotEnabled ? 'text-emerald-400' : 'text-zinc-400 hover:text-zinc-100'}`} aria-label={autopilotEnabled ? 'Autopilot: On' : 'Autopilot: Off'} title={autopilotEnabled ? 'Autopilot is active — Flint will automatically fix safe issues as you work' : 'Enable Autopilot to let Flint auto-fix safe issues in the background'}>
                    ...
                    {hasSeenViolation && autopilotEnabled && governedFixCount > 0 && <button type="button" onClick={() => {
        if (governedCode && activeFilePath) {
          setOpen(false);
          applyGovernedCode();
        }
      }} className="flex w-full items-center gap-2 px-3 py-1.5 text-[var(--spacing.2, 8px)] text-emerald-400 transition-colors hover:bg-zinc-800 hover:text-emerald-300" title={`Apply ${governedFixCount} governance fixes (Cmd+Shift+G)`}>
                            <span className="h-2 w-2 animate-pulse rounded-full bg-emerald-400" />
                            {governedFixCount} fixes ready
                        </button>}

                    {/* Divider before meta items */}
                    {(betaInfo != null || updateState === 'available' || updateState === 'ready') && <div className="my-1 border-t border-zinc-800" />}

                    {/* Beta chip */}
                    {betaInfo && <button type="button" onClick={() => {
        setOpen(false);
        setFeedbackOpen(true);
      }} className="flex w-full items-center gap-2 px-3 py-1.5 text-[var(--spacing.2, 8px)] text-indigo-400 transition-colors hover:bg-zinc-800 hover:text-indigo-300" title={`Beta build ${betaInfo.buildId}${betaInfo.daysRemaining != null ? ` — ${betaInfo.daysRemaining} day${betaInfo.daysRemaining === 1 ? '' : 's'} remaining` : ''} — click to send feedback`}>
                            <MessageSquare className="h-3 w-3" />
                            Beta {betaInfo.daysRemaining != null ? `(${betaInfo.daysRemaining}d)` : ''} — Send feedback
                        </button>}

                    {/* Update available */}
                    {updateState === 'available' && updateInfo && <button type="button" onClick={() => {
        setOpen(false);
        handleUpdateDownload();
      }} className="flex w-full items-center gap-2 px-3 py-1.5 text-[var(--spacing.2, 8px)] text-emerald-400 transition-colors hover:bg-zinc-800 hover:text-emerald-300" title={`Update available: v${updateInfo.version} — click to download`}>
                            <Download className="h-3 w-3" />
                            Update v{updateInfo.version} available
                        </button>}

                    {/* Update ready to install */}
                    {updateState === 'ready' && updateInfo && <button type="button" onClick={() => {
        setOpen(false);
        handleUpdateInstall();
      }} className="flex w-full items-center gap-2 px-3 py-1.5 text-[var(--spacing.2, 8px)] text-violet-400 transition-colors hover:bg-zinc-800 hover:text-violet-300" title={`v${updateInfo.version} downloaded — click to restart and install`}>
