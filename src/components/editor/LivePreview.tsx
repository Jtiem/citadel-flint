...
import { FLINT_INTERACTION_SCRIPT, FLINT_INTERACTION_STYLES } from '../../preview-vendor/flint-interaction';
import { getLibraryShims, getGenericShims, type LibraryShimBundle } from '../../preview-vendor/shims/index';
...
import { parseCodeToAST, injectFlintIds, generateCodeFromAST } from '../../core/ast-parser';
import { LanguageRegistry } from '../../core/adapters/types';
import { vueAdapter } from '../../core/adapters/VueAdapter';
import { publishPresence, publishPresenceImmediate } from '../../services/PresenceService';
 ...
 */
export function buildSrcdoc(js: string, tailwindConfigJson: string, libraryShims: LibraryShimBundle | null, tokenCssVars = ''): string {
 ...
 * @param tailwindConfigJson  JSON string produced by `generateTailwindConfig`.
 */
export function buildVueSrcdoc(js: string, css: string, tailwindConfigJson: string): string {
 ...
 * @param tailwindConfigJson  JSON string produced by `generateTailwindConfig`.
 */
export function buildSvelteSrcdoc(js: string, css: string, tailwindConfigJson: string): string {
  // JSON-safe embedding: escape </script> sequences and bare `<` chars so that
  // `</script>` inside the compiled code cannot terminate the enclosing script.
  const safeJson = JSON.stringify(js).replace(/</g, '\\u003c');
  return `<!DOCTYPE html>
...
export function LivePreview() {
  ...
  useEffect(() => {
    window.flintAPI?.scope?.getActiveLibrary?.().then((result: {
      library: string | null;
    }) => {
   ...
   */
  const handleSmartDrop = useCallback((targetNodeId: string) => {
      ...
      case 'html':
        {
          ...
          if (ast !== null) {
            ...
            if (!cancelled && iframeRef.current !== null) {
              ...
              const _srcdocHtml = buildHtmlSrcdoc(injectedHtml, tailwindConfigJson);
              iframeRef.current.srcdoc = _srcdocHtml;
              setLivePreviewHtml(_srcdocHtml); // RUNTIME.1 snapshot
      ...
      case 'vue':
        {
          ...
          window.flintAPI.transformVue(codeWithIds).then(({
            js,
            css,
            error
          }) => {
            ...
            if (iframeRef.current !== null) {
              const _srcdocHtml = buildVueSrcdoc(js, css ?? '', tailwindConfigJson);
              iframeRef.current.srcdoc = _srcdocHtml;
              setLivePreviewHtml(_srcdocHtml); // RUNTIME.1 snapshot
              renderedVersionRef.current = thisVersion;
              setIsStale(false);
      ...
      case 'svelte':
        {
          ...
          const svelteAdapter = LanguageRegistry.hasAdapter('svelte') ? LanguageRegistry.getAdapter('fake.svelte') : null;
          ...
          setIsTransforming(true);
          window.flintAPI.transformSvelte(codeToTransform).then(({
            js,
            css,
            error
          }: {
            ...
            setTransformError(null);
            if (iframeRef.current !== null) {
              const _srcdocHtml = buildSvelteSrcdoc(js, css ?? '', tailwindConfigJson);
              iframeRef.current.srcdoc = _srcdocHtml;
              setLivePreviewHtml(_srcdocHtml); // RUNTIME.1 snapshot
              renderedVersionRef.current = thisVersion;
              setIsStale(false);
      ...
      default:
        {
          ...
          const codeToTransform = freshAst !== null ? (() => {
            injectFlintIds(freshAst);
            return generateCodeFromAST(freshAst);
          })() : previewCode;
          setIsTransforming(true);
          window.flintAPI.transformCode(codeToTransform).then(({
            js,
            error
          }) => {
            if (cancelled) return;
            ...
            if (iframeRef.current !== null) {
              ...
              const _srcdocHtml = buildSrcdoc(executeCode, tailwindConfigJson, libraryShims, tokenCssVars);
              iframeRef.current.srcdoc = _srcdocHtml;
              setLivePreviewHtml(_srcdocHtml); // RUNTIME.1 snapshot
              renderedVersionRef.current = thisVersion;
              setIsStale(false);
  ...
  const handleHydroPaste = async (figmaPayload: string) => {
    ...
    try {
      ...
      if (!editorState.visualTree || editorState.visualTree.length === 0) {
        ...
        const screenName = (() => {
          try {
            return JSON.parse(figmaPayload).screenName || 'FigmaScreen';
          } catch {
            return 'FigmaScreen';
          }
        })();
  ...
  useEffect(() => {
    function handleMessage(e: MessageEvent): void {
      ...
      if (msg.type === 'FIGMA_PASTE') {
        ...
        if (typeof msg.targetId === 'string' && typeof msg.position === 'string' && sourceId !== null && msg.targetId !== sourceId) {
  ...
  function handleShieldFinish(e: React.MouseEvent<HTMLDivElement>, commit: boolean): void {
  ...
  useEffect(() => {
    ...
    if (iframe?.contentWindow == null) return;
    iframe.contentWindow.postMessage({
      type: 'SET_INTERACT_MODE',
      enabled: canvasMode === 'interact'
    }, getIframeOrigin(previewUrl));
  ...
  const getErrorHint = (msg: string): string | null => {
    if (/is not defined/i.test(msg) || /ReferenceError/i.test(msg)) return 'Missing import?';
    if (/unexpected token/i.test(msg) || /unterminated/i.test(msg) || /SyntaxError/i.test(msg)) return 'Unclosed tag?';
    if (/is not a function/i.test(msg) || /undefined/i.test(msg)) return 'Undefined component?';
    if (/cannot read propert/i.test(msg)) return 'Null reference — check your data';
    return null;
  };
  return <div className="relative flex h-full w-full flex-col">
      ...
      <div className="flex shrink-0 items-center gap-2 border-b border-zinc-800/60 bg-zinc-900/40 px-3 py-1.5">
        ...
        <div className="flex items-center rounded border border-zinc-700/60 p-0.5">
          <button type="button" title="Design mode: click to select AST nodes" onClick={() => setCanvasMode('design')} className={`flex items-center gap-1 rounded px-2 py-0.5 text-[10px] font-medium transition-colors ${canvasMode === 'design' ? 'bg-indigo-600 text-white' : 'text-zinc-500 hover:text-zinc-300'}`}>
            <MousePointer2 size={10} />
            Design
          </button>
          <button type="button" title="Interact mode: test native events" onClick={() => setCanvasMode('interact')} className={`flex items-center gap-1 rounded px-2 py-0.5 text-[10px] font-medium transition-colors ${canvasMode === 'interact' ? 'bg-emerald-600 text-white' : 'text-zinc-500 hover:text-zinc-300'}`}>
            <Hand size={10} />
            Interact
          </button>
        </div>
        ...
        <span data-testid="framework-badge" className="ml-1 rounded bg-zinc-800 px-1.5 py-0.5 text-[var(--spacing.2, 8px)] font-medium text-zinc-400">
          {detectedFramework}
        </span>

        {/* GLASS.3.1B: Stale preview indicator */}
        {isStale && !isTransforming && <div data-testid="stale-indicator" className="ml-auto flex items-center gap-1.5">
      ...
      {transformError !== null && <div data-testid="transform-error" role="alert" aria-live="assertive" className="shrink-0 border-b border-red-700/40 bg-red-900/10 rounded p-3">
          <div className="flex items-start gap-2">
            <AlertTriangle size={14} className="mt-0.5 shrink-0 text-red-400" />
            <div className="min-w-0 flex-1">
              {/* Error type badge */}
              <span data-testid="error-type-badge" className="mb-1 inline-block rounded bg-red-900/40 px-1.5 py-0.5 text-[var(--spacing.2, 8px)] font-semibold uppercase tracking-wider text-red-300">
                {parseErrorType(transformError)}
              </span>

              {/* Error message — max 3 lines unless expanded */}
              <pre data-testid="error-message" className={`mt-1 whitespace-pre-wrap break-all font-mono text-[11px] text-red-400 ${!errorExpanded ? 'line-clamp-3' : ''}`}>
                {transformError}
              </pre>

              {/* "Show more" toggle — visible only when the message likely exceeds 3 lines */}
              {transformError.length > 200 && <button type="button" data-testid="error-expand-toggle" aria-expanded={errorExpanded} onClick={() => setErrorExpanded(prev => !prev)} className="mt-1 text-[var(--spacing.2, 8px)] text-red-300/70 underline hover:text-red-300">
                  {errorExpanded ? 'Show less' : 'Show more'}
                </button>}

              {/* Contextual hint */}
              {getErrorHint(transformError) !== null && <p data-testid="error-hint" className="mt-1.5 text-[var(--spacing.2, 8px)] italic text-red-300/60">
      ...
      {autopilotEnabled && governedCode && <div className="flex shrink-0 items-center gap-2 border-b border-zinc-700 bg-zinc-800/90 px-3 py-1 text-xs">
      ...
      <div className="relative flex min-h-0 flex-1 flex-col items-center overflow-hidden bg-zinc-950">
        ...
        {previewBreakpoint !== 'desktop' && <div className="z-10 mt-1 rounded bg-zinc-900/80 px-2 py-0.5 text-[var(--spacing.2, 8px)] text-zinc-500" data-testid="breakpoint-label">
            {previewBreakpoint === 'mobile' ? '375px' : '768px'}
          </div>}

        {/* Inner container — constrained width for mobile/tablet, full width for desktop */}
        <div className="relative flex-1 transition-all duration-300" data-testid="breakpoint-container" style={BREAKPOINT_WIDTHS[previewBreakpoint] !== undefined ? {
      ...
      <div data-testid="live-preview-drop-zone" className={`relative min-h-0 h-full outline-none transition-all ${isCardDragOver ? 'ring-2 ring-inset ring-indigo-500 ring-dashed' : ''}`} tabIndex={0} onDragOver={e => {
          if (canvasMode !== 'design') return;
          // Accept component files from FileExplorer and component cards from the canvas
          if (e.dataTransfer.types.includes('application/flint-component-file') || e.dataTransfer.types.includes('application/flint-component-card')) {
            e.preventDefault();
          ...
          if (e.currentTarget && !e.currentTarget.contains(e.relatedTarget as Node | null)) {
            setIsCardDragOver(false);
            setPendingCardDrag(null);
            setSmartHoveredId(null);
          }
        ...
        {isCardDragOver && visualTree.length > 0 && <div data-testid="smart-insert-panel" className="absolute right-0 top-0 z-40 flex h-full w-44 flex-col border-l border-zinc-700/50 bg-zinc-900/95 backdrop-blur-sm">
            {/* Panel header */}
            <div className="border-b border-zinc-800 px-3 py-2 shrink-0">
              <h3 className="text-xs font-medium text-zinc-400 uppercase tracking-wider">
                Insert at
              </h3>
              {pendingCardDrag && <p className="mt-0.5 truncate font-mono text-[var(--spacing.2, 8px)] text-indigo-400">
                  &lt;{pendingCardDrag.name} /&gt;
                </p>}
            </div>

            {/* Tree node rows — max 10 to avoid clutter */}
            <div className="flex-1 overflow-y-auto py-1" data-testid="smart-insert-node-list">
              {visualTree.slice(0, 10).map(node => <div key={node.id} data-testid={`smart-insert-node-${node.id}`} className={`
                    flex cursor-pointer items-center gap-2 px-3 py-1.5
                    text-[11px] transition-colors
                    ${smartHoveredId === node.id ? 'bg-indigo-900/30 text-indigo-300 border-l-2 border-indigo-500/50' : 'text-zinc-400 hover:bg-zinc-800 hover:text-zinc-100'}
                  `.trim()} onMouseEnter={() => setSmartHoveredId(node.id)} onMouseLeave={() => setSmartHoveredId(null)} onDragOver={e => {
                e.preventDefault();
                e.stopPropagation();
                e.dataTransfer.dropEffect = 'copy';
              ...
              {visualTree.length > 10 && <div className="px-3 py-1 text-[var(--spacing.2, 8px)] text-zinc-600 italic">
        ...
        <iframe
          ...
          key={previewUrl ?? 'srcdoc'} ref={iframeRef} title="Live Preview" className={`absolute inset-0 h-full w-full bg-zinc-900 ${showGoverned && governedCode ? 'border-2 border-emerald-500/30' : 'border-0'}`}
          ...
          sandbox="allow-scripts allow-forms" {...previewUrl != null ? {
            src: previewUrl
          } : {}} onLoad={handleIframeLoad} />
        {/* ── GLASS.3.1A: Transform loading overlay ──────────────────────── */}
        {isTransforming && <div data-testid="transform-loading-overlay" className="pointer-events-none absolute inset-0 z-30 flex items-center justify-center bg-zinc-950/40">
        ...
        {isDragging && canvasMode === 'design' && <div className="absolute inset-0 z-50 cursor-grabbing" onMouseMove={handleShieldMouseMove} onMouseUp={e => handleShieldFinish(e, true)} onMouseLeave={e => handleShieldFinish(e, false)}>
            {/* Ghost: lightweight outline following the cursor */}
            {ghostPos !== null && <div className="pointer-events-none absolute h-10 w-20 rounded border-2 border-blue-500 bg-blue-500/10" style={{
              left: ghostPos.x,
              top: ghostPos.y
            }} />}
            {/* Remote cursors: SVG overlays for other connected users */}
            {remotePresence.map(user => <div key={user.id} className="pointer-events-none absolute" style={{
              left: user.x,
              top: user.y
            }}>
                <svg width="14" height="20" viewBox="0 0 14 20" fill="none" aria-hidden="true">
                  <path d="M0 0L0 16L4 11L7 18L9.5 17L6.5 10L12 10Z" fill="#ef4444" stroke="white" strokeWidth="0.75" />
                </svg>
                <span className="absolute left-4 top-0 whitespace-nowrap rounded bg-red-500 px-1 py-0.5 text-[var(--spacing.2, 8px)] font-medium leading-tight text-white">
                  {user.user_id}
                </span>
              </div>)}
          </div>}
