 ...
 *  - No inline uppercase utility. Section header uses --text-label token.
 *  - Legend ("ΔE<2 auto-fixable") is @schemaRole metadata.
 *
...
function DriftRow({
  ...
  return <div className="group flex flex-col gap-1 border-b border-zinc-800/60 px-3 py-2 last:border-0">
            {/* Detected class + colour swatch */}
            <div className="flex items-center gap-1.5">
                {/* Colour swatch */}
                <span className="h-3 w-3 shrink-0 rounded-sm border border-white/10" style={{
        backgroundColor: candidate.rawValue
      }} title={candidate.rawValue} />
                <span className="min-w-0 flex-1 truncate font-mono text-amber-400" style={{ fontSize: 'var(--text-micro)' }}>
                    {candidate.fullClass}
                </span>
                {match !== null ? <span className={`shrink-0 rounded border px-1 py-px text-[10px] ${deltaEBadgeClass(match.deltaE)}`}>
                        {deltaELabel(match.deltaE)}{' '}
                        <span className="opacity-60 font-mono">ΔE {match.deltaE.toFixed(1)}</span>
                    </span> : <span style={{ fontSize: 'var(--text-micro)', color: 'var(--text-tertiary)' }}>No off-brand colors found</span>}
            </div>

            {/* Suggestion row */}
            {match !== null && <div className="flex items-center gap-1.5 pl-4">
                    <span style={{ fontSize: 'var(--text-micro)', color: 'var(--text-tertiary)' }}>→</span>
                    {/* Token colour swatch */}
                    <span className="h-2.5 w-2.5 shrink-0 rounded-sm border border-white/10" style={{
        backgroundColor: match.tokenValue
      }} title={match.tokenValue} />
                    <span className="min-w-0 flex-1 truncate font-mono" style={{ fontSize: 'var(--text-micro)', color: 'var(--text-tertiary)' }}>
                        {match.tokenPath}
                    </span>
                    {match.systemizable && (
                      /* @schemaRole cta-secondary — fixes a drift warning, not a headline action */
                      <button
                        data-schema-role="cta-secondary"
                        type="button"
                        onClick={() => onFix(candidate, match)}
                        className="flex shrink-0 items-center gap-0.5 rounded border border-emerald-800/60 bg-emerald-900/30 px-1.5 py-px text-emerald-400 transition-colors hover:bg-emerald-900/60"
...
export function DriftDetector() {
  ...
  const allCandidates = useMemo(() => ast !== null ? scanArbitraryColors(ast as any) : [], [ast]);

  // Filter to the selected node only.
  const candidates = useMemo(() => allCandidates.filter(c => c.nodeId === selectedNodeId), [allCandidates, selectedNodeId]);
  ...
  return <div className="flex flex-col border-t border-zinc-800">
            {/* Section header — "Color Alignment" label is primary-content role */}
            <div className="flex shrink-0 items-center gap-1.5 px-3 py-1.5">
                <AlertTriangle className="h-3 w-3 text-amber-500" />
                <span
                  data-schema-role="primary-content"
                  className="font-semibold text-amber-500/80"
                ...
                {systemizableCount > 0 && (
                  ...
                  <span
                    ...
                    className="ml-auto rounded bg-emerald-900/40 px-1.5 py-px text-emerald-400"
                    style={{ fontSize: 'var(--text-micro)' }}
                  >
        ...
        return <DriftRow key={candidate.fullClass} candidate={candidate} match={match} onFix={handleFix} />;
      })}
            </div>

            {/* Legend — @schemaRole metadata: ΔE thresholds are passive info, no user action */}
            <div
              data-schema-role="metadata"
              className="flex items-center gap-3 border-t border-zinc-800/60 px-3 py-1.5"
            >
                <CheckCircle2 className="h-2.5 w-2.5 text-emerald-500" />
                <span style={{ fontSize: 'var(--text-micro)', color: 'var(--text-tertiary)' }}>ΔE&lt;2 auto-fixable</span>
                <span style={{ fontSize: 'var(--text-micro)', color: 'var(--text-tertiary)' }}>·</span>
                <span style={{ fontSize: 'var(--text-micro)', color: 'var(--text-tertiary)' }}>ΔE&gt;5 noticeable drift</span>
            </div>
        </div>;
}
