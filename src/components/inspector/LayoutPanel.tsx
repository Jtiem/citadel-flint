 ...
 *  - All text-[var(--spacing.*)] font-sizes replaced with --text-* token vars.
 *  - All text-zinc-{400..700} replaced with --text-* token color vars.
 *  - No inline uppercase. "Layout & Sizing" label uses PanelTabLabel role via
 *    data-schema-role="primary-content" (section-header semantic, not all-caps).
 *
...
function AlignmentGrid({
  ...
  for (let row = 0; row < 3; row++) {
    for (let col = 0; col < 3; col++) {
      ...
      const isActive = activeJustification === cellJustify && activeAlignment === cellItems;
      cells.push(<button key={`${row}-${col}`} type="button" title={`${cellJustify} ${cellItems}`} className="flex h-6 w-6 items-center justify-center rounded transition-colors hover:bg-zinc-700/60" onClick={() => onCellClick(cellJustify, cellItems)}>
                    <div className={`h-1.5 w-1.5 rounded-full transition-colors ${isActive ? 'bg-indigo-400' : 'bg-zinc-600 hover:bg-zinc-400'}`} />
                </button>);
    }
  }
  return <div className="grid shrink-0 grid-cols-3 gap-0.5 rounded-md bg-zinc-800/80 p-1 ring-1 ring-inset ring-zinc-700/50">
            {cells}
        </div>;
}
...
export function LayoutPanel({
  ...
  function handleTokenChange(prefix: string, type: TokenType, newCls: string | null): void {
    const allPossible = new Set(tokens.filter(t => t.token_type === type).map(t => tokenToClass(t.token_path, t.token_type, prefix)));
    const parts = className.split(/\s+/).filter(c => c !== '' && !allPossible.has(c));
  ...
  function getOptionsFor(tokenType: TokenType, prefix: string) {
    return tokens.filter(t => t.token_type === tokenType).map(t => ({
  ...
  function flowBtnCls(value: string): string {
    ...
    return ['flex items-center justify-center rounded p-1.5 transition-colors', isActive ? 'bg-indigo-600/25 text-indigo-400 ring-1 ring-inset ring-indigo-500/40' : 'bg-zinc-800/60 hover:bg-zinc-700/60 hover:text-zinc-300'].join(' ');
  }

  // ── Render ────────────────────────────────────────────────────────────────

  return <div className="flex flex-col gap-2 border-b border-zinc-800 px-3 py-3">
            {/* Section header — primary-content role; NOT uppercase (uses text-label token) */}
            <span
            ...
            <div className="flex items-center justify-between">
                <span
                  ...
                  style={{ fontSize: 'var(--text-body)', color: 'var(--text-tertiary)' }}
                >
                  Auto Layout
                </span>
                <div className="flex gap-1 justify-end">
                    <button type="button" title="Wrap (flex-wrap)" className={flowBtnCls('flex-wrap')} style={{ color: activeFlow === 'flex-wrap' ? 'var(--text-accent)' : 'var(--text-tertiary)' }} onClick={() => handleFlow('flex-wrap')}>
                        <WrapText className="h-3.5 w-3.5" />
                    </button>
                    <button type="button" title="Vertical (flex-col)" className={flowBtnCls('flex-col')} style={{ color: activeFlow === 'flex-col' ? 'var(--text-accent)' : 'var(--text-tertiary)' }} onClick={() => handleFlow('flex-col')}>
                        <ArrowDown className="h-3.5 w-3.5" />
                    </button>
                    <button type="button" title="Horizontal (flex-row)" className={flowBtnCls('flex-row')} style={{ color: activeFlow === 'flex-row' ? 'var(--text-accent)' : 'var(--text-tertiary)' }} onClick={() => handleFlow('flex-row')}>
            ...
            <div className="flex gap-3">
                <AlignmentGrid isColumn={isColumn} activeAlignment={activeAlignment} activeJustification={activeJustification} onCellClick={handleCellClick} />
                <div className="flex flex-col gap-2 flex-1 min-w-0">
                    <div className="flex gap-2">
                        <div className="flex-1 min-w-0" title="Gap">
                            <CompactSelect icon={<Grid className="h-3 w-3" />} value={getActiveTokenClass('gap-', 'dimension')} onChange={cls => handleTokenChange('gap-', 'dimension', cls)} options={getOptionsFor('dimension', 'gap-')} />
                        </div>
                        <div className="flex-1 min-w-0" title="Padding">
                            <CompactSelect icon={<Maximize className="h-3 w-3" />} value={getActiveTokenClass('p-', 'dimension')} onChange={cls => handleTokenChange('p-', 'dimension', cls)} options={getOptionsFor('dimension', 'p-')} />
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="flex gap-2">
                <div className="flex flex-1 items-center gap-1.5" title="Width">
                    <CompactSelect
                      icon={<span className="font-bold w-3 text-center" style={{ fontSize: 'var(--text-label)', color: 'var(--text-secondary)' }}>W</span>}
                      value={activeWidth}
                ...
                <div className="flex flex-1 items-center gap-1.5" title="Height">
                    <CompactSelect
                      icon={<span className="font-bold w-3 text-center" style={{ fontSize: 'var(--text-label)', color: 'var(--text-secondary)' }}>H</span>}
                      value={activeHeight}
