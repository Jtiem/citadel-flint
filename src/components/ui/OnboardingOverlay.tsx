...
export function OnboardingOverlay({
  ...
  const tooltipPositionClass = current.side === 'right' ? 'right-72 top-1/2 -translate-y-1/2' : 'left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2';
  return (
    // Full-screen backdrop
    <div className="fixed inset-0 z-50 bg-black/50">
            {/* Tooltip card */}
            <div className={`absolute ${tooltipPositionClass} w-72 rounded-lg border border-indigo-500 bg-zinc-900 p-4 shadow-xl shadow-black/40`}>
                {/* Dismiss button */}
                <button type="button" onClick={complete} className="absolute right-2 top-2 rounded p-1 text-zinc-500 hover:bg-zinc-800 hover:text-zinc-300 transition-colors" aria-label="Skip onboarding">
                    <X size={14} />
                </button>

                {/* Step counter */}
                <p className="mb-2 text-[var(--spacing.2, 8px)] font-medium uppercase tracking-widest text-indigo-400">
                ...
                <div className="mt-4 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1.5">
                        {STEPS.map((_, i) => <span key={i} className={`block h-1.5 rounded-full transition-all ${i === step ? 'w-4 bg-indigo-400' : 'w-1.5 bg-zinc-700'}`} />)}
                    </div>

                    {/* Next / Got it */}
                    <button type="button" onClick={handleNext} className="rounded px-3 py-1.5 text-xs font-medium bg-indigo-600 text-zinc-100 hover:bg-indigo-500 transition-colors">
                        {isLast ? 'Got it' : 'Next'}
                    </button>
                </div>
            </div>
