...
} from '../../core/rulePackRegistryClient'
...
function PackStatusBadge({ status, isActive }: StatusBadgeProps) {
    if (isActive) {
        return (
            <span className="rounded border border-emerald-500/30 bg-emerald-900/20 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-emerald-400">
                Active
            </span>
        )
    }
...
function PackRow({ pack, isActive, isToggling, onEnable, onDisable }: PackRowProps) {
    ...
    return (
        ...
        >
            ...
            <div className="min-w-0 flex-1">
                ...
                {pack.jurisdictions.length > 0 && (
                    <div className="mt-1 flex flex-wrap gap-1">
                        {pack.jurisdictions.map((j) => (
                            <span
                                key={j}
                                className="rounded bg-zinc-800 px-1 py-0.5 font-mono text-[9px] text-zinc-600"
                            >
                                {j}
                            </span>
                        ))}
            ...
            {!isComingSoon && (
                <button
                    ...
                    aria-label={isActive ? `Disable ${pack.name}` : `Enable ${pack.name}`}
                    className={`shrink-0 rounded border px-2.5 py-1 text-[10px] font-medium transition-colors disabled:cursor-not-allowed disabled:opacity-40 ${
                        isActive
                            ? 'border-zinc-700 bg-zinc-800 text-zinc-400 hover:border-red-700/40 hover:bg-red-900/10 hover:text-red-400'
                            : 'border-indigo-500/30 bg-indigo-900/20 text-indigo-400 hover:border-indigo-500/60 hover:bg-indigo-900/40 hover:text-indigo-300'
                    }`}
                >
...
function DomainSection({
    ...
    const activePacks = packs.filter(
        (p) =>
            p.status === 'active' ||
            (p.preset !== undefined && activePresets.includes(p.preset)),
    )
    ...
    return (
        <div className="border-b border-zinc-800/60 last:border-0">
            ...
            <button
                ...
                className="flex w-full items-center gap-2 px-4 py-2.5 text-left transition-colors hover:bg-zinc-800/20"
            ...
            >
                ...
                {activePacks.length > 0 && (
                    <span className="shrink-0 rounded border border-emerald-500/30 bg-emerald-900/20 px-1.5 py-0.5 text-[10px] text-emerald-400">
                    ...
                    {packs.map((pack) => {
                        ...
                        return (
                            <PackRow
                                ...
                                onEnable={() => onEnablePack(pack.id)}
                                onDisable={() => onDisablePack(pack.id)}
                            />
                        )
                    })}
                </div>
    ...
    const filteredPacks = useMemo(() => {
        ...
        return RULE_PACK_REGISTRY.filter((pack) => {
            ...
            const matchesSearch =
                ...
                pack.jurisdictions.some((j) => j.toLowerCase().includes(q))
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className="relative flex-1">
                        ...
                        <input
                            ...
                            className="w-full rounded border border-zinc-700/50 bg-zinc-800 py-1.5 pl-7 pr-2 text-xs text-zinc-300 placeholder-zinc-600 transition-colors focus:border-indigo-500/50 focus:outline-none focus:ring-1 focus:ring-indigo-500/30"
                    ...
                    <select
                        value={domainFilter}
                        onChange={(e) => setDomainFilter(e.target.value as ComplianceDomain | 'all')}
                        className="rounded border border-zinc-700/50 bg-zinc-800 py-1.5 px-2 text-xs text-zinc-300 transition-colors focus:border-indigo-500/50 focus:outline-none"
