...
export function FixPreviewDrawer({
  ...
  const handleAlwaysAutoChange = useCallback((checked: boolean) => {
    setAlwaysAuto(checked);
    setPrefs({
      fixMode: checked ? 'auto' : 'preview'
    });
  }, [setPrefs]);
  ...
  return <div className="border-t border-indigo-500/30 bg-zinc-900" role="region" aria-label={isBatch ? `Batch fix preview — ${items.length} changes` : 'Fix preview'}>
            {/* ── Header ───────────────────────────────────────────────── */}
            <div className="flex items-center gap-2 border-b border-zinc-800 px-3 py-2">
                ...
                <span className="flex-1 text-xs font-medium text-zinc-200">
                    {isBatch ? `Preview ${items.length} fixes` : 'Preview fix'}
                </span>
                <button type="button" onClick={onCancel} aria-label="Dismiss fix preview" className="rounded p-0.5 text-zinc-500 transition-colors hover:bg-zinc-800 hover:text-zinc-100">
                    <X size={12} />
                </button>
            </div>
      ...
      <ol className="divide-y divide-zinc-800/50">
                        {items.map((item, idx) => {
          const diffResult = diffLines(`className="${item.hardcodedClass}"`, `className="${item.tokenClass}"`);
          return <li key={item.nodeId + idx} className="px-3 py-2 space-y-1">
                                    <p className="text-[var(--spacing.2, 8px)] text-zinc-500 truncate" title={item.label}>
      ...
      <div className="px-3 py-2 space-y-1.5">
                        {items[0] && <>
                                <p className="text-[var(--spacing.2, 8px)] text-zinc-500 truncate" title={items[0].label}>
                                    {items[0].label}
                                </p>
                                <DiffBlock lines={diffLines(`className="${items[0].hardcodedClass}"`, `className="${items[0].tokenClass}"`)} />
                            </>}
                    </div>}
            </div>
            ...
            <div className="border-t border-zinc-800 px-3 py-2 space-y-1">
                <label className="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" checked={alwaysAuto} onChange={e => handleAlwaysAutoChange(e.target.checked)} className="h-3 w-3 rounded border-zinc-600 bg-zinc-800 accent-indigo-500" aria-label="Always auto-fix without preview" />
                    <span className="text-[var(--spacing.3, 12px)] text-zinc-400">
                        Always auto-fix (skip preview)
                    </span>
                </label>
                {alwaysAuto && <p className="flex items-center gap-1 text-[var(--spacing.2, 8px)] text-zinc-600">
                        <AlertTriangle size={9} className="text-amber-500 shrink-0" aria-hidden="true" />
                        Auto-fixes will apply immediately.{' '}
                        <button type="button" onClick={onOpenSettings} className="inline-flex items-center gap-0.5 text-indigo-400 hover:text-indigo-300 transition-colors" aria-label="Open Policy Settings to change fix behavior">
            ...
            <div className="flex gap-2 border-t border-zinc-800 px-3 py-2">
                <button type="button" onClick={handleApply} className="flex items-center gap-1 rounded bg-indigo-600/20 px-3 py-1 text-xs font-medium text-indigo-300 transition-colors hover:bg-indigo-600/30" aria-label={isBatch ? `Apply all ${items.length} fixes` : 'Apply fix'}>
                    <CheckCircle2 size={11} aria-hidden="true" />
                    {isBatch ? `Apply all (${items.length})` : 'Apply fix'}
                </button>
                <button type="button" onClick={onCancel} className="rounded px-3 py-1 text-xs text-zinc-500 transition-colors hover:bg-zinc-800 hover:text-zinc-300" aria-label="Cancel and discard fix">
                    Cancel
                </button>
            </div>
        </div>;
