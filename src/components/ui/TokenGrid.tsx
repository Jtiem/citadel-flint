...
function SyncBadge({
  ...
  return <span className={`shrink-0 rounded border px-1.5 py-0.5 text-[10px] ${SYNC_BADGE_STYLES[status]}`} data-testid="sync-badge">
...
function UsageBadge({
  ...
  if (usageCount === 0) {
    return <SeverityChip severity="advisory" label="dead" data-testid="dead-token-badge" aria-label="Dead token: not used in any file" />;
  }
  const isHighUsage = usageCount > 10;
  return <span className={`shrink-0 rounded border px-1.5 py-0.5 text-[10px] ${isHighUsage ? 'border-emerald-400/20 bg-emerald-400/10 text-emerald-400' : 'border-zinc-600/40 bg-zinc-800/60 text-zinc-400'}`} data-testid="usage-badge" aria-label={`Used in ${usageCount} file${usageCount !== 1 ? 's' : ''}`} title={files.length > 0 ? `Files: ${files.slice(0, 5).join(', ')}${files.length > 5 ? ` and ${files.length - 5} more` : ''}` : undefined}>
...
function DriftBadge({
  ...
  return <span data-testid="drift-badge" title={`Local: ${drift.localValue}\nFigma: ${drift.figmaValue}`}>
            <SeverityChip severity="amber" label="drifted" aria-label={`Drifted from Figma: local ${drift.localValue}, Figma ${drift.figmaValue}`} />
        </span>;
}
...
export function ContrastBadge({
  ...
  if (grade === 'fail') {
    return <span data-testid="contrast-inline-badge" title={ratio ? `Contrast ratio ${ratio.toFixed(1)}:1 — fails WCAG AA` : 'Fails WCAG AA contrast'}>
                <SeverityChip severity="critical" label="contrast fail" aria-label={`Contrast: ${ratio ? `${ratio.toFixed(1)}:1` : ''} FAIL`} />
            </span>;
  }
  return <span className="shrink-0 rounded border border-emerald-400/20 bg-emerald-400/10 px-1.5 py-0.5 text-[var(--spacing.2, 8px)] font-medium text-emerald-400" data-testid="contrast-inline-badge" aria-label={`Contrast: ${ratio ? `${ratio.toFixed(1)}:1` : ''} ${grade.toUpperCase()}`} title={ratio ? `Contrast ratio ${ratio.toFixed(1)}:1 — passes WCAG ${grade.toUpperCase()}` : `Passes WCAG ${grade.toUpperCase()}`}>
            {grade.toUpperCase()}
        </span>;
}

/** Compute the best contrast grade for a token from its contrast pairs. */
export function getBestContrastGrade(tokenPath: string, contrastMap: Map<string, ContrastPair[]>): {
  grade: ContrastBadgeGrade;
  ...
  if (bestPair.passAAA) return {
    grade: 'aaa',
    ratio: bestPair.ratio
  };
  if (bestPair.passAA) return {
    grade: 'aa',
    ratio: bestPair.ratio
  };
  return {
...
function isMotionToken(token: DesignToken): boolean {
  ...
  return path.includes('duration') || path.includes('easing') || path.includes('motion') || path.includes('transition') || path.includes('animation');
}
...
function MotionPreview({
  ...
  return <span className="relative inline-block h-3 w-8 rounded-full bg-zinc-700/50" data-testid="motion-preview" aria-label={`Motion preview: ${value}`} title={`Motion: ${value}`}>
            <span className="absolute left-0 top-0 h-3 w-3 rounded-full bg-indigo-400" style={{
...
export function detectScaleGaps(dimensionTokens: DesignToken[]): ScaleGap[] {
  const nums = dimensionTokens.map(t => {
...
function ScaleGapWarning({
  gaps
}: {
  gaps: ScaleGap[];
}) {
  if (gaps.length === 0) return null;
  return <div className="flex flex-wrap items-center gap-1 px-3 py-1 border-b border-amber-500/10 bg-amber-500/5" data-testid="scale-gap-warning" role="alert" aria-label="Scale gap detected">
            {gaps.map((gap, i) => <span key={i} className="inline-flex items-center gap-1 rounded border border-amber-400/20 bg-amber-400/10 px-1.5 py-0.5 text-[var(--spacing.2, 8px)] text-amber-400">
...
function ColorSwatchLarge({
  ...
  return <span className="inline-block h-8 w-8 shrink-0 rounded-full border border-white/20 shadow-sm" style={{
    backgroundColor: value
  }} aria-label={`Color swatch: ${label}, ${value}`} role="img" />;
}
function ColorSwatchSmall({
  value,
  label
}: {
  value: string;
  ...
  return <span className="inline-block h-3.5 w-3.5 shrink-0 rounded-full border border-white/20 shadow-sm" style={{
    backgroundColor: value
  }} aria-label={`Color swatch: ${label}, ${value}`} role="img" />;
}
function TypographySpecimen({
  ...
  return <span className="text-lg text-zinc-300" style={style} aria-label={`Typography specimen for ${token.token_path}: ${token.token_value}`}>
            Aa
        </span>;
}
function DimensionRuler({
  ...
  return <span className="inline-block h-2 shrink-0 rounded-full bg-blue-400/50" style={{
    width: barWidth
  }} aria-label={`Spacing ruler: ${value}`} role="img" />;
}
function ShadowSwatch({
  ...
  return <span className="inline-block h-8 w-8 shrink-0 rounded-lg bg-zinc-700" style={{
    boxShadow: value
  }} aria-label={`Shadow swatch: ${label}, ${value}`} role="img" />;
}
function OpacitySwatch({
  ...
  return <span className="inline-block h-8 w-8 shrink-0 rounded-lg bg-indigo-400" style={{
    opacity: isNaN(opacityNum) ? 1 : opacityNum
  }} aria-label={`Opacity swatch: ${label}, ${value}%`} role="img" />;
}
...
function DimensionBar({
  ...
  return <span className="inline-block h-1.5 shrink-0 rounded-full bg-blue-400/50" style={{
    width: barWidth
  }} aria-label={`Spacing: ${value}`} role="img" />;
}
export function TokenRow({
  ...
  return <div className="flex cursor-pointer items-center gap-2 border-b border-zinc-800/40 px-3 py-1.5 hover:bg-zinc-800/30" role="row" tabIndex={0} onClick={onClick} onKeyDown={e => {
    if ((e.key === 'Enter' || e.key === ' ') && onClick) {
      ...
      onClick();
            ...
            {token.token_type === 'color' && <ColorSwatchSmall value={token.token_value} label={token.token_path} />}
            {token.token_type === 'dimension' && <DimensionBar value={token.token_value} />}

            {/* Path + value */}
            <div className="min-w-0 flex-1" role="gridcell">
                <p className="truncate font-mono text-[var(--spacing.2, 8px)] text-zinc-500" title={token.token_path}>
                    {token.token_path}
                </p>
                {/* Read-only value — tooltip explains how to edit */}
                <p className="mt-0.5 truncate font-mono text-[var(--spacing.3, 12px)] text-zinc-300" title={figmaConnected ? 'Token values are managed through Figma. Use Envoy sync to update.' : 'Token values are managed in design-tokens.json. Use MCP tools to update.'} style={token.token_type === 'fontFamily' ? {
        fontFamily: token.token_value
            ...
            {/* MINT.2b: Usage badge */}
            {usageResult && <UsageBadge usageCount={usageResult.usageCount} files={usageResult.files} />}
            ...
            {token.mode !== 'default' && <span className="shrink-0 rounded px-1 py-0.5 text-[var(--spacing.2, 8px)] font-medium uppercase text-zinc-400 ring-1 ring-zinc-700">
                    {token.mode}
                </span>}
        </div>;
}
...
function TokenGridCard({
  ...
  return <div className={`flex cursor-pointer flex-col items-center gap-1.5 rounded-lg border p-3 transition-colors hover:border-zinc-700 ${isDead ? 'border-red-500/20 bg-red-950/20' : 'border-zinc-800/60 bg-zinc-900/50'}`} data-testid="token-grid-card" tabIndex={0} onClick={onClick} onKeyDown={e => {
    if ((e.key === 'Enter' || e.key === ' ') && onClick) {
      e.preventDefault();
      onClick();
    }
  }} role="button" aria-label={`Token ${token.token_path}: ${token.token_value}. Click for details.`}>
            {/* Visual specimen */}
            <div className="flex items-center gap-2">
                {token.token_type === 'color' && <>
                        <ColorSwatchLarge value={token.token_value} label={`${token.token_path} light`} />
                        {darkModeToken && <ColorSwatchLarge value={darkModeToken.token_value} label={`${token.token_path} dark`} />}
                    </>}
                {(token.token_type === 'fontFamily' || token.token_type === 'fontWeight' || token.token_type === 'lineHeight') && <TypographySpecimen token={token} />}
                {token.token_type === 'dimension' && <DimensionRuler value={token.token_value} />}
                {token.token_type === 'shadow' && <ShadowSwatch value={token.token_value} label={token.token_path} />}
                {token.token_type === 'opacity' && <OpacitySwatch value={token.token_value} label={token.token_path} />}
                {token.token_type === 'letterSpacing' && <span className="text-lg text-zinc-300" style={{
        letterSpacing: token.token_value
      }} aria-label={`Letter spacing specimen: ${token.token_value}`}>
                        Aa
                    </span>}
                {token.token_type === 'string' && <span className="text-xs text-zinc-400">{token.token_value}</span>}
                {token.token_type === 'boolean' && <span className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${token.token_value === 'true' ? 'bg-emerald-400/10 text-emerald-400' : 'bg-zinc-700 text-zinc-400'}`}>
                        {token.token_value}
                    </span>}
            </div>

            {/* MINT.1c: Mode columns for color tokens */}
            {token.token_type === 'color' && darkModeToken && <div className="flex w-full items-center gap-2 text-[var(--spacing.2, 8px)]" data-testid="mode-columns">
            ...
            {/* Token path + value */}
            <p className="w-full truncate text-center font-mono text-[var(--spacing.2, 8px)] text-zinc-500" title={token.token_path}>
                {token.token_path}
            </p>
            <p className="w-full truncate text-center font-mono text-[var(--spacing.2, 8px)] text-zinc-400">
                {token.token_value}
            </p>

            {/* MINT.2b: Usage badge */}
            {usageResult && <div className="mt-0.5">
                    <UsageBadge usageCount={usageResult.usageCount} files={usageResult.files} />
                </div>}

            {/* MINT.2c: Drift indicator */}
            {drift && <div className="mt-0.5">
                    <DriftBadge drift={drift} />
                </div>}

            {/* MINT.3b: Contrast badge */}
            {contrastGrade && <div className="mt-0.5">
                    <ContrastBadge grade={contrastGrade} ratio={contrastRatio} />
                </div>}

            {/* MINT.3d: Motion preview */}
            {isMotionToken(token) && <div className="mt-0.5">
                    <MotionPreview token={token} />
                </div>}

            {/* Sync badge */}
            {syncStatus && <div className="mt-0.5">
...
const GRID_FRIENDLY_TYPES = new Set<TokenType>(['color', 'shadow', 'opacity', 'fontFamily', 'fontWeight', 'lineHeight', 'letterSpacing', 'dimension']);
...
export function TokenDriftView({
  ...
  return <div className="px-3 py-3" data-testid="token-drift-view">
            <DriftGroupSection driftedTokens={driftedTokens} tokensByPath={tokensByPath} onPullOne={onPullOne} onSelect={onSelect} currentPullingPath={currentPullingPath} />
        </div>;
}
export function TokenGroupSection({
  ...
  return <div>
      ...
      const primaryTokens = viewMode === 'grid' ? group.filter(t => t.mode.toLowerCase() !== 'dark') : group;

      // MINT.3d: Detect scale gaps for dimension tokens
      const scaleGaps = tokenType === 'dimension' ? detectScaleGaps(group) : [];
      return <div key={tokenType}>
                        {/* Type sub-header */}
                        <div className="flex items-center gap-1.5 border-b border-zinc-800/60 bg-zinc-900/40 px-3 py-1">
                            <span className={`inline-block h-1.5 w-1.5 rounded-full ${TYPE_DOT[tokenType] ?? 'bg-zinc-500'}`} />
                            <span className="text-[var(--spacing.2, 8px)] font-medium uppercase tracking-widest text-zinc-400">
                                {TYPE_LABEL[tokenType] ?? tokenType}
                            </span>
                            <span className="ml-auto text-[var(--spacing.2, 8px)] text-zinc-500">
                                {group.length}
                            </span>
                        </div>
                        ...
                        {viewMode === 'grid' && isGridFriendly(tokenType) ? <div className="grid gap-2 p-2" style={{
          gridTemplateColumns: 'repeat(auto-fill, minmax(120px, 1fr))'
        }} role="grid" aria-label={`${TYPE_LABEL[tokenType] ?? tokenType} tokens grid`}>
                                {primaryTokens.map(token => {
            const contrast = contrastMap ? getBestContrastGrade(token.token_path, contrastMap) : {
              grade: null,
              ratio: undefined
            };
            return <TokenGridCard key={token.id} token={token} darkModeToken={darkTokensByPath.get(token.token_path)} syncStatus={getSyncStatus(token)} usageResult={usageMap?.get(token.token_path) ?? null} drift={driftByName.get(token.token_path) ?? null} contrastGrade={contrast.grade} contrastRatio={contrast.ratio} onClick={onTokenSelect ? () => onTokenSelect(token) : undefined} />;
          })}
                            </div> : <div role="grid" aria-label={`${TYPE_LABEL[tokenType] ?? tokenType} tokens list`}>
                                {group.map(token => {
            const contrast = contrastMap ? getBestContrastGrade(token.token_path, contrastMap) : {
              grade: null,
              ratio: undefined
            };
            return <TokenRow key={token.id} token={token} syncStatus={getSyncStatus(token)} figmaConnected={figmaConnected} usageResult={usageMap?.get(token.token_path) ?? null} drift={driftByName.get(token.token_path) ?? null} contrastGrade={contrast.grade} contrastRatio={contrast.ratio} onClick={onTokenSelect ? () => onTokenSelect(token) : undefined} />;
          })}
                            </div>}
                    </div>;
    })}
