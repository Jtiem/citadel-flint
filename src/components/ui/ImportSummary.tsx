...
import { Check, AlertTriangle, X, ArrowRight, Eye, Zap, Undo2, ShieldCheck } from 'lucide-react';
...
function Tier1FixRow({
  fix
}: {
  fix: IngestionFix;
}) {
  return <div className="flex items-center gap-2 px-3 py-1.5">
            <Check size={10} className="shrink-0 text-emerald-400" />
            <span className="min-w-0 flex-1 font-mono text-[var(--spacing.2, 8px)] text-zinc-400 truncate">
                {fix.originalValue}
            </span>
            <ArrowRight size={9} className="shrink-0 text-zinc-600" />
            <span className="font-mono text-[var(--spacing.2, 8px)] text-emerald-400 truncate max-w-[var(--spacing.12, 48px)]">
                {fix.fixedToClass}
            </span>
        </div>;
}
...
function Tier2FlagRow({
  flag,
  onSnap,
  isSnapping
}: Tier2FlagRowProps) {
  const distLabel = flag.distanceUnit === 'deltaE' ? `ΔE ${flag.distance.toFixed(1)} from ${flag.suggestedClass}` : `${flag.distance}px from ${flag.suggestedClass}`;
  return <div className="flex items-center gap-2 px-3 py-1.5">
            <AlertTriangle size={10} className="shrink-0 text-amber-400" />
            <div className="min-w-0 flex-1">
                <span className="block font-mono text-[var(--spacing.2, 8px)] text-zinc-300 truncate">
                    {flag.originalValue}
                </span>
                <span className="text-[var(--spacing.2, 8px)] text-zinc-500">{distLabel}</span>
            </div>
            <button type="button" onClick={() => onSnap(flag)} disabled={isSnapping} aria-label={`Apply token fix: replace ${flag.originalValue} with ${flag.suggestedToken}`} title="Apply the suggested design token to this value" className="shrink-0 flex items-center gap-1 rounded border border-indigo-500/30 bg-indigo-600/10 px-1.5 py-0.5 text-[var(--spacing.2, 8px)] text-indigo-400 transition-colors hover:bg-indigo-600/20 hover:text-indigo-300 disabled:cursor-not-allowed disabled:opacity-50">
                <Zap size={9} />
...
function Tier3FlaggedRow({
  nodeId,
  onView
}: Tier3FlaggedRowProps) {
  return <div className="flex items-center gap-2 px-3 py-1.5">
            <X size={10} className="shrink-0 text-red-400" />
            <span className="min-w-0 flex-1 font-mono text-[var(--spacing.2, 8px)] text-red-400 truncate">
                #{nodeId}
            </span>
            <button type="button" onClick={() => onView(nodeId)} aria-label={`View node ${nodeId} on canvas`} className="shrink-0 flex items-center gap-1 rounded border border-zinc-700/50 bg-zinc-800/60 px-1.5 py-0.5 text-[var(--spacing.2, 8px)] text-zinc-400 transition-colors hover:bg-zinc-700 hover:text-zinc-200">
                <Eye size={9} />
                View
            </button>
        </div>;
...
function ToastCard() {
  ...
  const summaryText = allClean ? `${tier1Count} token${tier1Count !== 1 ? 's' : ''} matched` : [tier1Count > 0 && `${tier1Count} auto-matched`, tier2Count > 0 && `${tier2Count} need review`, tier3Count > 0 && `${tier3Count} flagged`].filter(Boolean).join(' · ');
  return <div className={`flex w-full max-w-sm overflow-hidden rounded-lg border bg-zinc-900 shadow-xl ${allClean ? 'border-emerald-800/40' : 'border-indigo-500/30'}`} role="status" aria-live="polite" aria-label="Import summary" onMouseEnter={() => setInteracted(true)}>
            {/* Left accent bar */}
            ...
            <div className="flex min-w-0 flex-1 items-start gap-2.5 px-3 py-2.5">
                <span className="mt-0.5 shrink-0">
                    {allClean ? <ShieldCheck size={13} className="text-emerald-400" /> : <Check size={13} className="text-indigo-400" />}
                </span>
                ...
                <div className="min-w-0 flex-1">
                    ...
                    {!allClean && <button type="button" onClick={() => {
          setInteracted(true);
          openPanel();
                ...
                {/* Dismiss */}
                <button type="button" onClick={dismiss} aria-label="Dismiss import summary" className="ml-1 shrink-0 rounded p-0.5 text-zinc-600 transition-colors hover:bg-zinc-800 hover:text-zinc-300">
                    <X size={12} />
                </button>
            </div>
        </div>;
...
export function ImportSummaryPanelView() {
  ...
  return <div className="flex flex-col h-full" aria-label="Import summary panel">
            {/* Panel header */}
            <div className="flex items-center justify-between border-b border-zinc-800 px-3 py-2 shrink-0">
                ...
                <button type="button" onClick={dismiss} aria-label="Dismiss import summary panel" className="rounded p-0.5 text-zinc-600 transition-colors hover:bg-zinc-800 hover:text-zinc-300">
                    <X size={12} />
                </button>
            </div>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto">
                ...
                {tier1Count > 0 && <div>
                        <div className="border-b border-zinc-800/60 px-3 py-1.5 flex items-center gap-1.5">
                            ...
                            <span className="text-[var(--spacing.2, 8px)] font-medium text-emerald-400 uppercase tracking-wider">
                                Auto-healed ({tier1Count})
                            </span>
                        </div>
                        <div className="divide-y divide-zinc-800/40 bg-emerald-900/5">
                            {summary.tier1Fixed.map(fix => <Tier1FixRow key={`${fix.nodeId}-${fix.ruleId}`} fix={fix} />)}
                        </div>
                    </div>}
                ...
                {tier2Count > 0 && <div>
                        <div className="border-b border-zinc-800/60 px-3 py-1.5 flex items-center gap-1.5">
                            ...
                            <span className="text-[var(--spacing.2, 8px)] font-medium text-amber-400 uppercase tracking-wider">
                                Needs review ({tier2Count})
                            </span>
                        </div>
                        <div className="divide-y divide-zinc-800/40 bg-amber-900/5">
                            {summary.tier2Flagged.map(flag => <Tier2FlagRow key={`${flag.nodeId}-${flag.ruleId}`} flag={flag} onSnap={handleSnap} isSnapping={snappingNodeId === flag.nodeId} />)}
                        </div>
                    </div>}
                ...
                {tier3Count > 0 && <div>
                        <div className="border-b border-zinc-800/60 px-3 py-1.5 flex items-center gap-1.5">
                            ...
                            <span className="text-[var(--spacing.2, 8px)] font-medium text-red-400 uppercase tracking-wider">
                                Flagged ({tier3Count})
                            </span>
                        </div>
                        <div className="divide-y divide-zinc-800/40 bg-red-900/5">
                            <div className="px-3 py-2">
                                <p className="text-[var(--spacing.2, 8px)] text-zinc-500 leading-relaxed">
                ...
                {/* Empty state */}
                {tier1Count === 0 && tier2Count === 0 && tier3Count === 0 && <div className="flex items-center gap-2 px-3 py-4">
            ...
            <div className="shrink-0 border-t border-zinc-800 px-3 py-2">
                <p className="mb-2 text-[var(--spacing.2, 8px)] text-zinc-500">
                    Healed in {Math.round(summary.healTimeMs)}ms
                </p>

                {undoError !== null && <p className="mb-2 text-[var(--spacing.2, 8px)] text-red-400">{undoError}</p>}

                <div className="flex items-center justify-between gap-2">
                    {tier1Count > 0 && <button type="button" onClick={() => {
          void handleUndoAllHeals();
        }} disabled={undoing} aria-label="Undo all auto-healed token fixes" className="flex items-center gap-1 rounded border border-zinc-700/50 bg-zinc-800/60 px-2 py-1 text-[var(--spacing.2, 8px)] text-zinc-400 transition-colors hover:bg-zinc-700 hover:text-zinc-200 disabled:cursor-not-allowed disabled:opacity-50">
                            <Undo2 size={9} />
                            {undoing ? 'Undoing…' : 'Undo all heals'}
                        </button>}

                    <button type="button" onClick={dismiss} aria-label="Dismiss import summary" className="ml-auto rounded border border-zinc-700 px-2.5 py-1 text-[var(--spacing.2, 8px)] text-zinc-400 transition-colors hover:border-zinc-600 hover:text-zinc-200">
                        Dismiss
                    </button>
                </div>
...
export function ImportSummaryToastMount() {
  ...
  return <div className="fixed bottom-16 right-4 z-40 flex flex-col gap-2" aria-label="Import summary toast">
            <ToastCard />
        </div>;
}