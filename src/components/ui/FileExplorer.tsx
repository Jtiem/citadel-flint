 ...
 */
function buildFileParentMap(
    nodes: FileTreeNode[],
    parentMap: Map<string, FileTreeNode> = new Map(),
    parent?: FileTreeNode,
 ...
 */
function FileItem({ node, depth, focusedPath, setFocusedPath }: FileItemProps) {
...
function DirectoryItem({ node, depth, focusedPath, setFocusedPath }: DirectoryItemProps) {
        ...
        >
            ...
            {isExpanded &&
                node.children?.map((child) => (
                    <FileNode key={child.path} node={child} depth={depth + 1} focusedPath={focusedPath} setFocusedPath={setFocusedPath} />
                ))}
        </div>
    )
}
 ...
 */
function FileNode({ node, depth = 0, focusedPath, setFocusedPath }: FileNodeProps) {
...
export function FileExplorer() {
    ...
    const flatVisible = useMemo(
        () => flattenVisibleFileTree(topChildren, expandedFolders),
        [topChildren, expandedFolders],
    )
    const parentMap = useMemo(
        () => buildFileParentMap(topChildren),
        [topChildren],
    )
    ...
    const handleKeyDown = useCallback((e: React.KeyboardEvent<HTMLDivElement>) => {
        ...
        const currentIndex = focusedPath !== null
            ? flatVisible.findIndex((n) => n.path === focusedPath)
            : -1
    ...
    if (!workspaceFiles) {
        return (
            <div className="flex flex-col items-center justify-center gap-2 px-4 py-8 text-center">
                ...
                <p className="text-xs text-zinc-500 max-w-[240px]">Load a project to browse its file tree</p>
                <button
                    type="button"
                    onClick={() => void handleOpenFolder()}
                    className="mt-1 rounded border border-indigo-500/40 bg-indigo-900/20 px-3 py-1.5 text-xs text-indigo-400 transition-colors hover:bg-indigo-900/40 hover:text-indigo-300"
                >
                    Open Folder
                </button>
            </div>
        ...
        <div className="py-1">
            ...
            >
                {workspaceFiles.children?.map((node) => (
                    <FileNode key={node.path} node={node} focusedPath={focusedPath} setFocusedPath={setFocusedPath} />
                ))}
            </div>
        </div>
    )
