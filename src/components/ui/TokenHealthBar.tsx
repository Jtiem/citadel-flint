...
function HealthGradePill({
  grade,
  score
}: HealthGradePillProps) {
  return <span className={['inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5', 'text-[11px] font-semibold leading-none', GRADE_PILL_CLASS[grade]].join(' ')} aria-label={`Health grade ${grade}, score ${score} out of 100`} data-testid="health-grade-pill">
            <span className="text-[var(--spacing.3, 12px)] font-bold leading-none" data-testid="health-grade-letter">
                {grade}
            </span>
            <span className="text-[var(--spacing.2, 8px)] font-medium opacity-80" data-testid="health-score-number">
                {score}/100
            </span>
        </span>;
}
...
export function TokenHealthBar({
  ...
  return <div className="flex shrink-0 flex-wrap items-center gap-2 border-b border-zinc-800/60 px-3 py-2" role="status" aria-label="Token health summary" data-testid="token-health-bar">
            {/* Leading element: grade pill — only when health data available */}
            {health && <HealthGradePill grade={health.grade} score={health.score} />}
            ...
            {showPersistentErrorChip && <SeverityChip severity="critical" label="Connection expired" data-testid="health-chip-sync-error" />}

            {/* Total tokens pill */}
            <span className="inline-flex items-center gap-1 rounded-full bg-zinc-800 px-2.5 py-0.5 text-[var(--spacing.3, 12px)] font-medium text-zinc-300" data-testid="health-total">
                <span className="inline-block h-1.5 w-1.5 rounded-full bg-indigo-400" />
                {totalTokens} token{totalTokens !== 1 ? 's' : ''}
            </span>

            {/* Separator when health chips follow */}
            {health && (health.buckets.contrastFails > 0 || health.buckets.drifted > 0 || health.buckets.dead > 0 || health.buckets.scaleGaps > 0 || health.buckets.pendingConflicts > 0) && <span className="text-zinc-700" aria-hidden="true">•</span>}

            {/* Severity breakdown chips — only non-zero buckets rendered */}
            {health && health.buckets.contrastFails > 0 && <SeverityChip severity="critical" label="contrast fail" count={health.buckets.contrastFails} data-testid="health-chip-contrast" />}

            {health && health.buckets.drifted > 0 && <SeverityChip severity="amber" label="drifted" count={health.buckets.drifted} data-testid="health-chip-drifted" />}

            {health && health.buckets.pendingConflicts > 0 && <SeverityChip severity="amber" label="pending conflict" count={health.buckets.pendingConflicts} data-testid="health-chip-pending" />}

            {health && health.buckets.dead > 0 && <SeverityChip severity="advisory" label="dead" count={health.buckets.dead} data-testid="health-chip-dead" />}

            {health && health.buckets.scaleGaps > 0 && <SeverityChip severity="advisory" label="scale gap" count={health.buckets.scaleGaps} data-testid="health-chip-scale-gaps" />}

            {/* Figma sync: when connected and no drift, show success pill */}
            {figmaConnected && health && health.buckets.drifted === 0 && <span className="inline-flex items-center gap-1 rounded-full bg-emerald-400/10 px-2.5 py-0.5 text-[var(--spacing.3, 12px)] font-medium text-emerald-400" data-testid="health-sync">
                    <span className="inline-block h-1.5 w-1.5 rounded-full bg-emerald-400" />
                    In sync
                </span>}

            {/* Coverage pill — only when usage data is available */}
            {usageFileCount > 0 && <span className="inline-flex items-center gap-1 rounded-full bg-zinc-800 px-2.5 py-0.5 text-[var(--spacing.3, 12px)] font-medium text-zinc-400" data-testid="health-coverage">
            ...
            {hasSyncCluster && <div className={hasEmitCluster ? undefined : 'ml-auto'}>
                    <SyncActionCluster figmaConnected={figmaConnected} driftCount={clusterDriftCount} pendingConflictCount={clusterConflictCount} localEditCount={clusterLocalEditCount} syncOp={syncOp ?? null} onPull={onPull ?? (() => {})} onPush={onPush ?? (() => {})} onResolve={onResolve ?? (() => {})} onConnect={onConnect} />
                </div>}
            ...
            {/* MINT.5 Phase 3 §3.1 — Emit dropdown at trailing edge.
                ...
                to the far right. */}
            {hasEmitCluster && <div className={hasSyncCluster ? undefined : 'ml-auto'} data-testid="emit-cluster">
                    <EmitDropdown emitOp={emitOp ?? null} onEmit={onEmit!} disabled={false} />
                </div>}
        </div>;
}