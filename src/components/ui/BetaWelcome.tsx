...
import { ShieldCheck, Sparkles, ArrowRight, Eye, Wrench, PackageCheck, MessageSquare } from 'lucide-react';
...
const TEST_CHECKLIST = ['Open the demo project and see drift items appear', 'Click "Auto-Fix" on an issue and watch the preview update', 'Try the Export Gate — can you export with issues?', 'Check the governance dashboard (Health tab in the sidebar)', 'Send us feedback via the Beta chip in the status bar'];
export function BetaWelcome({
  ...
  return <div className="flex h-screen flex-col bg-gradient-to-br from-gray-950 via-gray-900 to-gray-950">
            {/* Header */}
            <header className="flex shrink-0 items-center justify-between border-b border-gray-800 px-6 py-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-[var(--spacing.2, 8px)] text-zinc-400">Governance Engine for AI-Generated UI</p>
                    </div>
                </div>
                {buildId && <span className="rounded border border-indigo-700/40 bg-indigo-900/10 px-2 py-0.5 text-xs text-indigo-400">
            ...
            <main className="flex flex-1 items-start justify-center overflow-y-auto px-6 pt-12 pb-12">
                <div className="w-full max-w-2xl space-y-10">
                    ...
                    <div className="text-center">
                        <h2 ref={headingRef} tabIndex={-1} className="text-2xl font-bold text-zinc-100 outline-none">
                    ...
                    <div className="grid gap-4">
                        {VALUE_STEPS.map((step, i) => <div key={step.title} className="flex items-start gap-4 rounded-xl border border-zinc-800 bg-zinc-900/50 px-5 py-4">
                                ...
                                <div className="min-w-0 flex-1">
                                    <div className="flex items-center gap-2">
                                        ...
                                        {i < VALUE_STEPS.length - 1 && <ArrowRight className="h-3 w-3 text-zinc-700" />}
                                    </div>
                    ...
                    <div className="rounded-xl border border-zinc-800 bg-zinc-900/30 px-5 py-4">
                        ...
                        <ul className="mt-3 space-y-2">
                            {TEST_CHECKLIST.map(item => <li key={item} className="flex items-start gap-2 text-xs text-zinc-400">
                    ...
                    <div className="flex flex-col items-center gap-3">
                        <button type="button" onClick={handleTryDemo} disabled={loading} className="flex items-center gap-2 rounded-xl border border-indigo-500 bg-indigo-500/10 px-8 py-3 text-sm font-semibold text-indigo-300 transition-colors hover:bg-indigo-500/20 disabled:cursor-not-allowed disabled:opacity-50">
                            <Sparkles className="h-4 w-4" />
                            {loading ? 'Setting up demo...' : 'Try the Demo Project'}
                        </button>
                        <button type="button" onClick={handleSkip} className="text-xs text-zinc-500 transition-colors hover:text-zinc-400">
