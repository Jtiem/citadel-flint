...
function parseShadowStorybook(markdown: string): RegistrySearchResult[] {
    ...
    for (const section of sections) {
        ...
        results.push({ name, importPath, description, variants })
    }
    ...
    useEffect(() => {
        ...
        debounceRef.current = setTimeout(async () => {
            try {
                ...
                const result: MCPCallResult = await window.flintAPI.mcp.callTool(
        ...
        <div className="flex flex-col gap-2">
            ...
            <div className="relative flex items-center">
                ...
                <input
                    ...
                    className="w-full rounded-md border border-zinc-700/60 bg-zinc-800/60 py-1.5 pl-8 pr-3 text-xs text-zinc-200 placeholder-zinc-500 outline-none focus:border-indigo-500/60 focus:ring-0"
                />
                {loading && (
                    <Loader2
                        role="status"
                        aria-label="Searching"
                        className="absolute right-2.5 h-3.5 w-3.5 animate-spin text-zinc-500"
                    />
                )}
            </div>
            ...
            {results !== null && results.length === 0 && error === null && (
                <p className="px-1 text-[10px] text-zinc-500">No components found</p>
            )}
            ...
            {results !== null && results.length > 0 && (
                <div className="flex flex-col gap-1.5">
                    {results.map((component) => (
                        <div
                            key={component.name}
                            className="flex items-start justify-between gap-2 rounded-md border border-zinc-700/50 bg-zinc-800/40 px-2.5 py-2"
                        >
                            ...
                            <button
                                ...
                                className="shrink-0 rounded px-2 py-1 text-[10px] font-medium text-indigo-300 hover:bg-indigo-900/30 hover:text-indigo-200 transition-colors"
                            >
                                Insert
                            </button>
                        </div>
