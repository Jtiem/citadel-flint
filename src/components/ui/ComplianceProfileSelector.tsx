...
function ProfileRow({
  ...
  return <label htmlFor={checkboxId} className={`flex cursor-pointer items-start gap-3 rounded px-3 py-2.5 transition-colors ${isDisabled ? 'cursor-not-allowed opacity-40' : isChecked ? 'bg-indigo-900/10 hover:bg-indigo-900/20' : 'hover:bg-zinc-800/40'}`}>
            {/* Custom checkbox */}
            <div className="relative mt-0.5 shrink-0">
                <input id={checkboxId} type="checkbox" checked={isChecked} disabled={isDisabled} onChange={e => onToggle(e.target.checked)} className="sr-only" aria-describedby={`${checkboxId}-desc`} aria-label="[NEEDS LABEL]" />
                <div className={`flex h-4 w-4 items-center justify-center rounded border transition-colors ${isChecked ? 'border-indigo-500/60 bg-indigo-600' : 'border-zinc-600 bg-zinc-800'}`} aria-hidden="true">
            ...
            <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                    ...
                    {isComingSoon && <span className="rounded border border-zinc-700/40 bg-zinc-800/60 px-1 py-0.5 text-[var(--spacing.2, 8px)] font-medium uppercase tracking-wider text-zinc-600">
                            Soon
                        </span>}
                </div>
                <p id={`${checkboxId}-desc`} className="mt-0.5 text-[var(--spacing.2, 8px)] leading-snug text-zinc-500">
                    {profile.description}
                </p>
            </div>

            {/* Region badge */}
            <span className="mt-0.5 shrink-0 rounded bg-zinc-800 px-1.5 py-0.5 font-mono text-[var(--spacing.2, 8px)] text-zinc-600">
                {profile.region}
            </span>
        </label>;
}
...
export function ComplianceProfileSelector({
  ...
  const enabledCount = useMemo(() => {
    ...
    for (const profile of profiles) {
      const isActive = RULE_PACK_REGISTRY.find(p => p.id === profile.packId)?.status === 'active' || profile.preset !== undefined && activePresets.includes(profile.preset);
  ...
  return <div className="flex flex-col">
            ...
            <div className="shrink-0 border-b border-zinc-800 px-4 py-3">
                ...
                <p className="mt-0.5 text-[var(--spacing.2, 8px)] text-zinc-500">
                    ...
                    {enabledCount > 0 && <span className="ml-1 text-indigo-400">{enabledCount} active</span>}
                </p>
            </div>
        ...
        const isChecked = RULE_PACK_REGISTRY.find(p => p.id === profile.packId)?.status === 'active' || profile.preset !== undefined && activePresets.includes(profile.preset);
        return <ProfileRow key={`${profile.packId}:${profile.label}`} profile={profile} isChecked={isChecked} isToggling={isToggling} onToggle={enabled => onToggleJurisdiction(profile.packId, enabled)} />;
      })}
            </div>
