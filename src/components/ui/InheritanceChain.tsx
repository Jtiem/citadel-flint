...
function ChainNode({ nodeRef, kind }: ChainNodeProps) {
    // Shorten display: "@flint/healthcare" → "healthcare", "./team.yaml" → "team.yaml"
    const displayName = nodeRef.startsWith('@flint/')
        ? nodeRef.slice('@flint/'.length)
        : nodeRef.startsWith('./')
        ? nodeRef.slice(2)
        : nodeRef
    ...
    return (
        <div
            className={`flex flex-col items-start rounded border px-2 py-1.5 ${NODE_BORDER[kind]} ${NODE_BG[kind]}`}
            title={nodeRef}
        >
...
export function InheritanceChain({ chain, isLoading }: InheritanceChainProps) {
                ...
                {isLoading ? (
                    ...
                    <div className="flex flex-wrap items-center gap-1.5">
                        {nodes.map((nodeRef, index) => {
                            ...
                            return (
                                <div key={`${nodeRef}-${index}`} className="flex items-center gap-1.5">
                                    {index > 0 && (
                                        <ChevronRight
                                            size={10}
