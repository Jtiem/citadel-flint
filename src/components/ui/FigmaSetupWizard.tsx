...
function StepIndicator({
  ...
  return <div className="flex items-center gap-2.5">
            <div className="relative flex h-5 w-5 shrink-0 items-center justify-center">
                {status === 'completed' ? <CheckCircle2 size={18} className="text-emerald-400" /> : status === 'current' ? <>
                        <div className="absolute h-5 w-5 motion-safe:animate-pulse rounded-full bg-indigo-500/30" />
                        <div className="h-3 w-3 rounded-full bg-indigo-400" />
                    </> : <div className="h-3 w-3 rounded-full bg-zinc-700" />}
            </div>
            <span className={status === 'completed' ? 'text-xs text-emerald-400' : status === 'current' ? 'text-xs font-medium text-zinc-100' : 'text-xs text-zinc-500'}>
                {index}. {label}
            </span>
        </div>;
}
...
function CopyField({
  ...
  return <div className="space-y-1">
            <p className="text-[var(--spacing.2, 8px)] font-medium uppercase tracking-wider text-zinc-500">{label}</p>
            <div className="flex items-center gap-2 rounded border border-zinc-700 bg-zinc-900 px-2.5 py-1.5">
                ...
                <button type="button" onClick={onCopy} className="shrink-0 rounded p-0.5 text-zinc-400 transition-colors hover:bg-zinc-800 hover:text-zinc-100" title={copied ? 'Copied!' : `Copy ${label}`} aria-label={copied ? 'Copied!' : `Copy ${label}`}>
                    {copied ? <Check size={13} className="text-emerald-400" /> : <Copy size={13} />}
                </button>
            </div>
            {copied && <p className="text-[var(--spacing.2, 8px)] text-emerald-400">Copied!</p>}
        </div>;
}
...
function TroubleshootingSection() {
  ...
  return <div className="mt-3 border-t border-zinc-800 pt-3">
      ...
      setOpen(v => !v);
            ...
            {open && <div className="mt-2 space-y-2 text-xs text-zinc-400">
                    <div>
...
export function FigmaSetupWizard({
  visible,
  onClose
}: FigmaSetupWizardProps) {
  ...
  useEffect(() => {
    ...
    setCopied(null);
    let cancelled = false;
    window.flintAPI.figma.status().then(status => {
      if (cancelled) return;
  ...
  return <div data-testid="figma-setup-wizard" className="mt-2 rounded-lg border border-zinc-700 bg-zinc-900 p-4 text-left">
            {/* ── Step indicators ───────────────────────────────────────────── */}
            ...
            {step === 'checking' && <div className="flex items-center gap-2 text-xs text-zinc-400">
                    <Loader2 size={13} className="motion-safe:animate-spin text-indigo-400" />
                    Checking server status…
                </div>}

            {step === 'error' && <div className="space-y-2">
            ...
            {step === 'configure' && figmaStatus && <div className="space-y-3">
                    ...
                    <CopyField label="Endpoint" value={endpoint} copied={copied === 'endpoint'} onCopy={() => {
        ...
        setStep('waiting');
            ...
            {step === 'waiting' && <div className="space-y-2">
                    <div className="flex items-center gap-2 text-xs text-zinc-400">
                        <Loader2 size={13} className="motion-safe:animate-spin text-indigo-400" />
                        Waiting for first sync from Figma…
                    </div>
                    <p className="text-[var(--spacing.3, 12px)] text-zinc-500">
                        Open the Flint plugin in Figma and click{' '}
                        <strong className="text-zinc-400">Sync Variables</strong>.
                    </p>
                </div>}

            {step === 'success' && <div className="flex items-center gap-2 rounded border border-emerald-500/30 bg-emerald-900/10 px-3 py-2">
