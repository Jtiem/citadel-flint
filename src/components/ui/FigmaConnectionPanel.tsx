...
export function FigmaConnectionPanel({
  ...
  const fetchStatus = useCallback(() => {
    window.flintAPI.figma?.status().then(setFigmaStatus).catch(err => console.warn('[Flint] FigmaConnectionPanel: failed to fetch Figma status', err));
  }, []);
  const fetchTokenCounts = useCallback(() => {
    window.flintAPI.mcp?.callTool('flint_sync_check', {}).then(result => {
      ...
      try {
        ...
        console.warn('[Flint] FigmaConnectionPanel: failed to parse sync check result', err);
      }
  ...
  const handleSync = useCallback(async (direction: 'pull' | 'push') => {
    ...
    try {
      ...
      push({
        ...
        message: isError ? result?.content?.[0]?.text ?? 'Unknown error.' : direction === 'pull' ? 'Figma tokens pulled to local project.' : 'Local tokens pushed to Figma.',
  ...
  return <div className="flex h-full flex-col bg-zinc-950 text-zinc-300" data-testid="figma-connection-panel">
            {/* ── Header ──────────────────────────────────────────────────────── */}
            ...
            <div className="min-h-0 flex-1 overflow-y-auto">
                ...
                <section className="border-b border-zinc-800/60 px-4 py-3">
                    <h3 className="mb-2 text-[var(--spacing.2, 8px)] font-medium uppercase tracking-wider text-zinc-500">Status</h3>
                ...
                {isConnected && figmaStatus?.lastWebhookAt === null && <section className="border-b border-indigo-800/40 bg-indigo-900/10 px-4 py-3" data-testid="first-sync-banner">
                        ...
                        <button type="button" onClick={() => void handleSync('pull')} disabled={syncOp !== null} className="flex items-center gap-1.5 rounded bg-indigo-600 px-3 py-1.5 text-xs font-medium text-white transition-colors hover:bg-indigo-500 disabled:cursor-not-allowed disabled:opacity-50" data-testid="first-sync-pull-btn">
                ...
                {/* ── 2. Sync Actions ──────────────────────────────────────────── */}
                <section className="border-b border-zinc-800/60 px-4 py-3">
                    <h3 className="mb-2 text-[var(--spacing.2, 8px)] font-medium uppercase tracking-wider text-zinc-500">Sync Actions</h3>
                    <div className="flex gap-2">
                        <button type="button" onClick={() => handleSync('pull')} disabled={!isConnected || syncOp !== null} className={`flex flex-1 items-center justify-center gap-1.5 rounded px-3 py-2 text-xs font-medium text-white transition-colors ${!isConnected || syncOp !== null ? 'cursor-not-allowed bg-indigo-600 opacity-50' : 'bg-indigo-600 hover:bg-indigo-500'}`} data-testid="figma-panel-pull">
                            {syncOp === 'pull' ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Download className="h-3.5 w-3.5" />}
                            Pull from Figma
                        </button>
                        <button type="button" onClick={() => handleSync('push')} disabled={!isConnected || syncOp !== null} className={`flex flex-1 items-center justify-center gap-1.5 rounded px-3 py-2 text-xs font-medium transition-colors ${!isConnected || syncOp !== null ? 'cursor-not-allowed bg-zinc-700 text-zinc-300 opacity-50' : 'bg-zinc-700 text-zinc-200 hover:bg-zinc-600'}`} data-testid="figma-panel-push">
                            {syncOp === 'push' ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Upload className="h-3.5 w-3.5" />}
                            Push to Figma
                        </button>
                    </div>
                    <div className="mt-2 flex gap-2">
                        <button type="button" onClick={fetchStatus} className="flex flex-1 items-center justify-center gap-1.5 rounded border border-zinc-700 bg-zinc-800/60 px-2 py-1.5 text-xs text-zinc-400 transition-colors hover:border-zinc-600 hover:text-zinc-200">
                            <RefreshCw className="h-3 w-3" />
                            Refresh
                        </button>
                        <button type="button" onClick={handleDisconnect} disabled={!isConnected} className="flex flex-1 items-center justify-center gap-1.5 rounded border border-red-700/40 bg-red-900/10 px-2 py-1.5 text-xs text-red-400 transition-colors hover:border-red-600/60 hover:text-red-300 disabled:cursor-not-allowed disabled:opacity-40">
                ...
                {/* ── 3. Token Mapping ─────────────────────────────────────────── */}
                <section className="border-b border-zinc-800/60 px-4 py-3">
                    <h3 className="mb-2 text-[var(--spacing.2, 8px)] font-medium uppercase tracking-wider text-zinc-500">Token Mapping</h3>
                ...
                <section className="px-4 py-3">
                    <h3 className="mb-2 text-[var(--spacing.2, 8px)] font-medium uppercase tracking-wider text-zinc-500">Recent Sync History</h3>
                    {syncHistory.length === 0 ? <p className="text-xs text-zinc-600">No sync operations this session.</p> : <ul className="space-y-1">
                            {syncHistory.map((entry, i) => <li key={i} className="flex items-center gap-2 rounded px-2 py-1.5 text-xs">
                                    {entry.success ? <CheckCircle2 className="h-3 w-3 shrink-0 text-emerald-400" /> : <AlertTriangle className="h-3 w-3 shrink-0 text-red-400" />}
