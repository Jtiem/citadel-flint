...
function renderOpen(overrides?: {
  ...
  const {
    rerender,
    unmount
  } = render(<BetaFeedbackModal open={true} onClose={onClose} />);
  return {
    onClose,
    rerender,
    unmount
...
describe('BetaFeedbackModal', () => {
  ...
  describe('Escape key', () => {
    ...
    it('does not throw when Escape pressed without onClose', () => {
      renderOpen();
      expect(() => fireEvent.keyDown(document, {
        key: 'Escape'
      })).not.toThrow();
    });
  });
  describe('screenshot capture', () => {
    ...
    it('shows thumbnail when captureScreenshot returns a base64 string', async () => {
      ...
      (window.flintAPI.beta!.captureScreenshot as ReturnType<typeof vi.fn>).mockResolvedValueOnce(fakeBase64);
      renderOpen();
      await act(async () => {
        fireEvent.click(screen.getByLabelText('Attach screenshot'));
      });
    ...
    it('does not show thumbnail when captureScreenshot returns null', async () => {
      ;
      (window.flintAPI.beta!.captureScreenshot as ReturnType<typeof vi.fn>).mockResolvedValueOnce(null);
      renderOpen();
      await act(async () => {
        fireEvent.click(screen.getByLabelText('Attach screenshot'));
      });
    ...
    it('modal is still usable when captureScreenshot throws', async () => {
      ;
      (window.flintAPI.beta!.captureScreenshot as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new Error('IPC failed'));
      renderOpen();
      await act(async () => {
        fireEvent.click(screen.getByLabelText('Attach screenshot'));
      });
  ...
  describe('submission', () => {
    it('calls submitFeedback with category, severity, and description', async () => {
      ...
      await waitFor(() => {
        expect(window.flintAPI.beta!.submitFeedback).toHaveBeenCalledWith(expect.objectContaining({
          category: 'bug',
          severity: 'annoying',
    ...
    it('button shows "Saving..." while submitting', async () => {
      ...
      (window.flintAPI.beta!.submitFeedback as ReturnType<typeof vi.fn>).mockReturnValueOnce(new Promise<{
        saved: boolean;
      }>(r => {
        resolve = r;
      }));
    ...
    it('form stays open when submitFeedback throws', async () => {
      ;
      (window.flintAPI.beta!.submitFeedback as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new Error('disk full'));
      renderOpen();
  ...
  describe('beta info banner', () => {
    ...
    it('shows banner when daysRemaining is set', async () => {
      ;
      (window.flintAPI.beta!.getInfo as ReturnType<typeof vi.fn>).mockResolvedValueOnce({
  ...
  describe('form reset', () => {
    it('clears description when modal is reopened', () => {
      const {
        rerender
      } = render(<BetaFeedbackModal open={true} onClose={vi.fn()} />);
