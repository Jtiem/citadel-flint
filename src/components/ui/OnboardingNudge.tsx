...
function countFiles(node: {
  ...
  return (node.children ?? []).reduce((acc, child) => acc + countFiles(child as {
    type: string;
    children?: {
      type: string;
      children?: unknown[];
...
export function OnboardingNudge({
  ...
  const shouldShow = !dismissed && workspaceFiles !== null && fileCount <= 1 && !hasTokens;
  ...
  return <div className="mx-2 mb-2 rounded-lg border border-indigo-500/30 bg-indigo-950/50 p-3" role="status" aria-label="Getting started suggestions">
            {/* Header row */}
            <div className="flex items-start justify-between gap-2">
                <p className="text-xs font-semibold text-indigo-300">
                    Get started
                </p>
                <button type="button" onClick={handleDismiss} className="shrink-0 rounded p-0.5 text-zinc-500 hover:bg-zinc-800 hover:text-zinc-300 transition-colors" aria-label="Dismiss onboarding nudge">
                    <X size={12} />
                </button>
            </div>

            {/* Hint text */}
            <p className="mt-1 text-[var(--spacing.3, 12px)] leading-relaxed text-zinc-400">
                Your project is ready. Import tokens from Figma or start editing the canvas.
            </p>

            {/* Action buttons */}
            <div className="mt-2.5 flex flex-col gap-1.5">
                <button type="button" onClick={handleConnectFigma} className="flex items-center gap-2 rounded border border-indigo-500/30 bg-indigo-600/20 px-2.5 py-1.5 text-[var(--spacing.3, 12px)] font-medium text-indigo-300 transition-colors hover:border-indigo-500/60 hover:bg-indigo-600/30 hover:text-indigo-200">
                    <Figma size={12} />
                    Connect Figma
                </button>
                <button type="button" onClick={handleStartEditing} className="flex items-center gap-2 rounded border border-zinc-700 bg-zinc-800/60 px-2.5 py-1.5 text-[var(--spacing.3, 12px)] font-medium text-zinc-300 transition-colors hover:border-zinc-600 hover:bg-zinc-700/60 hover:text-zinc-100">
