...
describe('SeverityChip — color classes', () => {
  // GLASSTYPO.1 Group C: advisory text color migrated from zinc-400 to --text-secondary CSS var.
  // Critical and amber use semantic colors (allowed per Mithril rules); advisory uses the token.
  it.each<[ChipSeverity, string, string]>([['critical', 'red-400', 'red-500/10'], ['amber', 'amber-400', 'amber-400/10']])('%s severity renders correct palette (%s text, %s background)', (severity, textToken, bgToken) => {
    render(<SeverityChip severity={severity} label="violations" data-testid="chip" />);
    const chip = screen.getByTestId('chip');
    expect(chip.className).toContain(textToken);
    expect(chip.className).toContain(bgToken);
  });

  it('advisory severity renders token-based text color (--text-secondary) and zinc-800 background', () => {
    render(<SeverityChip severity="advisory" label="violations" data-testid="chip" />);
    const chip = screen.getByTestId('chip');
    // GLASSTYPO.1: advisory text uses CSS var token, not zinc-400 class
    expect(chip.className).toContain('color:var(--text-secondary)');
    expect(chip.className).toContain('zinc-800');
  });
  it('critical severity renders red dot when no icon is provided', () => {
    const {
      container
    } = render(<SeverityChip severity="critical" label="issues" data-testid="chip" />);
    // The severity dot is a sibling span with aria-hidden="true" inside the chip
    const dots = container.querySelectorAll('[aria-hidden="true"]');
    const dotSpans = Array.from(dots).filter(el => el.tagName === 'SPAN' && (el as HTMLElement).className.includes('rounded-full'));
  ...
  it('amber severity renders amber dot when no icon is provided', () => {
    const {
      container
    } = render(<SeverityChip severity="amber" label="issues" data-testid="chip" />);
    const dots = container.querySelectorAll('[aria-hidden="true"]');
    const dotSpans = Array.from(dots).filter(el => el.tagName === 'SPAN' && (el as HTMLElement).className.includes('rounded-full'));
  ...
  it('advisory severity renders zinc dot when no icon is provided', () => {
    const {
      container
    } = render(<SeverityChip severity="advisory" label="issues" data-testid="chip" />);
    const dots = container.querySelectorAll('[aria-hidden="true"]');
    const dotSpans = Array.from(dots).filter(el => el.tagName === 'SPAN' && (el as HTMLElement).className.includes('rounded-full'));
...
describe('SeverityChip — count rendering', () => {
  it('does not render a count element when count is undefined', () => {
    render(<SeverityChip severity="critical" label="errors" data-testid="chip" />);
    const chip = screen.getByTestId('chip');
  ...
  it('renders count before label when count is 0', () => {
    render(<SeverityChip severity="advisory" label="warnings" count={0} data-testid="chip" />);
    const chip = screen.getByTestId('chip');
    expect(chip.textContent).toContain('0');
    expect(chip.textContent).toContain('warnings');
  });
  it('renders count=1 alongside label', () => {
    render(<SeverityChip severity="amber" label="drifted" count={1} data-testid="chip" />);
    const chip = screen.getByTestId('chip');
    expect(chip.textContent).toContain('1');
    expect(chip.textContent).toContain('drifted');
  });
  it('renders count=3 alongside label', () => {
    render(<SeverityChip severity="critical" label="contrast fails" count={3} data-testid="chip" />);
    const chip = screen.getByTestId('chip');
...
describe('SeverityChip — aria-label', () => {
  it('computes default aria-label from count + severity + label', () => {
    render(<SeverityChip severity="critical" label="contrast fails" count={3} />);
  ...
  it('computes aria-label without count prefix when count is undefined', () => {
    render(<SeverityChip severity="advisory" label="dead tokens" />);
  ...
  it('uses the aria-label prop override when provided', () => {
    render(<SeverityChip severity="amber" label="drifted" count={5} aria-label="5 token drift warnings" />);
  ...
  it('aria-label includes count=0 in prefix', () => {
    render(<SeverityChip severity="critical" label="violations" count={0} />);
...
describe('SeverityChip — keyboard tab order', () => {
  ...
  it('chip is a span element (not a button or anchor)', () => {
    render(<SeverityChip severity="amber" label="warnings" data-testid="chip" />);
    const chip = screen.getByTestId('chip');
...
describe('SeverityChip — icon prop', () => {
  it('renders icon content when icon prop is provided', () => {
    render(<SeverityChip severity="critical" label="errors" icon={<svg data-testid="custom-icon" />} data-testid="chip" />);
