...
export function ViolationsList({
  ...
  return <div>
            {/* BatchActionBar — batch fix CTAs */}
            <BatchActionBar acceptedCount={acceptedCount} autoFixableCount={autoFixableCount} a11yFixableCount={a11yFixableCount} manualCount={manualCount} onApplyAccepted={onApplyAccepted} onAutoFixMithril={onAutoFixMithril} onFixAllA11y={onFixAllA11y} onReviewManual={onReviewManual} sessionProgress={sessionProgress} isBaselineSet={isBaselineSet} effortEstimate={effortEstimate} />
            ...
            <div className="divide-y divide-zinc-800/50" data-testid="violations-list">
                ...
                {resurfacedCardKeys.size > 0 && <div data-testid="resurfaced-section">
                        <div className="flex items-center gap-2 px-3 py-1.5 bg-amber-900/10 border-b border-amber-700/20">
                            ...
                            <span style={{ fontSize: 'var(--text-label)', fontWeight: 'var(--text-label-weight)', color: 'oklch(0.845 0.098 92)' }}>Resurfaced</span>
                        </div>
                    </div>}
        ...
        void resurfaceTick;
        return <ViolationCard key={card.cardKey} issue={card.warning} type="mithril" cardKey={card.cardKey} isPinned={card.isPinned} isFlagged={card.isFlagged} isDeferred={card.isDeferred} deferExpiresAtMs={card.deferExpiresAtMs} isDeferSuccess={card.isDeferSuccess} deferSuccessMsg={card.deferSuccessMsg} resurfaceTick={resurfaceTick} isResurfaced={card.isResurfaced} isAiSourced={card.isAiSourced} isExpanded={card.isExpanded} isDiffOpen={card.isDiffOpen} isDiffLoading={card.isDiffLoading} diffData={card.diffData} isDeferFormOpen={card.isDeferFormOpen} fixItem={card.fixItem} provenance={card.provenance} deferReason={card.deferReason} deferDuration={card.deferDuration} onToggleExpand={() => onToggleExpand(card.cardKey)} onFix={() => card.fixItem && onFix(card.cardKey, card.fixItem)} onPreviewFix={() => onPreviewFix(card.cardKey)} onAcceptFix={() => card.fixItem && onAcceptFix(card.cardKey, card.fixItem)} onSkipFix={() => onSkipFix(card.cardKey)} onFlag={() => onFlag(card.cardKey)} onUnflag={() => onUnflag(card.cardKey)} onDefer={d => onDefer(card.cardKey, d)} onDeferReasonChange={r => onDeferReasonChange(card.cardKey, r)} onDeferDurationChange={d => onDeferDurationChange(card.cardKey, d)} onSubmitDefer={() => onSubmitDefer(card.cardKey)} onCancelDefer={() => onCancelDefer(card.cardKey)} onPin={() => onPin(card.cardKey)} onOverride={onOverride ? reason => onOverride(card.cardKey, reason) : undefined} getNodeName={getNodeName} activeFilePath={activeFilePath} navigationIndex={card.navigationIndex} overrideReason={card.overrideReason} overrideActor={card.overrideActor} overrideTimestamp={card.overrideTimestamp} />;
      })}

                {/* A11y violation cards */}
                {a11yCards.map(card => {
        void resurfaceTick;
        return <ViolationCard key={card.cardKey} issue={card.warning} type="a11y" cardKey={card.cardKey} indexInList={card.indexInList} isPinned={card.isPinned} isFlagged={card.isFlagged} isDeferred={card.isDeferred} deferExpiresAtMs={card.deferExpiresAtMs} isDeferSuccess={card.isDeferSuccess} deferSuccessMsg={card.deferSuccessMsg} resurfaceTick={resurfaceTick} isResurfaced={card.isResurfaced} isAiSourced={card.isAiSourced} isExpanded={card.isExpanded || card.isPinned} isDeferFormOpen={card.isDeferFormOpen} fixItem={null} diffData={null} isDiffOpen={false} isDiffLoading={false} provenance={card.provenance} deferReason={card.deferReason} deferDuration={card.deferDuration} onToggleExpand={() => onToggleExpand(card.cardKey)} onFix={() => {/* a11y violations use handleA11yFix in parent */}} onPreviewFix={() => {/* no inline diff for a11y */}} onAcceptFix={() => {/* no inline diff for a11y */}} onSkipFix={() => {/* no inline diff for a11y */}} onFlag={() => onFlag(card.cardKey)} onUnflag={() => onUnflag(card.cardKey)} onDefer={d => onDefer(card.cardKey, d)} onDeferReasonChange={r => onDeferReasonChange(card.cardKey, r)} onDeferDurationChange={d => onDeferDurationChange(card.cardKey, d)} onSubmitDefer={() => onSubmitDefer(card.cardKey)} onCancelDefer={() => onCancelDefer(card.cardKey)} onPin={() => onPin(card.cardKey)} onOverride={onOverride ? reason => onOverride(card.cardKey, reason) : undefined} getNodeName={getNodeName} activeFilePath={activeFilePath} navigationIndex={card.navigationIndex} overrideReason={card.overrideReason} overrideActor={card.overrideActor} overrideTimestamp={card.overrideTimestamp} />;
      })}

                {/* COUNSEL.2.1: Deferred section — muted cards at the bottom */}
                {deferredCardKeys.size > 0 && <div data-testid="deferred-section">
                        <div className="flex items-center gap-2 px-3 py-1.5 bg-zinc-900/50 border-b border-zinc-800/40 border-t border-t-zinc-800/40">
                            ...
                            <span style={{ fontSize: 'var(--text-label)', fontWeight: 'var(--text-label-weight)', color: 'var(--text-tertiary)' }}>
                ...
                {/* Overrides */}
                {overridesExist && <div className="flex items-start gap-2 px-3 py-2">
                        <span className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-amber-400" aria-hidden="true" />
                        <div className="flex-1">
                            <p style={{ fontSize: 'var(--text-body)', fontWeight: 500, color: 'var(--text-primary)' }}>Unapplied Overrides</p>
                            <p style={{ fontSize: 'var(--text-label)', color: 'var(--text-secondary)' }}>Property overrides are blocking export. Apply or revert them to unblock.</p>
                        </div>
                    </div>}
            </div>
        </div>;
