...
interface CoverageBarProps {
     ...
     */
    coverages: Record<string, { covered: number; total: number }> | null
...
function JurisdictionRow({ jurisdiction, covered, total }: JurisdictionRowProps) {
    ...
    return (
        <div className="flex items-center gap-2 py-1">
            {/* Jurisdiction label */}
            <span
                className="w-20 shrink-0 truncate font-mono text-[10px] text-zinc-400"
                title={jurisdiction}
            >
                {jurisdiction}
            </span>
            ...
            {/* Progress bar */}
            <div
                className={`h-1.5 flex-1 overflow-hidden rounded-full ${barTrackClass(percent)}`}
                role="progressbar"
                aria-valuenow={percent}
                aria-valuemin={0}
                aria-valuemax={100}
                aria-label={`${jurisdiction} coverage: ${percent}%`}
            >
                <div
                    className={`h-full rounded-full transition-all duration-300 ${barColorClass(percent)}`}
                    style={{ width: `${percent}%` }}
                />
            </div>
...
export function CoverageBar({ coverages, isLoading }: CoverageBarProps) {
    ...
    const sortedEntries = useMemo<JurisdictionCoverageEntry[]>(() => {
        ...
        return Object.entries(coverages)
            .map(([jurisdiction, { covered, total }]) => ({ jurisdiction, covered, total }))
        ...
        <>
            ...
            <div className="border-b border-t border-zinc-800 px-3 py-2">
                <div className="flex items-center justify-between">
                    ...
                    {overall !== null && (
                        <span className={`text-[10px] font-mono font-medium ${percentTextClass(overall)}`}>
                            {overall}% overall
                        </span>
                    )}
                </div>
