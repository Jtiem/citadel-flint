...
function RecipeSection() {
  ...
  const handleInsert = useCallback((recipe: ComponentRecipe) => {
    injectComponent('', recipe.jsxSnippet, recipe.imports.join('\n'));
  }, [injectComponent]);
  return <div className="border-b border-zinc-800" data-testid="recipe-section">
            <button type="button" onClick={() => setExpanded(v => !v)} className="flex w-full items-center gap-1.5 px-3 py-2 text-[var(--spacing.2, 8px)] font-semibold uppercase tracking-wider text-zinc-400 hover:text-zinc-200 transition-colors" data-testid="recipe-section-toggle">
                {expanded ? <ChevronDown size={10} /> : <ChevronRight size={10} />}
                Recipes ({BUILTIN_RECIPES.length})
            </button>
            {/* Collapsed preview: show 2 recipe name chips as read-only labels */}
            {!expanded && <div className="flex flex-wrap gap-1 px-3 pb-2" data-testid="recipe-section-preview">
                    {BUILTIN_RECIPES.slice(0, 2).map(recipe => <span key={recipe.id} className="text-xs text-zinc-400 bg-zinc-800 rounded px-2 py-0.5" data-testid={`recipe-preview-chip-${recipe.id}`}>
                            {recipe.name}
                        </span>)}
                </div>}
            {expanded && <div className="flex flex-col gap-1 px-3 pb-2" data-testid="recipe-section-list">
                    {BUILTIN_RECIPES.map(recipe => <button key={recipe.id} type="button" onClick={() => handleInsert(recipe)} disabled={!activeFilePath} title={recipe.description} data-testid={`recipe-panel-chip-${recipe.id}`} className="flex items-center gap-2 rounded border border-zinc-800 bg-zinc-900/50 px-2 py-1.5 text-left text-xs text-zinc-300 transition-colors hover:border-zinc-700 hover:text-zinc-100 disabled:opacity-40 disabled:cursor-not-allowed">
                            <span className="font-medium">{recipe.name}</span>
                            <span className="text-[var(--spacing.2, 8px)] text-zinc-500">{recipe.category}</span>
                        </button>)}
                </div>}
        </div>;
}
...
export function ComponentPanel() {
  ...
  const handleInsert = useCallback((name: string, importPath: string) => {
    ...
    injectComponent(targetNodeId, `<${name} />`, `import { ${name} } from '${importPath}';`);
  }, [activeFilePath, activeSelection, injectComponent, pushNotification]);
  return <div className="flex h-full flex-col" data-testid="component-panel">
            ...
            <div className="flex flex-col gap-1.5 border-b border-zinc-800 px-3 py-2">
                <div className="relative">
                    ...
                    <input type="text" placeholder="Search components..." value={searchInput} onChange={e => setSearchInput(e.target.value)} data-testid="component-panel-search" className="w-full rounded border border-zinc-800 bg-zinc-900 py-1 pl-7 pr-2 text-xs text-zinc-200 placeholder-zinc-600 outline-none focus:border-indigo-500/50" aria-label="[NEEDS LABEL]" />
                </div>
                <div className="flex items-center justify-between">
                    <select value={categoryFilter} onChange={e => setCategoryFilter(e.target.value as ComponentCategory | 'all')} data-testid="component-panel-category-filter" className="rounded border border-zinc-800 bg-zinc-900 px-1.5 py-0.5 text-[var(--spacing.2, 8px)] text-zinc-300 outline-none focus:border-indigo-500/50" aria-label="[NEEDS LABEL]">
                        {CATEGORY_OPTIONS.map(opt => <option key={opt.value} value={opt.value}>
                                {opt.label}
                            </option>)}
                    </select>
                    <span className="text-[var(--spacing.2, 8px)] text-zinc-500" data-testid="component-panel-count">
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-3 py-2">
                {isLoading && <div className="flex items-center justify-center py-8" data-testid="component-panel-loading">
                        <div className="h-4 w-4 motion-safe:animate-spin rounded-full border-2 border-zinc-700 border-t-indigo-500" />
                    </div>}

                {!isLoading && filteredCards.length === 0 && <div data-testid="component-panel-empty">
                        <EmptyState icon={<Boxes className="h-5 w-5 text-zinc-600" />} title={cards.length === 0 ? 'No components registered yet.' : 'No components match your search.'} description={cards.length === 0 ? 'Open a project with a flint-manifest.json to see components here.' : undefined} />
                    </div>}

                <div className="flex flex-col gap-1.5">
                    {filteredCards.map(card => <ComponentPanelCard key={card.id} name={card.name} category={card.category} variantCount={card.variantCount} importPath={card.importPath} onSelect={() => handleSelect(card.filePath)} onInsert={() => handleInsert(card.name, card.importPath)} />)}
                </div>
            </div>
        </div>;
}