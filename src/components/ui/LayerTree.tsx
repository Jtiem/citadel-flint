 ...
 */
function flattenVisibleTree(layers: VisualLayer[], collapsedIds: Set<string>): VisualLayer[] {
 ...
 */
function buildParentMap(layers: VisualLayer[], parentMap: Map<string, VisualLayer> = new Map(), parent?: VisualLayer): Map<string, VisualLayer> {
  for (const layer of layers) {
...
function LayerRow({
  ...
  function handleDragOver(e: React.DragEvent): void {
    ...
    if (pct < 0.25) position = 'before';else if (pct > 0.75) position = 'after';else position = 'inside';
    ...
    if (lb === null || lb.targetId !== layer.id || lb.position !== position) {
      ...
      window.dispatchEvent(new CustomEvent(`${BRAND.productLower}:dragOver`, {
        detail: {
          targetId: layer.id,
          position
        }
  ...
  return <>
             ...
             */}
            <div ref={rowRef} role="treeitem" aria-level={depth + 1} aria-selected={isSelected} aria-expanded={hasChildren ? !isCollapsed : undefined} tabIndex={focusedId === layer.id ? 0 : -1} data-layer-id={layer.id} draggable={!isLocked} className={`group relative flex w-full transition-opacity ${isDragging ? 'opacity-40' : ''} ${isLocked ? 'cursor-not-allowed' : ''}`} onMouseEnter={() => setHoveredId(layer.id)} onMouseLeave={() => setHoveredId(null)} onFocus={() => setFocusedId(layer.id)} onDragStart={isLocked ? undefined : handleDragStart} onDragEnd={isLocked ? undefined : handleDragEnd} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} title={isLocked ? 'Locked by a collaborator — editing disabled' : undefined}>
                {/* Before indicator — thin blue line at the top of the row */}
                {dropPosition === 'before' && <div className="pointer-events-none absolute inset-x-0 top-0 z-10 h-0.5 bg-indigo-400" />}

                {/* After indicator — thin blue line at the bottom of the row */}
                {dropPosition === 'after' && <div className="pointer-events-none absolute inset-x-0 bottom-0 z-10 h-0.5 bg-indigo-400" />}

                <button type="button" className={`flex min-w-0 flex-1 items-center gap-1.5 py-1.5 pl-[var(--indent)] pr-7 text-left text-xs transition-colors ${dropPosition === 'inside' ? 'bg-indigo-400/10 text-gray-200' : isSelected ? 'bg-indigo-600/30 text-indigo-300' : isHovered ? 'bg-gray-700/40 text-gray-200' : 'text-gray-400 hover:bg-gray-800/60 hover:text-gray-200'}`} style={{
        '--indent': `${depth * 14 + 8}px`
        ...
        setSelectedNode(layer.id);
                     ...
                     */}
                    {hasChildren ? <span className="shrink-0 text-zinc-500 transition-colors hover:text-zinc-300" onClick={handleChevronClick}>
                            {isCollapsed ? <ChevronRight className="h-2.5 w-2.5" /> : <ChevronDown className="h-2.5 w-2.5" />}
                        </span> : <span className="h-2.5 w-2.5 shrink-0" />}

                    <Icon className={`h-3 w-3 shrink-0 ${isSelected ? 'opacity-80' : 'opacity-40'}`} />
                    <span className="min-w-0 flex-1 truncate font-medium">{name}</span>
                    {hasViolation && <span title="Design drift — colour mismatch detected">
                    ...
                    {hasA11yViolation && <span title="Accessibility gap">
                    ...
                    {annotationCount > 0 && <span title={`${annotationCount} open annotation${annotationCount !== 1 ? 's' : ''}`} className="flex h-3.5 min-w-3.5 shrink-0 items-center justify-center rounded-full bg-indigo-600/40 border border-indigo-500/50 px-0.5">
                            <span className="text-[var(--spacing.2, 8px)] font-bold leading-none text-indigo-400">
                                {annotationCount}
                            </span>
                        </span>}
                    {/* Phase C.2: lock badge — shows when a remote user holds this node */}
                    {isLocked && <span title="Locked by a collaborator">
                            <Lock className="h-2.5 w-2.5 shrink-0 text-blue-400" />
                        </span>}
                    {name !== tag && <span className="shrink-0 font-mono text-[var(--spacing.2, 8px)] text-zinc-500 opacity-0 transition-opacity duration-150 group-hover:opacity-100">
                            {tag}
                        </span>}
                </button>

                {/* Jump-to-source button — visible only on row hover */}
                <button type="button" title={`Jump to line ${layer.line}`} className="absolute right-1 top-1/2 -translate-y-1/2 rounded p-0.5 text-zinc-500 opacity-0 transition-opacity group-hover:opacity-100 hover:text-indigo-400" onClick={e => {
        e.stopPropagation();
        setJumpToLine(layer.line);
            ...
            {!isCollapsed && layer.children.map(child => <LayerRow key={child.id} layer={child} depth={depth + 1} collapsedIds={collapsedIds} onToggleCollapsed={onToggleCollapsed} lockedIds={lockedIds} focusedId={focusedId} setFocusedId={setFocusedId} />)}
        </>;
}
...
export function LayerTree() {
  ...
  const flatVisible = useMemo(() => flattenVisibleTree(visualTree, collapsedIds), [visualTree, collapsedIds]);
  const parentMap = useMemo(() => buildParentMap(visualTree), [visualTree]);
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    const currentIndex = focusedId !== null ? flatVisible.findIndex(l => l.id === focusedId) : -1;
  ...
  if (visualTree.length === 0) {
    return <div className="flex flex-col items-center justify-center gap-2 px-4 py-8 text-center h-full">
                <Layers className="h-6 w-6 text-zinc-600" />
                <p className="text-sm text-zinc-400">No component layers</p>
                <p className="text-xs text-zinc-500 max-w-[var(--spacing.12, 48px)]">Open a .tsx file to see its layer structure</p>
            </div>;
  }
  return <div ref={treeRef} role="tree" aria-label="Component layer tree" className="h-full overflow-y-auto py-2" onKeyDown={handleKeyDown}>
            {visualTree.map(layer => <LayerRow key={layer.id} layer={layer} depth={0} collapsedIds={collapsedIds} onToggleCollapsed={toggleCollapsed} lockedIds={lockedIds} focusedId={focusedId} setFocusedId={setFocusedId} />)}
        </div>;
}