...
import { GOVERNANCE_RULES_MANIFEST, GOVERNANCE_CATEGORIES, type GovernanceRule, type RuleSeverity, type GovernanceCategory } from '../../core/governanceRulesManifest';
...
const PLANNED_BADGE = <span className="rounded border border-zinc-700/40 bg-zinc-800/60 px-1.5 py-0.5 text-[var(--spacing.2, 8px)] font-medium uppercase tracking-wider text-zinc-600" title="This rule is in development and will be enforced in a future update.">
        In development
    </span>;
...
function SeverityBadge({
  ...
  if (severity === 'critical') {
    return <span className="rounded border border-red-700/40 bg-red-900/10 px-1.5 py-0.5 text-[var(--spacing.2, 8px)] font-semibold uppercase tracking-wider text-red-400">
                Critical
            </span>;
  }
  if (severity === 'warning') {
    return <span className="rounded border border-amber-500/30 bg-amber-900/20 px-1.5 py-0.5 text-[var(--spacing.2, 8px)] font-semibold uppercase tracking-wider text-amber-400">
                Warning
            </span>;
  }
...
function RuleRow({
  ...
  return <div ref={rowRef} className={`flex items-center gap-3 px-4 py-2.5 hover:bg-zinc-800/30 transition-colors ${isModified ? 'border-l-2 border-indigo-500/40' : 'border-l-2 border-transparent'} ${isPlanned ? 'opacity-50' : ''} ${isFocused ? 'ring-1 ring-inset ring-indigo-500/40 bg-indigo-900/10' : ''}`}>
            <SwitchToggle checked={isEnabled} onChange={(val: boolean) => onToggle(rule.id, val)} aria-label={`Toggle ${rule.name}`} size="sm" />

            <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                    <span className="font-mono text-[var(--spacing.2, 8px)] text-zinc-400">{rule.id}</span>
                    {isModified && <button type="button" onClick={() => onReset(rule.id)} className="rounded bg-indigo-900/20 px-1 py-0.5 text-[var(--spacing.2, 8px)] text-indigo-400 transition-colors hover:bg-indigo-900/40 hover:text-indigo-300" title={`Reset ${rule.id} to default`}>
                            modified
                        </button>}
                </div>
                <p className={`mt-0.5 text-xs leading-snug transition-colors ${isEnabled ? 'text-zinc-300' : 'text-zinc-600 line-through'}`}>
                    {rule.name}
                </p>
                {/* EDU-04: one-line description surfaces taxonomy content inline */}
                {description && <p className="mt-0.5 text-[var(--spacing.2, 8px)] text-zinc-600 leading-snug">
                        {description}
                    </p>}
            </div>
...
function CategorySidebar({
  ...
  return <aside className="flex w-40 shrink-0 flex-col border-r border-zinc-800 overflow-y-auto">
            <div className="shrink-0 border-b border-zinc-800 px-3 py-2">
                <h3 className="text-[var(--spacing.2, 8px)] font-medium uppercase tracking-wider text-zinc-400">
                    Categories
                </h3>
            </div>
        ...
        const count = cat === 'All' ? GOVERNANCE_RULES_MANIFEST.length : counts[cat] ?? 0;
        return <button key={cat} type="button" onClick={() => onChange(cat)} className={`flex w-full items-center justify-between px-3 py-2 text-left text-xs transition-colors hover:bg-zinc-800/40 ${isActive ? 'bg-indigo-600/20 text-indigo-300' : 'text-zinc-400'}`}>
                            <span className="truncate">{cat}</span>
                            <span className={`ml-1 shrink-0 rounded px-1 py-0.5 text-[10px] ${isActive ? 'bg-indigo-900/40 text-indigo-400' : 'bg-zinc-800 text-zinc-600'}`}>
                                {count}
                            </span>
                        </button>;
      })}
...
export function GovernancePanel({
  ...
  const allCategoryRules = activeCategory === 'All' ? GOVERNANCE_RULES_MANIFEST : GOVERNANCE_RULES_MANIFEST.filter(r => r.category === activeCategory);
  ...
  const handleToggle = useCallback((ruleId: string, enabled: boolean) => {
    ...
    window.flintAPI.governance.recordOverride({
      ...
      newValue: isReturningToDefault ? null : {
        enabled
      },
      filePath: activeFilePath ?? ''
    });
  }, [setOverride, resetOverride, overrides, activeFilePath]);
  const handleResetRule = useCallback((ruleId: string) => {
    resetOverride(ruleId);
  ...
  const handleEnablePack = useCallback(async (packId: string) => {
    setIsToggling(true);
    try {
  ...
  const handleDisablePack = useCallback(async (packId: string) => {
    setIsToggling(true);
    try {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" role="button" tabIndex={0} onClick={e => {
    ...
    if ((e.key === 'Enter' || e.key === ' ') && e.target === e.currentTarget) onClose();
            ...
            <FocusTrap>
            <div role="dialog" aria-modal="true" aria-labelledby="governance-panel-title" className="flex h-[var(--spacing.1, 4px)] w-full max-w-3xl flex-col overflow-hidden rounded-xl border border-zinc-800 bg-zinc-900 shadow-2xl" onClick={e => e.stopPropagation()}>
                {/* ── Header ── */}
                <div className="flex shrink-0 items-center gap-3 border-b border-zinc-800 px-5 py-4">
                    ...
                    <div className="flex-1 min-w-0">
                        <h2 id="governance-panel-title" className="text-sm font-semibold text-zinc-100">
                            Governance Rules
                        </h2>
                        <p className="mt-0.5 text-xs text-zinc-400">
                            {GOVERNANCE_RULES_MANIFEST.length} rules
                            {modifiedCount > 0 && <span className="ml-1.5 text-indigo-400">
                                    · {modifiedCount} modified
                                </span>}
                        </p>
                    </div>
                    <button type="button" onClick={onClose} aria-label="Close governance panel" className="shrink-0 rounded p-1 text-zinc-400 transition-colors hover:bg-zinc-800 hover:text-zinc-100">
                        <X className="h-4 w-4" />
                    </button>
                </div>
                ...
                <div className="shrink-0 flex items-center gap-0 border-b border-zinc-800 px-5" role="tablist" aria-label="Governance panel sections">
                    {([{
          ...
          }] as const).map(tab => <button key={tab.id} type="button" role="tab" aria-selected={activeTab === tab.id} aria-controls={`governance-tabpanel-${tab.id}`} id={`governance-tab-${tab.id}`} onClick={() => setActiveTab(tab.id)}
          // EDU-07: subtitle shown as tooltip on each tab
          title={tab.subtitle} className={`border-b-2 px-3 py-2 text-xs font-medium transition-colors ${activeTab === tab.id ? 'border-indigo-500 text-indigo-300' : 'border-transparent text-zinc-400 hover:text-zinc-300'}`}>
                            {tab.label}
                        </button>)}
                </div>

                {/* EDU-07: Active tab subtitle — shown below the tab bar */}
                {activeTab === 'rules' && <p className="shrink-0 px-5 py-1.5 text-[var(--spacing.2, 8px)] text-zinc-600 border-b border-zinc-800/50">
                        Individual checks that run on your code
                    </p>}
                {activeTab === 'packs' && <p className="shrink-0 px-5 py-1.5 text-[var(--spacing.2, 8px)] text-zinc-600 border-b border-zinc-800/50">
                        Pre-built rule sets for common standards — enabling a pack activates all its rules
                    </p>}
                {activeTab === 'profiles' && <p className="shrink-0 px-5 py-1.5 text-[var(--spacing.2, 8px)] text-zinc-600 border-b border-zinc-800/50">
                ...
                <div className="flex min-h-0 flex-1 overflow-hidden">
                    {activeTab === 'rules' && <>
                            {/* Category sidebar */}
                            <CategorySidebar activeCategory={activeCategory} counts={categoryCounts} onChange={setActiveCategory} />

                            {/* Rule list — Issue 4: tabpanel role with matching aria-labelledby */}
                            <div className="flex-1 overflow-y-auto" role="tabpanel" id="governance-tabpanel-rules" aria-labelledby="governance-tab-rules">
                                {/* Section header */}
                  ...
                  return <RuleRow key={rule.id} rule={rule} override={overrides[rule.id]} onToggle={handleToggle} onReset={handleResetRule} isFocused={isFocused} rowRef={isFocused ? focusedRowRef : undefined} />;
                })}
                                </div>

                                {/* Planned rules — collapsed by default */}
                                {plannedRules.length > 0 && <div className="border-t border-zinc-800/40">
                                        <button type="button" onClick={() => setShowPlanned(v => !v)} className="flex w-full items-center gap-1.5 px-4 py-2.5 text-left text-[var(--spacing.3, 12px)] text-zinc-400 hover:text-zinc-300 transition-colors">
                                            <span className={`transition-transform duration-150 ${showPlanned ? 'rotate-90' : ''}`}>▸</span>
                                            Also coming ({plannedRules.length})
                                        </button>
                                        {showPlanned && <div className="divide-y divide-zinc-800/40">
                                                {plannedRules.map(rule => <RuleRow key={rule.id} rule={rule} override={overrides[rule.id]} onToggle={handleToggle} onReset={handleResetRule} isFocused={rule.id === focusRuleId} rowRef={rule.id === focusRuleId ? focusedRowRef : undefined} />)}
                                            </div>}
                                    </div>}
                            </div>
                        </>}
          ...
          <div className="flex-1 overflow-y-auto" role="tabpanel" id="governance-tabpanel-packs" aria-labelledby="governance-tab-packs">
                            <RuleCatalogPanel activePresets={activePresets} onEnablePack={packId => void handleEnablePack(packId)} onDisablePack={packId => void handleDisablePack(packId)} isToggling={isToggling} />
                        </div>}

                    {activeTab === 'profiles' &&
          // Issue 4: tabpanel role for profiles tab
          <div className="flex-1 overflow-y-auto" role="tabpanel" id="governance-tabpanel-profiles" aria-labelledby="governance-tab-profiles">
                            <ComplianceProfileSelector activePresets={activePresets} onToggleJurisdiction={(packId, enabled) => void (enabled ? handleEnablePack(packId) : handleDisablePack(packId))} isToggling={isToggling} />
                        </div>}
                </div>

                {/* ── S5.7: Persistence context banner ── */}
                {activeTab === 'rules' && modifiedCount > 0 && <div className="shrink-0 flex items-center gap-2 border-t border-amber-900/30 bg-amber-950/20 px-5 py-2" role="status" aria-live="polite">
                        <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-amber-400" />
                        <p className="text-[var(--spacing.2, 8px)] text-amber-400/80">
                            Toggles apply immediately to live audits · <strong className="font-semibold">Save</strong> to persist across sessions
                        </p>
                    </div>}
                {(activeTab === 'packs' || activeTab === 'profiles') && <div className="shrink-0 flex items-center gap-2 border-t border-zinc-800/60 bg-zinc-900 px-5 py-2">
                        <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-zinc-600" />
                        <p className="text-[var(--spacing.2, 8px)] text-zinc-600">
                ...
                <div className="flex shrink-0 items-center justify-between gap-3 border-t border-zinc-800 px-5 py-3">
                    ...
                    {activeTab === 'rules' ? <button type="button" onClick={handleReset} disabled={modifiedCount === 0} className="flex items-center gap-1.5 rounded border border-zinc-700 px-3 py-1.5 text-xs text-zinc-400 transition-colors hover:border-zinc-600 hover:text-zinc-200 disabled:cursor-not-allowed disabled:opacity-40">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {activeTab === 'rules' && <button type="button" onClick={() => void handleSave()} disabled={saving} className={`flex items-center gap-1.5 rounded px-3 py-1.5 text-xs font-medium text-white transition-colors active:scale-95 disabled:opacity-60 ${modifiedCount > 0 ? 'bg-indigo-600 ring-1 ring-indigo-400/40 hover:bg-indigo-500' : 'bg-indigo-600 hover:bg-indigo-500'}`}>
