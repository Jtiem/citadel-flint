...
export function FirstSyncPrompt({
  ...
  return <div role="status" aria-live="polite" className="flex items-center gap-3 border-b border-emerald-500/30 bg-emerald-900/15 px-3 py-2.5" data-testid="first-sync-prompt">
            ...
            <button type="button" onClick={handleReview} className="flex shrink-0 items-center gap-1 rounded bg-emerald-600/80 px-2.5 py-1 text-[var(--spacing.3, 12px)] font-medium text-white transition-colors hover:bg-emerald-500" data-testid="first-sync-review-button">
                Review tokens
                <ArrowRight size={11} aria-hidden="true" />
            </button>
            <button type="button" onClick={handleDismiss} aria-label="Dismiss first sync prompt" className="shrink-0 rounded p-0.5 text-emerald-500/60 transition-colors hover:text-emerald-300" data-testid="first-sync-dismiss">
