...
function ColorSwatch({
    ...
    return (
        <span
            role="img"
            aria-label={`${label} swatch for ${tokenName}: ${value}`}
            className="inline-block h-5 w-5 shrink-0 rounded-full border border-white/20 shadow-sm"
...
export function TokenDriftRow({
    ...
    return (
        <div
            role="button"
            tabIndex={0}
            aria-label={`Open detail for ${tokenName}`}
            data-testid={`drift-row-${tokenName}`}
            data-token-name={tokenName}
            onClick={handleRowClick}
            onKeyDown={handleKeyDown}
            className={[
                'flex items-center gap-2 rounded border border-zinc-800 bg-zinc-900/40',
                'px-2 py-1.5 text-xs text-zinc-300',
                'hover:bg-zinc-900 focus:outline-none focus:ring-1 focus:ring-indigo-400/50',
                'cursor-pointer',
            ].join(' ')}
        >
            ...
            {isColor ? (
                <ColorSwatch value={drift.localValue} role="local" tokenName={tokenName} />
            ) : (
                <span
                    className="shrink-0 font-mono text-[11px] text-zinc-500"
                    aria-label={`Local value: ${drift.localValue}`}
                >
                    {drift.localValue}
                </span>
            )}
            ...
            {isColor ? (
                <ColorSwatch value={drift.figmaValue} role="figma" tokenName={tokenName} />
            ) : (
                <span
                    className="shrink-0 font-mono text-[11px] text-zinc-500"
                    aria-label={`Figma value: ${drift.figmaValue}`}
                >
                    {drift.figmaValue}
                </span>
            )}
            ...
            {isColor && hasDeltaE && (
                <SeverityChip
                    severity={deltaEToSeverity(drift.deltaE as number)}
                    label={`ΔE ${formatDeltaE(drift.deltaE as number)}`}
                    data-testid={`drift-delta-${tokenName}`}
                />
            )}
            ...
            <button
                type="button"
                data-role="pull-one"
                data-testid={`drift-pull-${tokenName}`}
                onClick={handlePull}
                disabled={isPulling}
                aria-label={`Pull ${tokenName} from Figma`}
                className={[
                    'shrink-0 rounded border border-indigo-500/30 bg-indigo-500/10',
                    'px-2 py-0.5 text-[11px] font-medium text-indigo-300',
                    'hover:bg-indigo-500/20 focus:outline-none focus:ring-1 focus:ring-indigo-400',
