...
export function EmitDropdown({
  ...
  return <div className="relative" data-testid="emit-dropdown">
      {/* ── Trigger ── */}
      <button ref={triggerRef} type="button" onClick={handleTriggerClick} onKeyDown={handleTriggerKeyDown} disabled={triggerDisabled} aria-haspopup="menu" aria-expanded={isOpen} aria-controls={isOpen ? menuId : undefined} data-testid="emit-trigger" title="Emit tokens" className={clsx('inline-flex items-center gap-1.5 rounded-md border px-2 py-1 text-xs font-medium transition-colors', 'border-zinc-700 bg-zinc-900 text-zinc-200', 'hover:border-zinc-600 hover:bg-zinc-800', 'disabled:cursor-not-allowed disabled:opacity-40')}>
        {emitOp !== null ? <Loader2 className="h-3 w-3 animate-spin" aria-hidden="true" data-testid="emit-spinner" /> : <ArrowUpFromLine className="h-3 w-3" aria-hidden="true" />}
        <span>Emit</span>
        <ChevronDown className={clsx('h-3 w-3 transition-transform', isOpen && 'rotate-180')} aria-hidden="true" />
      </button>

      {/* ── Menu ── */}
      {isOpen && <div ref={menuRef} id={menuId} role="menu" aria-label="Emit tokens" onKeyDown={handleMenuKeyDown} data-testid="emit-menu" className={clsx('absolute right-0 top-full z-50 mt-1', 'min-w-[200px] rounded-lg border border-zinc-700/50 bg-zinc-900', 'py-1 shadow-xl')}>
          {/* Section headers — purely visual, platform groups */}
          {PLATFORMS.map(({
        platform,
        label
      }, platformIdx) => <div key={platform} role="none">
              {/* Platform section divider (except first) */}
              {platformIdx > 0 && <div className="my-1 border-t border-zinc-800" role="separator" aria-hidden="true" />}
          ...
          return <div key={previewItem.testId} ref={el => {
            itemRefs.current[previewIdx] = el;
          }} role="menuitem" tabIndex={focusedIndex === previewIdx ? 0 : -1} data-testid={previewItem.testId} onClick={() => handleItemSelect(previewItem)} onKeyDown={e => handleItemKeyDown(e, previewItem)} className={clsx('mx-1 flex cursor-pointer items-center rounded-md px-2 py-1.5 text-xs text-zinc-300', 'hover:bg-zinc-800 hover:text-zinc-100', 'focus:bg-zinc-800 focus:text-zinc-100 focus:outline-none')}>
          ...
          return <div key={writeItem.testId} ref={el => {
            itemRefs.current[writeIdx] = el;
          }} role="menuitem" tabIndex={focusedIndex === writeIdx ? 0 : -1} data-testid={writeItem.testId} onClick={() => handleItemSelect(writeItem)} onKeyDown={e => handleItemKeyDown(e, writeItem)} className={clsx('mx-1 flex cursor-pointer items-center rounded-md px-2 py-1.5 text-xs text-zinc-300', 'hover:bg-zinc-800 hover:text-zinc-100', 'focus:bg-zinc-800 focus:text-zinc-100 focus:outline-none')}>
