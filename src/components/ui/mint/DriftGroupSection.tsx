 ...
 */
function groupDriftsByCollection(
    drifts: readonly TokenDrift[],
    tokensByPath: DriftGroupSectionProps['tokensByPath'],
): GroupedDrift[] {
...
export function DriftGroupSection({
    ...
    return (
        <div data-testid="drift-group-section" className="flex flex-col gap-3">
            {groups.map(({ collection, drifts }) => {
                const headingId = `drift-group-${collection}`
                return (
                    <section
                        key={collection}
                        aria-labelledby={headingId}
                        data-testid={`drift-group-${collection}`}
                            ...
                            {drifts.map(drift => {
                                ...
                                return (
                                    <TokenDriftRow
                                        ...
                                        onPullOne={onPullOne}
                                        onSelect={onSelect}
                                        isPulling={currentPullingPath === drift.tokenName}
                                    />
                                )
                            })}
                        </div>
