...
describe('ConfirmResolveDialog', () => {
    it('renders nothing when isOpen=false', () => {
        const { container } = render(<ConfirmResolveDialog {...makeProps({ isOpen: false })} />)
    ...
    it('onConfirm is called with "prefer-figma" when default selection is confirmed', () => {
        ...
        render(<ConfirmResolveDialog {...makeProps({ onConfirm })} />)
    ...
    it('fires onCancel on Cancel button click', () => {
        ...
        const onConfirm = vi.fn()
        render(<ConfirmResolveDialog {...makeProps({ onConfirm, onCancel })} />)
    ...
    it('fires onCancel on Escape key', () => {
        const onCancel = vi.fn()
        render(<ConfirmResolveDialog {...makeProps({ onCancel })} />)
    ...
    it('renders the conflict count in the heading with correct pluralization', () => {
        const { rerender } = render(<ConfirmResolveDialog {...makeProps({ conflictCount: 1 })} />)
        expect(screen.getByText(/Resolve 1 conflict\?/i)).toBeDefined()

        rerender(<ConfirmResolveDialog {...makeProps({ conflictCount: 4 })} />)
