...
function categoryBadgeClass(category: string): string {
    switch (category) {
        case 'primitive':     return 'bg-blue-900/30 text-blue-400'
        case 'molecule':      return 'bg-purple-900/30 text-purple-400'
        case 'organism':      return 'bg-amber-900/30 text-amber-400'
        case 'page':          return 'bg-emerald-900/30 text-emerald-400'
        case 'layout':        return 'bg-cyan-900/30 text-cyan-400'
...
export function ComponentPanelCard({
    ...
    const handleDragStart = useCallback(
        (e: React.DragEvent) => {
            ...
            e.dataTransfer.setData(
                ...
                JSON.stringify({ name, importPath }),
            )
        },
        [name, importPath],
    )
    ...
    return (
        <div
            ...
            data-testid={`component-panel-card-${name}`}
            onClick={onSelect}
            onKeyDown={(e) => {
        ...
        >
            ...
            {draggable && (
                <div
                    ...
                    className="flex shrink-0 cursor-grab items-center text-zinc-600 hover:text-zinc-400 active:cursor-grabbing"
                    data-testid={`component-panel-drag-${name}`}
            ...
            <div className="h-8 w-8 shrink-0 overflow-hidden rounded bg-zinc-800">
                {thumbnailUrl ? (
                    ...
                    <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-zinc-800 to-zinc-700">
                        <span
                            className="font-mono text-xs font-bold text-zinc-500 select-none"
                            aria-hidden="true"
                        >
                            {firstLetter}
                        </span>
            ...
            <div className="flex min-w-0 flex-1 flex-col gap-0.5">
                ...
                <div className="flex items-center gap-1.5">
                    <span
                        className={`text-[10px] px-1.5 py-0.5 rounded ${categoryBadgeClass(category)}`}
                    >
                        {category}
                    </span>
                    {variantCount > 0 && (
