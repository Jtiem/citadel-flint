...
import { Search, Shield, GitBranch, Settings, Loader2, LayoutGrid, Zap, RotateCcw, History, Bot, RefreshCw, Plug, Download, X } from 'lucide-react';
...
function parseRegistryMarkdown(markdown: string): RegistryResult[] {
  ...
  for (const section of sections) {
    ...
    const descLine = section.split('\n').slice(1).map(l => l.trim()).find(l => l.length > 0 && !l.startsWith('**') && !l.startsWith('---') && !l.startsWith('#'));
    results.push({
      name,
      importPath,
...
export function CommandPalette({
  ...
  const closeAndRun = useCallback((action: () => void | Promise<void>) => {
    setOpen(false);
  ...
  const callMcp = useCallback(async (tool: string, params: Record<string, unknown>, successTitle: string) => {
  ...
  // ── Governance ───────────────────────────────────────────────────
  {
    ...
    action: () => closeAndRun(async () => {
      ...
      pushNotification({
        type: 'info',
        severity: 'success',
        title: `Fixed ${fixOps.length} issues`,
        message: 'Auto-fixable token drifts resolved',
        autoDismissMs: 4000
      });
    })
  }, {
  ...
  // ── Settings / Tools ─────────────────────────────────────────────
  {
    ...
    action: () => closeAndRun(() => {
      if (onOpenSetupWizard) {
        ...
        window.flintAPI.mcp?.reconnect?.().catch(err => console.warn('[Flint] CommandPalette: MCP reconnect failed', err));
      }
    })
  }],
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        ...
        const res: MCPCallResult = await window.flintAPI.mcp.callTool('flint_query_registry', {
          query,
          limit: 5
        });
        if (res.isError) {
  ...
  const flatItems = useMemo<FlatItem[]>(() => {
    ...
    if (registryResults && registryResults.length > 0) {
      registryResults.forEach((r, i) => items.push({
        kind: 'registry',
        result: r,
        index: items.length + i
      }));
    }
    filteredCommands.forEach(c => items.push({
      kind: 'command',
      command: c,
      index: items.length
    }));
  ...
  return createPortal(<div role="dialog" aria-modal="true" aria-label="Command palette" className="fixed inset-0 z-[9999] flex items-start justify-center pt-[var(--spacing.1, 4px)]">
            {/* Backdrop */}
            <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setOpen(false)} aria-hidden="true" />

            {/* Palette card */}
            <div className="relative z-10 w-full max-w-[var(--spacing.12, 48px)] mx-4 rounded-xl border border-zinc-700 bg-zinc-900 shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
                {/* Search row */}
                <div className="flex items-center gap-2.5 border-b border-zinc-800 px-4 py-3">
                    <Search className="h-4 w-4 shrink-0 text-zinc-400" />
                    <input ref={inputRef} type="text" value={query} onChange={e => setQuery(e.target.value)} placeholder="Search commands and components…" className="flex-1 bg-transparent text-sm text-zinc-100 placeholder-zinc-500 outline-none" aria-label="Command search" aria-autocomplete="list" aria-controls="command-palette-list" />
                    {registryLoading && <Loader2 className="h-3.5 w-3.5 shrink-0 animate-spin text-zinc-500" />}
                    <button type="button" onClick={() => setOpen(false)} aria-label="Close command palette" className="text-zinc-600 hover:text-zinc-400 transition-colors">
                        <X className="h-3.5 w-3.5" />
                    </button>
                </div>

                {/* Results */}
                <div ref={listRef} id="command-palette-list" role="listbox" aria-label="Commands" className="max-h-[var(--spacing.12, 48px)] overflow-y-auto py-2">
                    {/* Registry results (at the top when query active) */}
                    {registryResults && registryResults.length > 0 && <div>
                            <div className="px-3 py-1.5 text-[var(--spacing.2, 8px)] font-semibold uppercase tracking-wider text-zinc-500">
                                Assets
                            </div>
                            {registryResults.map(result => {
            const fi = flatIndexForRegistry(result);
            const isSelected = fi === selectedIndex;
            return <button key={result.name} type="button" role="option" aria-selected={isSelected} data-selected={isSelected} onClick={() => handleInsertComponent(result)} className={`flex w-full items-center gap-3 px-3 py-2 text-left transition-colors ${isSelected ? 'bg-indigo-600/20 text-white' : 'text-zinc-200 hover:bg-zinc-800'}`}>
                                        ...
                                        <span className="min-w-0 flex-1">
                                            ...
                                            {result.description && <span className="block truncate text-[var(--spacing.3, 12px)] text-zinc-500">
                                                    {result.description}
                                                </span>}
                                        </span>
                                        <span className="shrink-0 rounded bg-indigo-900/50 px-1.5 py-0.5 text-[var(--spacing.2, 8px)] font-medium text-indigo-400">
                                            Insert
                                        </span>
                                    </button>;
          })}
                    ...
                    {query.length >= 2 && registryResults !== null && registryResults.length === 0 && !registryLoading && <div className="px-3 py-1.5 text-[var(--spacing.2, 8px)] font-semibold uppercase tracking-wider text-zinc-500">
                            Assets — no matches
                        </div>}

                    {/* Static command groups */}
                    {groupedCommands.length > 0 ? groupedCommands.map(({
          category,
          commands
        }) => <div key={category}>
                                <div className="px-3 py-1.5 text-[var(--spacing.2, 8px)] font-semibold uppercase tracking-wider text-zinc-500">
            ...
            const isSelected = fi === selectedIndex;
            return <button key={cmd.id} type="button" role="option" aria-selected={isSelected} data-selected={isSelected} onClick={() => void cmd.action()} className={`flex w-full items-center gap-3 px-3 py-2 text-left transition-colors ${isSelected ? 'bg-indigo-600/20 text-white' : 'text-zinc-200 hover:bg-zinc-800'}`}>
                                            <span className={`shrink-0 ${isSelected ? 'text-indigo-400' : 'text-zinc-500'}`}>
                                                {cmd.icon}
                                            </span>
                                            <span className="flex-1 text-sm">{cmd.label}</span>
                                            {cmd.kbd && <span className="shrink-0 font-mono text-[var(--spacing.2, 8px)] text-zinc-600">
                                                    {cmd.kbd}
                                                </span>}
                                        </button>;
          })}
                            </div>) : !registryLoading && <div className="px-4 py-8 text-center text-sm text-zinc-500">
                                No commands match &ldquo;{query}&rdquo;
                            </div>}
                </div>

                {/* Footer hint */}
                <div className="flex items-center gap-4 border-t border-zinc-800 px-4 py-2 text-[var(--spacing.2, 8px)] text-zinc-600">
