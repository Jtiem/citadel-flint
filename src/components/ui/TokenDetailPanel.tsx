...
export function TokenDetailPanel({
  ...
  return <FocusTrap initialFocusRef={closeButtonRef} onClose={onClose}>
        <div className="fixed inset-y-0 right-0 z-40 flex w-80 flex-col border-l border-zinc-700 bg-zinc-900 shadow-2xl" role="dialog" aria-modal="true" aria-label={`Token detail: ${token.token_path}`} data-testid="token-detail-panel">
            {/* Header */}
            <div className="flex shrink-0 items-center justify-between border-b border-zinc-800 px-4 py-3">
                <h3 className="truncate text-sm font-semibold text-zinc-100">
                    Token Detail
                </h3>
                <button ref={closeButtonRef} type="button" onClick={onClose} aria-label="Close token detail panel" className="rounded p-1 text-zinc-500 transition-colors hover:text-zinc-300" data-testid="token-detail-close">
                    <X className="h-4 w-4" />
                </button>
            </div>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4 space-y-5">
                ...
                <section>
                    ...
                    <p className="mt-1 break-all font-mono text-xs text-zinc-200" data-testid="token-detail-path">
                        {token.token_path}
                    </p>
                </section>
                ...
                <section>
                    ...
                    <div className="mt-1.5 flex items-center gap-2">
                        {isColor && <span className="inline-block h-8 w-8 shrink-0 rounded-lg border border-white/20 shadow-sm" style={{
              backgroundColor: token.token_value
            }} aria-label={`Color swatch: ${token.token_value}`} role="img" data-testid="token-detail-swatch" />}
                        <span className="font-mono text-sm text-zinc-200" data-testid="token-detail-value">
                            {token.token_value}
                        </span>
                    </div>
                    {/* Dark mode counterpart */}
                    {darkModeToken && <div className="mt-2 flex items-center gap-2">
                            {isColor && <span className="inline-block h-6 w-6 shrink-0 rounded border border-white/20 shadow-sm" style={{
              backgroundColor: darkModeToken.token_value
            }} aria-label={`Dark mode swatch: ${darkModeToken.token_value}`} role="img" />}
                ...
                <section>
                    ...
                    <div className="mt-1.5" data-testid="token-detail-usage">
                        {usageCount === 0 ? <div className="flex items-center gap-1.5 text-xs text-red-400">
                                ...
                                {files.length > 0 && <ul className="mt-1.5 space-y-0.5" data-testid="token-detail-file-list">
                                        {files.slice(0, 10).map(f => <li key={f} className="flex items-center gap-1.5 truncate text-[var(--spacing.3, 12px)] text-zinc-400">
                                                <FileText size={10} className="shrink-0 text-zinc-600" aria-hidden="true" />
                                                {f}
                                            </li>)}
                                        {files.length > 10 && <li className="text-[var(--spacing.3, 12px)] text-zinc-500">
                ...
                {/* Drift status */}
                {drift && <section data-testid="token-detail-drift">
                        ...
                        <div className="mt-1.5 rounded border border-amber-500/20 bg-amber-900/10 px-3 py-2 text-xs">
                            ...
                            <div className="mt-1.5 grid grid-cols-2 gap-2">
                                <div>
                                    <span className="text-[var(--spacing.2, 8px)] text-zinc-500">Local</span>
                                    <p className="font-mono text-xs text-zinc-300">{drift.localValue}</p>
                                </div>
                                <div>
                                    <span className="text-[var(--spacing.2, 8px)] text-zinc-500">Figma</span>
                ...
                {/* Sync status */}
                {syncStatus && <section data-testid="token-detail-sync">
                ...
                {/* Contrast pairings */}
                {contrastPairs && contrastPairs.length > 0 && <section data-testid="token-detail-contrast">
                        <SectionLabel label="Contrast pairings" />
                        <div className="mt-1.5 space-y-1">
                            {contrastPairs.slice(0, 5).map((pair, i) => <div key={i} className="flex items-center justify-between rounded border border-zinc-800 bg-zinc-800/40 px-2 py-1.5 text-[var(--spacing.3, 12px)]">
                                    <span className="truncate text-zinc-300">
                                        vs {pair.bg}
                                    </span>
                                    <span className={`shrink-0 font-medium ${pair.passAAA ? 'text-emerald-400' : pair.passAA ? 'text-emerald-400' : 'text-red-400'}`}>
                ...
                <section data-testid="token-detail-provenance">
                    ...
                    <dl className="mt-1.5 grid grid-cols-2 gap-x-3 gap-y-1.5 text-[var(--spacing.3, 12px)]">
...
function SectionLabel({
  ...
  return <span className="text-[var(--spacing.2, 8px)] font-medium uppercase tracking-wider text-zinc-500">
            {label}
        </span>;
}