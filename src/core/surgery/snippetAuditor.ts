 ...
 * Flint-compliant code by enforcing two invariants:
 *
 ...
 *          data-flint-id={`node-${index}`}
 *      These are scaffold placeholders; `injectFlintIds` in the AST canvas
 ...
 *          ΔE < 2.0  → replaced with var(--flint-token-<path>)
 ...
 * e.g. "color.brand.primary" → "var(--flint-token-color-brand-primary)"
 */
function tokenPathToCssVar(tokenPath: string): string {
    return `var(--flint-token-${tokenPath.replace(/\./g, '-')})`
}

/**
 * Returns true when the JSX element's opening tag already carries a
 * `data-flint-id` attribute (prevents double-injection on re-audit).
 */
function hasFlintId(elem: import('@babel/types').JSXElement): boolean {
...
/**
 * Injects `data-flint-id={\`node-${<indexName>}\`}` onto a JSX element.
 * No-op if the attribute is already present.
 */
function injectFlintId(
    elem: import('@babel/types').JSXElement,
    indexName: string
): void {
    if (hasFlintId(elem)) return
...
/**
 * Audits a raw JSX snippet from a Ralph Loop and returns Flint-compliant code.
 *
 ...
 *
 * @returns Cleaned, Flint-compliant JSX source string.
 */
export function auditSnippet(rawCode: string, tokens: DesignToken[] = []): string {
    ...
    // ── Step 2: Commandment 3 — inject index + data-flint-id ───────────────
    ...
    traverse(ast, {
        CallExpression: {
            enter(path) {
                ...
                // Stamp data-flint-id on the root JSX element of this loop
                const rootJSX = findRootJSX(callback.body)
                if (rootJSX !== null) {
                    injectFlintId(rootJSX, indexName)
                }
