...
| **Orchestration** | **Flint Auditor** | Complexity analysis that routes tasks to "Flash" or "Thinking" AI models. |
...
* **Buffer Hardening:** Every buffer loaded via `loadBuffer` has `injectFlintIds(ast)` applied immediately.
...
* **Registry Tracking:** `flint-registry.db` maintains global state for local project discovery.
...
* **Preload Isolation:** `window.flintAPI` provides safe access to native file system dialogs and Git interactions.
...
### **Module L: Flint Auditor / Orchestration**
...
* **Ingestion Server:** Loopback HTTP server (port 4545). Authenticated via `x-flint-secret`.
...
* **Auto-Hydration:** `/ingest-ast` triggers `flint:hydro-paste-auto` for zero-click Figma-to-Flint sync.
...
1. **Architecture Spec:** Always consult `.flint-context/architecture.md` and `.antigravityrules`.
...
5. **Documentation Autopilot:** No session ends without a `[FLINT-PULSE-v7.x]` block update.
