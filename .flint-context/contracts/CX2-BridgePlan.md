# Contract: CX.2 -- flint_plan Orchestration Tool
...
`flint_plan` is a new MCP tool that accepts a natural language intent string and returns a structured, deterministic execution plan. The plan is an ordered sequence of Flint tool calls, decision points requiring human judgment, and success criteria. The tool does not execute anything -- it returns a plan that the calling agent then executes step by step.

The tool lives entirely in the headless MCP engine (`flint-mcp/`). Zero Glass changes. Zero IPC changes. Zero store changes. The implementation creates two new files (`planService.ts` and `plan.ts`) and defers the `server.ts` registration to avoid conflict with the ING.3 swarm territory.
...
**Scope estimation:** When a `projectRoot` is provided, the plan service reads `.flint/debt-history.json` and `.flint/design-tokens.json` to estimate violation counts and file scope. This enriches the plan with concrete numbers (e.g., "~47 files, ~200 violations") instead of generic placeholders. When `projectRoot` is absent or files are unreadable, the plan degrades gracefully to generic estimates.
...
|------|------------|-------------|
| `flint-mcp/src/core/planService.ts` | NEW -- intent classification + plan generation | flint-ast-surgeon |
| `flint-mcp/src/tools/plan.ts` | NEW -- MCP tool handler (FLINT_PLAN_TOOL const + handler) | flint-ast-surgeon |
| `flint-mcp/src/__tests__/planService.test.ts` | NEW -- service unit tests (30+ tests) | flint-test-writer |
| `flint-mcp/src/__tests__/plan.tool.test.ts` | NEW -- tool handler integration tests (10+ tests) | flint-test-writer |
| `flint-mcp/src/server.ts` | DEFERRED MODIFY -- register `flint_plan` in ListTools + CallTool | flint-ast-surgeon (post-ING.3) |

---

## 2. Type Contracts (Source of Truth for Phase 2)

All types are defined in `flint-mcp/src/core/planService.ts` and re-exported from `flint-mcp/src/tools/plan.ts`. No new shared types file is needed -- these types do not cross any process boundary.
...
/**
 * A step that invokes a Flint MCP tool.
 */
export interface ToolStep {
    ...
    kind: 'tool'
    /** The MCP tool name to invoke. Must be a real, registered Flint tool. */
    tool: string
    /**
...
```typescript
/**
 * A complete execution plan returned by flint_plan.
 */
...
```typescript
/**
 * Input parameters for the flint_plan MCP tool.
 */
export interface FlintPlanParams {
    /**
     ...
     * service reads .flint/ files to enrich scope estimates with real data
...
Each intent type maps to a fixed plan template. The template is parameterized by `glob` and enriched with scope estimates from `.flint/` files when `projectRoot` is available.
...
const TOKEN_MIGRATION_TEMPLATE: PlanStep[] = [
    ...
    },
    {
        step: 5,
        kind: 'decision',
        description: 'For tier-2/3 violations that could not be auto-fixed, review the remaining violations and decide: apply manual fixes via flint_ast_mutate, or defer them.',
...
const ACCESSIBILITY_SWEEP_TEMPLATE: PlanStep[] = [
    {
        step: 1,
        kind: 'tool',
        tool: 'flint_accessibility_report',
        params: { projectRoot: '{{projectRoot}}', glob: '{{glob}}' },
...
const FIGMA_SYNC_TEMPLATE: PlanStep[] = [
    {
        ...
        tool: 'flint_status',
        params: {},
        purpose: 'Verify Flint MCP server is running and Figma ingestion endpoint is reachable.',
        requiresReview: false,
    },
    {
        step: 2,
        kind: 'tool',
        tool: 'flint_sync_tokens',
...
const UNKNOWN_INTENT_TEMPLATE: PlanStep[] = [
    {
        ...
        rationale: 'flint_plan supports 5 task types: token-migration, accessibility-sweep, full-governance-audit, figma-sync, and debt-remediation. The provided intent did not match any of these categories with sufficient confidence.',
        suggestedOptions: [
...
// riskLevel: 'low'
// successCriteria: ["User clarifies intent and re-invokes flint_plan with a more specific request"]
```
...
```typescript
/**
 * Scope context assembled from .flint/ files.
...
/**
 * Estimate scope by reading .flint/ files. Never throws.
 * Falls back gracefully when files are missing.
 *
 * Files read:
 *   .flint/debt-history.json  -- latest snapshot for violation count, score, grade
 *   .flint/design-tokens.json -- token count
 *
...
- Without data: `"Files matching ${glob || '**/*.tsx'} (scope estimate unavailable -- no .flint/ data found)"`
 ...
 *
 * @param args - Validated FlintPlanParams
...
The following additions to `flint-mcp/src/server.ts` are DEFERRED until the ING.3 swarm territory clears `server.ts` from its MODIFY list.
...
**ListToolsRequestSchema (add FLINT_PLAN_TOOL to the tools array):**
```typescript
FLINT_PLAN_TOOL,
```
...
case 'flint_plan': {
    const result = handleFlintPlan(
        args as unknown as FlintPlanParams,
        flintConfig,
    )
    return result
}
...
### 7.1 `flint-mcp/src/core/planService.ts`

Contains:
- All type exports: `PlanIntentType`, `PlanIntent`, `ToolStep`, `DecisionStep`, `PlanStep`, `ExecutionPlan`, `FlintPlanParams`, `ScopeEstimate`
...
### 7.2 `flint-mcp/src/tools/plan.ts`

Contains:
- `FLINT_PLAN_TOOL` constant (MCP tool schema)
- `handleFlintPlan()` function (MCP handler)
- Re-exports types from `planService.ts` for external consumers

### 7.3 `flint-mcp/src/__tests__/planService.test.ts`

Contains unit tests for the plan service (see Section 8).

### 7.4 `flint-mcp/src/__tests__/plan.tool.test.ts`
...
| CX2-22 | accessibility-sweep intent | Returns 7 steps; step 1 is flint_accessibility_report |
...
| Test ID | Scenario | Assertion |
|---------|----------|-----------|
| CX2-31 | Valid projectRoot with .flint/debt-history.json | healthScore and violationCount populated |
| CX2-32 | Valid projectRoot, no .flint/ directory | All scope fields are null, no throw |
...
| CX2-45 | Every ToolStep.tool references a real Flint tool name | All tool names in all templates exist in the known tool list |
...
- [x] **C4 Local-First Only** -- no external URLs, no network calls. Classification is pure string matching. Scope estimation reads local `.flint/` files only.
- [x] **C8 Audit-First Execution** -- `flint_plan` is itself an audit-first pattern: classify the task before choosing a model or execution strategy. The plan includes `flint_audit` as the first tool step in every template.
- [x] **C12 Atomic Queuing** -- `flint_plan` itself performs no file writes. Tool steps that reference `flint_fix` or `flint_ast_mutate` are executed by the calling agent, which routes through the existing atomic pipelines.
...
- [x] **C15 Granular AST Tools Only** -- every `ToolStep.tool` in every template references a real, registered Flint MCP tool. No raw code generation.
...
1. **flint-ast-surgeon**: Implement `flint-mcp/src/core/planService.ts`
   - All types
...
2. **flint-ast-surgeon**: Implement `flint-mcp/src/tools/plan.ts`
   - FLINT_PLAN_TOOL constant
   - handleFlintPlan() function
   - Re-exports
...
3. **flint-test-writer**: Implement `flint-mcp/src/__tests__/planService.test.ts` (CX2-01 through CX2-48 minus tool handler tests)
4. **flint-test-writer**: Implement `flint-mcp/src/__tests__/plan.tool.test.ts` (CX2-36 through CX2-45)
...
5. **flint-ast-surgeon**: Modify `flint-mcp/src/server.ts`
   - Import plan tool
...
6. **flint-integration-validator**: Run full test suite + TSC.
...
| `estimateScope()` reads files synchronously, blocking the event loop | Low | C4 | Capped at 500 file entries via `readdir`. `.flint/` files are small (< 100KB). Total I/O budget < 20ms. If this becomes a problem, the function can be made async without changing the public API. |
...
The `server.ts` registration is deferred because the ING.3 swarm currently claims `server.ts` as a MODIFY target (see `.flint-context/ACTIVE-SWARM-TERRITORY.md`). When ING.3 is complete and its territory is released:

1. Add the import line for `plan.ts`
2. Add `FLINT_PLAN_TOOL` to the `ListToolsRequestSchema` handler's tools array
3. Add a `case 'flint_plan'` to the `CallToolRequestSchema` handler's switch
4. Run `cd flint-mcp && npm test` and `npx tsc --noEmit` to verify no regressions
...
- `flint-mcp/src/server.ts` -- deferred (ING.3 territory)
- `flint-mcp/src/types.ts` -- no new shared types needed
