...
**Security Review Ref:** `.flint-context/reviews/SEC-surface-review.md`
...
Flint's core value proposition is deterministic governance enforcement. The AI Orchestrator (`electron/orchestrator.ts`) is constrained by two commandments:

- **Commandment 15 (Granular AST Tools Only):** The AI model MUST only emit operations from the versioned Flint Tool Catalog (`flint_read_code`, `flint_read_tokens`, `flint_audit_mithril`, `flint_audit_a11y`, `flint_update_props`, `flint_update_text`, `flint_insert_node`, `flint_wrap_node`, `flint_delete_node`, `flint_add_class`, `flint_remove_class`, `flint_search_design_system`). Raw code string generation is prohibited.
...
**Anthropic branch (lines 903+):** Fully compliant. Uses the `tools` parameter with `FLINT_TOOLS` catalog. The AI can only propose changes via tool_use blocks. Each tool_use is validated in-memory before being surfaced. This is the reference implementation.
...
A user who configures an OpenAI or Gemini API key gets a fundamentally different product: an ungovernanced chat assistant that can suggest arbitrary code changes with no design system enforcement, no accessibility checks, no Mithril safety, and no in-memory type validation. This is not a degraded experience -- it is the complete absence of the governance guarantees Flint promises.

---

## 2. Decision Drivers

1. **Governance integrity:** Flint is a governance product. Silently disabling governance for a subset of users undermines the product's credibility.
2. **Implementation effort:** Adding tool-use support for OpenAI and Gemini requires implementing their respective function-calling/tool-use APIs, mapping FLINT_TOOLS to their schema formats, parsing their tool_use responses, and feeding tool results back into the conversation loop.
...
Implement OpenAI function-calling and Gemini tool-use equivalents of the Flint AST Tool Catalog.

**Changes required:**
- Map `FLINT_TOOLS` (Anthropic format) to OpenAI `functions` / `tools` parameter format
- Map `FLINT_TOOLS` to Gemini `tools` / `functionDeclarations` format
...
1. Flint's entire reason for existence is deterministic governance enforcement. A "governance engine" that silently turns off governance for a subset of users is not credible. Option B is honest about the current constraint.
...
2. Each adapter maps `FLINT_TOOLS` to the provider's format
...
|---|-------------|---------|--------------|
| 15 | Granular AST Tools Only | Yes | The only remaining active code path (Anthropic) enforces tool-use via FLINT_TOOLS. Non-Anthropic paths are blocked. |
...
|------|----------|------------|
| Users with OpenAI/Gemini keys cannot use Flint AI | Medium | This is the intended outcome. The error message is clear and actionable: "configure an Anthropic API key." Users retain full Glass observability; only the AI chat path is gated. |
