...
**Security Review Ref:** `.flint-context/reviews/SEC-surface-review.md` findings: "srcdoc iframe has no sandbox attribute", "No CSP set via session.webRequest", "postMessage uses wildcard origin '*'"
...
1. **iframe sandbox attribute** -- Add `sandbox="allow-scripts allow-forms"` to the LivePreview iframe so injected code from Figma ingestion cannot escape the iframe to access `window.parent.flintAPI`.
...
|------|------------|-------------|-------|
| `src/components/editor/LivePreview.tsx` | MODIFY | `flint-design-engineer` | Add `sandbox` attr, add origin check in `handleMessage` |
| `electron/main.ts` | MODIFY | `flint-electron-ipc` | Add `session.webRequest.onHeadersReceived` CSP injection in `createWindow()` |
| `src/components/editor/__tests__/LivePreview.test.tsx` | MODIFY or CREATE | `flint-test-writer` | Tests for sandbox attr + origin rejection |
| `electron/__tests__/main-csp.test.ts` | CREATE | `flint-test-writer` | Test that CSP header callback is registered |
...
- Event listeners inside the iframe (click, hover, drag handlers installed by the Flint interaction scripts) continue to work.
- React rendering via `ReactDOM.createRoot()` inside the iframe continues to work.

**What the sandbox blocks:**
- `window.parent.flintAPI` -- The iframe can no longer access the parent's `window` object or any APIs exposed on it. This is the core security fix. Previously, malicious code injected via Figma `/ingest-ast` could call `window.parent.flintAPI.saveFile(...)`, `window.parent.flintAPI.terminal.spawn(...)`, etc.
...
- Assert: editorStore selectedNode changed to 'flint-test-node'
```
...
|------|-------|---------------|-------------|
| 1 | `flint-design-engineer` | A | Add `sandbox` attr to iframe in LivePreview.tsx |
| 2 | `flint-design-engineer` | A | Add origin validation in `handleMessage` in LivePreview.tsx |
| 3 | `flint-electron-ipc` | A | Add CSP `session.webRequest.onHeadersReceived` in main.ts `createWindow()` |
| 4 | `flint-test-writer` | B (after A) | Write tests SEC1-01 through SEC1-04 |
| 5 | `flint-test-writer` | B | Run full test suite, TSC, report counts |
