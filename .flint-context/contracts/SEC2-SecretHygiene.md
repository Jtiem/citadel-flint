...
**Security Review Ref:** `.flint-context/reviews/SEC-surface-review.md` findings: "Hardcoded webhook secret in production binary", "Secret logged to console", "Webhook secret returned to renderer via IPC"

---

## 1. Summary

The ingestion server authenticates Figma plugin requests via an `x-flint-secret` header. Currently, the secret is a hardcoded compile-time constant (`'flint-dev-secret-phase2'`) that ships in every binary, is logged to the console on startup, and is returned to the renderer via IPC. This contract replaces the hardcoded secret with a per-session cryptographic random value, removes all console logging of the secret, removes the secret from the renderer-facing IPC surface, and cleans up the stale type declarations and renderer code that reference it.

**Partial fix already in place:** The `figma:status` IPC handler in `electron/main.ts` (line 2351-2353) already strips the `secret` field before returning to the renderer. However, the `FigmaStatus` type in `flint-api.d.ts`, the `preload.ts` return type, and two renderer files (`StatusBar.tsx`, `FigmaSetupWizard.tsx`) still reference `secret`. These must be cleaned up.
...
|------|------------|-------------|-------|
| `electron/ingestion-server.ts` | MODIFY | `flint-electron-ipc` | Replace hardcoded secret with injected per-session secret; remove `secret` from `getFigmaStatus()` return; remove console.log of secret |
| `electron/main.ts` | MODIFY | `flint-electron-ipc` | Generate random secret in `app.whenReady()`; pass to ingestion server init; no changes to `figma:status` handler (already strips secret) |
| `electron/preload.ts` | MODIFY | `flint-electron-ipc` | Remove `secret: string` from `figma.status()` return type |
| `src/types/flint-api.d.ts` | MODIFY | `flint-electron-ipc` | Remove `secret` field from `FigmaStatus` interface |
| `src/components/editor/StatusBar.tsx` | MODIFY | `flint-design-engineer` | Remove `secret` and `secretTruncated` variables (dead code since handler already strips it) |
| `src/components/ui/FigmaSetupWizard.tsx` | MODIFY | `flint-design-engineer` | Remove `secret` variable usage |
| `electron/__tests__/ingestion-secret.test.ts` | CREATE | `flint-test-writer` | Tests for per-session secret generation and hygiene |
...
Remove line 35:
```typescript
// REMOVE: const FLINT_SECRET = process.env.FLINT_SECRET ?? 'flint-dev-secret-phase2'
```
...
export function startIngestionServer(secret: string): void {
    if (server) {
        console.warn('[Flint] Ingestion server already running.')
        return
    }
...
In `handleRequest`, line 135, replace `FLINT_SECRET` with `flintSecret`:
...
The `getFigmaStatus()` function currently returns `secret: FLINT_SECRET`. Remove this field entirely:
...
Remove line 368:
```typescript
// REMOVE: console.log(`[Flint] x-flint-secret: ${FLINT_SECRET}`)
```

The server start log on line 367 (`[Flint] Ingestion server listening on http://127.0.0.1:${port}`) remains.
...
export function stopIngestionServer(): void {
    if (server) {
        server.close(() => {
            console.log('[Flint] Ingestion server stopped.')
        })
        server = null
        flintSecret = null  // Clear secret on stop
    }
}
```
...
**File:** `src/types/flint-api.d.ts` (near line 408-418)
...
- Send a request to the server with the correct x-flint-secret header
- Assert: request succeeds (200)
- Send a request with the wrong x-flint-secret header
- Assert: request fails (401)
- Send a request with no x-flint-secret header
- Assert: request fails (401)
```
...
- Assert: no console.log call contains 'x-flint-secret'
```
...
|------|-------|---------------|-------------|
| 1 | `flint-electron-ipc` | A | Modify `electron/ingestion-server.ts`: remove hardcoded secret, change `startIngestionServer` signature, remove `secret` from `getFigmaStatus`, remove console.log |
| 2 | `flint-electron-ipc` | A | Modify `electron/main.ts`: generate `sessionSecret`, pass to `startIngestionServer`, simplify `figma:status` handler |
| 3 | `flint-electron-ipc` | A | Modify `electron/preload.ts`: remove `secret` from `figma.status()` return type |
| 4 | `flint-electron-ipc` | A | Modify `src/types/flint-api.d.ts`: remove `secret` from `FigmaStatus` interface |
| 5 | `flint-design-engineer` | A | Modify `StatusBar.tsx` and `FigmaSetupWizard.tsx`: remove dead `secret` code |
| 6 | `flint-test-writer` | B (after A) | Write tests SEC2-01 through SEC2-05 |
| 7 | `flint-test-writer` | B | Run full test suite, TSC, report counts |
...
- [ ] `FigmaStatus` interface in `flint-api.d.ts` does not include `secret`
