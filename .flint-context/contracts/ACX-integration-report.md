...
| Store Isolation | PASS | `importSummaryStore.ts` does not call `window.flintAPI` or cross-import stores |
...
### ComplexityRouter -- MCP Side (flint-mcp/src/core/complexityRouter.ts)
...
- MCP router: provides a `flint_assess_complexity` tool for external agents to assess task complexity
...
- `FLINT_SENTINEL_PROMPT_DEF` -- PASS (exact match with contract Section 6)
  - `name: "flint-sentinel"` -- PASS
  - `arguments[0].required: false` -- PASS
  - `description` matches contract verbatim -- PASS
- `getFlintSentinelContent(domain?, projectRoot?)` -- PASS (signature matches)
...
- `isEnrichableTool()` correctly identifies `flint_ast_mutate`, `flint_fix`, `flint_audit` -- PASS
...
| Type declaration | PASS | `src/types/flint-api.d.ts:1165-1167` -- `context?: { getEnriched: () => Promise<EnrichedContext> }` |
...
| Renderer usage | PASS | `src/hooks/useContextSync.ts:198` -- `window.flintAPI.syncContext(ctx)` |

---

## FlintContext Coherence

The new `FlintContext` fields in `src/types/flint-api.d.ts` (lines 848-873) match what `useContextSync.ts` actually writes:
...
| `selectedNodeSummary` | `{ tagName, flintId, className, props, childCount, parentId } \| null` (optional) | Visual tree walk to find selected node | PASS |
...
| Check | Result |
|-------|--------|
| `sessionContext.ts` in flint-mcp (no electron/ imports) | PASS -- imports only `node:fs`, `node:path` |
| `contextPush.ts` in flint-mcp (no electron/ imports) | PASS -- imports from `./events.js` and `../types.js` |
| `useContextSync.ts` in src/ (no fs/sqlite imports) | PASS -- imports only from React, stores, and types |
| `orchestrator.ts` does not import from flint-mcp/ | PASS -- no `flint-mcp` imports |
...
| C14 (Bypass Prohibition) | `sessionContext.ts` reads `.flint/` JSON files (not `flint.db`). `contextPush.ts` uses `appendMCPEvent()` from the event bus. No direct `fs.writeFile` for governed files. | PASS |
| C15 (Granular AST Tools Only) | No new tools added to `FLINT_TOOLS` catalog. The router only selects models, not tools. | PASS |
...
| `importSummaryStore.ts` does not call `window.flintAPI` | PASS (comment explicitly documents this) |
| Complexity router types live in `electron/orchestrator.ts` (not in a store) | PASS |

Note: The `window.flintAPI` references found in other store files (`governanceStore.ts`, `orchestratorStore.ts`, `editorStore.ts`, `canvasStore.ts`, etc.) are pre-existing architectural debt not introduced by ACX. These are not new violations.
...
1. **[WARNING]** `types.ts` `ComplexityFactor` missing `contribution` field -- `flint-mcp/src/types.ts:232` has `ComplexityFactor` without the `contribution: number` property that `flint-mcp/src/core/complexityRouter.ts:37` declares. The local type in `complexityRouter.ts` is correct (it has `contribution`), but any external consumer importing from `types.ts` would get an incomplete type. **Fix**: Add `contribution: number` to the `ComplexityFactor` interface in `types.ts`.

2. **[WARNING]** `server.ts` sentinel default domain is `"ui"` -- `flint-mcp/src/server.ts:544` defaults the sentinel domain to `"ui"` when no argument is provided (`?? "ui"`). The sentinel's `getFlintSentinelContent()` does not recognize `"ui"` as a known domain (known domains are `general`, `healthcare`, `fintech`, `e-commerce`, `government`, `enterprise-saas`). It will fall back to `"general"` via the unknown-domain path with a `console.warn`. This works correctly (graceful degradation) but emits an unnecessary warning on every default sentinel invocation. **Fix**: Change `"ui"` to `"general"` on `server.ts:544` or remove the fallback entirely (let `getFlintSentinelContent(undefined)` handle it).

3. **[WARNING]** `SessionContext` type diverges from contract -- The implemented `SessionContext` in `flint-mcp/src/types.ts` and `flint-mcp/src/core/sessionContext.ts` uses a flattened structure compared to the contract's nested interfaces (`ActiveFileContext`, `ViolationSnapshot` with `ViolationDetail[]`, `HealthSnapshot`, `TokenSnapshot` with `TokenUsage[]`). This is a pragmatic adaptation (the MCP server reads from `.flint/` JSON files, not SQLite), but it means the resource payload shape differs from the contract's type specification. This should be documented as a "Contract Deviation" so future consumers of `flint://session-context` know the actual shape.

4. **[WARNING]** Pre-existing MCP test failure -- `flint-mcp/src/tests/project-scaffold.test.ts` fails with ENOENT on template directory. Not ACX-related. Should be fixed separately or skipped with a clear TODO.
...
|---------|---------------|----------------|----------|
| 1 | flint-ast-surgeon | Add `contribution: number` to `ComplexityFactor` in `flint-mcp/src/types.ts` | Low |
| 2 | flint-ast-surgeon | Change `?? "ui"` to `?? undefined` on `flint-mcp/src/server.ts:544` | Low |
| 3 | flint-architect | Document SessionContext shape deviation in contract appendix | Low |
| 4 | flint-test-writer | Fix or skip `project-scaffold.test.ts` template path | Low |
