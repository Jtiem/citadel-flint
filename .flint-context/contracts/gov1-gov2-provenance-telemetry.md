...
1. **Provenance registry lives in flint-mcp** as a static TypeScript map (`ruleProvenanceRegistry.ts`). It is NOT a JSON config file -- it is code, versioned with the engine, so that provenance data is always consistent with the rules it describes.
...
3. **GOV.2 detection point is the governanceStore action**, not AST scanning for `data-flint-override` attributes. Reason: `data-flint-override` is a planned future attribute that does not yet exist in the codebase. The actual override mechanism today is `governanceStore.setOverride(ruleId, { enabled, severity })`, called from GovernancePanel. We intercept at that call site.

4. **GOV.2 IPC is a fire-and-forget write** -- the renderer calls `window.flintAPI.governance.recordOverride(...)` and does not await a meaningful return value. The main process writes to SQLite synchronously via `GovernanceEventService.recordEvent()`.

5. **StatusBar override count is fetched via IPC on mount + on each override action** -- not derived from governanceStore (which only holds the current overrides, not the historical count). The badge count comes from `window.flintAPI.governance.getOverrideCount()`.
...
|------|------------|-------------|
| `flint-mcp/src/core/governance/ruleProvenanceRegistry.ts` | NEW FILE -- static provenance map for all 49 rules | flint-ast-surgeon |
| `flint-mcp/src/core/governance/types.ts` | MODIFY -- add `RuleProvenance` interface | flint-ast-surgeon |
| `flint-mcp/src/core/governance/eventService.ts` | MODIFY -- add `getOverrideCount(sessionId)` method | flint-ast-surgeon |
| `flint-mcp/src/server.ts` | MODIFY -- register `flint_audit_report` tool, enrich `flint_audit` output | flint-ast-surgeon |
| `flint-mcp/src/tools/auditReport.ts` | NEW FILE -- `flint_audit_report` MCP tool handler | flint-ast-surgeon |
| `electron/main.ts` | MODIFY -- add `governance:record-override` and `governance:override-count` IPC handlers | flint-electron-ipc |
| `electron/preload.ts` | MODIFY -- expose `governance.recordOverride()` and `governance.getOverrideCount()` on flintAPI | flint-electron-ipc |
| `src/types/flint-api.d.ts` | MODIFY -- add `GovernanceAPI`, `RuleProvenance`, `ComplianceSummary` types | flint-state-architect |
| `src/components/ui/ExportModal.tsx` | MODIFY -- add "Compliance Summary" section with provenance data | flint-design-engineer |
| `src/components/editor/StatusBar.tsx` | MODIFY -- add "Overrides (N)" badge | flint-design-engineer |
| `src/components/ui/GovernancePanel.tsx` | MODIFY -- fire IPC on setOverride/resetOverride/resetAll | flint-design-engineer |
| `flint-mcp/src/core/governance/__tests__/ruleProvenanceRegistry.test.ts` | NEW FILE -- provenance registry tests | flint-test-writer |
| `flint-mcp/src/core/governance/__tests__/eventService.test.ts` | MODIFY -- add getOverrideCount tests | flint-test-writer |
| `src/components/__tests__/ExportModal.test.tsx` | NEW FILE or MODIFY -- compliance summary rendering tests | flint-test-writer |
| `src/components/__tests__/StatusBar.test.tsx` | NEW FILE or MODIFY -- overrides badge tests | flint-test-writer |

---

## Type Contracts (Source of Truth for Phase 2)

### New Types -- flint-mcp (Node.js side)

```typescript
// flint-mcp/src/core/governance/types.ts -- ADDITIONS
...
### New Types -- renderer side (mirror in flint-api.d.ts)

```typescript
// src/types/flint-api.d.ts -- ADDITIONS

/**
 * Regulatory source authority (renderer-side mirror of flint-mcp type).
 */
export type SourceAuthority =
    | 'WCAG 2.1 AA'
    | 'WCAG 2.2 AA'
 ...
 * sourceAuthority: 'Flint Design System' and regulatoryReference: 'N/A'.
 */
...
| New Store Deps | (none) |
| New IPC Calls | `window.flintAPI.governance.getComplianceSummary(ruleIds)` |
...
- An "Export Audit Report (JSON)" button calls `navigator.clipboard.writeText(JSON.stringify(complianceSummary))` to copy the structured audit report. Future: this could trigger the `flint_audit_report` MCP tool for a richer report.
...
| New Props | (none) |
| New Store Deps | (none) |
| New IPC Calls | `window.flintAPI.governance.getOverrideCount()` |
...
- To avoid polling, the main process pushes `flint:governance-override-recorded` after each `recordEvent`, and the renderer subscribes via a new `onOverrideRecorded` listener in the preload surface.
...
| New IPC Calls | `window.flintAPI.governance.recordOverride(payload)` |
...
## MCP Tool: flint_audit_report

### Tool Definition

```typescript
export const FLINT_AUDIT_REPORT_TOOL = {
    name: 'flint_audit_report',
...
```typescript
// flint-mcp/src/types.ts -- LinterWarning addition
...
- [x] **C9 Process Boundary** -- New IPC channels properly defined. No `fs` or `sqlite` in `src/`. All cross-boundary calls go through `window.flintAPI.governance.*`.
...
|------|------|-------|------------|
| 0a | `flint-mcp/src/core/governance/types.ts` | flint-ast-surgeon | -- |
| 0b | `flint-mcp/src/types.ts` | flint-ast-surgeon | -- |
| 0c | `src/types/flint-api.d.ts` | flint-state-architect | -- |

### Group 1: MCP Engine (parallel within group, depends on Group 0)

| Step | File | Agent | Depends On |
|------|------|-------|------------|
| 1a | `flint-mcp/src/core/governance/ruleProvenanceRegistry.ts` (NEW) | flint-ast-surgeon | 0a |
| 1b | `flint-mcp/src/core/governance/eventService.ts` (add `getOverrideCount`) | flint-ast-surgeon | 0a |
| 1c | `flint-mcp/src/core/MithrilLinter.ts` (add `ruleId` to warnings) | flint-ast-surgeon | 0b |
| 1d | `flint-mcp/src/core/A11yLinter.ts` (add `ruleId` to violation output -- see note) | flint-ast-surgeon | 0b |
| 1e | `flint-mcp/src/tools/auditReport.ts` (NEW) | flint-ast-surgeon | 1a, 1c, 1d |
| 1f | `flint-mcp/src/server.ts` (register `flint_audit_report` tool) | flint-ast-surgeon | 1e |

**Note on A11y ruleId:** The A11yLinter returns `A11yViolations = Record<string, string[]>` (flintId -> message list). The ruleId is embedded in each message string (e.g., `"A11Y-001: ..."`). For GOV.1, we do NOT change the A11yLinter return type -- we parse ruleIds from message prefixes in the auditReport handler using a simple `msg.match(/^(A11Y-\d{3})/)?.[1]` extraction. This avoids breaking the A11y violation pipeline.

### Group 2: IPC Layer (parallel, depends on Group 0)

| Step | File | Agent | Depends On |
|------|------|-------|------------|
| 2a | `electron/main.ts` | flint-electron-ipc | 0a, 0c |
| 2b | `electron/preload.ts` | flint-electron-ipc | 0c |

### Group 3: UI Layer (parallel within group, depends on Groups 1 + 2)

| Step | File | Agent | Depends On |
|------|------|-------|------------|
| 3a | `src/components/ui/ExportModal.tsx` | flint-design-engineer | 0c, 2b |
| 3b | `src/components/editor/StatusBar.tsx` | flint-design-engineer | 0c, 2b |
| 3c | `src/components/ui/GovernancePanel.tsx` | flint-design-engineer | 0c, 2b |

### Group 4: Tests (parallel with Group 3)

| Step | File | Agent | Depends On |
|------|------|-------|------------|
| 4a | `flint-mcp/src/core/governance/__tests__/ruleProvenanceRegistry.test.ts` (NEW) | flint-test-writer | 1a |
| 4b | `flint-mcp/src/core/governance/__tests__/eventService.test.ts` (MODIFY) | flint-test-writer | 1b |
| 4c | `src/components/__tests__/ExportModal.test.tsx` | flint-test-writer | 3a |
| 4d | `src/components/__tests__/StatusBar.test.tsx` | flint-test-writer | 3b |

### Group 5: Integration Review (sequential, after all)

| Step | Task | Agent | Depends On |
|------|------|-------|------------|
| 5 | Full TSC check + IPC symmetry + contract fidelity | flint-code-reviewer | all |
...
To avoid polling in StatusBar, the main process pushes a `flint:governance-override-recorded` event after each successful `recordEvent` call for `eventType: 'override'`. The preload surface exposes:
...
governance: {
    ...
    onOverrideRecorded: (cb: () => void): (() => void) => {
        const listener = () => cb()
        ipcRenderer.on('flint:governance-override-recorded', listener)
        return () => ipcRenderer.removeListener('flint:governance-override-recorded', listener)
    },
}
```
...
GOV.2 telemetry recording happens in GovernancePanel (component layer), not in governanceStore. This is intentional -- calling `window.flintAPI` inside a Zustand store action violates the architectural anti-pattern rule. The GovernancePanel already imports and calls `setOverride` from the store; the IPC call is a separate statement in the same event handler.

### R4: IPC Channel Naming Collision

New channels use the `governance:` prefix, which does not collide with any existing prefix (`tokens:`, `ast:`, `sync:`, `ai:`, `flint:`, `menu:`, `dialog:`, `file:`, `registry:`, `project:`, `preview:`, `terminal:`, `code:`).

### R5: Missing `saveRuleOverrides` / `getRuleOverrides` IPC Handlers

The existing `governanceStore` calls `window.flintAPI.saveRuleOverrides()` and `window.flintAPI.getRuleOverrides()`, but no IPC handlers exist for these in `electron/main.ts` or `electron/preload.ts`. The store falls back to localStorage. GOV.2 does NOT fix this gap -- it adds telemetry for override events, not persistence for the override state itself. The existing fallback to localStorage remains. This is a known gap tracked separately.

### R6: FlintContext Type Missing

`useContextSync.ts` imports `FlintContext` from `'../types/flint-api'`, but this type is not defined in `flint-api.d.ts`. This is a pre-existing issue not caused by GOV.1/GOV.2. The contextSync hook should be extended to include override count in the `FlintContext.violations` object once the type is formalized, but that is a separate concern.

---

## Testing Requirements

### flint-mcp tests (vitest.config.ts)
...
1. `/Users/tiemann/Lunar-Elevator-Flint/flint-mcp/src/core/governance/ruleProvenanceRegistry.ts`
2. `/Users/tiemann/Lunar-Elevator-Flint/flint-mcp/src/tools/auditReport.ts`
3. `/Users/tiemann/Lunar-Elevator-Flint/flint-mcp/src/core/governance/__tests__/ruleProvenanceRegistry.test.ts`
4. `/Users/tiemann/Lunar-Elevator-Flint/.flint-context/contracts/gov1-gov2-provenance-telemetry.md` (this file)

### Modified Files (11)

1. `/Users/tiemann/Lunar-Elevator-Flint/flint-mcp/src/core/governance/types.ts`
2. `/Users/tiemann/Lunar-Elevator-Flint/flint-mcp/src/core/governance/eventService.ts`
3. `/Users/tiemann/Lunar-Elevator-Flint/flint-mcp/src/core/governance/__tests__/eventService.test.ts`
4. `/Users/tiemann/Lunar-Elevator-Flint/flint-mcp/src/types.ts`
5. `/Users/tiemann/Lunar-Elevator-Flint/flint-mcp/src/server.ts`
6. `/Users/tiemann/Lunar-Elevator-Flint/flint-mcp/src/core/MithrilLinter.ts`
7. `/Users/tiemann/Lunar-Elevator-Flint/electron/main.ts`
8. `/Users/tiemann/Lunar-Elevator-Flint/electron/preload.ts`
9. `/Users/tiemann/Lunar-Elevator-Flint/src/types/flint-api.d.ts`
10. `/Users/tiemann/Lunar-Elevator-Flint/src/components/ui/ExportModal.tsx`
11. `/Users/tiemann/Lunar-Elevator-Flint/src/components/editor/StatusBar.tsx`
12. `/Users/tiemann/Lunar-Elevator-Flint/src/components/ui/GovernancePanel.tsx`
