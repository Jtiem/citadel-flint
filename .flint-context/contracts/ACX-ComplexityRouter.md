...
- If the previous assistant turn used `flint_insert_node` or `flint_wrap_node`: Compound floor
...
console.log(`[Flint ACX] tier=${assessment.tier} model=${resolvedModel} reason="${assessment.reasoning}"`)
...
|------|------------|-------------|
| `electron/orchestrator.ts` | MODIFY — add `ComplexityTier`, `ComplexitySignal`, `ComplexityAssessment`, `RouterInput` types; add `classifyComplexity()`, `buildRouterInput()`, `loadCurrentViolationCount()`, `TIER_TO_MODEL`, `ESCALATION_PATH` constants; modify Anthropic branch of `sendChatMessage()` to invoke the router | flint-orchestrator |
| `electron/__tests__/orchestrator.complexity.test.ts` | NEW FILE — unit tests for the classification algorithm | flint-test-writer |
...
- [x] **C15 Granular AST Tools Only** — the router does not add or remove tools. The `FLINT_TOOLS` catalog is unchanged.
...
| Prior structural tool | priorToolNames=['flint_insert_node'] | compound (prior tool raise) |
...
1. `/Users/tiemann/Lunar-Elevator-Flint/electron/__tests__/orchestrator.complexity.test.ts`
2. `/Users/tiemann/Lunar-Elevator-Flint/.flint-context/contracts/ACX-ComplexityRouter.md` (this file)

### Modified Files (1)

1. `/Users/tiemann/Lunar-Elevator-Flint/electron/orchestrator.ts`
...
Extract `toolInput.targetId` from each `ChatMessage` in the prior tool calls. Deduplicate with a `Set`. Return `Set.size`. Messages without a `targetId` in their input are ignored. Read-only tools (`flint_read_code`, `flint_read_tokens`) never have a `targetId` and are correctly excluded.
...
- The `FLINT_TOOLS` catalog is immutable in this contract. No tools are added or removed.
