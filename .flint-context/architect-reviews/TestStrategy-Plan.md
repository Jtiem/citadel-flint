...
These prove Flint's core value proposition. If any of these break, the product is lying.
...
| 5 | `flint_audit` tool → returns violations with correct ruleIds and severity | **MCP audit works headless** | MCP |
| 6 | `flint_fix` tool → auto-fixes token violation → re-audit passes | **Auto-fix actually fixes** | MCP |
| 7 | `flint_debt_report` → scans project → returns score 0-100 with correct grade | **Debt report is accurate** | MCP |
| 8 | `flint_accessibility_report` → returns WCAG criterion pass/fail breakdown | **A11y report is structured** | MCP |
| 9 | `flint_audit_report` → violations enriched with provenance metadata | **Compliance trail works** | MCP |
...
| # | Test | What it proves | Domain |
|---|------|---------------|--------|
| 11 | `applyMutationBatch([move, inject, fixToken])` → AST valid → `data-flint-id` preserved | **Batch mutations are atomic** | AST |
...
| 18 | `injectFlintIds(ast)` → every JSXElement has unique `data-flint-id` | **ID preservation holds** | AST |
...
| 21 | `.flint/context.json` written by useContextSync → readable by MCP server | **Context Flint file is valid JSON** | Integration |
| 22 | `.flint/annotations.json` written by MCP → readable by Glass annotationStore | **Annotation sync works** | Integration |
| 23 | `flint://dashboard` resource → returns DashboardData with score + history | **MCP resource is live** | MCP |
| 24 | `flint://violations/{filePath}` resource → returns audit for that file | **Per-file violations work** | MCP |
| 25 | `flint://capabilities` resource → lists all 13 tools with descriptions | **Tool discovery works** | MCP |
...
| File | Tests | Runner |
|------|-------|--------|
| `flint-mcp/src/__tests__/safety-promises.test.ts` | 1-10 | vitest (Node) |
| `flint-mcp/src/__tests__/data-integrity.test.ts` | 11-18 | vitest (Node) |
| `flint-mcp/src/__tests__/boundary-contracts.test.ts` | 19-25 | vitest (Node) |
