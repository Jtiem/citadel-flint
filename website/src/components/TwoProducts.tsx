  ...
  return <section className="py-section bg-surface-raised">
      <div className="section-container">
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-2 gap-6 max-w-page mx-auto">
          {twoProducts.products.map((product, i) => <FadeIn key={product.name} delay={i * 120}>
              <div className={`rounded-2xl border shadow-card hover:shadow-card-hover transition-shadow p-6 md:p-8 ${i === 0 ? 'bg-accent-subtle/40 border-border-subtle border-l-4 border-l-accent' : 'bg-white border-border-subtle'}`}>
                ...
                <ul className="space-y-2.5">
                  {product.highlights.map(item => <li key={item} className="flex items-start gap-2.5">
