# Bridge MCP — Demo Fixtures

Runnable, repeatable demo scenarios for the Bridge governance engine. Each subfolder is a self-contained story: an input file (or pair of files) that demonstrates one capability of Bridge MCP, plus a `README.md` describing what to run and what to expect.
...
The demos assume Bridge MCP is built and running. Connect a client (Claude Code, Cursor, or any MCP-compatible IDE) to the server before stepping through a demo.
...
| 02 | `02-self-correcting/` | Bridge's in-memory TSC loop catches three AI-hallucinated type errors before they reach the user as a diff |
...
| 04 | `04-sentinel/` | The Bridge Sentinel rejects an AI-generated order form for violating Hick's Law (10-button toolbar) and Miller's Law (16 always-visible fields) |
...
| 07 | _(planned)_ `07-figma-sync/` | Bidirectional token sync: Figma variables pushed via SDI webhook, Bridge reconciles drift against the running design system |
| 08 | _(planned)_ `08-ci-gate/` | `bridge audit` run as a CI step — SARIF output, exit code 1 on critical violations, PR blocked until resolved |
...
Each demo is meant to be opened in a Bridge-connected IDE session. The general pattern:

1. Point the MCP server at the demo directory as the working root.
2. Open the component file in the IDE.
3. Invoke the relevant MCP tool listed in the demo README (e.g. `audit_ui_component`, `bridge_ast_mutate`, `bridge_query_registry`).
4. Observe violations, auto-fixes, or recovered AST nodes in Bridge Glass.
