...
export default function PricingCard({
  ...
  return <article data-flint-id="pricing-card-root" className="relative flex flex-col overflow-hidden rounded-xl border-0 shadow-2xl">
      {/* Badge — DRIFT: #FF3333 used instead of color.danger (#DC2626) */}
      <div data-flint-id="pricing-card-badge" style={{
      ...
      <div data-flint-id="pricing-card-header" style={{
      backgroundColor: "var(--color.primary-hover, #0052CC)"
    }} className="px-6 py-7">
        <h3 data-flint-id="pricing-card-plan-name" className="text-white text-lg font-semibold m-0">
          {planName}
        </h3>
      ...
      {/* Features */}
      <div data-flint-id="pricing-card-features" className="flex-1 px-6 py-6">
        <ul className="list-none m-0 p-0 flex flex-col gap-3">
          {features.map((feature, i) => <li key={i} className="flex items-start gap-2.5 text-sm" style={{
          color: feature.included ? '#374151' : '#9CA3AF',
          textDecoration: feature.included ? 'none' : 'line-through'
        }}>
              <svg width="18" height="18" viewBox="0 0 18 18" fill="none" aria-hidden="true" className="shrink-0 mt-px" style={{
            color: feature.included ? '#00AAFF' : 'transparent'
          }}>
                {feature.included && <path d="M3.5 9.5l3.5 3.5 7-8" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />}
              </svg>
              {feature.text}
            </li>)}
        </ul>
      ...
      <div className="px-6 pb-7">
        <button data-flint-id="pricing-card-cta" onClick={() => onSelectPlan(planName)} style={{
        backgroundColor: "var(--color.primary-hover, #0052CC)"
